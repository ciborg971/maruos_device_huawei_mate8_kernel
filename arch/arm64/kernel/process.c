...
void machine_restart(char *cmd)
{
	...
	if (arm_pm_restart) {
		...
		/*for fpga test*/
		unsigned int tmp;
		sctrl_addr = ioremap((phys_addr_t)0xfff0a000,0x1000);
		tmp = readl(SOC_SCTRL_SCPEREN1_ADDR(sctrl_addr));
		writel(tmp | (0x1<<31), SOC_SCTRL_SCPEREN1_ADDR(sctrl_addr));
		while(1) {
			writel(0xdeadbeef, (sctrl_addr+ 4));
		}
	}
