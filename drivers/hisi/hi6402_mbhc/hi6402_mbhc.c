...
void hi6402_mbhc_on(struct hi6402_mbhc_platform_data *pdata)
{
	...
	hi6402_irq_clr_btn_irqs(pdata->p_irq);
	/* unmask btn irqs */
	hi6402_irq_unmask_btn_irqs(pdata->p_irq);
...
void hi6402_plug_in_detect(struct hi6402_mbhc_platform_data *pdata)
{
	...
	if (pdata->hs_3_pole_max_voltage >= saradc_value) {
		...
		/* real invert headset */
		if(pdata->hs_4_pole_min_voltage > saradc_value) {
...
/* not sure whether hi6402_pll_unlock_handler would be used in future */
#if 0
...
/* not sure whether pll_unlock_err would be used in future */
#if 0
pll_unlock_err:
#endif
...
late_initcall_sync(hi6402_mbhc_init);
