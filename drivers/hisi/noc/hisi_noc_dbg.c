...
static struct dentry *noc_dbg_file_dentry = NULL;
					      ...
					      uint *reg_value)
{
	...
	if (i >= ARRAY_SIZE(tbl_noc_dbg_cmd))
		...
		if (cmd == E_NOC_DBG_CMD_WR_REG) {
			if (-1 ==
			    sscanf(kn_buf, "set %s %s 0x%x", reg_str[0],
				   reg_str[1], reg_value))
				cmd = E_NOC_DBG_CMD_INVALID;
		} else if (cmd == E_NOC_DBG_CMD_RD_REG) {
			if (-1 ==
			    sscanf(kn_buf, "get %s %s", reg_str[0], reg_str[1]))
				cmd = E_NOC_DBG_CMD_INVALID;
		}
	}
	return cmd;
			       ...
			       uint *reg_value)
{
	...
	case E_NOC_DBG_CMD_RD_REG:
		...
		for (i = 0; i < cmdtbl_len; i++) {
			if (!strncmp
			    (reg_str[0], tbl_noc_dbg_reg[i].p_reg_name, 128)) {
		...
		if (match_index >= cmdtbl_len) {
			...
			if (NULL == (node = get_probe_node((const char *)reg_str[1]))) {
				goto err_out;
			}
		}
		if (!is_noc_node_available(node)) {
			pr_err
			    ("[%s]: this node is power down, cannot access.\n",
			     node->name);
			goto err_out;
		}
		...
		if (E_NOC_DBG_CMD_WR_REG == cmd_in) {
			if (-1 ==
			    noc_dbg_wr_reg(pt_dbg_reg, reg_addr, reg_value))
				goto err_out;
		} else if (E_NOC_DBG_CMD_RD_REG == cmd_in) {
			if (-1 ==
			    noc_dbg_rd_reg(pt_dbg_reg, reg_addr, reg_value))
				goto err_out;
			  ...
			  size_t bytes, loff_t *off)
{
	...
	byte_writen =
	    simple_write_to_buffer(kn_buf, NOC_DBG_STRING_SIZE, off, userbuf,
				   bytes);
	if (byte_writen <= 0) {
	...
	/* parse string */
	cmd_in =
	    noc_dbg_get_cmd_typ(kn_buf, (char (*)[NOC_CMD_CELL_SIZE])reg_str,
				&reg_value);
...
int noc_dbg_creat_node(void)
{
	...
	_noc_root = noc_get_debugfs_dentry();
	if (NULL != _noc_root) {
		noc_dbg_file_dentry =
		    debugfs_create_file("dbg", 0660, _noc_root, NULL,
					&noc_dbg_fops);
		if ((NULL == noc_dbg_file_dentry) || IS_ERR(noc_dbg_file_dentry)) {
