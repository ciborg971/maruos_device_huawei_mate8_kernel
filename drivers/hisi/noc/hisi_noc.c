...
static unsigned int g_noc_init = 0;

static unsigned int nodes_array_idx = 0;

static struct dentry *noc_root = NULL;

const struct hisi_noc_data *g_config_hisi_noc_data = NULL;
...
static const struct hisi_noc_data hisi6250_noc_data = {
	.platform_id = HISI_NOC_HI6250
};
...
static struct of_device_id hisi_noc_match[] = {
	...
	{.compatible = "hisilicon,hi6250-noc",
	 .data = &hisi6250_noc_data,
	 },
...
extern void hi3xxx_noc_err_probe_hanlder(void __iomem *base,
...
int noc_node_try_to_giveup_idle(struct noc_node *node)
{
	...
	status =
	    readl_relaxed(SOC_PMCTRL_NOC_POWER_IDLE_ADDR(noc_dev->pmctrl_base));
...
unsigned long noc_get_irq_status(void __iomem *pctrl_base)
{
	...
	ret = readl_relaxed(pctrl_base + PCTRL_NOC_IRQ_STAT3);
	ret = (ret << 32) | readl_relaxed(pctrl_base + PCTRL_NOC_IRQ_STAT1);
	ret = ret & HI3XXX_NOC_PCTRL_IRQ_MASK;
	pr_info("noc irq status = 0x%llx\n", ret);
	return ret;
}
...
static irqreturn_t hisi_noc_irq_handler(int irq, void *data)
{
	...
	pending = noc_get_irq_status(pctrl_base);
...
NOC_ERROR_CHECK:
					if (is_noc_node_available(node)) {
						...
						if (giveup_idle_retry++ < 5) {
							noc_node_try_to_giveup_idle
							    (node);
							pr_err
							    ("noc_node_try_to_giveup_idle , %d time\n",
							     giveup_idle_retry);
							goto NOC_ERROR_CHECK;
						}
						pr_err
						    ("Finally, noc_node: %s powerdown state cann't be changed!\n",
						     node->name);
						BUG_ON(1);	/* while(1); dead loop for manually check out what happened on the noc bus*/
					}
				}
				break;
...
static void get_noc_node_clock(struct device_node *np, struct noc_node *node)
{
	...
	for (i = 0; i < HISI_NOC_CLOCK_MAX; i++) {
		...
		ret =
		    of_property_read_u32_array(np, clock_name, clk_reg_u32, 2);
		if (ret) {
			continue;
		} else {
			if (clk_reg_u32[1] > 32) {	/* 32 bit register */
				pr_err
				    ("[%s]:NOC node[%s] %s  invalid mask_bit=%d\n",
			...
			if (g_noc_debug)
				pr_info
				    ("[%s]:NOC node[%s] %s  offset=0x%x, mask_bit=%d\n",
				     __func__, node->name, clock_name,
...
static int register_noc_node(struct device_node *np)
{
	...
	if (g_error_enable) {
		...
		ret =
		    of_property_read_u32(np, "eprobe-hwirq",
					 &node->eprobe_hwirq);
		if (ret < 0) {
			node->eprobe_hwirq = -1;
		...
		if (node->eprobe_hwirq >= 0) {
			ret =
			    of_property_read_u32(np, "eprobe-offset",
						 &node->eprobe_offset);
			if (ret < 0) {
				node->eprobe_hwirq = -1;
				pr_debug("the node doesnot have err probe!\n");
...
static void register_irqdata(void)
{
	...
	for (i = 0; i < nodes_array_idx; i++) {
		...
		if (node->eprobe_hwirq >= HI3XXX_NOC_MAX_IRQ_NR) {
			pr_err
			    ("[%s]: node->eprobe_hwirq(%d) is out of range(%d)!\n",
			     __func__, node->eprobe_hwirq,
			     HI3XXX_NOC_MAX_IRQ_NR);
			continue;
		}
		if ((node->hwirq_type >= NOC_ERR_PROBE_IRQ)
		    && (node->hwirq_type <= NOC_TRANS_PROBE_IRQ)) {
			hi3xxx_noc_irqdata[node->eprobe_hwirq].type =
			    node->hwirq_type;
...
static void enable_errprobe(struct device *dev)
{
	...
	for (i = 0; i < nodes_array_idx; i++) {
		...
		if (is_noc_err_node_available(node)) {
			if (1 == g_noc_debug) {
				pr_err
				    ("Start to enable error probe for bus %s.\n",
				     node->name);
			}
...
void enable_noc_transcation_probe(struct device *dev)
{
	...
	for (i = 0; i < nodes_array_idx; i++) {
		...
		if (is_noc_transcation_node_available(node)) {
			if (1 == g_noc_debug) {
				pr_err
				    ("Start to enable transaction probe for bus %s.\n",
				     node->name);
			}
...
void enable_noc_packet_probe(struct device *dev)
{
	...
	for (i = 0; i < nodes_array_idx; i++) {
		...
		if (is_noc_packet_node_available(node)) {
			if (1 == g_noc_debug) {
				pr_err
				    ("Start to enable packet probe for bus %s.\n",
				     node->name);
			}
...
static void hisi_noc_get_property(struct device_node *np)
{
	...
	if (of_property_read_bool(np, "noc_timeout_enable")) {
		pr_err("noc_timeout_enable!");
		g_noc_timeout_enable = 1;
	}
}

static int noc_set_platform_info(struct platform_device *pdev, uint platform)
{
/* buses information select among platforms*/
	noc_set_buses_info(platform);
/* rounted information select among platforms*/
	noc_set_routeid_info(platform);

	return 0;
}

static int hisi_noc_hi3xxx_part_probe(struct platform_device *pdev,
				      struct device_node *np)
{
	struct hisi_noc_device *noc_dev = NULL;
	...
	ret =
	    devm_request_irq(&pdev->dev, noc_dev->hi3xxx_irq,
	...
	if (g_noc_timeout_enable) {
		...
		ret =
		    devm_request_irq(&pdev->dev, noc_dev->timeout_irq,
...
static int hisi_noc_probe(struct platform_device *pdev)
{
	...
	noc_set_platform_info(pdev, NOC_PLATFORM_ID);
	...
	noc_dev =
	    devm_kzalloc(&pdev->dev, sizeof(struct hisi_noc_device),
			 GFP_KERNEL);
	if (!noc_dev) {
	...
	ret = hisi_noc_hi3xxx_part_probe(pdev, np);
	if (ret != 0) {
		pr_err("hisi_noc_hi3xxx_part_probe err\n");
		goto err;
	}
