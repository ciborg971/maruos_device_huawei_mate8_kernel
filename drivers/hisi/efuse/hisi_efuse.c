...
int set_efuse_securitydebug_value(unsigned char *pu8Buffer, unsigned int timeout)
{
    ...
    memmove((void*)efuse_buf, (void*)pu8Buffer, sizeof(unsigned int));
    #ifdef CONFIG_HI6XXX_EFUSE
        BSP_IPC_SpinLock(IPC_SEM_EFUSE);
    #endif
...
int get_efuse_freq_value(unsigned char *pu8Buffer, unsigned int u32Length)
{
    ...
    ret = invoke_efuse_fn(EFUSE_FN_GET_FREQ, (u64)g_phy_addr, sizeof(unsigned int), 0);
    if(OK == ret)
    {
        memmove((void*)pu8Buffer, (void*)efuse_buf, u32Length);
    }
...
int get_efuse_kce_value(unsigned char *pu8Buffer, unsigned int u32Length, unsigned int timeout)
{
    ...
    if(NULL == pu8Buffer)
    {
       ...
       return -EFAULT;
        ...
        BSP_IPC_SpinLock(IPC_SEM_EFUSE);
    #endif
    ret = invoke_efuse_fn(EFUSE_FN_GET_KCE, (u64)(g_phy_addr + (g_efuse_group_max << 2)), u32Length, timeout);
    #ifdef CONFIG_HI6XXX_EFUSE
        BSP_IPC_SpinUnLock(IPC_SEM_EFUSE);
    #endif
    if(OK == ret)
    {
        memmove((void*)pu8Buffer, (void*)(efuse_buf  + (g_efuse_group_max << 2)), u32Length);
    }
    return ret;
}
...
int set_efuse_kce_value(unsigned char *pu8Buffer, unsigned int u32Length, unsigned int timeout)
{
    ...
    memmove((void*)(efuse_buf + (g_efuse_group_max << 2)), (void*)pu8Buffer, u32Length);
    #ifdef CONFIG_HI6XXX_EFUSE
        BSP_IPC_SpinLock(IPC_SEM_EFUSE);
    #endif
    ret = invoke_efuse_fn(EFUSE_FN_SET_KCE, (u64)(g_phy_addr + (g_efuse_group_max << 2)), u32Length, timeout);
    #ifdef CONFIG_HI6XXX_EFUSE
        BSP_IPC_SpinUnLock(IPC_SEM_EFUSE);
    #endif
...
int bsp_efuse_read(unsigned int * pBuf, const unsigned int group, const unsigned int num)
{
    ...
    switch(group)
    {
        case EFUSE_CHIPID_GROUP_START:
            ...
            bytes = num*4;
			ret = invoke_efuse_fn(EFUSE_FN_GET_CHIPID, (u64)g_phy_addr + (g_efuse_group_max << 2), bytes, EFUSE_MAILBOX_TIMEOUT_1000MS);
    		if(OK != ret )
    		{
			...
			memcpy((void*)pBuf, (void*)(efuse_buf + (g_efuse_group_max << 2)), bytes);
            break;
        ...
        case EFUSE_DIEID_GROUP_START:
            ...
            bytes = num*4;
			ret = invoke_efuse_fn(EFUSE_FN_GET_DIEID, (u64)g_phy_addr + (g_efuse_group_max << 2), bytes, EFUSE_MAILBOX_TIMEOUT_1000MS);
    		if(OK != ret )
    		{
        ...
        case EFUSE_KCE_GROUP_START:
            ...
            bytes = num*4;
            ret = get_efuse_kce_value((char*)(efuse_buf + (g_efuse_group_max << 2)), bytes, EFUSE_MAILBOX_TIMEOUT_1000MS);
    		if(OK != ret )
    		{
    			efuse_print_info(log_level_error, "%s: %d: bsp_efuse_read: get_efuse_dieid_value failed.\n", __func__, __LINE__);
    			return ret;
...
int bsp_efuse_write(unsigned int *pBuf, const unsigned int group, const unsigned int num)
{
    ...
    switch(group)
    {
        case EFUSE_KCE_GROUP_START:
            ...
            bytes = num*4;
            memcpy((void *)(efuse_buf + (g_efuse_group_max << 2)), (void*)pBuf, bytes);
            ret = set_efuse_kce_value((char *)(efuse_buf + (g_efuse_group_max << 2)), bytes, EFUSE_MAILBOX_TIMEOUT_1000MS);
    		if(OK != ret )
    		{
...
int get_efuse_sltfinishflag_value(unsigned char *pu8Buffer, unsigned int u32Length, unsigned int timeout)
{
    ...
    if(NULL == pu8Buffer)
    {
       ...
       return -EFAULT;
        ...
        BSP_IPC_SpinLock(IPC_SEM_EFUSE);
    #endif
    ret = invoke_efuse_fn(EFUSE_FN_GET_SLTFINISHFLAG, (u64)g_phy_addr, u32Length, timeout);
    #ifdef CONFIG_HI6XXX_EFUSE
        BSP_IPC_SpinUnLock(IPC_SEM_EFUSE);
    #endif
    if(OK == ret)
...
int set_efuse_sltfinishflag_value(unsigned char *pu8Buffer, unsigned int u32Length, unsigned int timeout)
{
    ...
    memmove((void*)efuse_buf, (void*)pu8Buffer, u32Length);
        ...
        BSP_IPC_SpinLock(IPC_SEM_EFUSE);
    #endif
    ret = invoke_efuse_fn(EFUSE_FN_SET_SLTFINISHFLAG, (u64)g_phy_addr, u32Length, timeout);
    #ifdef CONFIG_HI6XXX_EFUSE
        BSP_IPC_SpinUnLock(IPC_SEM_EFUSE);
    #endif
...
int set_efuse_djtagvalue_value(unsigned char *pu8Buffer, unsigned int timeout)
{
    ...
    memmove((void*)efuse_buf, (void*)pu8Buffer, sizeof(unsigned int));
        ...
        BSP_IPC_SpinLock(IPC_SEM_EFUSE);
    #endif
    ret = invoke_efuse_fn(EFUSE_FN_SET_DJTAGDEBUG, (u64)g_phy_addr, 0, timeout);
    #ifdef CONFIG_HI6XXX_EFUSE
        BSP_IPC_SpinUnLock(IPC_SEM_EFUSE);
    #endif
...
int get_efuse_djtagvalue_value(unsigned char *pu8Buffer, unsigned int timeout)
{
    ...
    if(NULL == pu8Buffer)
    {
       ...
       return -EFAULT;
        ...
        BSP_IPC_SpinLock(IPC_SEM_EFUSE);
    #endif
    ret = invoke_efuse_fn(EFUSE_FN_GET_DJTAGDEBUG, (u64)g_phy_addr, 0, timeout);
    #ifdef CONFIG_HI6XXX_EFUSE
        BSP_IPC_SpinUnLock(IPC_SEM_EFUSE);
    #endif
    if(OK == ret)
    {
        memmove((void*)pu8Buffer, (void*)efuse_buf, sizeof(unsigned int));
    }
    return ret;
}
...
static long efusec_ioctl(struct file *file, u_int cmd, u_long arg)
{
	...
	case HISI_EFUSE_READ_CHIPID:
		...
		ret = get_efuse_chipid_value(efuse_buf, bytes, EFUSE_MAILBOX_TIMEOUT_1000MS);
		if (OK != ret )
		{
	...
	case HISI_EFUSE_WRITE_CHIPID:
		...
		ret = set_efuse_chipid_value(efuse_buf, bytes, EFUSE_MAILBOX_TIMEOUT_1000MS);
		if (OK != ret )
		{
	...
	case HISI_EFUSE_READ_AUTHKEY:
		...
		bytes = GET_BYTE_NUM(bits_width);
		ret = get_efuse_authkey_value(efuse_buf, bytes, EFUSE_MAILBOX_TIMEOUT_1000MS);
		if (OK != ret)
		{
	...
	case HISI_EFUSE_WRITE_AUTHKEY:
		...
		ret = set_efuse_authkey_value(efuse_buf, bytes, EFUSE_MAILBOX_TIMEOUT_1000MS);
		if (OK != ret)
		{
	...
	case HISI_EFUSE_READ_DEBUGMODE:
		...
		bytes = GET_BYTE_NUM(bits_width);
		ret = get_efuse_securitydebug_value(efuse_buf, bytes, EFUSE_MAILBOX_TIMEOUT_1000MS);
		if (OK != ret)
		{
	...
	case HISI_EFUSE_WRITE_DEBUGMODE:
		...
		ret = set_efuse_securitydebug_value(efuse_buf, EFUSE_MAILBOX_TIMEOUT_1000MS);
		if (OK != ret)
		{
    ...
    case HISI_EFUSE_READ_DIEID:
		...
		bytes = GET_BYTE_NUM(bits_width);
        ret = get_efuse_dieid_value(efuse_buf, bytes, EFUSE_MAILBOX_TIMEOUT_1000MS);
        if (OK != ret)
		{
	...
	case HISI_EFUSE_TEST_READ_CHIPID:
	    GET_BITS_WIDTH(bits_width, efuse_mem_attr_chipid);
		bytes = GET_BYTE_NUM(bits_width);
		ret = bsp_efuse_read(efuse_buf, EFUSE_CHIPID_GROUP_START, EFUSE_CHIPID_GROUP_WIDTH);
		if (OK != ret )
		{
    ...
    case HISI_EFUSE_TEST_READ_DIEID:
	    GET_BITS_WIDTH(bits_width, efuse_mem_attr_dieid);
		bytes = GET_BYTE_NUM(bits_width);
        ret = bsp_efuse_read(efuse_buf, EFUSE_DIEID_GROUP_START, EFUSE_DIEID_GROUP_WIDTH);
        if (OK != ret)
		{
            efuse_print_info(log_level_error, "%s: %d: efusec_ioctl: bsp_efuse_read failed.\n", __func__, __LINE__);
            break;
      ...
      case HISI_EFUSE_TEST_READ_KCE:
        ret = bsp_efuse_read(efuse_buf, EFUSE_KCE_GROUP_START, EFUSE_KCE_GROUP_WIDTH);
        if (OK != ret)
		{
            efuse_print_info(log_level_error, "%s: %d: efusec_ioctl: bsp_efuse_read failed.\n", __func__, __LINE__);
            break;
        ...
        /*send back to user*/
        if (copy_to_user(argp, efuse_buf, EFUSE_KCE_GROUP_WIDTH << 2))
		{
            ret = -EFAULT;
        }
    ...
    case HISI_EFUSE_TEST_WRITE_KCE:
        /*get data from user*/
		if (copy_from_user(efuse_buf, argp, EFUSE_KCE_GROUP_WIDTH << 2))
		{
			ret = -EFAULT;
			break;
		}
        ret = bsp_efuse_write(efuse_buf, EFUSE_KCE_GROUP_START, EFUSE_KCE_GROUP_WIDTH);
        if (OK != ret)
		{
    ...
    case HISI_EFUSE_READ_THERMAL:
		...
		bytes = GET_BYTE_NUM(bits_width);
		ret = get_efuse_thermal_value(efuse_buf, bytes, EFUSE_MAILBOX_TIMEOUT_1000MS);
		if (OK != ret)
		{
    ...
    case HISI_EFUSE_TEST_DISPLAY:
        ...
        ret = test_efuse_display(efuse_buf, bytes, EFUSE_MAILBOX_TIMEOUT_1000MS);
        if (OK != ret)
        {
    ...
    case HISI_EFUSE_WRITE_DJTAGDEBUG:
		...
		ret = set_efuse_djtagvalue_value(efuse_buf, EFUSE_MAILBOX_TIMEOUT_1000MS);
		if (OK != ret)
		{
    ...
    case HISI_EFUSE_READ_DJTAGDEBUG:
		ret = get_efuse_djtagvalue_value(efuse_buf, EFUSE_MAILBOX_TIMEOUT_1000MS);
		if (OK != ret)
		{
		...
		/*send back to user*/
		if (copy_to_user(argp, efuse_buf, sizeof(unsigned int)))
		{
			ret = -EFAULT;
		}
        break;
        ...
        case HISI_EFUSE_WRITE_SLTFINISHFLAG:
            ...
            ret = set_efuse_sltfinishflag_value(efuse_buf, bytes, EFUSE_MAILBOX_TIMEOUT_1000MS);
            if (OK != ret )
            {
        ...
        case HISI_EFUSE_READ_SLTFINISHFLAG:
            GET_BITS_WIDTH(bits_width, efuse_mem_attr_sltfinishflag);
            bytes = GET_BYTE_NUM(bits_width);
            ret = get_efuse_sltfinishflag_value(efuse_buf, bytes, EFUSE_MAILBOX_TIMEOUT_1000MS);
            if (OK != ret )
            {
...
int efuse_test_debug(unsigned int test_case)
{
    ...
    case 1:
        ...
        break;
    case 2:
        ret = get_efuse_carrierid_value(efuse_buf, 2, EFUSE_MAILBOX_TIMEOUT_1000MS);
...
void hi6xxx_smc_start_work(u64 a0)
{
    ...
    schedule_delayed_work_on(0,&efuse_monitor_work, msecs_to_jiffies(EFUSE_MAILBOX_TIMEOUT_1000MS));
    return;
}
...
#ifdef CONFIG_ATFDRIVER
	invoke_efuse_fn = atfd_hisi_service_efusec_smc;
#ifdef HISI_EFUSE_DEBUG
 ...
 * block allocated by dma_alloc_coherent.*/
#ifndef CONFIG_ATFDRIVER
    efuse_buf = (unsigned char *)kmalloc((g_efuse_group_max << 2) * 2, GFP_KERNEL);
#else
    ...
    g_phy_addr = bl31_smem_base + data[0];
    efuse_buf = (unsigned char *)ioremap(bl31_smem_base + data[0], data[1]);
#endif
    if(NULL == efuse_buf) {
        ret = -EFAULT;
	...
	efuse_print_info(log_level_error, "g_phy_addr=0x%lx, efuse_buf=0x%lx, efuse init success", (unsigned long)g_phy_addr, (unsigned long)efuse_buf);
