...
#endif
	/* set global variable */
	(VOS_VOID)VOS_MemSet(&gbl_drx_cur_vote_status, 0, sizeof(drx_msp_debug_vote_s));
}
...
VOS_VOID drx_msp_fid_vote_lock(drx_msp_fid_vote_e e_drx_vote)
{
	...
	if(0 ==g_drx_vote_sem)
	{
	    ...
	    {
	    	diag_printf("%s: create sem failed!\n",__FUNCTION__);
	        return;
	    }
	}
	...
	{
		diag_printf("%s: e_drx_vote si too larger!\n",__FUNCTION__);/* [false alarm]:alarm */
		return ;
	}
...
{
	diag_printf("----drx debug info begin: \n");
	diag_printf("gbl_drx_msp_vote_status is 0x%x!\n",gbl_drx_msp_vote_status);
    diag_printf("drx_msp_cur_vote_status is 0x%x!\n",gbl_drx_cur_vote_status.drx_msp_cur_vote_status);
	diag_printf("----drx debug info end!\n");
}
