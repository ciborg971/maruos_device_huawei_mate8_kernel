...
{
    {SCM_CHANNEL_UNINIT, SOCP_CODER_SRC_LOM_IND2, SOCP_CODER_DST_OM_IND,   SOCP_DATA_TYPE_0, SOCP_ENCSRC_CHNMODE_CTSPACKET, SOCP_CHAN_PRIORITY_3, SCM_CODER_SRC_BDSIZE, SCM_CODER_SRC_RDSIZE, VOS_NULL_PTR, VOS_NULL_PTR, VOS_NULL_PTR, VOS_NULL_PTR},
...
SCM_CODER_SRC_CFG_STRU      g_astSCMCoderSrcCfg[SCM_CODER_SRC_NUM]=
{
    {SCM_CHANNEL_UNINIT, SOCP_CODER_SRC_LOM_CNF1, SOCP_CODER_DST_OM_CNF,   SOCP_DATA_TYPE_0, SOCP_ENCSRC_CHNMODE_CTSPACKET, SOCP_CHAN_PRIORITY_3, SCM_CODER_SRC_BDSIZE, SCM_CODER_SRC_RDSIZE, VOS_NULL_PTR, VOS_NULL_PTR, VOS_NULL_PTR, VOS_NULL_PTR},
...
#ifdef SCM_ACORE
SCM_CODER_DST_DEBUG_STRU g_astScmCoderDstDebugInfo[SCM_CODER_DST_NUM] = {
        {"DST_CNF",0,0,0},
        {"DST_IND",0,0,0}
};
#endif
...
VOS_UINT32 SCM_CoderDstChanMemInit(VOS_VOID)
{
    ...
    /* 非延迟写入下的处理 */
    if ((VOS_TRUE != stLogCfg.logOnFlag)
    ...
    g_astSCMCoderDstCfg[SCM_CODER_DST_IND_CHANNEL].pucBufPHY    = (VOS_UINT8*)((VOS_UINT_PTR)stLogCfg.ulPhyBufferAddr);
...
{
    static VOS_UINT32 ulTotal = 0;
    static VOS_UINT32 ulSlice = 0;

    if(u32Event == SOCP_EVENT_OUTBUFFER_OVERFLOW)
    {
        ulTotal ++;
        if((VOS_GetSlice() - ulSlice) > 327680)/*超过10s上报一次*/
        {
            DIAG_TRACE("DST Channel %s OverFlow %d times!\n",((ulDstChID&0xff) == 0)?"CNF":"IND",ulTotal);
...
VOS_VOID SCM_CoderDstSpeedInd(SOCP_CODER_DST_ENUM_U32  enChanID)
{
    ...
    if (SOCP_CODER_DST_OM_CNF == enChanID)
    {
        ...
        ulCfgNum     = SCM_CODER_TYPE_CNF;
    }
    else
    {
        pstDebugInfo = &g_stAcpuDebugInfo.stIndDebugInfo;
        ulCfgNum     = SCM_CODER_TYPE_IND;
    }

    ulSlice     = VOS_GetSlice() - g_astScmCoderDstDebugInfo[ulCfgNum].ulLastSlice;
    ulLength    = pstDebugInfo->ulUSBSendLen - g_astScmCoderDstDebugInfo[ulCfgNum].ulSendPacketLength;
    ulPacket    = pstDebugInfo->ulUSBSendNum - g_astScmCoderDstDebugInfo[ulCfgNum].ulSendPacketNum;
    if((ulSlice > DIAG_DEBUG_TXT_LOG_DELAY)&&(g_astScmCoderDstDebugInfo[ulCfgNum].ulLastSlice))
    {
        DIAG_TRACE("%s,Packet:%d,Length:%d,Slice:%d\n",g_astScmCoderDstDebugInfo[ulCfgNum].pucChanName,ulPacket,ulLength,ulSlice);
        g_astScmCoderDstDebugInfo[ulCfgNum].ulLastSlice         = VOS_GetSlice();
        g_astScmCoderDstDebugInfo[ulCfgNum].ulSendPacketLength  = pstDebugInfo->ulUSBSendLen;
        g_astScmCoderDstDebugInfo[ulCfgNum].ulSendPacketNum     = pstDebugInfo->ulUSBSendNum;
    }
    if(!(g_astScmCoderDstDebugInfo[ulCfgNum].ulLastSlice))
    {
        g_astScmCoderDstDebugInfo[ulCfgNum].ulLastSlice = VOS_GetSlice();
    }
}
