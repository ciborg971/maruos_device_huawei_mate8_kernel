...
typedef struct __LPHY_REQ_TIMING_CONST_ADVANCE_STRU__
{
    ...
    SINT16                       sAdvance;        /*将初始化搜索的结果强制提前多少个(0.25Ts)*/
...
typedef struct __TIMING_NOISE_STRU__
{
	...
	SINT16     sTotalGaindB[LPHY_MAX_RX_ANTENNA_NUM];     //子帧的AGC总增益;
	UINT32     ulInitFlag;
...
typedef struct __TIMING_EST_STRU__ /*estimate*/
{
    ...
    SINT16                          sDelta;             /*最近一次的定时偏差估计值,单位:0.5Ts*/
    SINT16                          usTrefParThreshold; /*进行提前量折算的PAR域值*/
...
typedef struct __TIMING_STT_STRU__
{
    /*外部模块触发的事件*/
    TIMING_EVT_ENUM_UINT32          enEvents;           /*输入定时跟踪事件*/
    SINT16                          sResetCompDelta;    /*解调定时突变导致系统定时调整量0.25Ts*/
    UINT16                          usDemCounter;       /*解调定时跟踪输出计数*/
    SINT32                          lSingleIOAcc;       /*单入单出累加器*/
    SINT32                          lSysTimeAccValue;   /*系统定时调整累积和值*/
    SINT32                          lLastAvgAcc;        /*系统定时累和平均值*/
    SINT32                          lLastCompsent;      /*累和平均置减去单入单出累和值剩下的补偿值*/
    SINT32                          lLastRemainder;     /*上一次剩下的调整量*/

    SINT32                          lAdjustValue;       /*系统定时调整量,0.5TS单位*/
...
typedef struct __TIMING_DTA_STRU__
{
    ...
    /*通过原始值算出来的给AFC的调整量*/
    SINT32                          lNormalTimeAdjust;        /*单播调整量*/
...
typedef struct __TIMING_MBMS_DTA_STRU__
{
    ...
    /*通过原始值算出来的给AFC的调整量*/
    SINT32                          lMbmsTimeAdjust;          /*多播调整量*/
    SINT32                          lAdjustValue;             /*给AFC的调整量*/
...
typedef struct __TIMING_MODULE_STRU__
{
	...
	*/
    ...
    TIMING_STT_STRU                 stStt[LPHY_MAX_UL_CARRIER_NUM];              /*系统定时跟踪*/
    TIMING_STA_STRU                 stSta[LPHY_MAX_UL_CARRIER_NUM];              /*系统定时调整*/
...
typedef struct __TIMING_MODULE_SIG_QUALITY_STRU__
{
    ...
    SINT32                          lCurFOAdjHz;          /* 当前频偏调整量 */
    SINT32                          lMCurFOAdjHz;         /* 当前频偏调整量 */
    SINT32                          lAccFOHz;             /* 频偏调整量累加值 */
    SINT32                          lMAccFOHz;            /* 频偏调整量累加值 */
