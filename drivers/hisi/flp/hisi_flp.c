...
typedef struct flp_device {
    ...
    unsigned int            pdr_update_config ;
...
typedef struct flp_port {
    ...
    unsigned int            discard_flag ;  /*when multiple so start to get data,just discard history data*/
}flp_port_t;
...
extern unsigned long g_flp_pdr_awake_flag;
...
static ssize_t  flp_read_cmd(flp_port_t *flp_port, char __user *buf, size_t count,unsigned int type)
{
    ...
    if((pdata->read_index + len)>  pdata->buf_size){
        ret = copy_to_user(buf,pdata->data_buf + pdata->read_index ,pdata->buf_size - pdata->read_index);
...
static int  flp_stop_cmd(flp_port_t *flp_port, unsigned int timeout,unsigned int  type)
{
    ...
    if(FLP_PDR_DATA == type){
        ...
        if(0 == g_flp_dev.pdr_start_count) {
            ...
            g_flp_dev.pdr_update_config = TRUE;
...
static int  flp_start_cmd(flp_port_t *flp_port, const char __user *buf,
                                 size_t len,unsigned int  type)
{
    ...
    if(FLP_PDR_DATA == type){
        ...
        if(flp_port->pdr_config.report_interval/flp_port->pdr_config.report_precise >PDR_DATA_MAX_COUNT ){
            printk(KERN_ERR "flp_start_cmd error  line[%d]\n",__LINE__);
            return -EINVAL;
        }
...
static int  flp_update_cmd(flp_port_t *flp_port, const char __user *buf,
                                 size_t len,unsigned int  type)
{
    unsigned int count;   /* [false alarm]:fortify */
    struct list_head    *pos;
...
static void copy_data_to_buf(flp_data_buf_t * pdata,char * data,unsigned int len,unsigned int align)
{
    unsigned int deta;
...
static int get_pdr_data_from_mcu(const pkt_header_t *head)
{
    ...
    for (j=0;j<count ; j++) {
        pevent = data + j ;
        pevent->time.msec = syscounter_to_utc(pevent->time.msec) ;
#if SAVE_PDR_DATA_TO_FTRACE
    ...
    list_for_each(pos,&g_flp_dev.list){
        ...
        if(flp_port->discard_flag){
            if(count < PDR_SINGLE_SEND_COUNT) {
                flp_port->discard_flag = 0;
            }
            continue ;
        }
        printk(HISI_FLP_DEBUG "rate[%d],interval[%d],nextid[%d],aquiredid[%d]\n",
                flp_port->rate,flp_port->interval,flp_port->nextid,flp_port->aquiredid);
        pdata = &flp_port->pdr_buf ;
        printk(HISI_FLP_DEBUG "count[%d],readindex[%d],writeindex[%d]\n",pdata->data_count,
                                                        pdata->read_index,pdata->write_index);
        mutex_lock(&pdata->data_mutex);
...
#if 0
        if ((flp_port->pdr_config.report_times) &&
            (flp_port->nextid/flp_port->pdr_config.report_interval ==
            flp_port->pdr_config.report_times)){
            flp_port->channel_type &= ~FLP_PDR_DATA;
        }
#endif
        if((g_flp_dev.pdr_update_config) &&(count < PDR_SINGLE_SEND_COUNT)){
            flp_port->rate = flp_port->pdr_config.report_precise/g_flp_dev.pdr_config.report_precise ;
            flp_port->interval = flp_port->pdr_config.report_interval/g_flp_dev.pdr_config.report_precise ;
            flp_port->nextid *= flp_port->rate;
        }
        mutex_unlock(&pdata->data_mutex);
        printk(HISI_FLP_DEBUG "%s %d: len:%d\n", __func__, flp_port->port_type,flp_port->pdr_buf.data_count);
    }
    /*short package indecate history date sending complete*/
    if((g_flp_dev.pdr_update_config) &&(count < PDR_SINGLE_SEND_COUNT)){
        g_flp_dev.pdr_update_config = 0;
    }
...
static int get_ar_data_from_mcu(const pkt_header_t *head)
{
    ...
    for(j=0; j<count; j++){
        pevent = data + j ;
        pevent->time.msec = syscounter_to_elapsetime(pevent->time.msec) ;
#if SAVE_PDR_DATA_TO_FTRACE
    ...
    list_for_each(pos,&g_flp_dev.list){
        ...
        for(j=0; j<count; j++){
            ...
            if((flp_port->ar_config.activity_list[pevent->activity].event_type) &&
                ...
                copy_data_to_buf(pdata,(char *)pevent,sizeof(ar_activity_event_t),sizeof(ar_activity_event_t));
            }
        }
        mutex_unlock(&pdata->data_mutex);
        printk(HISI_FLP_DEBUG "%s %d: len:%d\n", __func__, flp_port->port_type,flp_port->ar_buf.data_count);
    }
    mutex_unlock(&g_flp_dev.lock);
    return len;
}
...
static long flp_pdr_ioctl(flp_port_t *flp_port, unsigned int cmd, unsigned long arg)
{
    ...
    switch (cmd){
        ...
        case FLP_IOCTL_PDR_START(1) :
            printk(HISI_FLP_DEBUG "%s 0x%x: start\n", __func__, cmd);

            if(cmd == FLP_IOCTL_PDR_START(0)){
            ...
            mutex_lock(&g_flp_dev.lock);
            flp_start_cmd(flp_port,(char __user *)arg,sizeof(pdr_start_config_t),FLP_PDR_DATA);
        ...
        case FLP_IOCTL_PDR_STOP(1) :
            printk(HISI_FLP_DEBUG "%s 0x%x: stop\n", __func__, cmd);
            if(!flp_port->pdr_buf.data_buf) {
        ...
        case FLP_IOCTL_PDR_WRITE(1) :
            printk(HISI_FLP_DEBUG "%s %d: write\n", __func__, __LINE__);
        ...
        case FLP_IOCTL_PDR_READ(1) :
            printk(HISI_FLP_DEBUG "%s %d: read\n", __func__, __LINE__);
            ret = copy_from_user(&para, (void *)arg,sizeof(buf_info_t));
            if(ret) {
                printk(KERN_ERR "flp_ioctl copy_from_user error[%ld]\n",ret);
                return -EFAULT;
        ...
        default :
            printk(KERN_ERR "flp_ioctrl input cmd[0x%x] error\n", cmd);
            return -EFAULT;
    }
...
static long flp_ar_ioctl(flp_port_t *flp_port, unsigned int cmd, unsigned long arg)
{
    ...
    switch (cmd){
        ...
        case FLP_IOCTL_AR_START(2) :
            ...
            if(cmd == FLP_IOCTL_AR_START(0)){
                flp_port->port_type = FLP_TAG_FLP ;
            } else if (cmd == FLP_IOCTL_AR_START(1)) {
                flp_port->port_type = FLP_TAG_GPS ;
            } else {
            ...
            mutex_lock(&g_flp_dev.lock);
            flp_start_cmd(flp_port,(char __user *)arg,sizeof(ar_start_config_t),FLP_AR_DATA);
            mutex_unlock(&g_flp_dev.lock);
            break ;
        ...
        case FLP_IOCTL_AR_UPDATE(2) :
            printk(HISI_FLP_DEBUG "%s %d: update\n", __func__, __LINE__);
            mutex_lock(&g_flp_dev.lock);
            ret = flp_update_cmd(flp_port,(char __user *)arg,sizeof(ar_start_config_t),FLP_AR_DATA);
            mutex_unlock(&g_flp_dev.lock);
            break ;
        ...
        case FLP_IOCTL_AR_FLUSH(2) :
            printk(HISI_FLP_DEBUG "%s %d: flush\n", __func__, __LINE__);
            send_cmd_from_kernel(TAG_AR,CMD_CMN_CONFIG_REQ,CMD_FLP_AR_FLUSH_REQ,NULL,0);
            break ;
        default :
...
static long flp_common_ioctl(flp_port_t *flp_port, unsigned int cmd, unsigned long arg)
{
    ...
    switch(cmd) {
        ...
        case FLP_IOCTL_COMMON_SLEEP :
            printk(HISI_FLP_DEBUG "%s %d: sleep\n", __func__, __LINE__);
            if (!(flp_port->channel_type )) {
                return -EFAULT;
            }
            ret = copy_from_user(&data, (void *)arg,sizeof(unsigned int));
            if(ret) {
            ...
            if (timer_pending(&flp_port->sleep_timer))
            {
                del_timer(&flp_port->sleep_timer);
            }
            mod_timer(&flp_port->sleep_timer,jiffies + msecs_to_jiffies(data));
            break ;

        case FLP_IOCTL_COMMON_AWAKE_RET :
            printk(HISI_FLP_DEBUG "%s %d: awake_ret\n", __func__, __LINE__);
            ret = copy_to_user( (void *)arg, &g_flp_pdr_awake_flag,sizeof(unsigned int));
            if(ret) {
                printk(KERN_ERR "flp_ioctl copy_to_user error[%ld]\n",ret);
                return -EFAULT;
            }
            g_flp_pdr_awake_flag = 0;
            break ;
        default :
            printk(KERN_ERR "flp_ar_ioctrl input cmd[0x%x] error\n", cmd);
            return -EFAULT;
...
static int flp_open(struct inode *inode, struct file *filp)
{
    ...
    setup_timer(&flp_port->sleep_timer,flp_sleep_timeout,0);
    mutex_lock(&g_flp_dev.lock);
...
int  hisi_flp_register(void)
{
    ...
    printk(HISI_FLP_DEBUG "%s %d: regster tag[%d] req[%d]\n", __func__, __LINE__, TAG_PDR,CMD_FLP_PDR_DATA_REQ);
