...
/*************************************hifi系统ipc******************************************/
#if  defined(CONFIG_HIFI_IPC_3650) || defined(CONFIG_HIFI_IPC_6250)
...
static void hisi_hifi_default_complete (rproc_msg_t *ack_buffer,
				rproc_msg_len_t ack_buffer_len,
				int error,
				void *data)
{
	if(error){
 		 pr_err("hisi_hifi_default_complete: error = %d\n", error);
	}
	return ;
...
#endif	
		ret = RPROC_ASYNC_SEND(rproc_id, (mbox_msg_t *)ipcMsg, 2, ASYNC_MSG, hisi_hifi_default_complete, NULL);
		if(ret)
		  {
