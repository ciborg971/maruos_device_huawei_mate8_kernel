...
#define TELE_MNTN_AREA_ADDR                                 (HISI_RESERVED_LPMX_CORE_PHYMEM_BASE)
...
#define TELE_MNTN_GLOBALLOCK_INIT(lock, val)                do{lock = (val);}while(0)
#define TELE_MNTN_GLOBAL_LOCK(lock, flag, ret)              do{ret = hwspin_lock_timeout((lock), TELE_MNTN_LOCK_TIMEOUT);}while(0)
#define TELE_MNTN_CHECK_GLOBAL_LOCK_RET(ret, cmd, out)      do{ \
                                                                if(ret) \
                                                                { \
                                                                    cmd; \
...
#define TELE_MNTN_GLOBAL_UNLOCK(lock, flag)                 hwspin_unlock(lock)
...
#define TELE_MNTN_PHY_TO_VIRT(addr, size)                   ((u8_t *)(addr))/*((u8_t *)DRV_PHY_TO_VIRT(addr))*/
