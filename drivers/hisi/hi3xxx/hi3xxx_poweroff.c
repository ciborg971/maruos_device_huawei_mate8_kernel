...
#endif
    if (g_bbox_fpga_flag == 0) {
        tmp = hisi_pmic_reg_read(PMIC_HRESET_PWRDOWN_CTRL_ADDR(0));
        if (!(tmp & 0x1)) {
            printk(KERN_ERR "[%s], PMIC_HRESET_PWRDOWN_CTRL_ADDR is 0x%x\n", __FUNCTION__, tmp);
            hisi_pmic_reg_write(PMIC_HRESET_PWRDOWN_CTRL_ADDR(0), (tmp | 0x1));
        }
    }

    writel(0x1<<31, (char*)SOC_SCTRL_SCPEREN1_ADDR(sysctrl_base)); /*DDR bypass*/
    while(1) {
        writel(0xdeadbeef, (char*)SOC_SCTRL_SCSYSSTAT_ADDR(sysctrl_base));
        value = readl((char*)SOC_SCTRL_SCPERCLKEN1_ADDR(sysctrl_base)) & (0x1<<31);
        printk(KERN_ERR "[%s], 0xdeadbeef write fail , value = 0x%x\n", __FUNCTION__, value);
    }
}
