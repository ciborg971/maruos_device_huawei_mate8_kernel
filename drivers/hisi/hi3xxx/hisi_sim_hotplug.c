...
#define HI6421V500_PMU_STATUS1              (0x007) /* PMU STATUS1 */
#define HI6421V500_PMU_LDO11_ONOFF_ECO      (0x06E) /* PMU LDO11_ONOFF_ECO */
#define HI6421V500_PMU_LDO11_VSET           (0x06F) /* PMU LDO11_VSET */
#define HI6421V500_PMU_LDO12_ONOFF_ECO      (0x070) /* PMU LDO11_ONOFF_ECO */
#define HI6421V500_PMU_LDO12_VSET           (0x071) /* PMU LDO11_VSET */
#define HI6421V500_PMU_SIM_CTRL             (0x0DE) /* PMU SIM_CTRL */
#define HI6421V500_PMU_SIM_DEB_CTRL         (0x0DF) /* PMU SIM_DEB_CTRL */
#define HI6421V500_PMU_IRQ_MASK3            (0x0E4) /* PMU IRQ_MASK_3 */
#define HI6421V500_PMU_IRQ3                 (0x123) /* PMU IRQ3 */

//Next lines are sim related regitster value
#define LDO11_ONOFF_VALUE                   (0x02)
#define LDO11_VSET_VALUE                    (0x01)
#define LDO12_ONOFF_VALUE                   (0x02)
#define LDO12_VSET_VALUE                    (0x01)

#define HI6421_IRQ_LDO11_ADDR               (0x6E)
#define HI6421_IRQ_LDO12_ADDR               (0x70)
...
static void sim_pmu_hpd_read(void)
{
    u32 pmu_status1         = 0;
    u32 pmu_irq3            = 0;
    u32 pmu_irq_mask3       = 0;
    ...
    pmu_status1         = READ_PMU_SIM(HI6421V500_PMU_STATUS1);
    pmu_irq3            = READ_PMU_SIM(HI6421V500_PMU_IRQ3);
    pmu_irq_mask3       = READ_PMU_SIM(HI6421V500_PMU_IRQ_MASK3);
    pmu_sim_ctrl        = READ_PMU_SIM(HI6421V500_PMU_SIM_CTRL);
    pmu_sim_deb_ctrl    = READ_PMU_SIM(HI6421V500_PMU_SIM_DEB_CTRL);
    pmu_ldo11_onoff     = READ_PMU_SIM(HI6421V500_PMU_LDO11_ONOFF_ECO);
    pmu_ldo11_vset      = READ_PMU_SIM(HI6421V500_PMU_LDO11_VSET);
    pmu_ldo12_onoff     = READ_PMU_SIM(HI6421V500_PMU_LDO12_ONOFF_ECO);
    pmu_ldo12_vset      = READ_PMU_SIM(HI6421V500_PMU_LDO12_VSET);
...
static void sim_pmu_hpd_write(struct hisi_sim_hotplug_info *info)
{
    ...
    WRITE_PMU_SIM(info->hpd_interrupt_to_close_ldo, HI6421V500_PMU_SIM_CTRL);

    // set debounce waiting time
    WRITE_PMU_SIM(info->hpd_debounce_wait_time, HI6421V500_PMU_SIM_DEB_CTRL);

    // enable PMU, set 1.8v by default
    WRITE_PMU_SIM(LDO11_ONOFF_VALUE,    HI6421V500_PMU_LDO11_ONOFF_ECO);
    WRITE_PMU_SIM(LDO11_VSET_VALUE,     HI6421V500_PMU_LDO11_VSET);
    WRITE_PMU_SIM(LDO12_ONOFF_VALUE,    HI6421V500_PMU_LDO12_ONOFF_ECO);
    WRITE_PMU_SIM(LDO12_VSET_VALUE,     HI6421V500_PMU_LDO12_VSET);
}
...
static void update_sim_state_info(int sim_id, int sim_pluged)
{
    if (SIM0 == sim_id)
    {
        ...
        {
            sim0_hotplug_state_store(sim_kobj, attrs[SIM0], STR_SIM_CARD_IN, strlen(STR_SIM_CARD_IN));
        }
        else
        {
            sim0_hotplug_state_store(sim_kobj, attrs[SIM0], STR_CLEAR_SIM_CARD_STATE, strlen(STR_CLEAR_SIM_CARD_STATE));
        }
    }
    else if (SIM1 == sim_id)
    {
        if (SIM_CARD_IN == sim_pluged)
        {
            sim1_hotplug_state_store(sim_kobj, attrs[SIM1], STR_SIM_CARD_IN, strlen(STR_SIM_CARD_IN));
        }
        else
        {
            sim1_hotplug_state_store(sim_kobj, attrs[SIM1], STR_CLEAR_SIM_CARD_STATE, strlen(STR_CLEAR_SIM_CARD_STATE));
        }
    }
    else
    {
...
static irqreturn_t sim_hpd_irq_handler(int irq, void *data)
{
    ...
    {
        /* close ldo11 */
        WRITE_PMU_SIM(HI6421_IRQ_LDO11_VALUE, HI6421_IRQ_LDO11_ADDR);
        LOGI("%s: IRQ HPD0 happend, software close ldo11.\n", __func__);
    }
    else if (SIM1 == info->sim_id)
    {
        /* close ldo12 */
        WRITE_PMU_SIM(HI6421_IRQ_LDO12_VALUE, HI6421_IRQ_LDO12_ADDR);
