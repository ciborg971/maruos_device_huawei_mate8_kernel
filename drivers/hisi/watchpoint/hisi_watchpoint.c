			      ...
			      size_t count, loff_t *ppos)
{
	...
	pt_kn_buf +=
	    snprintf(pt_kn_buf, WP_DBG_SIZE_PAGE, "s addr size(4) rw(1-3)\n");
	pt_kn_buf +=
	    snprintf(pt_kn_buf, WP_DBG_SIZE_PAGE, "    set watchpoint\n");
	pt_kn_buf +=
	    snprintf(pt_kn_buf, WP_DBG_SIZE_PAGE, "    size: default is 4\n");
	pt_kn_buf +=
	    snprintf(pt_kn_buf, WP_DBG_SIZE_PAGE,
		     "    rw   : 1-r, 2-w, 3-rw\n");
	pt_kn_buf += snprintf(pt_kn_buf, WP_DBG_SIZE_PAGE, "c addr\n");
	pt_kn_buf +=
	    snprintf(pt_kn_buf, WP_DBG_SIZE_PAGE, "    clear watchpoint\n");
			       ...
			       size_t count, loff_t *ppos)
{
	...
	byte_writen =
	    simple_write_to_buffer(kn_buf, WP_DBG_SIZE_PAGE, ppos, user_buf,
				   count);
			       ...
			       size_t count, loff_t *ppos)
{
	...
	pt_kn_buf +=
	    snprintf(pt_kn_buf, WP_DBG_SIZE_PAGE, "      malloc memory\n");
	pt_kn_buf +=
	    snprintf(pt_kn_buf, WP_DBG_SIZE_PAGE,
		     "      size  : size in byte\n");
	pt_kn_buf +=
	    snprintf(pt_kn_buf, WP_DBG_SIZE_PAGE, "      example: m 4\n");
	pt_kn_buf += snprintf(pt_kn_buf, WP_DBG_SIZE_PAGE, "f addr\n");
	pt_kn_buf +=
	    snprintf(pt_kn_buf, WP_DBG_SIZE_PAGE, "      free memory\n");
	pt_kn_buf +=
	    snprintf(pt_kn_buf, WP_DBG_SIZE_PAGE,
		     "      addr  : in hex, such as 0x..\n");
	pt_kn_buf +=
	    snprintf(pt_kn_buf, WP_DBG_SIZE_PAGE, "      example: f 0x..\n");
			       ...
			       size_t count, loff_t *ppos)
{
	...
	if (tget_mem_addr != NULL) {
		pt_kn_buf +=
		    snprintf(pt_kn_buf, WP_DBG_SIZE_PAGE, "addr(0x%p) = 0x%x\n",
	...
	pt_kn_buf +=
	    snprintf(pt_kn_buf, WP_DBG_SIZE_PAGE, "    example: w 30\n");
...
static int __init hw_break_module_init(void)
{
	...
	/* status */
	if (!debugfs_create_file("stat", 0660, wp_df_dir, NULL,
						&wp_state_fops)) {
	...
	/* set */
	if (!debugfs_create_file("cfg", 0660, wp_df_dir, NULL,
						&wp_config_fops)) {
	...
	/* tst malloc */
	if (!debugfs_create_file("tm", 0660, wp_df_dir, NULL,
						&wp_tgetm_fops)) {
	...
	/* tst access */
	if (!debugfs_create_file("ta", 0660, wp_df_dir, NULL,
						&wp_taccess_fops)) {
