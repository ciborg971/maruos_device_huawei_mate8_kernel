...
#if defined(CONFIG_ARCH_HI6250) //fixme: hi6250
...
static void slimbus_dump_state(slimbus_dump_state_t type)
{
	...
	case SLIMBUS_DUMP_LOSTMS:
		state[0] = readl((void __iomem *)(general_cfg.regBase + 0x24));
		state[1] = readl((void __iomem *)(general_cfg.regBase + 0x28));
		state[2] = readl((void __iomem *)(asp_base_reg + 0x8));
		state[3] = readl((void __iomem *)(asp_base_reg + 0x14));
		state[4] = readl((void __iomem *)(asp_base_reg + 0x18));
		state[5] = readl((void __iomem *)(asp_base_reg + 0x20));
		state[6] = readl((void __iomem *)(asp_base_reg + 0x38));

		SLIMBUS_DEV_LIMIT_INFO("0x24:%#x, 0x28:%#x\n", state[0], state[1]);
