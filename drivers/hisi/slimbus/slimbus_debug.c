...
extern int  slimbus_hi6402_read_1byte(
				void                   *pdata,
				unsigned int            reg);
extern int  slimbus_hi6402_read_4byte(
				void                   *pdata,
				unsigned int            reg);
extern void slimbus_hi6402_write_1byte(
				void                   *pdata,
				unsigned int            reg,
				unsigned int            val);
extern void slimbus_hi6402_write_4byte(
				void                   *pdata,
				unsigned int            reg,
				unsigned int            val);
extern void slimbus_hi6402_read_pageaddr();
				 ...
				 const char *buf, size_t size)
{
	...
	switch (simgdown) {
		...
		case 2:
			...
			slimbus_hi6402_write_1byte(NULL, 0x20007040, 0x3);
			slimbus_hi6402_write_1byte(NULL, 0x20007214, 0xff);
			slimbus_hi6402_write_1byte(NULL, 0x200072f2, 0x3);
			slimbus_hi6402_write_1byte(NULL, 0x20007045, 0xff);
			slimbus_hi6402_write_1byte(NULL, 0x20007052, 0x50);
			slimbus_hi6402_write_4byte(NULL, 0x20000810, 0x200);
			slimbus_hi6402_write_4byte(NULL, 0x20000814, 0x20010000);
			slimbus_hi6402_write_4byte(NULL, 0x20000818, 0x10000000);
			slimbus_hi6402_write_4byte(NULL, 0x2000081c, 0x47711007);
			slimbus_hi6402_write_1byte(NULL, 0x200073d8, 0x1);
			slimbus_hi6402_write_1byte(NULL, 0x20007031, 0x11);

			ret = slimbus_bus_configure(SLIMBUS_BUS_CONFIG_IMGDOWN);
			break;
		case 3:
			printk(KERN_EMERG"[%s:%d] configure and active slimbus channel!\n", __FUNCTION__, __LINE__);
			ret = slimbus_track_configure(SLIMBUS_DEVICE_HI6402, SLIMBUS_TRACK_IMAGE_LOAD, NULL);
			ret = slimbus_track_activate(SLIMBUS_DEVICE_HI6402, SLIMBUS_TRACK_IMAGE_LOAD);
	...
	if (sregpagerd >= 0x10000000 && sregpagerd <= 0x20008000) {
		...
		if ((sregpagerd >= 0x10000000 && sregpagerd <= 0x10001000)
			...
			for (i=0; i<240; i+=16) {
				ret += sprintf(buf+ret, " %4x %4x %4x %4x \n",
							slimbus_hi6402_read_4byte(NULL, sregpagerd+i), slimbus_hi6402_read_4byte(NULL, sregpagerd+i+4),
							slimbus_hi6402_read_4byte(NULL, sregpagerd+i+8), slimbus_hi6402_read_4byte(NULL, sregpagerd+i+12));
				msleep(20);
			}
		}
		...
		if (sregpagerd >= 0x20007000 && sregpagerd <= 0x20008000) {
			...
			for (j=0; j<=0x1ff; j+=16) {
				for (i=j; i<j+16; i++) {
					ret += sprintf(buf+ret, " %3x ",slimbus_hi6402_read_1byte(NULL, sregpagerd+i));
				}
				ret += sprintf(buf+ret, "\n");
			}
		}
				 ...
				 char *buf)
{
	...
	if (sregrd >= 0x20007000 && sregrd < 0x20008000) {
		gregvalue = slimbus_hi6402_read_1byte(SLIMBUS_DEVICE_HI6402, sregrd);
				 ...
				 const char *buf, size_t size)
{
	...
	if (sregwr >= 0x20007000 && sregwr <= 0x20008000) {
		slimbus_hi6402_write_1byte(SLIMBUS_DEVICE_HI6402, sregwr, gregvalue);
				 ...
				 char *buf)
{
	...
	ret += sprintf(buf+ret, "0x2: switch to hi6402 \n");
				 ...
				 const char *buf, size_t size)
{
	...
	{
		case 1:
			slimbus_track_deactivate(SLIMBUS_DEVICE_HI6402, stracktype);
			break;
		case 2:
			slimbus_track_activate(SLIMBUS_DEVICE_HI6402, stracktype);
			break;
		case 3:
			slimbus_track_deactivate(SLIMBUS_DEVICE_HI6402, stracktype);
			slimbus_track_activate(SLIMBUS_DEVICE_HI6402, stracktype);
		...
		case 4:
			slimbus_track_deactivate(SLIMBUS_DEVICE_HI6402, stracktype);
			slimbus_track_remove(SLIMBUS_DEVICE_HI6402, stracktype);
			break;
		case 5:
			slimbus_track_configure(SLIMBUS_DEVICE_HI6402, stracktype, NULL);
			slimbus_track_activate(SLIMBUS_DEVICE_HI6402, stracktype);
			break;
		default:
			break;
				 ...
				 const char *buf, size_t size)
{
	...
	{
		case 0:
			slimbus_pause_clock(SLIMBUS_DEVICE_HI6402, SLIMBUS_RT_FAST_RECOVERY);
		case 1:
			slimbus_pause_clock(SLIMBUS_DEVICE_HI6402, SLIMBUS_RT_CONSTANT_PHASE_RECOVERY);
		case 2:
			slimbus_pause_clock(SLIMBUS_DEVICE_HI6402, SLIMBUS_RT_UNSPECIFIED_DELAY);
		default:
				 ...
				 const char *buf, size_t size)
{
	...
	for (i=0; i<1000; i++) {
		val = slimbus_hi6402_read_1byte(NULL, 0x20007000);
		if (i<=4) {
	...
	start_time = get_timeus();
	for (i=0; i<1000; i++) {
		slimbus_hi6402_write_1byte(NULL, 0x2000703a, i);
		val = slimbus_hi6402_read_1byte(NULL, 0x2000703a);
		if (i<=4) {
			printk(KERN_ERR"[%s:%d] %x! \n", __FUNCTION__, __LINE__, val);
		}
	...
	start_time = get_timeus();
	for (i=0; i<1000; i++) {
		val = slimbus_hi6402_read_4byte(NULL, sregtest);
		if (i<=4) {
			printk(KERN_ERR"[%s:%d] %x! \n", __FUNCTION__, __LINE__, val);
		}
	}
	...
	start_time = get_timeus();
	for (i=0; i<1000; i++) {
		slimbus_hi6402_write_4byte(NULL, sregtest, i);
		val = slimbus_hi6402_read_4byte(NULL, sregtest);
		if (i<=4) {
			printk(KERN_ERR"[%s:%d] %x! \n", __FUNCTION__, __LINE__, val);
		}
