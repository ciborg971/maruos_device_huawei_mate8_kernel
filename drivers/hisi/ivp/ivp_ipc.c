...
static inline struct ivp_ipc_packet *ivp_ipc_alloc_packet(size_t len)
{
    ...
    packet = kmalloc(sizeof(struct ivp_ipc_packet), GFP_KERNEL | GFP_ATOMIC);
    if (packet == NULL) {
        ivp_err("malloc packet fail.");
        return NULL;
    }

    packet->buff = kzalloc(sizeof(char) * len, GFP_KERNEL | GFP_ATOMIC);
    if (packet->buff == NULL) {
...
static void rproc_complete_func(rproc_msg_t *ack_buffer,
                rproc_msg_len_t ack_buffer_len,
                int error,
                void *data)
{
    if (error) {
        ivp_warn("IVP msg send failed.[%d]", error);
    }
}
...
#if 0
{
    ...
    ret = RPROC_ASYNC_SEND(pdev->send_ipc, (rproc_msg_t *) tmp_buff, size/sizeof(rproc_msg_len_t), ASYNC_MSG, rproc_complete_func, NULL);
