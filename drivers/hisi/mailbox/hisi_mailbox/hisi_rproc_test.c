...
static void  test_all_sync_ipc_callback(rproc_msg_t *ack_buffer,
				rproc_msg_len_t ack_buffer_len,
				int error, void *data)
{
...
*****************************************************************************************************/	
	...
	while(1)
	{
		...
		for(index =HISI_RPROC_LPM3_MBX13; index<HISI_RPROC_ISP_MBX0; index++)
		{
			...
			if(syntypye%2)
				err = RPROC_ASYNC_SEND(index, (rproc_msg_t *)lpm3Msg, 8,  ASYNC_MSG, test_all_sync_ipc_callback, NULL);
			else
				err = RPROC_SYNC_SEND(index,(rproc_msg_t *) lpm3Msg,8, SYNC_MSG, ack_buffer, 2);
			if (err ) 
			{
				kfree(lpm3Msg);
				lpm3Msg = NULL;
...
static int rproc_performance_cnt(struct timespec *ptimespec, unsigned char rproc_id,  union ipc_data *msg, 
									unsigned char len,  unsigned char synctpye, unsigned char acktype)
{
	...
	for(index = 0; index < PRESSURE_TIMES; index++)
	{
		...
		{
			ret |= RPROC_SYNC_SEND(rproc_id,(rproc_msg_t *) msg, len, acktype, ack_buffer, 2);
		}
		else if(ASYNC_SEND == synctpye)
		{
			ret |= RPROC_ASYNC_SEND(rproc_id,(rproc_msg_t *) msg,len, acktype, NULL, NULL);
		}
 ...
 int test_rproc_performance(unsigned char objmax)
{
	...
	for(i = 0; i<objmax; i++)
	{
		...
		ret  |= rproc_performance_cnt(ptimespec, rproc_id, msg, MAX_MAIL_SIZE, SYNC_SEND, ASYNC_MSG);
		show_rproc_performance(ptimespec, rproc_id, SYNC_SEND);
		ret  |= rproc_performance_cnt(ptimespec, rproc_id, msg, MAX_MAIL_SIZE, ASYNC_SEND, ASYNC_MSG);
...
*****************************************************************************************************/	
	...
	while(1)
	{
		err = RPROC_ASYNC_SEND(rproc_lpm3, (rproc_msg_t *)lpm3Msg, 8,  ASYNC_MSG, test_all_sync_ipc_callback, NULL);
...
extern int g_TxTaskCnt;

int show_txtaskcnt(void)
{
    pr_err("xxxxxxxxx g_TxTaskCnt is  %d\n",g_TxTaskCnt);
    return g_TxTaskCnt;
}
