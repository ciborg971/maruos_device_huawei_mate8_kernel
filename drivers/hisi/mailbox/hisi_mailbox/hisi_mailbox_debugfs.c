...
static void mbox_debugfs_complete(rproc_msg_t *tx_ack_buffer,
				rproc_msg_len_t tx_ack_buffer_len,
				int tx_error,
				void *tx_data)
{
	struct mbox_task *task_info = (struct mbox_task *)tx_data;
	struct hisi_mbox_task *tx_task = container_of(tx_data, struct hisi_mbox_task, data);
	int i;
...
static void mbox_debufs_mbox_show(void)
{
	...
	for (i = 0; (i < DEBUGFS_MBOXES_NUM) && (debugfs_mboxes[i].mbox); i++) {
...
void mbox_debugfs_unregister(struct hisi_mbox_device **list)
{
