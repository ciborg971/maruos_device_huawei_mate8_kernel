...
static struct hisi_rproc_info rproc_table[ ] = {
	...
	},
	{
		...
		.mbox_rp = HISI_MAILBOX_RP_MODEM_BBE16_MBX23,
	},
	{
		.rproc_id = HISI_RPROC_MODEM_BBE16_MBX24,
		.mbox_rp = HISI_MAILBOX_RP_MODEM_BBE16_MBX24,
	},
#endif	
	{
...
#if 0	 /*use maybe in the future*/	 	
	{
		.rproc_id = HISI_RPROC_LPM3_MBX29,
		.mbox_rp = HISI_MAILBOX_RP_LPM3_MBX29,
	},
	{
...
int   hisi_rproc_flush_tx(rproc_id_t rproc_id)
{
	...
	for (i = 0; i < sizeof(rproc_table)/sizeof(struct hisi_rproc_info); i++) {
		...
		if (rproc->mbox->tx  && rproc_id == rproc->rproc_id )
			{
				left_in_kfifo = kfifo_len(&rproc->mbox->tx->fifo);
				/*if left_in_kfifo is not zero, wake up the tx_thread*/
				if(left_in_kfifo)
				{
					wake_up_interruptible(&rproc->mbox->tx->tx_wait);
					/*call printk to delay sometime for the tx_thread to send and record if this happens */
					RPROC_PR_ERR("%s 's kfifo left %d when flush\n",rproc->mbox->name, left_in_kfifo);
				}
