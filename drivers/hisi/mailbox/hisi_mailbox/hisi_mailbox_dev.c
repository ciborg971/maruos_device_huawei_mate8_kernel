...
static int
hisi_mdev_check(struct hisi_mbox_device *mdev,
...
static void hisi_mdev_ack(struct hisi_mbox_device *mdev,
				mbox_msg_t *msg,
				mbox_msg_len_t len)
{
	return;
					...
					mbox_msg_len_t len, int ack_mode)
{
	...
	__ipc_cpu_imask_clr(priv->idev->base, IPCBITMASK(priv->src)|IPCBITMASK(priv->des), priv->index);
...
static int
hisi_mdev_send_msg(struct hisi_mbox_device *mdev, 
...
hisi_mdev_irq_to_mdev(struct hisi_mbox_device *_mdev, struct list_head *list, int irq)
{
...
static int
hisi_mdev_get(struct hisi_ipc_device *idev,
