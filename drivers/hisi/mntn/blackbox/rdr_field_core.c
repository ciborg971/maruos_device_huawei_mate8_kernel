...
void rdr_field_top_init(void)
{
	...
	memcpy(pbb->top_head.product_version, RDR_PRODUCT_VERSION,
	    strlen(RDR_PRODUCT_VERSION) > 16 ? 16 : strlen(RDR_PRODUCT_VERSION));
...
int rdr_field_init(void)
{
    ...
    pbb = hisi_bbox_map(rdr_reserved_phymem_addr(), rdr_reserved_phymem_size());
    if (pbb == NULL) {
    ...
    tmp_pbb = (struct rdr_struct_s *)vmalloc(rdr_reserved_phymem_size());
    if (tmp_pbb == NULL) {
...
void rdr_show_base_info(int flag)
{
	...
	for (index = 0; index<RDR_AREA_MAXIMUM; index++) {
		BB_PRINT_PN("area[%d] addr[0x%x] size[0x%x]\n",
            index,
			pbb->area_info[index].offset,
