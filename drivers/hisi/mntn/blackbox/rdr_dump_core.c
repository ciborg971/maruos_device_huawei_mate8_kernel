...
int rdr_save_history_log(struct rdr_exception_info_s* p, char* date, bool is_save_done)
{
    int ret = 0;
    ...
    if (is_save_done) {
        snprintf(buf, HISTORY_LOG_SIZE,
            "system exception core [%s], reason [%s], time [%s]\n",
            rdr_get_exception_core(p->e_from_core),
            rdr_get_exception_type(p->e_exce_type), date );
    }
    else {
        snprintf(buf, HISTORY_LOG_SIZE,
            "system exception core [%s], reason [%s], time [%s][last_save_not_done]\n",
            rdr_get_exception_core(p->e_from_core),
            rdr_get_exception_type(p->e_exce_type), date );
    }
...
void rdr_save_dfx_baseinfo(char* logpath)
{
    BB_PRINT_START();
    //save pbb to fs.
    rdr_savebuf2fs(logpath, DFX_BIN,
            rdr_get_tmppbb(), rdr_get_pbb_size(), 0);
