...
static bool is_dsp_power_on(void)
{
	...
	power_status_addr = (unsigned int*)ioremap(DRV_DSP_POWER_STATUS_ADDR, 0x4);
...
static bool is_nmi_complete()
{
	unsigned int * nmi_flag_addr = NULL;
	bool is_complete = false;

	nmi_flag_addr = (unsigned int *)ioremap(DRV_DSP_NMI_FLAG_ADDR, 0x4);
...
static int dump_hifi_ddr(char *filepath)
{
	...
	full_text = (char *)vmalloc((size_t)(RDR_HIFI_DUMP_SIZE + RDR_COMMENT_LEN));
	if (NULL == full_text) {
...
{
	int i = 0;
	struct icc_channel_fifo* pstDstFifo = NULL;
	struct icc_channel_fifo* pstSrcFifo = NULL;
	struct icc_dbg* pstIccDbg = (struct icc_dbg*)ioremap_wc(HIFI_ICC_DEBUG_LOCATION, HIFI_ICC_DEBUG_SIZE);

	if (!pstIccDbg) {
		BB_PRINT_ERR("%s(): ioremap hifi pstIccDbg fail.\n", __func__);
		return -1;
	}

	if ((ICC_DEBUG_PROTECT_WORD1 != pstIccDbg->protectword1)
		|| (ICC_DEBUG_PROTECT_WORD2 != pstIccDbg->protectword2)) {
		BB_PRINT_ERR("%s(): check ICC_DEBUG_PROTECT_WORD fail [0x%x, 0x%x], do not save icc fifo.\n",
			__func__, pstIccDbg->protectword1, pstIccDbg->protectword2);
		iounmap(pstIccDbg);
		pstIccDbg = NULL;
		return -EINVAL;
	}
	...
	for (i = 0; i < 2; i++) {
		struct icc_channel* pstChannel = &pstIccDbg->pstIccChannels[i];
		/* 保存接收fifo */
		if (0 != pstChannel->fifo_recv) {
			pstDstFifo = (struct icc_channel_fifo*)(((char *)&(pstChannel->fifo_recv)) + *(unsigned int *)(&pstChannel->fifo_recv));
			pstSrcFifo = (struct icc_channel_fifo*)ioremap_wc(*((unsigned int*)pstDstFifo->data), pstDstFifo->size + sizeof(struct icc_channel_fifo));
			if (!pstSrcFifo) {
				BB_PRINT_ERR("rdr: remap hifi recv pstSrcFifo fail.\n");
				iounmap(pstIccDbg);
				pstIccDbg = NULL;
				return -1;
			}
			/* 拷贝fifo通道数据 */
			memcpy((void*)((char*)pstDstFifo + sizeof(struct icc_channel_fifo)), (void*)((char*)pstSrcFifo + sizeof(struct icc_channel_fifo)), pstDstFifo->size);
			/* 更新Fifo头信息 */
			pstDstFifo->magic = pstSrcFifo->magic;
			pstDstFifo->read  = pstSrcFifo->read;
			pstDstFifo->write = pstSrcFifo->write;
			iounmap(pstSrcFifo);
			pstSrcFifo = NULL;
		}

		/* 保存发送fifo */
		if (0 != pstChannel->fifo_send) {
			pstDstFifo = (struct icc_channel_fifo*)((char *)&(pstChannel->fifo_send) + *(unsigned int *)(&pstChannel->fifo_send));
			pstSrcFifo = (struct icc_channel_fifo*)ioremap_wc(*((unsigned int*)pstDstFifo->data), pstDstFifo->size + sizeof(struct icc_channel_fifo));

			if (!pstSrcFifo) {
				BB_PRINT_ERR("rdr: remap hifi send pstSrcFifo fail.\n");
				iounmap(pstIccDbg);
				pstIccDbg = NULL;
				return -1;
			}
...
static int dump_thread(void *arg)
{
	...
	while (soc_des.is_dump_thread_running) {
		if (down_interruptible(&soc_des.dump_sem)) {
			BB_PRINT_ERR("dump_thread down sem fail.\n");
			continue;
		}

		if (!soc_des.is_dump_thread_running) {
			BB_PRINT_PN("soc dump thread exit.\n");
			break;
		}
...
static int irq_handler_thread(void *arg)
{
	...
	while (soc_des.is_handler_thread_running) {
		...
		if (!soc_des.is_handler_thread_running) {
			BB_PRINT_ERR("soc handler thread exit.\n");
			break;
		}
...
int hisi_rdr_ipc_notify_lpm3(u32 *msg, unsigned long len)
{
	...
	ret = RPROC_ASYNC_SEND(HISI_RPROC_LPM3_MBX17, msg, len, ASYNC_MSG, NULL, NULL);
	if (ret != 0) {
...
int rdr_audio_soc_init(void)
{
	...
	soc_des.is_dump_thread_running = true;
	soc_des.is_handler_thread_running = true;
	...
	if (!soc_des.kdump_task) {
		soc_des.is_dump_thread_running = false;
...
void rdr_audio_soc_exit(void)
{
	...
	soc_des.is_handler_thread_running = false;
	soc_des.is_dump_thread_running = false;
	up(&soc_des.dump_sem);
	up(&soc_des.handler_sem);
