...
static char* s_internal_beta_infor[EDITION_INFOR_MAX] = {INTERNEL_BETA_FILE_1, INTERNEL_BETA_FILE_2, NULL};
static char* s_oversea_beta_infor[EDITION_INFOR_MAX] = {OVERSEA_BETA_FILE_1, OVERSEA_BETA_FILE_2, NULL};
...
unsigned int bbox_check_edition(void)
{
    ...
    for (i=0; i<EDITION_INFOR_MAX && s_internal_beta_infor[i]; i++) {
        BB_PRINT_DBG("%s: Is checking %s!\n", __func__, s_internal_beta_infor[i]);
    ...
    for (i=0; i<EDITION_INFOR_MAX && s_oversea_beta_infor[i]; i++) {
        BB_PRINT_DBG("%s: Is checking %s!\n", __func__, s_oversea_beta_infor[i]);
...
struct cmdword reboot_reason_map[] =
{
    ...
    {"AP_S_DDR_SELFREFLASH",       AP_S_DDR_SELFREFLASH},
...
struct blackbox_modid_list g_modid_list[]={
    ...
    {HISI_BB_MOD_RESERVED_START,    HISI_BB_MOD_RESERVED_END,       "blackbox reserved"},
    {HISI_BB_MOD_MODEM_LPS_START,   HISI_BB_MOD_MODEM_LPS_END,      "MODEM LPS"},
    {HISI_BB_MOD_MODEM_LMSP_START,  HISI_BB_MOD_MODEM_LMSP_END,     "MODEM LMSP"},
    {HISI_BB_MOD_RANDOM_ALLOCATED_START,HISI_BB_MOD_RANDOM_ALLOCATED_END,"blackbox random allocated"},
};
...
static int bb_reboot_nb(struct notifier_block *nb,
...
char *blackbox_get_modid_str(u32 modid)
{
    ...
    for (i=0; i<modid_size; ++i) {
        if(modid>=g_modid_list[i].modid_span_little && modid<=g_modid_list[i].modid_span_big) {
...
int rdr_common_early_init(void) {
    ...
    if (ret < 0 || NULL == prdr_dumpctrl || (len=strlen(prdr_dumpctrl)) > 16) {
    ...
    g_wait_dumplog_timeout = value; //120s
    BB_PRINT_DBG("[%s], get g_wait_dumplog_timeout [0x%x] in dts!\n", __func__,
        g_wait_dumplog_timeout);
...
void bbox_save_done(char *logpath, u32 step)
{
    ...
    BB_PRINT_START();
    if (!logpath || (PATH_MAXLEN < strlen(logpath)+strlen(BBOX_SAVE_DONE_FILENAME)+1)) {
    ...
    if (BBOX_SAVE_STEP_DONE == step) {
        ...
        strncat(path, BBOX_SAVE_DONE_FILENAME, strlen(BBOX_SAVE_DONE_FILENAME));

        oldfs = get_fs();
        set_fs(KERNEL_DS);

        /*在时间戳目录下面创建done文件*/
        fd = sys_open(path, O_CREAT | O_WRONLY, 0660);
        if (fd < 0) {
