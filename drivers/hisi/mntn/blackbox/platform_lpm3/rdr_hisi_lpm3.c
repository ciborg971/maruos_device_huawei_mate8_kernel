...
u64 rdr_lpm3_buf_addr = 0;
u32 rdr_lpm3_buf_len = 0;

char *g_lpmcu_rdr_ddr_addr = NULL;
...
static irqreturn_t m3_wdt_irq_handler(int irq, void *data)
{
	...
	pfn_rdr_system_error = rdr_system_error;
	g_modid = M3_WDT_TIMEOUT;
	g_irq = irq;
	up(&rdr_lpm3_sem);
	pr_err("exit %s\n", __func__);
#endif
...
static int rdr_lpm3_thread_body(void *arg)
{
    ...
    while (1) {
        ...
        if(pfn_cb_dumpdone != NULL) {
            ...
            lpmcu_ddr_base = (char*)ioremap((phys_addr_t)HISI_RESERVED_LPMX_CORE_PHYMEM_BASE,HISI_RESERVED_LPMX_CORE_PHYMEM_SIZE);
...
int __init rdr_lpm3_init(void)
{
	...
	pr_err("%s: (ap)->(lpm3) ipc send (0x%x 0x%x 0x%x)!\n", __func__, msg[0], msg[1], msg[2]);
...
#if 0
/* test code start */
void rdr_test_m3_wdt(void)
{
	rdr_system_error(M3_WDT_TIMEOUT, M3_WDT_TIMEOUT_IRQ_NUM, 0);
}

void rdr_test_ap_wdt(void)
{
	rdr_system_error(AP_WDT_TIMEOUT, AP_WDT_TIMEOUT_IRQ_NUM, 0);
}

int rdr_test_syscounter(void)
{
	u32 timestamp_h, timestamp_l;
	timestamp_h = readl(SOC_SYSCOUNTER_CNTCV_H32_ADDR(counter_base));
	timestamp_l = readl(SOC_SYSCOUNTER_CNTCV_L32_ADDR(counter_base));
	pr_err("[%s] %p 0x%x 0x%x start\n", __func__, counter_base, timestamp_h, timestamp_l);
	return 0;
}

s32 rdr_test_m3_context_save(void)
{
	s32 ret;
	u32 msg;
	msg = PSCI_MSG_TYPE_M3_CTXSAVE;
	ret = RPROC_ASYNC_SEND(HISI_RPROC_LPM3_MBX17, (mbox_msg_t *)&msg, 1,
	              ASYNC_MSG, NULL, NULL);
	if (ret != 0) {
		pr_err("RPROC_ASYNC_SEND failed! return 0x%x, &msg:(0x%x)\n", ret, msg);
...
void test_pfn_cb_dump_done( u32 modid, u64 coreid)
{
    pr_err("test_pfn_cb_dump_done: modid:%u,coreid:%llu\n", modid, coreid);
}
void test_dump()
{
    fn_dump(0x100, 0x10, current_core_id, "test", test_pfn_cb_dump_done);
}
void test_sys_err()
{
