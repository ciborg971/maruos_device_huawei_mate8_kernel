...
{
    if(!kthread_run(test_hung_task_thread_fn, NULL, "test_hung_task_thread_fn"))
    {
        printk(KERN_ERR "[%s], kthread_run:test_hung_task_thread_fn failed.\n", __func__);
        return -1;
    }
    printk(KERN_ERR "[%s], kthread_run:test_hung_task_thread_fn start.\n", __func__);
    return 0;
}
...
void ap_exch_buf_show(unsigned int offset, unsigned int size)
{
    ...
    ap_exch_hex_dump((unsigned char *)(g_hisiap_addr+ add_offset), size, 16);
}
...
/***************************************************************************
 Description : get time difference to collect the diff between hisicurtime and bboxslice
  History
  1.Date: 2015/07/10
    Author : lwx275086
