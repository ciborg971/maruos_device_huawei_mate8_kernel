...
int g_bbox_fpga_flag = -1;    /*-1 未初始化；0 非fpga板；1 fpga板*/
...
void get_product_version(char* version, size_t count)
{
    ...
    /*等待文件系统ok，读取/system/build.prop的ro.build.display.id=BalongV100R001C50B310属性*/
    /*不能在module_init调用*/
...
Description:    保存last_kmsg和fastbootlog.
Input:          void *arg暂不使用
Output:         NA
Return:         -1 : error, 0 : ok
********************************************************************************/
int save_last_kmsg(void *arg)
{
    ...
    /*申请一块内存用于存储遍历/mnt/pstore目录下所有文件的名字*/
    ...
    /*调用接口将/mnt/pstore目录下的所有文件名字存入pbuff中*/
