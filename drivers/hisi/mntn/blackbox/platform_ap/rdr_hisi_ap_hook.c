...
static const char* g_hook_trace_pattern[HK_MAX] = {
    "irq_switch::ktime,slice,vec,cpu,dir", /*IRQ*/
    "task_switch::ktime,cpu,pid,comm",   /*TASK*/
    "cpuidle::ktime,cpu,dir",            /*CPUIDLE*/
...
arch_timer_func_ptr g_arch_timer_func_ptr = NULL;

int register_arch_timer_func_ptr(arch_timer_func_ptr func)
{
    if (IS_ERR_OR_NULL(func))
    {
        printk(KERN_ERR "[%s], arch_timer_func_ptr [0x%p] is invalid!\n", __func__, func);
        return -EINVAL;
    }
...
int single_buffer_init(unsigned char* addr, unsigned int size, hook_type hk, unsigned int fieldcnt)
{
    ...
    printk(KERN_DEBUG "hisiap_ringbuffer_init: g_hook_trace_pattern[%d]: %s\n", hk, g_hook_trace_pattern[hk]);
    return hisiap_ringbuffer_init((struct hisiap_ringbuffer_s *)addr, size, fieldcnt, g_hook_trace_pattern[hk]);
}
     ...
     u32 cpu_num, u32 fieldcnt, const char* keys)
{
    ...
    for (i = 0; i < cpu_num; i++)
    {
        ...
        {
            buffer->percpu_addr[i] = buffer->percpu_addr[i-1] + buffer->percpu_length[i-1];
        }

        printk(KERN_DEBUG "[%s], [%d]: percpu_addr [0x%p], percpu_length [0x%x], fieldcnt [%d]\n", __func__, i,
            buffer->percpu_addr[i], buffer->percpu_length[i], fieldcnt);

        ret = hisiap_ringbuffer_init((struct hisiap_ringbuffer_s *)buffer->percpu_addr[i],
                        buffer->percpu_length[i], fieldcnt, keys);
        if (ret)
        {
            printk(KERN_ERR "[%s], cpu [%d] ringbuffer init failed!\n", __func__, i);
            return ret;
        }
    }
    return 0;
                            ...
                            hook_type hk, u32 ratio[][8])
{
    ...
    {
        printk(KERN_ERR "[%s], buffer_info [0x%p] buffer_addr [0x%p], buffer_size [0x%x]\n",
...
{
    unsigned int irq_record_ratio[8][8] =  {{16, 0, 0, 0, 0, 0, 0, 0},//HERE:[8][8]
                                               { 8, 8, 0, 0, 0, 0, 0, 0},
                                               { 8, 4, 4, 0, 0, 0, 0, 0},
                                               { 8, 4, 2, 2, 0, 0, 0, 0},
                                               { 8, 4, 2, 1, 1, 0, 0, 0},
    ...
    return hook_percpu_buffer_init(buffer_info, addr, size, sizeof(irq_info),
                                HK_IRQ, irq_record_ratio);
}
...
int task_buffer_init(percpu_buffer_info* buffer_info, unsigned char* addr, unsigned int size)
{
    ...
    return hook_percpu_buffer_init(buffer_info, addr, size, sizeof(task_info),
                                HK_TASK, task_record_ratio);
}

int cpuidle_buffer_init(percpu_buffer_info* buffer_info, unsigned char* addr, unsigned int size)
{
    unsigned int cpuidle_record_ratio[8][8] = {{16, 0, 0, 0, 0, 0, 0, 0},
                                               { 8, 8, 0, 0, 0, 0, 0, 0},
                                               { 8, 4, 4, 0, 0, 0, 0, 0},
                                               { 8, 4, 2, 2, 0, 0, 0, 0},
                                               { 4, 4, 4, 2, 2, 0, 0, 0},
    ...
    return hook_percpu_buffer_init(buffer_info, addr, size, sizeof(cpuidle_info),
                                HK_CPUIDLE, cpuidle_record_ratio);
}

int cpu_onoff_buffer_init(unsigned char* addr, unsigned int size)
{
    return single_buffer_init(addr, size, HK_CPU_ONOFF, sizeof(cpu_onoff_info));
}
...
{
    return single_buffer_init(addr, size, HK_HUNGTASK, sizeof(hung_task_info));
}

int buddy_fail_buffer_init(unsigned char* addr, unsigned int size)
{
    return single_buffer_init(addr, size, HK_BUDDY_FAIL, sizeof(buddy_fail_info));
}
...
void irq_trace_hook(unsigned int dir, unsigned int old_vec, unsigned int new_vec)
{
    ...
    g_hisi_last_irqs[info.cpu] = new_vec;

    hisiap_ringbuffer_write((struct hisiap_ringbuffer_s*)g_hook_percpu_buffer[HK_IRQ]->percpu_addr[info.cpu], (u8*)&info);
}
...
void task_switch_hook(const void *pre_task, void *next_task)
{
    ...
    g_last_task_ptr[info.cpu] = task;
    hisiap_ringbuffer_write((struct hisiap_ringbuffer_s*)g_hook_percpu_buffer[HK_TASK]->percpu_addr[info.cpu], (u8*)&info);
}
...
void cpuidle_stat_hook(u32 dir)
{
    ...
    hisiap_ringbuffer_write((struct hisiap_ringbuffer_s*)g_hook_percpu_buffer[HK_CPUIDLE]->percpu_addr[info.cpu], (u8*)&info);
}
...
void cpu_on_off_hook(u32 cpu, u32 on)
{
    ...
    hisiap_ringbuffer_write((struct hisiap_ringbuffer_s *)g_hook_buffer_addr[HK_CPU_ONOFF], (u8 *)&info);
}
...
asmlinkage void syscall_hook(u32 syscall_num, u32 dir)
{
    ...
    hisiap_ringbuffer_write((struct hisiap_ringbuffer_s *)g_hook_buffer_addr[HK_SYSCALL], (u8 *)&info);
}
...
void hung_task_hook(void *tsk, u32 timeout)
{
    ...
    hisiap_ringbuffer_write((struct hisiap_ringbuffer_s *)g_hook_buffer_addr[HK_HUNGTASK], (u8 *)&info);
}
...
void buddy_fail_hook(u32 order)
{
    ...
    hisiap_ringbuffer_write((struct hisiap_ringbuffer_s *)g_hook_buffer_addr[HK_BUDDY_FAIL], (u8 *)&info);
}
