 ...
 *	char *logpath
 */
void rdr_callback(struct rdr_exception_info_s *p_exce_info, u32 mod_id, char *logpath)
{
    ...
    list_for_each_safe(cur, next, &__rdr_exception_list) {
        ...
        if (e_type_info == NULL) {
            spin_lock(&__rdr_exception_list_lock);
            BB_PRINT_DBG("It might be better to look around here. %s:%d",__func__, __LINE__);
            continue;
        }
        if ((e_type_info->e_from_core != p_exce_info->e_from_core) && 
            (e_type_info->e_from_core & p_exce_info->e_notify_core_mask)) {
            if((u64)(e_type_info->e_callback) & BBOX_COMMON_CALLBACK){
                BB_PRINT_DBG("%s: call core common callback function.\n", __func__);
                ((rdr_e_callback)((u64)(e_type_info->e_callback) & ~BBOX_CALLBACK_MASK))(mod_id, logpath);
            }
        }
        spin_lock(&__rdr_exception_list_lock);
    }
    ...
    if((u64)(p_exce_info->e_callback) & ~BBOX_CALLBACK_MASK) {
        ...
        ((rdr_e_callback)((u64)(p_exce_info->e_callback) & ~BBOX_CALLBACK_MASK))(mod_id, logpath);
    }
}
/*
...
u32 rdr_check_modid(u32 modid, u32 modid_end)
{
	...
	list_for_each_safe(cur, next, &__rdr_exception_list) {
		e_type_info =
		      list_entry(cur, struct rdr_exception_info_s, e_list);
		if (e_type_info == NULL) {
