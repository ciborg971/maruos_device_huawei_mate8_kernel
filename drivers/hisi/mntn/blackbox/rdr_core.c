    ...
    if (p_exce_info != NULL &&
        ...
        list_for_each_safe(cur, next, &g_rdr_syserr_list) {
            ...
            if (e_cur->modid == p->modid)
                exist = 1;
				BB_PRINT_DBG("exception:[0x%x] disallow reentrant.  return.\n",
					modid);
                break;
        }
    }
...
void rdr_syserr_process(struct rdr_syserr_param_s *p)
{
    ...
    mask = rdr_notify_module_dump(mod_id,
        p_exce_info,
        path);

    BB_PRINT_DBG("rdr_notify_module_dump done. return mask=[0x%x]\n", mask);

    /* mask的值是根据p_exce_info->e_notify_core_mask获得的, 当mask为0时, 不走RDR框架导出LOG的流程. 确认人:刘海龙 */
    if (mask != 0) {
        while(wait_dumplog_timeout > 0) {
            ...
            if (mask != cur_mask){
                ...
                BB_PRINT_DBG("wait for dump done. use time:[%d], cur_maks[0x%x]\n",
                i*inter_ms, cur_mask);
                break;
            }
            BB_PRINT_DBG("wait for dump done. current status:[0x%x]\n", cur_mask);
        }
        if (wait_dumplog_timeout <= 0) {
            BB_PRINT_ERR("wait for dump status timeout... cur_mask[0x%x] "
                    "target_mask[0x%x]\n", cur_mask, mask);
        }
    }
    ...
    if (0 != mask)
    {
        if (check_himntn(HIMNTN_GOBAL_RESETLOG)) {
            ...
            if (p_exce_info->e_reset_core_mask & RDR_AP) {
                bbox_save_done(path, BBOX_SAVE_STEP1);    /*复位重启后还有一部分log需要保存*/
            }
            else {
                if(!in_atomic() && !irqs_disabled() && !in_irq())
                {
                    sys_sync();    /*确保之前的所有文件系统相关操作都能完成。*/
                }
                bbox_save_done(path, BBOX_SAVE_STEP_DONE);    /*此异常目录下的所有log都保存完毕*/
            }
        }
    } else {
...
void rdr_syserr_list_print(void)
{
    ...
    spin_lock(&g_rdr_syserr_list_lock);
    list_for_each_safe(cur, next, &g_rdr_syserr_list) {
        e_cur = list_entry(cur,
              struct rdr_syserr_param_s, syserr_list);
...
static int rdr_main_thread_body(void *arg)
{
	...
	while (!kthread_should_stop()) {
		jiffies = msecs_to_jiffies(1000); //1s
		if (down_timeout(&rdr_sem, jiffies)) {
            if (rdr_syserr_list_empty()) continue;
			/*return -1;*/
		}
...
static s32 __init rdr_init(void)
{
    ...
    rdr_main = kthread_run(rdr_main_thread_body, NULL, "blackbox_main_thread");
    if (!rdr_main) {
    ...
    rdr_bootcheck = kthread_run(rdr_bootcheck_thread_body, NULL, "blackbox_bootcheck_thread");
    if (!rdr_bootcheck) {
    ...
    if (!kthread_run(rdr_dump_init, NULL, "blackbox_dump_init")) {
