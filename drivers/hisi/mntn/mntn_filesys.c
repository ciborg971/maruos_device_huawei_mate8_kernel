...
int mntn_filesys_rm_all_file(const char *path)
{
    ...
    fd = sys_open(path, O_RDONLY, 0660);/*create file */
    if (fd < 0)
    {
    ...
    for (bpos = 0; bpos < nread;)
    {
	 ...
	 if (strlen(fullname) + strlen(d->d_name) >= MNTN_FILESYS_FNAME_LEN)
	 {
        ...
        if (ret)
        {
            MNTN_FILESYS_PRINT(KERN_ERR"mntn_err: fail to mntn_filesys_rm_file\n");
        }
...
void mntn_rm_old_log(const char *ppath, unsigned int unumber)
{
    ...
    if (NULL == ppath)
    {
        MNTN_FILESYS_PRINT(KERN_ERR"mntn_err: path is NULL when removing old log\n");
	 return;
    }
