...
static size_t s_last_fastbootlog_size = 0;
static char *s_fastbootlog_buff = NULL;
static size_t s_fastbootlog_size = 0;
...
static ssize_t last_fastbootlog_dump_file_read(struct file *file, char __user *userbuf, size_t bytes, loff_t *off)
{
	ssize_t copy;
...
static void bootloader_logger_dump(char *start, unsigned int size, const char *str)
{
    unsigned int i;
    char *p = start;
...
static void check_fastbootlog_head(struct fastbootlog_head *head, int *need_dump_whole)
{
	...
	if (head->lastlog_start >= FASTBOOT_DUMP_LOG_SIZE || head->lastlog_start < sizeof(struct fastbootlog_head)
		|| head->lastlog_offset >= FASTBOOT_DUMP_LOG_SIZE || head->lastlog_offset < sizeof(struct fastbootlog_head)) {
		printk(KERN_INFO "%s: last fastbootlog offset incorrect\n", __func__);
		*need_dump_whole = 1;
		return;
	}

	if (head->log_start >= FASTBOOT_DUMP_LOG_SIZE || head->log_start < sizeof(struct fastbootlog_head)
		|| head->log_offset >= FASTBOOT_DUMP_LOG_SIZE || head->log_offset < sizeof(struct fastbootlog_head)) {
...
static int __init fastbootlog_dump_init(void)
{
	...
	if (pfn_valid(__phys_to_pfn(FASTBOOT_DUMP_LOG_ADDR))) {
		...
		use_ioremap = 1;
		fastbootlog_buff = ioremap_wc(FASTBOOT_DUMP_LOG_ADDR, FASTBOOT_DUMP_LOG_SIZE);
	}
	if (!fastbootlog_buff) {
	...
	if (head->lastlog_offset < head->lastlog_start) {
		...
		lastlog_size = tmp_len + head->lastlog_offset - sizeof(struct fastbootlog_head);

		s_last_fastbootlog_buff = vmalloc(lastlog_size);
		if (!s_last_fastbootlog_buff) {
			printk(KERN_ERR "%s: fail to vmalloc %#x bytes s_last_fastbootlog_buff\n", __func__, lastlog_size);
			return -1;
		}
		memcpy(s_last_fastbootlog_buff, lastlog_start, tmp_len);
		lastlog_start = fastbootlog_buff + sizeof(struct fastbootlog_head);
		memcpy(s_last_fastbootlog_buff+tmp_len, lastlog_start, lastlog_size-tmp_len);
		...
		if (lastlog_size > 0) {
			...
			memcpy(s_last_fastbootlog_buff, lastlog_start, lastlog_size);
			s_last_fastbootlog_size = lastlog_size;
		}
	}
	...
	if (head->log_offset < head->log_start) {
		...
		log_size = tmp_len + head->log_offset - sizeof(struct fastbootlog_head);

		s_fastbootlog_buff = vmalloc(log_size);
		if (!s_fastbootlog_buff) {
			printk(KERN_ERR "%s: fail to vmalloc %#x bytes s_fastbootlog_buff\n", __func__, log_size);
			return -1;
		}
		memcpy(s_fastbootlog_buff, log_start, tmp_len);
		log_start = fastbootlog_buff+sizeof(struct fastbootlog_head);
		memcpy(s_fastbootlog_buff+tmp_len, log_start, log_size-tmp_len);
	...
	if (s_last_fastbootlog_buff) {
		bootloader_logger_dump(s_last_fastbootlog_buff, s_last_fastbootlog_size, "last");
		balong_create_log_proc_entry("last_fastboot_log", S_IRUSR|S_IRGRP, &last_fastbootlog_dump_file_fops, NULL);
	}
	if (s_fastbootlog_buff) {
		bootloader_logger_dump(s_fastbootlog_buff, s_fastbootlog_size, "current");
		balong_create_log_proc_entry("fastboot_log", S_IRUSR|S_IRGRP, &fastbootlog_dump_file_fops, NULL);
	}
