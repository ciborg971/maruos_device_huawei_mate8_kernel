...
static int lowmem_shrink(struct shrinker *s, struct shrink_control *sc)
{
	...
	static struct task_struct * last_selected = NULL;
	static pid_t last_pid = 0;
	static atomic_t atomic_lmk = ATOMIC_INIT(0);
	struct task_struct *selected_proc = NULL;
	...
	if (selected) {
		if (last_selected!=NULL && last_pid==selected->pid && last_selected==selected) {
			struct task_struct *t;

			lowmem_print(1, "lmk: selected '%s' pid repeat!\n", selected->comm);
			if (selected_proc) {
				rcu_read_lock();
				for_each_thread(selected_proc, t) {
					lowmem_print(1, "    tgid=%d  pid=%d comm=%s status=0x%lx   frozen=%d\n",
							t->tgid, t->pid, t->comm, t->state, (int)frozen(selected));
				}
				rcu_read_unlock();
			}
		}
		...
		lowmem_print(1, "nas skb page is %ld\n", atomic_long_read(&naspg_nr));
		lowmem_deathpending_timeout = jiffies + HZ;
		if (lowmem_debug_level >= 2 && selected_oom_score_adj == 0) {
			dump_meminfo();
		}

		if (selected->state&TASK_UNINTERRUPTIBLE && selected_proc) {
			lowmem_print(1, "    tgid=%d  pid=%d comm=%s state=0x%lx   frozen=%d\n",
					selected->tgid, selected_proc->pid, selected_proc->comm, selected->state, (int)frozen(selected));/* [false alarm]:selected_proc cant be null */
		}
