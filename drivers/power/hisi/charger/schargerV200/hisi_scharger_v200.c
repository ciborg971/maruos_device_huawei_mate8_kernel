            ...
            struct device_attribute *attr, const char *buf, size_t count)
{
    ...
    if(!strncmp(("reg_addr"),attr->attr.name,strlen("reg_addr")))
    {
        if(v <(u8)HI6522_REG_TOTAL_NUM && v >=(u8)0x00 )
        {
            info->reg = v;
            return count;
        }
...
static void hi6522_irq_work_handle(struct work_struct *work)
{
    ...
    if (irq_status[3] & CHG_IRQ3_LCD_LDO_OCP){
        SCHARGER_ERR("SCHARGER ERROR:CHG_IRQ3_LDO_OCP,reg[0x0A]:0x%x;lcd ocp count %d;open lcd ldo time %lu;lcd ldo ocp time %lu;open to ocp time %u\n",\
                    irq_status[3],lcd_ldo_ocp_count,lcd_ldo_open_time,lcd_ldo_ocp_time,jiffies_to_msecs(lcd_ldo_ocp_time - lcd_ldo_open_time));

        if(jiffies_to_msecs(lcd_ldo_ocp_time - lcd_ldo_open_time)< HI6522_OPEN_LDO_TO_OCP_TIME)
        {
            lcd_ldo_ocp_count++;
        }
        else
        {
            if(lcd_ldo_ocp_count <= HI6522_LCD_LDO_OCP_LIMIT_NUM)
            {
                lcd_ldo_ocp_count = 0;
            }
        }
    ...
    irq_status[3] = irq_status[3] & HI6522_NO_CLR_LDO_OCP;
	/*clean irq*/
