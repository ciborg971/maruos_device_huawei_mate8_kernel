...
static int hisi_force_full_timer(int curr_capacity, struct hisi_bci_device_info *di)
{
...
static int hisi_capacity_pulling_filter(int curr_capacity, struct hisi_bci_device_info *di)
{
    int index = 0;
...
static void hisi_get_error_info(struct hisi_bci_device_info *di)
{
    ...
    {
        if (batt_dsm_array[i].notify_enable && (BAT_BOARD_ASIC == is_board_type))
        {
...
{
    struct hisi_bci_device_info *di = container_of(work,
               struct hisi_bci_device_info, hisi_bci_monitor_work.work);

    hisi_get_battery_info(di);

    hisi_get_error_info(di);

    schedule_delayed_work(&di->hisi_bci_monitor_work,
...
static ssize_t hisi_bci_show_batt_removable(struct device *dev, struct device_attribute *attr, char *buf)
{
    return sprintf(buf, "%d\n", removable_batt_flag);
}

static ssize_t hisi_bci_poll_charge_start_event(struct device *dev, struct device_attribute *attr, char *buf)
{
...
static DEVICE_ATTR(batt_removable, (S_IWUSR | S_IRUGO),
                hisi_bci_show_batt_removable,
                NULL);

static DEVICE_ATTR(poll_charge_start_event, (S_IWUSR | S_IRUGO),
                hisi_bci_poll_charge_start_event,
                hisi_bci_set_charge_event);
...
static int hisi_bci_battery_probe(struct platform_device *pdev)
{
   ...
   struct device_node* np;
    ...
    INIT_DELAYED_WORK(&di->hisi_bci_monitor_work,
                hisi_bci_battery_work);
...
#ifdef CONFIG_PM
static int hisi_bci_battery_suspend(struct platform_device *pdev,pm_message_t state)
{
    struct hisi_bci_device_info *di = platform_get_drvdata(pdev);
