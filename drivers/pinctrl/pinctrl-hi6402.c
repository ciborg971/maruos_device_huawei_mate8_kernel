...
{
	hi6402_irq_write(pindev->p_irq_data, pindev->base_reg + reg, val);
}

static unsigned hi6402es_pinctrl_readl(struct hi6402_pinctrl_device *pindev, unsigned reg)
{
	return hi6402es_irq_read(pindev->es_p_irq_data, pindev->base_reg + reg);
}

static void hi6402es_pinctrl_writel(struct hi6402_pinctrl_device *pindev, unsigned val, unsigned reg)
{
	hi6402es_irq_write(pindev->es_p_irq_data, pindev->base_reg + reg, val);
}
...
static int hi6402_pinctrl_probe(struct platform_device *pdev)
{
	...
	if (of_get_property(np, "chip,type-es", NULL)) {
		pindev->es_p_irq_data = dev_get_drvdata(dev->parent);
		pindev->p_irq_data = NULL;
		pindev->read = hi6402es_pinctrl_readl;
		pindev->write = hi6402es_pinctrl_writel;
	} else {
		pindev->p_irq_data = dev_get_drvdata(dev->parent);
		pindev->es_p_irq_data = NULL;
		pindev->read = hi6402_pinctrl_readl;
		pindev->write = hi6402_pinctrl_writel;
	}
	if (!pindev->p_irq_data && !pindev->es_p_irq_data) {
		dev_err(dev, "could not get parent device\n");
		return -ENOENT;
	}
