...
struct cci_ace_port
{
	...
	unsigned int qcr; 	/* qos-control */
	unsigned int rtr; 	/* regulator-target */
	unsigned int sfr; 	/* salar */
	unsigned int qrr; 	/* qos-range */
};
...
struct cci_dev
{
	...
	int nb_cci_ports;
	unsigned int speculative_dis; 	/* speculative disable */
	void __iomem *cci_ctrl_base;
	unsigned long cci_ctrl_phys;
	void __iomem *cci_pmu_base;
	void __iomem *crgctrl_base;
};

static struct cci_dev *g_cci_dev;
...
static int __cci_ace_get_port(struct device_node *dn, int type)
{
	...
	for (i = 0; i < g_cci_dev->nb_cci_ports; i++) {
		ace_match = g_cci_dev->ports[i].type == type;
		if (ace_match && cci_portn == g_cci_dev->ports[i].dn)
			return i;
	}
	return -ENODEV;
}
...
{
	void __iomem *base = g_cci_dev->ports[port & (~PORT_VALID)].base;
	 ...
	 */
	while (readl_relaxed(g_cci_dev->cci_ctrl_base + CCI_CTRL_STATUS) & 0x1);
}
...
{
	if (port >= g_cci_dev->nb_cci_ports || g_cci_dev->ports[port].type == ACE_INVALID_PORT)
		return -ENODEV;
	/*
	 ...
	 */
	if (g_cci_dev->ports[port].type == ACE_PORT)
		return -EPERM;
...
void hisi_cci_enable_detect(u32 cluster)
{
	...
	val = readl(g_cci_dev->crgctrl_base + addr_offset) | (1 << bit_shift);
	writel(val, g_cci_dev->crgctrl_base + addr_offset);
	val = readl(g_cci_dev->crgctrl_base + addr_offset);
	val &= ~(1 << bit_shift);
	writel(val, g_cci_dev->crgctrl_base + addr_offset);
}
...
{
	pr_debug("cci_init_speculation: data=0x%x\n", g_cci_dev->speculative_dis);
	writel_relaxed(g_cci_dev->speculative_dis, g_cci_dev->cci_ctrl_base + CCI400_SPECCTRL);
...
static void cci_init_qos(void)
{
	...
	for(i = 0; i < g_cci_dev->nb_cci_ports; i++)
	{
		if(g_cci_dev->ports[i].base)
		{
			pr_debug("qcr:0x%x\n", 	g_cci_dev->ports[i].qcr);
			writel_relaxed(g_cci_dev->ports[i].qcr, g_cci_dev->ports[i].base + CCI400_SLAVE_QCR_OFFSET);

			/* Banwidth*/
			pr_debug("rtr: 0x%x\n", g_cci_dev->ports[i].rtr);
			writel_relaxed(g_cci_dev->ports[i].rtr, g_cci_dev->ports[i].base + CCI400_SLAVE_RTR_OFFSET);

			pr_debug("sfr: 0x%x\n", g_cci_dev->ports[i].rtr);
			writel_relaxed(g_cci_dev->ports[i].sfr, g_cci_dev->ports[i].base + CCI400_SLAVE_QRSFR_OFFSET);

			/* QoS: [2~4] */
			pr_debug("qrr: 0x%x\n", g_cci_dev->ports[i].qrr);
			writel_relaxed(g_cci_dev->ports[i].qrr, g_cci_dev->ports[i].base + CCI400_SLAVE_QRR_OFFSET);
		}
		else
		{
			break;
...
static int __init cci_probe(void)
{
	...
	struct cci_ace_port *ports;
	void __iomem *cci_ctrl_base = NULL;
	unsigned long cci_ctrl_phys = 0;
	int nb_cci_ports= 0;
	...
	g_cci_dev = (struct cci_dev *)kzalloc(sizeof(struct cci_dev), GFP_KERNEL);
	if (!g_cci_dev)
		return -ENOMEM;
	...
	g_cci_dev->cci_node = np;
	g_cci_dev->nb_cci_ports = nb_cci_ports;
	...
	g_cci_dev->cci_ctrl_base = cci_ctrl_base;
	g_cci_dev->cci_ctrl_phys = cci_ctrl_phys;
	...
	if (of_property_read_u32(np, "speculative-disable", &g_cci_dev->speculative_dis))
	{
		WARN(1, "Bad speculative disable data.\n");
		g_cci_dev->speculative_dis = 0; /* set the default value */
	}
	...
	for_each_child_of_node(np, cp) {
		...
		if (!ret) {
			...
			pr_debug("ports[%d].base=0x%p, ports[i].phys=0x%lx\n", i, ports[i].base, ports[i].phys);
		}
		if (ret || !ports[i].base) {
		...
		if (of_property_read_u32(cp, "qos-control", &ports[i].qcr))
		{
			WARN(1, "Bad qcr data.\n");
			ports[i].qcr = 0;
		}

		if (of_property_read_u32(cp, "regulator-target", &ports[i].rtr))
		{
			WARN(1, "Bad rtr data.\n");
			ports[i].rtr = 0;
		}

		if (of_property_read_u32(cp, "scalar", &ports[i].sfr))
		{
			WARN(1, "Bad sfr data.\n");
			ports[i].sfr = 0;
		}

		if (of_property_read_u32(cp, "qos-range", &ports[i].qrr))
		{
			WARN(1, "Bad qrr data.\n");
			ports[i].qrr = 0;
		}
	 ...
	 */
#ifndef CONFIG_ARM64
	sync_cache_w(g_cci_dev->cci_ctrl_base);
	sync_cache_w(g_cci_dev->cci_ctrl_phys);
	sync_cache_w(g_cci_dev->ports);
	sync_cache_w(&g_cci_dev->cpu_port);
	__sync_cache_range_w(g_cci_dev->ports, sizeof(*g_cci_dev->ports) * g_cci_dev-> nb_cci_ports);
#else
	__flush_dcache_area(g_cci_dev, sizeof(*g_cci_dev));
	__flush_dcache_area(&cpu_port, sizeof(cpu_port));
	__flush_dcache_area(g_cci_dev->ports, sizeof(*g_cci_dev->ports) * g_cci_dev->nb_cci_ports);
#endif
	...
	g_cci_dev->crgctrl_base = of_iomap(np, 0);
	if (!g_cci_dev->crgctrl_base ) {
	...
	pr_info("ARM64 CCI driver probed\n");
	return 0;
