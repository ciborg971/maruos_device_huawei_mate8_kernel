...
#define HI6402_CONFIG_REG 0x20007000
#define HI6402_AP_GPIO0_SEL 0x027
#define HI6402_IRQM_REG3 0x02c
...
struct hi6402_gpio {
	...
	struct hi6402_irq *p_irq_data;
	struct hi6402es_irq *es_p_irq_data;
	int p_irq;
	unsigned reg_offset;
	u8 gpioie;
	u8 gpiois;
...
{
	return chip->es_p_irq_data ? (u8)hi6402es_irq_read(chip->es_p_irq_data, chip->reg_offset + reg)
					: (u8)hi6402_irq_read(chip->p_irq_data, chip->reg_offset + reg); /* [false alarm]:funtion hi6402_gpio_readb return a unsigned value */
}

static void hi6402_gpio_writeb(struct hi6402_gpio *chip, u8 val, unsigned reg)
{
	chip->es_p_irq_data ? hi6402es_irq_write(chip->es_p_irq_data, chip->reg_offset + reg, (unsigned)val)
				: hi6402_irq_write(chip->p_irq_data, chip->reg_offset + reg, (unsigned)val);
}
...
static irqreturn_t hi6402_gpio_irq_handler(int irq, void *data)
{
	...
	if (chip->p_irq_data && hi6402_gpio_irq_is_awake(irq, &gpio))
	...
	for (i = 0; i < HI6402_AWAKE_GPIO_CNT ; i++) {
		if (((gc->base + offset) == gpio_awake_irq[i].binding_gpio) && (gpio_awake_irq[i].is_requested & 0x01)) {
			if ((gpio_awake_irq[i].flag & IRQ_TYPE_EDGE_RISING) && (!(gpio_awake_irq[i].is_requested & 0x02))) {
				ret = devm_request_threaded_irq(gc->dev,
						gpio_awake_irq[i].irq[0],
						NULL, hi6402_gpio_irq_handler,
						IRQF_NO_SUSPEND | IRQF_ONESHOT,
						dev_name(gc->dev), chip);
				if (ret) {
...
static void hi6402_gpio_awake_irq_ctrl(struct hi6402_gpio *chip, int gpio)
{
	...
	data = hi6402_irq_read(chip->p_irq_data,  HI6402_CONFIG_REG + HI6402_IRQM_REG3);
	...
	hi6402_irq_write(chip->p_irq_data,  HI6402_CONFIG_REG + HI6402_IRQM_REG3, data);
}
...
static int hi6402_gpio_irq_setup(struct hi6402_gpio *chip, struct platform_device *pdev)
{
	...
	ret = devm_request_threaded_irq(gc->dev, (unsigned)chip->p_irq,
					NULL, hi6402_gpio_irq_handler,
					IRQF_NO_SUSPEND | IRQF_ONESHOT,
					dev_name(gc->dev), chip);
	if (ret) {
	...
	if (chip->p_irq_data) {
		...
		for (i = 0; i < HI6402_AWAKE_GPIO_CNT; i++) {
			if ((gpio_awake_irq[i].binding_gpio == -1) && (j < size)) {
				...
				hi6402_irq_write(chip->p_irq_data,
					HI6402_CONFIG_REG + HI6402_AP_GPIO0_SEL + i,
...
static int hi6402_gpio_probe(struct platform_device *pdev)
{
	...
	if (of_get_property(np, "chip,type-es", NULL)) {
		chip->es_p_irq_data = dev_get_drvdata(dev->parent);
		chip->p_irq_data = NULL;

		if (of_get_property(np, "chip,clock-en", NULL)) {
			chip->audio_clk = devm_clk_get(dev, "pmu-audio-clk");
			if (IS_ERR(chip->audio_clk)) {
				dev_err(dev, "could not get pmu-audio-clk\n");
				ret = PTR_ERR(chip->audio_clk);
				return ret;
			}

			ret = clk_prepare_enable(chip->audio_clk);
			if (ret) {
				dev_err(dev, "could noe prepare clock enable\n");
				return ret;
			}
		}

	} else {
		chip->p_irq_data = dev_get_drvdata(dev->parent);
		chip->es_p_irq_data = NULL;
	}

	if (!chip->p_irq_data && !chip->es_p_irq_data) {
		dev_err(dev, "could not get parent data\n");
		ret = -ENOENT;
		goto error;
	}
	...
	chip->p_irq = platform_get_irq_byname(pdev, "hi6402_gpio");
	if (chip->p_irq < 0) {
