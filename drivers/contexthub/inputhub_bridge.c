...
void restart_iom3(void)
{
	...
	ret = RPROC_ASYNC_SEND(HISI_RPROC_LPM3_MBX17, (mbox_msg_t *)&reset_msg, 1, ASYNC_MSG, NULL, NULL);
	if (ret) {
	...
	msleep(10);
	 ...
	 ret = RPROC_ASYNC_SEND(HISI_RPROC_IOM3_MBX10, (mbox_msg_t *)&boot_iom3, 1, ASYNC_MSG, NULL, NULL);
         if (ret) {
...
int inputhub_mcu_send(const char *buf, unsigned int length)
{
    ...
    ret = RPROC_SYNC_SEND(HISI_RPROC_IOM3_MBX10 ,(mbox_msg_t *)buf, len, SYNC_MSG, NULL, 0);
    if(ret) {
...
{
    int ret = 0;
    ret = RPROC_ASYNC_SEND(HISI_RPROC_IOM3_MBX10, (mbox_msg_t *)&g_boot_iom3, 1, ASYNC_MSG, inputhub_power_on_complete, NULL);
    if (ret) {
...
recovery_iom3:
	...
	rc = RPROC_ASYNC_SEND(HISI_RPROC_LPM3_MBX17, &tx_buffer, 1, ASYNC_MSG, NULL, NULL);
	if (rc) {
	...
	rc = RPROC_SYNC_SEND(HISI_RPROC_IOM3_MBX10, &tx_buffer, 1, 1, &ack_buffer, 1);
	if (rc) {
