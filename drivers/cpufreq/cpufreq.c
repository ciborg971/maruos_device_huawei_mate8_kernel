...
static int cpufreq_add_dev(struct device *dev, struct subsys_interface *sif)
{
    ...
    /* hold the writer semaphore here to avoid race condition, protect
     * the initialization flow from being interrupted by cpufreq sysfs operations.
     */
#ifdef CONFIG_HISI_CPUFREQ
 ...
 */
#ifdef CONFIG_HISI_CPUFREQ
static void cpufreq_out_of_sync(struct cpufreq_policy *policy, unsigned int old_freq,
...
#ifdef CONFIG_HISI_CPUFREQ
			cpufreq_out_of_sync(policy, policy->cur, ret_freq);
#else
					...
					unsigned int event)
{
	...
	*/
	...
	if ((!policy->governor_enabled && (event == CPUFREQ_GOV_LIMITS || event == CPUFREQ_GOV_STOP)) ||
...
#ifdef CONFIG_HISI_CPUFREQ
	if (policy->min > (&data->cpuinfo)->max_freq || policy->max < (&data->cpuinfo)->min_freq) {
		ret = -EINVAL;
		goto error_out;
	}
#else
	...
	if (cpufreq_driver->setpolicy) {
		...
		if (policy->governor != data->governor) {
			...
			if (failed) {
				...
				pr_err("starting governor %s failed\n",
							data->governor->name);
				if (old_gov) {
...
int cpufreq_update_policy(unsigned int cpu)
{
	...
	if (cpufreq_driver->get) {
		...
		if (!data->cur) {
			pr_err("Driver did not initialize current freq");
			data->cur = policy.cur;
		} else {
			if (data->cur != policy.cur && cpufreq_driver->target)
#ifdef CONFIG_HISI_CPUFREQ
				cpufreq_out_of_sync(data, data->cur,
								policy.cur);
#else
