...
static DEFINE_PER_CPU(struct cpufreq_interactive_tunables *, saved_tunables);
...
static void free_tunables(struct cpufreq_policy *policy, struct cpufreq_interactive_tunables *tunables)
{
	int cpu;
	/*
	 * not really free, but save for next time use
	 * tunables is only freed at module_exit time
	 */
	for_each_cpu(cpu, policy->related_cpus) {
		per_cpu(saved_tunables, cpu) = tunables;
	}
}

static struct cpufreq_interactive_tunables *alloc_tunables(struct cpufreq_policy *policy)
{
	struct cpufreq_interactive_tunables *saved = per_cpu(saved_tunables, policy->cpu);
	struct cpufreq_interactive_tunables *tunables;

	if (saved) {
		int cpu;
		for_each_possible_cpu(cpu) {
			if (per_cpu(saved_tunables, cpu) == saved)
				per_cpu(saved_tunables, cpu) = NULL;
		}
		tunables = saved;
	}
	else {
		...
		unsigned int event)
{
	...
	case CPUFREQ_GOV_POLICY_INIT:
		...
		tunables = alloc_tunables(policy);
		if (!tunables)
			return -ENOMEM;
...
static void __exit cpufreq_interactive_exit(void)
{
	...
	for_each_possible_cpu(i) {
		struct cpufreq_interactive_tunables *tunables = per_cpu(saved_tunables, i);
		if (!tunables)
			continue;
		for_each_possible_cpu(j) {
			if (tunables != per_cpu(saved_tunables, j))
				continue;

			per_cpu(saved_tunables, j) = NULL;
		}
		if (tunables->above_hispeed_delay != default_above_hispeed_delay)
			kfree(tunables->above_hispeed_delay);
		if (tunables->target_loads != default_target_loads)
