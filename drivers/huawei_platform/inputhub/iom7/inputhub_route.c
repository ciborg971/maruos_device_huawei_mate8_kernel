...
ssize_t inputhub_route_read(unsigned short port, char __user *buf, size_t count)
{
    ...
    if ((route_item->pWrite.buffer_size > ROUTE_BUFFER_MAX_SIZE) ||  (route_item->pWrite.buffer_size + (full_pkg_length + LENGTH_SIZE) > ROUTE_BUFFER_MAX_SIZE))
    {
        hwlog_err( "%s:%d write buffer error buffer_size pkg_len\n",__func__,__LINE__,route_item->pWrite.buffer_size, full_pkg_length);
	 goto clean_buffer;
    }
     else
    {
...
static void set_notifier(void)
{
    ...
    if (fb_register_client(&fb_notify)) {
        ...
        dmd_log_report(DSM_SHB_ERR_MCU_SYS_MAIN_INIT, __func__, "register fb client failed\n");
    }

    /**register to reboot notifier**/
    if(register_reboot_notifier(&reboot_notify)){
        hwlog_err("register reboot notify failed in %s\n", __func__);
        dmd_log_report(DSM_SHB_ERR_MCU_SYS_MAIN_INIT, __func__, "register reboot notify failed\n");
    }
#ifdef CONFIG_IOM3_RECOVERY
    ...
    if(register_iom3_recovery_notifier(&recovery_notify)){
        ...
        dmd_log_report(DSM_SHB_ERR_MCU_SYS_MAIN_INIT, __func__, "register recovery notify failed\n");
    }
#endif
}
