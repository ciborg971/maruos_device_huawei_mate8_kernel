...
static irqreturn_t via_gpio_irq_handle(int irq, void* pt_data)
{
    ...
    {
        hwlog_info("%s %d: gpio %s irq level=%d\n", __func__, __LINE__, g_via_gpios.m_arr_via_gpios[p_via_gpio_attr->m_gpio_num].m_gpio_name, level);
    }
