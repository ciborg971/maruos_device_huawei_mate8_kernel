...
static int modem_tty_write(struct tty_struct* pt_tty, const unsigned char* pt_buf, int t_count)
{
    ...
    {
        hwlog_err("%s %d: NULL pointer! pt_tty=0x%x, pt_buf=0x%x, count=%d\n", \
                  __func__, __LINE__, (unsigned int)pt_tty, (unsigned int)pt_buf, t_count);
        return -1;
    }
...
int modem_tty_push(int t_tty_index, unsigned char* pt_payload_buf, int t_payload_len)
{
    ...
    {
        hwlog_err("%s %d: input payload buffer is 0x%x, length is %d\n", __func__, __LINE__, \
                  (unsigned int)pt_payload_buf, t_payload_len);
        return -1;
    }
...
static int modem_tty_dev_cleanup(int t_index)
{
    struct modem_tty_dev* p_tty_dev = NULL;
...
{
    struct mdoem_tty_dev* p_tty_dev = NULL;
    int index = 0;
