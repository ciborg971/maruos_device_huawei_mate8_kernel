...
static int modem_channel_write(unsigned int t_chan_num, const unsigned char* pt_payload_buf, int t_count)
{
    ...
    {
        apcp_current_subpacket_printk(pt_payload_buf, t_count, 1, t_chan_num);
    }
 ...
 */
int modem_tty_write_to_channel(unsigned int t_tty_index, unsigned char* pt_tty_buf, int t_count)
{
    int ret = -1;
    int wr_count = 0;
...
int modem_channel_push(void* pt_pkt_payload_buf, unsigned int t_chan_num, int t_payload_len, unsigned char tranHi)
{
    ...
    {
        hwlog_err("%s %d: NULL pointer! pt_pkt_payload_buf=0x%x\n", \
                  __func__, __LINE__, (unsigned int)pt_pkt_payload_buf);
        return -1;
    }
