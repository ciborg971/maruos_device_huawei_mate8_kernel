...
static int via_sdio_read(void* pt_rx_subpkt_buf, size_t t_len)
{
    ...
    if (ret)
    {
        hwlog_err("%s %d: sdio read with error code = %d, read bytecount = %d!!!\n",
    ...
    {
        hwlog_info("%s %d bytecnt:%d, sdio_readsb return:%d\n", __func__, __LINE__, t_len, ret);
    }
...
static int modem_sdio_remove(struct sdio_func* pt_sdio)
{
    hwlog_info("%s %d: Enter.\n", __func__, __LINE__);
...
static int modem_sdio_suspend(struct device* pt_dev, pm_message_t mesg)
{
    hwlog_info("%s %d: Enter.\n", __func__, __LINE__);
    set_ap_bus_suspend();
    hwlog_info("%s %d: Exit.\n", __func__, __LINE__);
...
int via_sdio_init(void)
{
    ...
    modem_register_reset_handle(MODEM_BUS_SDIO, modem_detect);
    hwlog_info("%s %d: Exit.\n", __func__, __LINE__);
    return ret;
