...
int modem_write(char* pt_pkt_buf, int t_len)
{
    ...
    {
        hwlog_err("%s %d: NULL pointer! pt_pkt_buf=0x%x, len=%d\n", \
                  __func__, __LINE__, (unsigned int)pt_pkt_buf, t_len);
        return -1;
    }
    ...
    hwlog_debug("%s %d: write subpacket addr=0x%x, packet length is %d.\n", \
                __func__, __LINE__, (unsigned int)p_tx_subpkt_buf, left_tx_pkt_len);
    ...
    while (left_tx_pkt_len)
    {
        ...
        if (ret)
        {
            hwlog_err("wait Host controller to resume FAILED!", __func__, __LINE__);
            ret = -5;
            goto down_out;
        }
...
int modem_read(char* pt_buf, int t_include_head, int t_head_size, int t_len)
{
    ...
    int ack_len = 0; /* length of handshake packet */
    char rst_buf[CBP_EXCEPT_STACK_LEN] = {0};
    modem_bus_t* p_cur_bus = NULL;
    unsigned int no_ack_times = 0;
    ...
    {
        hwlog_err("%s %d input parameter invalid, pt_buf=0x%x, t_len=%d\n", __func__, __LINE__, (unsigned int)pt_buf, t_len);
        ret = -1;
        goto out;
    }
    ...
    p_cur_buf = pt_buf;
    hwlog_debug("%s %d: read subpacket buf addr=0x%x, current read length is %d.\n", \
                __func__, __LINE__, (unsigned int)p_cur_buf, cur_read_len);
