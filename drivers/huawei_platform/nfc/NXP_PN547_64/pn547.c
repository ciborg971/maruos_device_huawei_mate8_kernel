...
static int g_nfc_age_test_state_flag = 0;
...
static ssize_t nfc_age_test_flag_state_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	return (ssize_t)(snprintf(buf,  MAX_ATTRIBUTE_BUFFER_SIZE-1, "%d", g_nfc_age_test_state_flag));
}

static ssize_t nfc_age_test_flag_state_store(struct device *dev, struct device_attribute *attr,
								const char *buf, size_t count)
{
	sscanf(buf, "%d", &g_nfc_age_test_state_flag);
	pr_info("%s: g_nfc_age_test_state_flag:%d\n", __func__, g_nfc_age_test_state_flag);
	return (ssize_t)count;
}
...
static struct device_attribute pn547_attr[] ={
	...
	__ATTR(nfc_age_test_flag_state, 0640, nfc_age_test_flag_state_show, nfc_age_test_flag_state_store),
};
                                                                ...
                                                                void *unused)
{
	...
	{
		pr_info("[%s]: %s powerdown with current sim_switch value: %d !\n", __func__, g_nfc_chip_type, nfcdev->sim_switch);
