...
static void synaptics_parse_chip_specific_dts(struct ts_device_data *chip_data)
{
	...
	if (!retval) {
		TS_LOG_INFO("get chip specific holster_switch_addr = %d\n", read_val);
	...
	if (!retval) {
		TS_LOG_INFO("get chip specific glove_switch_addr = %d\n", read_val);
	...
	if (!retval) {
		TS_LOG_INFO("get chip specific roi_control_addr = %d\n", read_val);
	...
	if (!retval) {
		TS_LOG_INFO("get chip specific roi_data_addr = %d\n", read_val);
...
static int synaptics_set_wakeup_gesture_enable_switch(u8 enable)
{
	...
	TS_LOG_INFO("%s, synaptics holster_enable_addr=%d, holster_bit_num=%d\n", __func__, holster_enable_addr, holster_bit_num);
...
static int synaptics_set_holster_switch(u8 holster_switch)
{
	...
	holster_enable_addr = g_ts_data.feature_info.holster_info.holster_switch_addr;
	holster_bit_num = g_ts_data.feature_info.holster_info.holster_switch_bit;
	TS_LOG_INFO("synaptics holster_enable_addr=%d, holster_bit_num=%d\n", holster_enable_addr, holster_bit_num);
...
static int synaptics_set_roi_switch(u8 roi_switch)
{
	...
	TS_LOG_INFO("roi_ctrl_write_addr=0x%4x, roi_control_bit=%d, roi_switch=%d\n", roi_ctrl_addr, roi_control_bit, roi_switch);
...
static int synaptics_calibrate_wakeup_gesture(void)
{
	...
	if (need_calibrate){
		synaptics_rmi4_i2c_write(rmi4_data, addr, &val_2, 1);
		do{
			count ++;
			mdelay(200);
	...
	TS_LOG_INFO("%s, no need to calibrate\n", __func__);
	return 0;
}
...
static int synaptics_calibrate(void)
{
	...
	if (need_calibrate){
		synaptics_rmi4_i2c_write(rmi4_data, addr, &val_1, 1);
		do{
			count ++;
			mdelay(200);
			synaptics_rmi4_i2c_read(rmi4_data, addr, &value, sizeof(value));
...
static int synaptics_after_resume(void *feature_info)
{
	...
	retval = synaptics_set_glove_switch(g_ts_data.feature_info.glove_info.glove_switch);
	if (retval < 0) {
		TS_LOG_ERR("Failed to set glove switch(%d), err: %d\n",
			g_ts_data.feature_info.glove_info.glove_switch, retval);
	}
#endif
