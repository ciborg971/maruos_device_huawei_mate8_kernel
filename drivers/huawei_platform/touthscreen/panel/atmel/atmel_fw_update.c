...
#define T37_COMMAND_RESEND_INDEX  2
	...
	if (buf) {
		...
		if ((u8)command_register[0] != cmd || command_register[1] != page) {
			TS_LOG_ERR("T37 page changed (%d,%d) -> (%d,%d)\n",
					cmd,
					page,
...
static int mxt_psd_get_data(struct mxt_data *data, struct psd_code *code)
{
	int ret = -EINVAL;
	...
	if (!code)
		...
		do {
			...
			if (ret == 0)  {
				TS_LOG_INFO("Found PID head %02x %02x.\n",code->head[0],code->head[1]);
		...
		if (ret == 0) 
			TS_LOG_INFO("Found PDS Data %02x %02x %02x %02x %02x %02x %02x %02x %02x\n",
...
static int mxt_check_reg_init(struct mxt_data *data)
{
	...
	if (config_crc > 0 && (config_crc != calculated_crc))
		TS_LOG_INFO("Config CRC different, calculated=%06X, file=%06X\n",
...
#if 0
int mxt_update_cfg_name_by_fw_name(struct mxt_data *data, char **file_name,
...
int mxt_check_firmware_version(struct mxt_data *data)
{
	...
	TS_LOG_INFO("Firmware does not need updating, firmware version and chiptype is the same:family_id:%d, variant_id:%d, version:%d, build:%d\n", data->info->family_id,
		data->info->variant_id, data->info->version, data->info->build);
	error = -EINVAL;
...
void mxt_update_fw_file_name(struct mxt_data *data, char *file_name)
{
	...
	if (update_sd_mode == 0) {
		 ...
		 */
		offset = snprintf(file_name, PAGE_SIZE, "../../../sdcard/firmware.fw");
	}
...
void mxt_update_config_file_name(struct mxt_data *data, char *file_name)
{
	...
	if (update_sd_mode == 0) {
		...
		offset = snprintf(file_name, PAGE_SIZE, "../../../sdcard/config.raw");
	}
...
int mxt_load_fw(struct mxt_data *data)
{
	...
	while (pos < fw->size) {
		...
		if (ret) {
			TS_LOG_INFO("mxt_check_bootloader fail\n");
			goto release_firmware;
		}
		...
		if (ret) {
			TS_LOG_INFO("mxt_bootloader_write fail\n");
			goto release_firmware;
		}
