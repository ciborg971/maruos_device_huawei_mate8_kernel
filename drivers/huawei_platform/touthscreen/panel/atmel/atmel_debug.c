...
#define MXT_T25_PROCESS_TIMEOUT 1000
...
//	if (data->T37_buf)
	...
	for (i = 0, j = 0; i < T37_tem_buf_size; i +=2, j++) {
		tem_value = (T37_tem_buf[i+1] << 8) | (T37_tem_buf[i]);
		tem_buf[j]  = tem_value;
		//printk("%5d\n", tem_buf[j]);
	}
					...
					const char *buf, size_t count)
{
	...
	TS_LOG_DEBUG("mxt_update_fw_store\n");

	if (!data)
		return -ENODEV;
#if 0
	error = mxt_update_file_name(data, &data->fw_name, buf, count, /*false*/data->alt_chip | ALT_CHIP_BIT_FW);
	if (error)
		return error;
	...
	if (error) {
		...
		TS_LOG_DEBUG("The firmware update succeeded\n");
		...
		const char *buf, size_t count)
{
	...
	int ret;
#if 0
	TS_LOG_DEBUG("mxt_update_cfg_store\n");
	...
	ret = mxt_update_file_name(data, &data->cfg_name, buf, count, data->alt_chip);
	if (ret)
		return ret;
	...
	do{
		...
		msleep(100);
		TS_LOG_ERR("wait interrupt release %d\n",wait);
	}while(--wait >= 0);
...
int mxt_t25_selftest(struct mxt_data *data)
{
	...
	TS_LOG_INFO("%x %x %x %x %x %x %x\n",  data->t25_msg[0], data->t25_msg[1], data->t25_msg[2], data->t25_msg[3], data->t25_msg[4],
		 data->t25_msg[5]);
	...
	struct device_attribute *attr, char *buf)
{
	...
	TS_LOG_INFO("[mxt]Plug cfg :\n");

	TS_LOG_INFO("[mxt]dcfg: t9_t100 start(%u,%u) size(%u,%u)\n",
			pl->init_cfg.t9_t100.x0,
			pl->init_cfg.t9_t100.y0,
			pl->init_cfg.t9_t100.xsize,
			pl->init_cfg.t9_t100.ysize);

	TS_LOG_INFO("[mxt]dcfg: t15 start(%u,%u) size(%u,%u)\n",
			pl->init_cfg.t15.x0,
			pl->init_cfg.t15.y0,
	...
	for (i = 0; i < MX_SUM; i++) {
		...
		TS_LOG_INFO("[mxt]dcfg: Matrix %d (%d,%d)~(%d,%d)\n",
				i,
				m->x0,
				m->y0,
				m->x1,
	...
	TS_LOG_INFO("[mxt]Plug obs :\n");
	TS_LOG_INFO("[mxt]status: Flag=0x%08lx\n",
			obs->flag);
	TS_LOG_INFO("[mxt]\n");
	...
	struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (count > 4 && count < sizeof(name)) {
		...
		TS_LOG_INFO("name %s, offset %d, ret %d\n",name,offset,ret);
		if (ret == 1) {
			if (strncmp(name, "pl", 2) == 0) {
				TS_LOG_INFO("cmd 'pl': %s\n",buf + offset);
				ret = sscanf(buf + offset, "t%d%n", &object, &ofs);
				TS_LOG_INFO("object %d, ofs %d, ret %d\n",object,ofs,ret);
				if (count > 5 &&  ret == 1) {
					offset += ofs;
					TS_LOG_INFO("parse %s\n",buf + offset);
					switch (object) {
						case 8:
						case 100:
							TS_LOG_ERR("[mxt]Not support command\n");
							break;
						default:
							TS_LOG_ERR("[mxt]Invalid object %d, %s\n",object, buf + offset);
							return -EINVAL;
					}
