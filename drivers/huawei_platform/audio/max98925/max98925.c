...
static int max98925_digital_mute(struct max98925_priv *max98925, int mute)
{
    ...
    if (mute) {
		hwlog_info("%s: maxim smartpa mute, is mute = %d\n",__func__, g_ismute);
		...
		hwlog_info("%s: maxim smartpa unmute, is mute = %d\n, ",__func__, g_ismute);
		regmap_update_bits(max98925->regmapL, MAX98925_R02D_GAIN,
...
static int max98925_open(struct inode *inode, struct file *filp)
{
	...
	if ((ret < 0) || (reg == 0x00)) {
		hwlog_err("max98925 initialization error (%d 0x%02X)\n",
			ret, reg);
		goto err_access;
	}
			   ...
			   void __user *p, int compat_mode)
{
	...
	case M98925_SET_DAIFORMAT:
		ret = get_user(value, pUser);
		hwlog_info("%s: maxim smartpa set daiformat: 0x%x\n",__func__, value);
	...
	case M98925_SET_BOOSTVOLT:
		ret = get_user(value, pUser);
		hwlog_info("%s: maxim smartpa set boost voltage: 0x%x\n",__func__, value);
			     ...
			     const struct i2c_device_id *id)
{
	...
	ret = of_property_read_u32(i2c_l->dev.of_node, "spk_gain", &max98925_data->spk_gain);
	if (ret) {
...
struct max98925_static_ops max98925_ops = {
    .max98925_rate_value = max98925_rate_value,
    .max98925_register_show = max98925_register_show,
    .max98925_readable_register = max98925_readable_register,
    .max98925_digital_mute = max98925_digital_mute,
    .max98925_set_slave = max98925_set_slave,
    .max98925_set_master = max98925_set_master,
    .max98925_set_clock = max98925_set_clock,
    .max98925_open = max98925_open,
    .max98925_release = max98925_release,
    .max98925_do_ioctl = max98925_do_ioctl,
    .max98925_ioctl = max98925_ioctl,
    .max98925_i2c_probe = max98925_i2c_probe,
    .max98925_i2c_remove = max98925_i2c_remove,
    .max98925_i2c_shutdown = max98925_i2c_shutdown,
};
#endif
