...
struct max98925_static_ops {
    int (*max98925_rate_value)(int rate, int clock, u8 *value, int *n, int *m);
    ssize_t (*max98925_register_show)(struct device *dev,	struct device_attribute *attr,	char *buf);
    bool (*max98925_readable_register)(struct device *dev, unsigned int reg);
    int (*max98925_digital_mute)(struct max98925_priv *max98925, int mute);
    int (*max98925_set_slave)(struct max98925_priv *max98925);
    void (*max98925_set_master)(struct max98925_priv *max98925);
    int (*max98925_set_clock)(struct max98925_priv *max98925, unsigned int rate);
    int (*max98925_open)(struct inode *inode, struct file *filp);
    int (*max98925_release)(struct inode *inode, struct file *filp);
    int (*max98925_do_ioctl)(struct file *file, unsigned int cmd, void __user *p, int compat_mode);
    long (*max98925_ioctl)(struct file *file, unsigned int command, unsigned long arg);
    int (*max98925_i2c_probe)(struct i2c_client *i2c_l, const struct i2c_device_id *id);
    int (*max98925_i2c_remove)(struct i2c_client *client);
    void (*max98925_i2c_shutdown)(struct i2c_client *client);
};
#endif
