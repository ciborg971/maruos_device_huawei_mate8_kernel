...
HWLOG_REGIST();
#else
#define hwlog_info(fmt,args...) do { printk(KERN_ERR     "[coul_core]" fmt, ## args); } while (0)
#define hwlog_warn(fmt,args...) do { printk(KERN_WARNING "[coul_core]" fmt, ## args); } while (0)
#define hwlog_err(fmt,args...)  do { printk(KERN_INFO    "[coul_core]" fmt, ## args); } while (0)
#endif
...
static void coul_clear_coul_time(void)
{
    ...
    unsigned int time_now = di->coul_dev_ops->get_coul_time();
                ...
                int *ret_pc_unusable)
{
    ...
    hwlog_err("%s,batt_temp_degc = %d,zero_voltage = %d\n",__func__,batt_temp_degc,zero_voltage);
...
int coul_battery_temperature_tenth_degree(void)
{
    ...
    {
        T_adc = hisi_adc_get_adc(HKADC_TEMP_BATTERY);
...
{
    int temp = coul_battery_temperature_tenth_degree();
...
static void get_battery_id_voltage_real(struct smartstar_coul_device *di)
{
    ...
    volt = hisi_adc_get_adc(HKADC_TEMP_BATT_ID);
    if(ret != 0){ // 0 == HKADC_OK
...
static int calculate_state_of_charge(struct smartstar_coul_device *di)
{
    ...
    if( 0 == g_eco_leak_uah)
    {
        ...
        hwlog_info("NOT EXIT FROM ECO, CHANGE <= 1, SOC_NEW = %d\n",soc);
    }
    else
    {
         hwlog_info("EXIT FROM ECO, CHANGE CAN BE BIGGER 1, SOC_NEW = %d\n",soc);
        g_eco_leak_uah = 0;
    }
...
static void coul_charging_done (struct smartstar_coul_device *di)
{
    ...
    if (CHARGING_STATE_CHARGE_START != di->charging_state) {
        hwlog_err("coul_charging_done, but pre charge state is %d \n",
                            di->charging_state);
        return;
    }
    /* enable coul irq */
        ...
        struct device_attribute *attr, char *buf)
{
    ...
    case COUL_SYSFS_GAUGELOG_HEAD:
        return sprintf(buf,"ss_VOL  ss_CUR  ss_ufSOC  ss_SOC  SOC  ss_RM  ss_FCC  ss_UUC  ss_CC  ss_dRC  pdFlag Temp  ss_OCV   rbatt  fcc    ");
    ...
    pm_message_t state)
{
    ...
    int current_sec = di->coul_dev_ops->get_coul_time();
    int wakeup_time = 0;
