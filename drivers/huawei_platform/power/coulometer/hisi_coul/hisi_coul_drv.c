...
#include <huawei_platform/power/hisi_coul_drv.h>
...
HWLOG_REGIST();
#else
#define hwlog_info(fmt,args...) do { printk(KERN_ERR     "[hisi_coul_drv]" fmt, ## args); } while (0)
#define hwlog_warn(fmt,args...) do { printk(KERN_WARNING "[hisi_coul_drv]" fmt, ## args); } while (0)
#define hwlog_err(fmt,args...)  do { printk(KERN_INFO    "[hisi_coul_drv]" fmt, ## args); } while (0)
#endif
