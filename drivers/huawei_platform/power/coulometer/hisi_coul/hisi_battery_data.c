...
#include <huawei_platform/power/hisi_battery_data.h>
...
HWLOG_REGIST();
#else
#define hwlog_info(fmt,args...) do { printk(KERN_ERR "[hisi_battery_data]" fmt, ## args); } while (0)
#define hwlog_warn(fmt,args...) do { printk(KERN_ERR "[hisi_battery_data]" fmt, ## args); } while (0)
...
static int get_dat(struct device_node* np, struct hisi_coul_battery_data *pdat)
{
    ...
    if((array_len <= 0)||(array_len%TEMP_PARA_TOTAL != 0)||(array_len > TEMP_PARA_LEVEL*TEMP_PARA_TOTAL))
    {
    ...
    if((array_len <= 0)||(array_len%VOLT_PARA_TOTAL != 0)||(array_len > VOLT_PARA_LEVEL*VOLT_PARA_TOTAL))
    {
    ...
    if((array_len <= 0)||(array_len%SEGMENT_PARA_TOTAL != 0)||(array_len > SEGMENT_PARA_LEVEL*SEGMENT_PARA_TOTAL))
    {
...
arch_initcall_sync(hisi_battery_init);
