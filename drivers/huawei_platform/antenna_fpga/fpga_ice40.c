...
#define FPGA_TALBE_OUTPUT_LEN 9
...
static int get_image_version(char *buf)
{
    ...
    for(i = 0; i < RETRY_TIME; i++) {
        ...
        ret = snprintf(offset, SPI_OUTPUT_DATA_LEN, "%02x", read_version[3]);
        if(ret < 0) {
            hwlog_err("%s snprintf errror!\n", __func__);
            return SPI_OUTPUT_DATA_LEN;
        }
        offset += ret;
        ret = snprintf(offset, SPI_OUTPUT_DATA_LEN, "%02x", read_version[4]);
        if(ret < 0) {
            hwlog_err("%s snprintf errror!\n", __func__);
            return SPI_OUTPUT_DATA_LEN;
        }
        offset += ret;
        ret = snprintf(offset, SPI_OUTPUT_DATA_LEN, "%02x", read_version[5]);
        if(ret < 0) {
            hwlog_err("%s snprintf errror!\n", __func__);
            return SPI_OUTPUT_DATA_LEN;
        }
        offset += ret;
        ret = snprintf(offset, SPI_OUTPUT_DATA_LEN, "%02x", read_version[6]);
...
int check_input_exc(const char *inputdata, const char *outputdata)
{
    ...
    for(i = 0; i < RETRY_TIME; i++){
        ...
        indata[5] = ((ret>>BYTE_PER_COUNT) & 0x1);
        indata[6] = (ret & 0xff);
        ...
        ret = snprintf(offset, SPI_OUTPUT_DATA_LEN, "%02x", outdata[3]);
        if(ret < 0) {
            hwlog_err("%s snprintf errror!\n", __func__);
            return FAIL;
        }
        offset += ret;
        ret = snprintf(offset, SPI_OUTPUT_DATA_LEN, "%02x", outdata[4]);
        if(ret < 0) {
            hwlog_err("%s snprintf errror!\n", __func__);
            return FAIL;
        }
        offset += ret;
        ret = snprintf(offset, SPI_OUTPUT_DATA_LEN, "%02x", outdata[5]);
        if(ret < 0) {
            hwlog_err("%s snprintf errror!\n", __func__);
            return FAIL;
        }
        offset += ret;
        ret = snprintf(offset, SPI_OUTPUT_DATA_LEN, "%02x\n", outdata[6]);
...
static int fpga_scan(char* string_org, int len)
{
    ...
    kfree(ice40_table);
    return scan_fpga_table(ice40_table, index_talbe);
}
...
static ssize_t show_testmode(struct device *pdev, struct device_attribute *attr, char *buf)
{
    ...
    ret = snprintf(offset, SPI_DATA_LEN, "%02x", outdata[3]);
    if(ret < 0) {
        hwlog_err("%s snprintf errror!\n", __func__);
        return SPI_OUTPUT_DATA_LEN;
    }
    offset += ret;
    ret = snprintf(offset, SPI_DATA_LEN, "%02x", outdata[4]);
    if(ret < 0) {
        hwlog_err("%s snprintf errror!\n", __func__);
        return SPI_OUTPUT_DATA_LEN;
    }
    offset += ret;
    ret = snprintf(offset, SPI_DATA_LEN, "%02x", outdata[5]);
    if(ret < 0) {
        hwlog_err("%s snprintf errror!\n", __func__);
        return SPI_OUTPUT_DATA_LEN;
    }
    offset += ret;
    ret = snprintf(offset, SPI_DATA_LEN, "%02x", outdata[6]);
    return SPI_OUTPUT_DATA_LEN;
}
