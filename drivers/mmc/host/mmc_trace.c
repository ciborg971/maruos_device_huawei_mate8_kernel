...
static ssize_t mmc_trace_proc_write(struct file * file, const char __user * userbuf,
...
static void mmc_trace_printf_putc_init(unsigned n, struct mmc_trace_client *client)
{
	...
	if (((u64)(&(q->data[q->head])) < start_addr) || ((u64)(&(q->data[q->head])) > start_addr + len)) {
...
static void mmc_trace_xputs(const char *s,
	                               void (*mmc_trace_printf_putc)(unsigned n, struct mmc_trace_client *client),
	                               struct mmc_trace_client *client)
{
	while (*s) {
...
static void mmc_trace_xprintf(const char *fmt, va_list ap,
               void (*mmc_trace_printf_putc)(unsigned n, struct mmc_trace_client *client),
               struct mmc_trace_client *client, unsigned int idex)
{
...
static void mmc_trace_rdr_dump( u32 modid, u32 etype, u64 coreid,
...
static int mmc_trace_tofile(MMC_TRACE_TYPE trace_type)
{
	...
	if(trace_type == TRACE_INIT) {
		...
		mmc_trace_size = ((struct mmc_trace_init *)(mmc_trace_fd->mmc_trace_init))->buff_size -
					sizeof(struct mmc_trace_init) - sizeof(struct queue);
		...
		mmc_trace_size = ((struct mmc_trace_comm *)(mmc_trace_fd->mmc_trace_comm))->buff_size -
					sizeof(struct mmc_trace_comm) - sizeof(struct queue);
		...
		mmc_trace_size = ((struct mmc_trace_perf*)(mmc_trace_fd->mmc_trace_perf))->buff_size -
				sizeof(struct mmc_trace_perf) - sizeof(struct queue);
	} else {
	...
	if((NULL == mmc_trace_filepath) || (NULL == mmc_trace_buffer) || (0 == mmc_trace_size)) {
...
void mmc_trace_perf_tofile(void)
{
	...
	record_act = (struct mmc_trace_perf_record_act *)kmalloc(sizeof(struct mmc_trace_perf_record_act), GFP_KERNEL);
...
void mmc_trace_record(struct mmc_host *host, struct mmc_request *mrq)
{
	...
	if(((host->index == EMMC) &&  mmc_trace_fd->mmc_trace_init_mmc_flag) ||
	    ...
	    ((host->index == SDIO) &&  mmc_trace_fd->mmc_trace_init_sdio_flag)) {
	...
	/*通用日志记录*/
		if( (mrq->cmd->opcode == CMD25) || (mrq->cmd->opcode == CMD18) || (mrq->cmd->opcode == CMD13)){
