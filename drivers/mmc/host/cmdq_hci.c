...
static int cmdq_enable(struct mmc_host *mmc)
{
	...
	if (!cq_host || !mmc->card || !mmc_card_mmc(mmc->card) ||
		...
		pr_err("%s: cmdq_enable fail -- cq_host:%p; mmc->card:%p,"
			...
			"mmc->card:%p\n", __func__, cq_host, mmc->card, mmc_card_mmc(mmc->card),
			mmc_card_cmdq(mmc->card), mmc->card);
		err = -EINVAL;
		goto out;
	}
	...
	pr_err("****** %s:line %u  desc_dma_base is 0x%x  desc_base is %p trans_desc_base is %p\n",
