...
static void sdhci_of_arasan_PHY_init(struct sdhci_host *host)
{
	...
	reg_data = reg_data | ((0x8 & SDHCI_PHY_CTRL1_DLL_TRIM_ICP_MASK) << SDHCI_PHY_CTRL1_DLL_TRIM_ICP);
	sdhci_writel(host, reg_data, SDHCI_PHY_CTRL1);
...
static void sdhci_set_sample(struct sdhci_host *host, unsigned char timing, int sam_phase)
{
	...
	pr_debug("%s: timing=%d,SDHCI_PHY_CTRL3=0X%x\n", __func__,
	    timing, sdhci_readl(host, SDHCI_PHY_CTRL3));
}
...
static int sdhci_of_arasan_tuning_find_condition(struct sdhci_host *host, unsigned char timing)
{
	...
	pr_debug("%s: timing is %d, tuning_current_sample = 0X%x, tuning_init_sample=0X%x\n", __func__,
				 timing, sdhci_arasan->tuning_current_sample, sdhci_arasan->tuning_init_sample);
	...
	if (sdhci_arasan->tuning_sample_count >= MAX_TUNING_LOOP) {
		...
		for (mask_lenth = (((SDHCI_TUNING_COUNT - 1 - SDHCI_TUNING_MIN_SAMPLE) >> 1) << 1) + 1;
		     mask_lenth >= 1; mask_lenth -= 2) {
			mask = ((unsigned long)1 << mask_lenth) - 1;
			for (i = (SDHCI_TUNING_MIN_SAMPLE + SDHCI_TUNING_COUNT - 1 - mask_lenth + 1) / 2,
			    j = 1;
			     (i <= SDHCI_TUNING_COUNT - 1 - mask_lenth + 1) &&
				 (i >= SDHCI_TUNING_MIN_SAMPLE);
			     i = ((SDHCI_TUNING_MIN_SAMPLE + SDHCI_TUNING_COUNT - 1 - mask_lenth + 1) /
				  2) +
				 ((j % 2) ? -1 : 1) * (j / 2)) {
				if ((sdhci_arasan->tuning_sample_flag & (mask << i)) ==
				    (mask << i)) {
		...
		if (ret) {
			pr_err("tuning err: no good sam_del, "
					    "timing is %d, tuning_init_sample=%d, tuning_flag = 0x%lx.\n",
				 timing, sdhci_arasan->tuning_init_sample, sdhci_arasan->tuning_sample_flag);
			ret = -1;
			sdhci_set_sample(host, timing, sdhci_arasan->tuning_init_sample);
		}
...
static int sdhci_of_arasan_tuning_move_strobe(struct sdhci_host *host)
{
	...
	for (loop = 0; loop < 2; loop++) {
		...
		if ((sdhci_arasan->tuning_strobe_move_sample > SDHCI_TUNING_STROBE_MAX_SAMPLE) ||
		    (sdhci_arasan->tuning_strobe_move_sample < SDHCI_TUNING_STROBE_MIN_SAMPLE)) {
			continue;
		} else {
			break;
		}
...
static int sdhci_arasan_resume(struct device *dev)
{
	...
	if (host->mmc->ios.timing >= MMC_TIMING_MMC_HS200) {
		pr_info("%s: tuning_move_sample=%d, tuning_init_sample=%d, tuning_strobe_move_sample=%d, tuning_strobe_init_sample=%d.\n",
			__func__, sdhci_arasan->tuning_move_sample, sdhci_arasan->tuning_init_sample,
