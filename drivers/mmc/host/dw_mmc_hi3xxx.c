...
static int hs_timing_config[][TUNING_INIT_TIMING_MODE][TUNING_INIT_CONFIG_NUM] = {
	...
	{ /*MMC*/
	  ...
	  { 400000000  , 7  , 6        , 0     , 15           , 15           , 50000000  } , /* 1: MMC_HS*/                                                                           /* 1: MMC_HS */
...
static void dw_mci_hs_set_ios(struct dw_mci *host, struct mmc_ios *ios)
{
	...
	if (priv->old_power_mode != ios->power_mode) {
		...
		case MMC_POWER_OFF:
			...
			if ((host->hw_mmc_id == DWMMC_SD_ID) &&
				!(check_himntn(HIMNTN_SD2JTAG) || check_himntn(HIMNTN_SD2DJTAG))) {
	...
	if (priv->old_timing != ios->timing) {
		...
		if (host->hw_mmc_id == DWMMC_SD_ID) {
			do {
				reg_value = readl(pericrg_base + PERI_CRG_PERSTAT4);
				reg_value &= GTCLK_SD_EN;
				if (retry == 0) {
					dev_err(host->dev,
						"ios ciu count = %d\n", host->ciu_clk->enable_count);
					break;
				}
				retry--;
			} while (reg_value);
...
static void dw_mci_hi3xxx_work_fail_reset(struct dw_mci *host)
{
	...
	do {
		retval = readl(pericrg_base + PERI_CRG_PERSTAT4);
		retval &= GTCLK_SD_EN;
		if (retry == 0) {
			dev_err(host->dev,
				"rest ciu count = %d\n", host->ciu_clk->enable_count);
...
int current_ptn_num = sizeof(partitions) / sizeof(struct partition);
int flash_find_ptn(const char* str, char* pblkname)
{
       ...
       for(n = 0; n < current_ptn_num; n++) {
               if(partitions[n].name != 0 &&(!strcmp(partitions[n].name, str))) {
                       snprintf(ptn_num, sizeof(ptn_num), "%d", n);
