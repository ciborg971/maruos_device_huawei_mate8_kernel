 ...
 * send mmc command
 */
static int k3v3_mmc_send_cmd(struct raw_k3v3_mmc_host *host,
 ...
 * cofind ext csd
 */
static int k3v3_mmc_update_ext_csd(struct raw_k3v3_mmc_host *host,
 ...
 * write function
 */
static int k3v3_mmc_write(char *buf, sector_t start_sect, sector_t nr_sects,
			unsigned int offset, unsigned int len)
{
	...
	/* FIXME */
	/* how to disable dma */
	writel((readl((void *)(host->mmcioaddr+EMMC_REG_OFF_CTRL)) & (~(BIT_DMA_ENABLE|BIT_INTERNAL_DMA))),
				(void *)(host->mmcioaddr+EMMC_REG_OFF_CTRL));
	...
	while (written_len < total_len) {
		...
		for (loop = 0; loop < temp_len; loop += 4) {
			writel(*(unsigned int *)(buf + written_len + loop),
					(void *)(host->mmcioaddr + EMMC_REG_OFF_FIFO_START));
		}
...
int raw_mmc_panic_write(struct raw_hd_struct *rhd, char *buf,
			unsigned int offset, unsigned int len)
{
	return k3v3_mmc_write(buf, rhd->start_sect, rhd->nr_sects,
			offset, len);
}
