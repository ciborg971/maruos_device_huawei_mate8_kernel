...
static inline void rpmb_combine_request(struct mmc_blk_ioc_data *data,
...
mmc_rpmb_basic_request(uint32_t index,
		       ...
		       uint32_t reliable)
{
	rpmb_combine_request(&rpmb_data->data[0], &request->frame[0], true,
			...
			uint32_t reliable)
{
	...
	rpmb_combine_request(&rpmb_data->data[1], status_frame, true, 0x1,
			     RPMB_BLK_SZ, false);
}
...
mmc_rpmb_result_request(uint32_t index,
			...
			uint32_t reliable)
{
	rpmb_combine_request(&rpmb_data->data[index], &request->frame[0], false,
			     block_count, RPMB_BLK_SZ, false);
}
...
static int32_t mmc_rpmb_work(struct rpmb_request *request)
{
	...
	hisi_rpmb.blkdev = blkdev_get_by_path("/dev/block/mmcblk0rpmb",
...
static int hisi_rpmb_debugfs_init(void)
{
	...
	debugfs_create_file("counter", S_IRUSR | S_IWUSR, debugfs_hisi_rpmb,
...
static int hisi_rpmb_device_init(void)
{
	...
	hisi_rpmb.rpmb_request = ioremap_nocache(rpmb_request_phy, mem_size);
	if (!hisi_rpmb.rpmb_request)
		goto err_node;

	if (atfd_hisi_rpmb_smc(RPMB_SVC_REQUEST_ADDR, rpmb_request_phy, 0x0,
			       0x0)) {
...
static int __init hisi_rpmb_init(void)
{
	hisi_rpmb.rpmb_workqueue = alloc_workqueue(
	    "hisi-rpmb-queue", WQ_MEM_RECLAIM | WQ_NON_REENTRANT, 1);
