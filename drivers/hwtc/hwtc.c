...
static int hwtc_open_generic_tr(struct inode *inode, struct file *filp)
{
	struct hwtc_array *tr = inode->i_private;

	if (hwtc_array_get(tr) < 0)
		return -ENODEV;
...
static int hwtc_release_generic_tr(struct inode *inode, struct file *filp)
{
	struct hwtc_array *tr = inode->i_private;
		      ...
		      size_t cnt, loff_t *ppos)
{
    ...
    hwtc_mark(&tr->table[HWTC_TABLE(info.id)], info.id);
...
static int hwtc_dump_entry(long fd, struct hwtc_entry_t *entry)
{
    ...
    for (; k != entry->time_index; k = ((k + MAX_TIME_RECORD_COUNT - 1) % MAX_TIME_RECORD_COUNT)) {
        time = &entry->time[k];
        if (0 != time->time_delta) {
            len = snprintf(buffer, DUMP_BUFFER_LEN, "B:%ld.%06ld s, E:%ld.%06ld s, Delta:%u us.\n",
    ...
    for (; s != entry->statistic_index; s = ((s+MAX_STATISTIC_RECORD_COUNT-1)%MAX_STATISTIC_RECORD_COUNT)) {
...
Return:         -1 : error, 0 : ok
********************************************************************************/
static int hwtc_dump(struct hwtc_array *tr, uint32_t tag)
{
    ...
    snprintf(file_name, FILE_NAME_LEN, "%s%s.0x%x", exce_dir, HWTC_LOG_NAME, tag); 
...
/*******************************************************************************
Function:       hwtc_dump_write 
Description:    control interface which dump table entry statistic info
Input:          ubuf: input tag value, dump this tag entry statistic info,
                if tag is 0xffffffff, it will dump all entry statistic info
Return:         -1 : error, >0 : ok
********************************************************************************/
static ssize_t
hwtc_dump_write(struct file *filp, const char __user *ubuf,
		      size_t cnt, loff_t *ppos)
{
    struct hwtc_array *tr = (struct hwtc_array*)filp->private_data;
    char buf[64];
    uint32_t tag;
    int ret;
...
static const struct file_operations hwtc_dump_fops = {
	.open		= hwtc_open_generic_tr,
	.llseek		= NULL,
	.release	= hwtc_release_generic_tr,
	.write		= hwtc_dump_write,
};
...
Input:          tag: module tag value, use to distinguish module
                name: function name
                id: table id
                threshold: time diff threshold,when time diff reach this threshold,
                it may do error process
Return:         -1 : error, >0 : ok
********************************************************************************/
static int32_t
...
void
hwtc_end(uint32_t tag, const char *name, int32_t *id)
{
    char buf[MAX_NAME_SIZE];
    int32_t ret;
    ...
    if (hwtc_is_enable) {
        HWTC_SET_END(*id);
        snprintf(buf, MAX_NAME_SIZE, "0x%x|0x%x|pid(%ld)|%s", *id, tag, sys_getpid(), name);
        ret = hwtc_record(tag, buf, *id, 0);
        if (ret < 0) {
            hwlog_warn("%s faild to hwtc record, ret(%d).\n", __func__, ret);
        } else {
            *id = ret;
...
static struct dentry *hwtc_create_file(const char *name,
				 umode_t mode,
				 ...
				 const struct file_operations *fops)
{
	...
	if (!ret) {
		hwlog_warn("Could not create debugfs '%s' entry\n", name);
    }

	return ret;
}

static uint32_t is_beta_version(void)
{
    bool is_beta = false;
 	mm_segment_t old_fs;
	...
	old_fs = get_fs();
	set_fs(KERNEL_DS);
	if (sys_faccessat(AT_FDCWD, "/system/etc/log_collect_service_beta.xml", O_RDWR)) {
        hwlog_info("%s this version is beta version.\n", __func__);
        is_beta = true;
    }
	set_fs(old_fs);
...
static void
init_hwtc_debugfs(struct hwtc_array *tr, struct dentry *d_tracer)
{
    hwtc_create_file("hwtc", 0220, d_tracer,
			  tr, &hwtc_fops);

    hwtc_create_file("hwtc_dump", 0220, d_tracer,
			  tr, &hwtc_dump_fops);
}

static struct dentry *hwtc_init_dentry_tr(struct hwtc_array *tr)
{
	if (tr->dir) {
 		return tr->dir;   
    }
	...
	tr->dir = debugfs_create_dir("hwtc", NULL);
	if (!tr->dir) {
 		hwlog_warn("Could not create debugfs directory 'hwtc'\n");   
    }

	return tr->dir;
}

static struct dentry *hwtc_init_dentry(void)
{
	return hwtc_init_dentry_tr(&global_hwtc);
}
...
static int hwtc_init_data(struct hwtc_array *tr)
{
    ...
    for (i=0; i<HWTC_CTBL_MAX; i++) {
        spin_lock_init(&tr->table[i].lock);
    }
...
static __init int hwtc_init_debugfs(void)
{
	struct dentry *d_tracer;

	d_tracer = hwtc_init_dentry();
	if (!d_tracer)
		return 0;

    hwtc_init_data(&global_hwtc);

	init_hwtc_debugfs(&global_hwtc, d_tracer);

    return 0;
}

fs_initcall(hwtc_init_debugfs); 
