    ...
    struct device_attribute *attr,char *buf)
{
    ...
    char tmp_buf[MAX_ATTRIBUTE_BUFFER_SIZE] = {0};

    if (!access_ok(VERIFY_WRITE, buf, MAX_ATTRIBUTE_BUFFER_SIZE)) {
        return -EINVAL;
    }

    rc = snprintf(tmp_buf, MAX_ATTRIBUTE_BUFFER_SIZE, "%d", state);

    if (copy_to_user(buf, tmp_buf, MAX_ATTRIBUTE_BUFFER_SIZE)) {
    ...
    struct device_attribute *attr, const char *buf, size_t count)
{
    ...
    char tmp_buf[MAX_ATTRIBUTE_BUFFER_SIZE] = {0};
    if (copy_from_user(tmp_buf, buf, MAX_ATTRIBUTE_BUFFER_SIZE)) {
        ...
        struct device_attribute *attr, char *buf)
{
    ...
    if(flash_ctrl->func_tbl->flash_check) {
        ...
        rc = snprintf(tmp_buf, MAX_ATTRIBUTE_BUFFER_SIZE, "%d", status);
    } else {
        cam_err("%s: flash check is NULL", __func__);
        rc = snprintf(tmp_buf, MAX_ATTRIBUTE_BUFFER_SIZE, "%d", FLASH_LED_ERROR);
    }
