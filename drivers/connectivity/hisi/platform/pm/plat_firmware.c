...
int32 read_msg(uint8 *data, int32 len)
{
    ...
    l_len = sdio_patch_readsb(data, len);
...
void *malloc_cmd_buf(uint8 *puc_cfg_info_buf, uint32 ul_index)
{
    ...
    PS_PRINT_INFO("cfg file cmd count: al_count[%d] = %d\n", ul_index, g_st_cfg_info.al_count[ul_index]);
...
int32 recv_device_mem(OS_KERNEL_FILE_STRU *fp, int32 len)
{
    uint8 *pdatabuf;
    int32 l_ret;
    int32 count;
    ...
    if (NULL == pdatabuf)
    {
        PS_PRINT_ERR("kmalloc recv buf fail\n");
        return -EFAIL;
    }
    ...
    while (len > lenbuf)
    {
        l_ret = read_msg(pdatabuf + lenbuf, len - lenbuf);
        if (l_ret > 0)
        {
            lenbuf += l_ret;
        }
...
int32 download_bfgx_cali_data(uint8 *Key, uint8 *Value)
{
    ...
    /*发送bfgx校准数据*/
    l_ret = msg_send_and_recv_except(buff_tx, len, MSG_FROM_DEV_FILES_OK);
    if(0 > l_ret)
    {
                                  ...
                                  OS_KERNEL_FILE_STRU *fp)
{
    ...
    uint32 sdio_transfer_limit = oal_sdio_func_max_req_size(oal_get_sdio_stru());
    ...
    while(remainder > 0)
    {
        ...
        size = min(remainder,sdio_transfer_limit);
        snprintf(buf_tx,sizeof(buf_tx),"%s%c0x%lx%c%d%c",
                                                ...
                                                size,
                                                COMPART_KEYWORD);
        PS_PRINT_INFO("read mem cmd:[%s]\n", buf_tx);
        send_msg(buf_tx, OS_STR_LEN(buf_tx));

        ret = recv_device_mem(fp, size);
        if(ret < 0)
        {
...
int32 sdio_device_mem_dump(struct st_wifi_dump_mem_info *pst_mem_dump_info, uint32 count)
{
    ...
    for (i = 0; i < count; i++)
    {
        ...
        ret = sdio_read_device_mem(&pst_mem_dump_info[i],fp);
        if (ret < 0)
        {
            filp_close(fp, NULL);
            break;
...
int32 exec_number_type_cmd(uint8 *Key, uint8 *Value)
{
    ...
    else if (!OS_STR_CMP((int8 *)Key, RMEM_CMD_KEYWORD))
    {
        ...
        string_to_num(flag, &num);

        PS_PRINT_DBG("recv len [%d]\n", num);

        l_ret = recv_device_mem(fp, num);
    }
...
int32 firmware_download(uint32 ul_index)
{
    ...
    g_ulDataBufLen = oal_sdio_func_max_req_size(oal_get_sdio_stru());
    ...
    for (i = 0; i < g_st_cfg_info.al_count[ul_index]; i++)
    {
        ...
        PS_PRINT_INFO("firmware down start cmd[%d]:type[%d], name[%s]\n", i, l_cmd_type, puc_cmd_name);
        ...
        PS_PRINT_INFO("firmware down finish cmd[%d]:type[%d], name[%s]\n", i, l_cmd_type, puc_cmd_name);
    }
