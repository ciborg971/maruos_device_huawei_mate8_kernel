...
const oal_uint8 *oal_cfg80211_find_ie(oal_uint8 uc_eid, const oal_uint8 *puc_ies, oal_int32 l_len)
{
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(2,6,34))
    return cfg80211_find_ie(uc_eid, puc_ies, l_len);
#else
    while (l_len > 2 && puc_ies[0] != uc_eid)
    {
        l_len   -= puc_ies[1] + 2;
        puc_ies += puc_ies[1] + 2;
    }
                                                    ...
                                                    oal_gfp_enum_uint8      en_gfp)
{
    ...
    action_data_len = ul_len - (OAL_IEEE80211_MIN_ACTION_SIZE - 1);
...
const oal_uint8 *oal_cfg80211_find_ie(oal_uint8 uc_eid, const oal_uint8 *puc_ies, oal_int32 ul_len)
{
    while (ul_len > 2 && puc_ies[0] != uc_eid)
    {
        ul_len -= puc_ies[1] + 2;
        puc_ies += puc_ies[1] + 2;
    }
...
/*lint -e578*//*lint -e19*/
oal_module_symbol(oal_cfg80211_find_ie);
