...
module_param(hisdio_intr_mode, int, S_IRUGO);
...
#endif
module_param(sdio_extend_func, uint, S_IRUGO);

oal_uint32 wifi_patch_enable = 1;
module_param(wifi_patch_enable, uint, S_IRUGO);

oal_uint8 oal_sdio_get_state(struct oal_sdio *hi_sdio);
...
struct oal_sdio* oal_get_sdio_stru(oal_void)
{
    return _hi_sdio_;
}
...
OAL_STATIC OAL_INLINE oal_int32 oal_sdio_xfercount_get(struct oal_sdio *hi_sdio, oal_uint32 *xfercount)
{
    ...
    /*read from 0x0c*/
    *xfercount = sdio_readl(hi_sdio->func, HISDIO_REG_FUNC1_XFER_COUNT, &ret);
...
oal_int32 oal_sdio_data_sg_irq(struct oal_sdio *hi_sdio)
{
    ...
    //OAL_IO_PRINT("rx feed wdg!\n");
    //wlan_pm_feed_wdg(hi_sdio->wlan_pm_info);
...
OAL_STATIC OAL_INLINE oal_int32 oal_sdio_msg_stat(struct oal_sdio *hi_sdio, oal_uint32* msg)
{
    ...
    *msg = sdio_readl(hi_sdio->func,HISDIO_REG_FUNC1_MSG_FROM_DEV,&ret);
#else
    *msg = sdio_readb(hi_sdio->func, HISDIO_REG_FUNC1_MSG_FROM_DEV, &ret);
#endif
    ...
    msg_old = sdio_readb(hi_sdio->func, HISDIO_REG_FUNC1_MSG_HIGH_FROM_DEV, &ret);
    if (ret < 0)
    {
...
OAL_STATIC OAL_INLINE oal_int32 oal_sdio_extend_buf_get(struct oal_sdio *hi_sdio)
{
    ...
    if(sdio_extend_func)
    {
        ...
        ret = sdio_memcpy_fromio(hi_sdio->func, (oal_void*)hi_sdio->sdio_extend,
...
OAL_STATIC OAL_INLINE oal_int32 oal_sdio_extend_buf_get(struct oal_sdio *hi_sdio)
{
    ...
    //if(sdio_extend_func)
    {
        oal_int32 ret = 0;
        oal_memset(hi_sdio->sdio_extend, 0 , sizeof(struct hisdio_extend_func));
        ret = sdio_memcpy_fromio(hi_sdio->func, (oal_void*)&hi_sdio->sdio_extend->credit_info,
                                                                   ...
                                                                   oal_netbuf_head_stru   * head)
{
    ...
    for(i = 0; i < HISDIO_EXTEND_REG_COUNT; i++)
    {
        ...
        netbuf = oal_netbuf_alloc(buff_len_t ,0,0);
        if(NULL == netbuf)
        {
            OAL_IO_PRINT("[WIFI][ERROR]rx no mem:%u\n", buff_len);
    ...
    {
        OAL_IO_PRINT("[WIFI][ERROR]scatt total len[%u] should = xfercount[%u],after pad len:%u\n",
                    sum_len,
...
#else
    netbuf = oal_netbuf_alloc(hi_sdio->sdio_extend->xfer_count ,0,0);
    if(NULL == netbuf)
    {
...
OAL_STATIC OAL_INLINE oal_int32 oal_sdio_get_func1_int_status(struct oal_sdio *hi_sdio, oal_uint8* int_stat)
{
    ...
    *int_stat = sdio_readb(hi_sdio->func, HISDIO_REG_FUNC1_INT_STATUS, &ret);
    if (OAL_UNLIKELY(ret < 0))
    {
...
OAL_STATIC OAL_INLINE oal_int32 oal_sdio_clear_int_status(struct oal_sdio *hi_sdio, oal_uint8 int_stat)
{
    ...
    sdio_writeb(hi_sdio->func, int_stat, HISDIO_REG_FUNC1_INT_STATUS, &ret);
    if (OAL_UNLIKELY(ret < 0))
    {
...
oal_int32 oal_sdio_do_isr(struct oal_sdio *hi_sdio)
{
    ...
    if(OAL_UNLIKELY(OAL_TRUE != oal_sdio_get_state(hi_sdio)))
    {
        if(printk_ratelimit())
            OAL_IO_PRINT("[W][%s]sdio closed,state:%u\n",__FUNCTION__,oal_sdio_get_state(hi_sdio));
        return -OAL_EFAIL;
    }
...
OAL_STATIC irqreturn_t wlan_gpio_irq(oal_int32 irq, oal_void *dev_id)
{
    ...
    oal_spin_lock(&hi_sdio->st_pm_state_lock);
    ...
    oal_spin_unlock(&hi_sdio->st_pm_state_lock);
...
oal_int32 oal_sdio_dev_init(struct oal_sdio *hi_sdio)
{
    ...
    oal_sdio_set_state(hi_sdio, (oal_uint8)OAL_TRUE);
    sdio_release_host(func);
...
#if 0
failed_multi_block:
    sdio_writeb(func, 0, HISDIO_REG_FUNC1_INT_ENABLE, &ret);
#endif
failed_enable_func1:
...
OAL_STATIC oal_void oal_sdio_dev_deinit(struct oal_sdio *hi_sdio)
{
    ...
    oal_sdio_set_state(hi_sdio, (oal_uint8)OAL_FALSE);
    sdio_release_host(func);
...
oal_void oal_sdio_set_state(struct oal_sdio *hi_sdio, oal_uint8 state)
{
    ...
    OAL_IO_PRINT("set sdio state from %u to %u\n", hi_sdio->state, state);
...
OAL_STATIC oal_int32 oal_sdio_probe(struct sdio_func *func, const struct sdio_device_id *ids)
{
    ...
    wake_lock_init(&hi_sdio->st_wklock_wifi, WAKE_LOCK_SUSPEND, "hi110x_wlan_wakelock");
    oal_spin_lock_init(&hi_sdio->st_wklock_spinlock);
    hi_sdio->ul_wklock_cnt      = 0;
#endif
...
void oal_sdio_wake_lock(struct oal_sdio *pst_hi_sdio)
{
#ifdef CONFIG_HAS_WAKELOCK
    oal_uint ul_flags;
    ...
    {
        wake_lock(&pst_hi_sdio->st_wklock_wifi);
    }
    pst_hi_sdio->ul_wklock_cnt++;
    oal_spin_unlock_irq_restore(&pst_hi_sdio->st_wklock_spinlock, &ul_flags);
#endif
}


void oal_sdio_wake_unlock(struct oal_sdio *pst_hi_sdio)
{
#ifdef CONFIG_HAS_WAKELOCK
    oal_uint ul_flags;
...
oal_uint oal_sdio_wakelock_active(struct oal_sdio *pst_hi_sdio)
{
#ifdef CONFIG_HAS_WAKELOCK
    return wake_lock_active(&pst_hi_sdio->st_wklock_wifi);
#endif
    return OAL_FALSE;
}
...
oal_void oal_sdio_wakelocks_release_detect(struct oal_sdio *pst_hi_sdio)
{
    ...
    {
        OAL_IO_PRINT("We still hold %s   %d wake locks, Now release all",
                    pst_hi_sdio->st_wklock_wifi.ws.name,
                    pst_hi_sdio->ul_wklock_cnt);
        pst_hi_sdio->ul_wklock_cnt = 1;
                                      ...
                                      oal_int32 rw)
{
    ...
    //OAL_IO_PRINT("sdio feed wdg!\n");
    if(OAL_PTR_NULL!=hi_sdio->pst_pm_callback->wlan_pm_feed_wdg)
    {
        hi_sdio->pst_pm_callback->wlan_pm_feed_wdg();
    }
...
#ifdef CONFIG_HAS_WAKELOCK
    wake_lock_destroy(&hi_sdio->st_wklock_wifi);
#endif
...
extern oal_int32 sdio_dev_init(struct sdio_func *func);
oal_int32 oal_sdio_device_panic_callback(void *data)
{
    ...
    ret = sdio_reset_comm(hi_sdio->func->card);
    if(ret < 0)
    {
        OAM_ERROR_LOG1(0,OAM_SF_ANY,"[E]try to reset sdio ip failed!ret=%d",ret);
        return OAL_SUCC;
    }
    ret = sdio_dev_init(hi_sdio->func);
    if (ret)
    {
        OAM_ERROR_LOG1(0,OAM_SF_ANY,"[E]try to init sdio ip failed!ret=%d",ret);
        return OAL_SUCC;
    }
    OAM_ERROR_LOG0(0,OAM_SF_ANY,"[E]Reset sdio ip ok.");

    ret = wifi_exception_mem_dump(mem_info,sizeof(mem_info)/sizeof(struct st_wifi_dump_mem_info));
    if( ret < 0)
    {
...
void oal_sdio_tc_extend_001(void)
{
    ...
    ret = sdio_memcpy_fromio(hi_sdio_debug->func, buf, HISDIO_FUNC1_EXTEND_REG_BASE, HISDIO_FUNC1_EXTEND_REG_LEN);
...
void oal_sdio_tc_mem_cp_from(int offset, int len)
{
    ...
    oal_memset(buf, 0 , OAL_NETBUF_LEN(netbuf));
    sdio_claim_host(hi_sdio_debug->func);
    ret = sdio_memcpy_fromio(hi_sdio_debug->func, buf, offset, len);
    sdio_release_host(hi_sdio_debug->func);
    if(ret)
    {
        printk("read failed ret=%d\n", ret);
...
void oal_sdio_read_func1(int offset, int len)
{
    ...
    {
        sdio_claim_host(hi_sdio_debug->func);
        *(buf+i) = sdio_readb(hi_sdio_debug->func, offset+i, &ret);
        sdio_release_host(hi_sdio_debug->func);
        if(ret)
        {
            printk("read error ret=%d\n", ret);
...
void oal_sdio_readsb(int offset, int len)
{
    int ret = 0;
    char *buf = NULL;
    oal_netbuf_stru * netbuf = oal_netbuf_alloc(len, 0, 0);
    ...
    oal_memset(buf, 0 , OAL_NETBUF_LEN(netbuf));
    sdio_claim_host(hi_sdio_debug->func);
    ret = sdio_readsb(hi_sdio_debug->func, buf, offset, len);
    sdio_release_host(hi_sdio_debug->func);
    if(ret)
    {
        printk("read failed ret=%d\n", ret);
