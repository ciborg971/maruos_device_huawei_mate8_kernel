...
oal_uint32  frw_event_lookup_process_entry(frw_event_mem_stru *pst_event_mem, frw_event_hdr_stru *pst_event_hrd)
{
    ...
    {
        OAM_ERROR_LOG2(0, OAM_SF_FRW, "{frw_event_lookup_process_entry::sub_table pointer is NULL! sub_type[0x%x], index[0x%x].}",
    ...
    {
        OAM_ERROR_LOG2(0, OAM_SF_FRW, "{frw_event_lookup_process_entry::sub type pointer is NULL! sub_type[0x%x], index[0x%x].}",
                       uc_sub_type, uc_index);
        return OAL_ERR_CODE_PTR_NULL;
    }
    ...
    if (OAL_PTR_NULL != pst_frw_event_table->pst_sub_table[uc_sub_type].p_tx_adapt_func)
    {
        if (OAL_SUCC != pst_frw_event_table->pst_sub_table[uc_sub_type].p_tx_adapt_func(pst_event_mem))
        {
            OAM_ERROR_LOG2(0, OAM_SF_FRW, "{frw_event_lookup_process_entry::tx adapt process failed! sub_type[0x%x], index[0x%x].}",
                           uc_sub_type, uc_index);
            return OAL_ERR_CODE_ALLOC_MEM_FAIL;
        }
...
*****************************************************************************/
oal_uint8 frw_sched_queue_not_empty(oal_uint32 ul_core_id)
{
    /* 返回OAL_TRUE
        1.调度队列非空
        2.调度队列里有非pause的队列
