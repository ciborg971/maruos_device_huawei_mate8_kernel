 ...
 */
int8  gac_cfg_file[INI_VAR_FILE_LEN] = {INI_FILE_PATH};
int8  gac_file[INI_VAR_FILE_LEN] = {INI_FILE_PATH};
/*
...
static int32 ini_find_mode(INI_FILE *fp, int32 modu, int8 *puc_var, int8 *puc_value)
{
	...
	{
		case INI_MODU_WIFI:			 
			strcpy(auc_mode_var, INI_MODE_VAR_WIFI);
			break;
		case INI_MODU_GNSS:			 
			strcpy(auc_mode_var, INI_MODE_VAR_GNSS);
			break;
		case INI_MODU_BT:			 
			strcpy(auc_mode_var, INI_MODE_VAR_BT);
			break;
		case INI_MODU_FM:			 
			strcpy(auc_mode_var, INI_MODE_VAR_FM);
			break;
		case INI_MODU_WIFI_PLAT:			 
			strcpy(auc_mode_var, INI_MODE_VAR_WIFI_PLAT);
			break;
		case INI_MODU_BFG_PLAT:			 
			strcpy(auc_mode_var, INI_MODE_VAR_BFG_PLAT);
			break;
		case INI_MODU_PLAT:			 
			return INI_SUCC;
			break;
...
static int32 ini_find_modu(INI_FILE *fp, int32 modu, int8 * puc_var, int8 *puc_value)
{
	...
	switch (modu)
	{
		case INI_MODU_WIFI:			 
			...
			{
				strcpy(auc_modu, INI_STR_WIFI_NORMAL);		
			}
			else if (INI_MODE_PERFORMANCE == l_mode_value)
			{
				strcpy(auc_modu, INI_STR_WIFI_PERFORMANCE);		
			}
			else if (INI_MODE_CERTIFY == l_mode_value)
			{
				strcpy(auc_modu, INI_STR_WIFI_CERTIFY);		
			}
            else if (INI_MODE_CERTIFY_CE == l_mode_value)
			{
				strcpy(auc_modu, INI_STR_WIFI_CERTIFY_CE);		
			}
		...
		case INI_MODU_GNSS:			 
			...
			{
				strcpy(auc_modu, INI_STR_GNSS_GPSGLONASS);		
			}
			else if (INI_MODE_BDGPS == l_mode_value)
			{
				strcpy(auc_modu, INI_STR_GNSS_BDGPS);		
			}
			else
			{
				INI_ERROR("not support mode!!!");
			...
			break;
		case INI_MODU_BT:			 
			strcpy(auc_modu, INI_STR_BT_NORMAL);		
			break;
		case INI_MODU_FM:			 
			strcpy(auc_modu, INI_STR_FM_NORMAL);		
			break;
		case INI_MODU_WIFI_PLAT:			 
			strcpy(auc_modu, INI_STR_WIFI_PLAT_NORMAL);		
			break;
		case INI_MODU_BFG_PLAT:			 
			strcpy(auc_modu, INI_STR_BFG_PLAT_NORMAL);		
			break;
		case INI_MODU_PLAT:			 
			strcpy(auc_modu, INI_STR_PLAT);		
			break;
		default :
			INI_ERROR("not suport modu type!!!");
			break;
...
int32 ini_find_var_value(int32 modu, int8 * puc_var, int8* puc_value)
{
	...
	if (sl_once)
	{
		...
		strcpy(g_board_version.board_version, puc_value);
		...
		strcpy(g_param_version.param_version, puc_value);
		fp->f_pos = 0;
	}
#endif
...
static int32 ini_write_mode_value(INI_FILE *fp, int32 modu, int8 * puc_var, int8 * puc_value)
{
	...
	int8 auc_mode_var[INI_STR_MODU_LEN];
	int8 *puc_val = NULL;
	int8 auc_change_bin[INI_STR_MODU_LEN * 2];
	int8 auc_cmd[INI_STR_MODU_LEN * 4];
    ...
    if (INI_MODU_WIFI == modu)
    {
        if ('0' == puc_value[0])
        {
            strcpy(auc_change_bin, INI_SDIOBIN_NORMAL);
        }
        else if ('1' == puc_value[0])
        {
            strcpy(auc_change_bin, INI_SDIOBIN_PERFORMANCE);
        }
        else if ('2' == puc_value[0])
        {
            strcpy(auc_change_bin, INI_SDIOBIN_CERTIFY);
        }
        else
        {
        ...
        sprintf(auc_cmd, "cp %s %s", auc_change_bin, INI_SDIOBIN_DEFAULT);	
