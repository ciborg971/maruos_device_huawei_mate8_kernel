...
int32 ps_exe_sys_func(struct ps_core_s *ps_core_d, uint8 *buf_ptr)
{
    ...
    case SYS_INF_PF_INIT:
        ...
        //complete(&ps_core_d->bfgx_init_ok);
        break;
    ...
    case SYS_INF_BFG_HEART_BEAT:
        PS_PRINT_INFO("receive heart beat from bfgx\n");
    ...
    case SYS_INF_DEV_AGREE_HOST_SLP:
        ...
        del_timer(&pm_data->dev_ack_timer);
                                ...
                                uint16 pkt_len, uint8 type)
{
    ...
    if (NULL == skb) {
        PS_PRINT_ERR("can't allocate mem for new debug skb\n");
        return -EINVAL;
    }
...
int32 ps_recv_debug_data(struct ps_core_s *ps_core_d, uint8 *buf_ptr)
{
    ...
    if ((PACKET_START_SIGNAL == buf_ptr[0])&&(PACKET_RX_FUNC_LAST_WORDS == buf_ptr[1]))
    {
        dbg_pkt_lenth = buf_ptr[5]*0x100;
        dbg_pkt_lenth = dbg_pkt_lenth + buf_ptr[4];
    ...
    if ((PACKET_START_SIGNAL == buf_ptr[0])&&(PACKET_RX_FUNC_DUMP_STACK_SIZE == buf_ptr[1]))
    {
        dbg_pkt_lenth = buf_ptr[5]*0x100;
        dbg_pkt_lenth = dbg_pkt_lenth + buf_ptr[4];
        PS_PRINT_ERR("prepare to recv bfgx stack size [%d]!\n", dbg_pkt_lenth);
        prepare_to_recv_bfgx_stack(dbg_pkt_lenth);
        return 0;
    }

    if ((PACKET_START_SIGNAL == buf_ptr[0])&&(PACKET_RX_FUNC_DUMP_STACK == buf_ptr[1]))
    {
        dbg_pkt_lenth = buf_ptr[5]*0x100;
        dbg_pkt_lenth = dbg_pkt_lenth + buf_ptr[4];
        if (dbg_pkt_lenth == rx_pkt_total_len)
        {
            ptr = buf_ptr + sizeof(struct st_om_mesg_heager);
            dump_stack_size = dbg_pkt_lenth - sizeof(struct st_om_mesg_heager);
            PS_PRINT_INFO("recv bfgx stack size [%d]!\n", dump_stack_size);
            bfgx_recv_dev_mem(ptr, dump_stack_size);
        }
...
int32 ps_recv_sepreated_data(struct ps_core_s *ps_core_d, uint8 *buf_ptr, uint8 subsys, uint8 sepreted_type)
{
    ...
    if (SUBSYS_STATE_OPEN != atomic_read(&pst_bfgx_data->subsys_state))
    {
        PS_PRINT_ERR("bfgx subsys %d has closed, no need to recv data\n", subsys);
    ...
    if (SEPRETED_RX_PKT_SEQ_CORRECT == seq_correct)
    {
        ...
        {
            PS_PRINT_ERR("sepreted rx data lenth err! give up this total pkt, subsys=%d\n", subsys);
    ...
    {
        PS_PRINT_ERR("bfgx sepreted rx seq is err! sepreted_type=%d, rx_prev_seq=%d\n",
                                                      sepreted_type, pst_sepreted_data->rx_prev_seq);
        spin_unlock(&pst_sepreted_data->sepreted_rx_lock);
        return -EINVAL;
    }
    ...
    if (RX_SEQ_LAST == sepreted_type)
    {
        ...
        if (ret < 0)
        {
            PS_PRINT_ERR("push sepreted rx data to skb failed! subsys=%d\n", subsys);
            spin_unlock(&pst_sepreted_data->sepreted_rx_lock);
            return -EINVAL;
        }

        /*现在skb中已经有正确完整的数据，唤醒等待数据的进程*/
        PS_PRINT_DBG("bfgx sepreted rx done!subsys=%d,qlen=%d\n", subsys, pst_bfgx_data->rx_queue.qlen);
        ...
        if (pst_bfgx_data->rx_queue.qlen > g_bfgx_rx_queue_max_num[subsys])
        {
            ...
            PS_PRINT_WARNING("bfgx sepreted rx queue too large! subsys=%d\n", subsys);
        }
    }
    spin_unlock(&pst_sepreted_data->sepreted_rx_lock);
...
int32 ps_recv_no_sepreated_data(struct ps_core_s *ps_core_d, uint8 *buf_ptr, uint8 subsys)
{
    ...
    if (SUBSYS_STATE_OPEN == atomic_read(&pst_bfgx_data->subsys_state))
    {
        ...
        if (ret < 0)
        {
            PS_PRINT_ERR("push no sepreted rx data to skb failed! subsys=%d\n", subsys);
            return -EINVAL;
        }

        PS_PRINT_DBG("bfgx no sepreted rx done!subsys=%d,qlen=%d\n", subsys, pst_bfgx_data->rx_queue.qlen);
        wake_up_interruptible(&pst_bfgx_data->rx_wait);
        if (pst_bfgx_data->rx_queue.qlen > g_bfgx_rx_queue_max_num[subsys])
        {
            tty_throttle(ps_core_d->tty);
            atomic_set(&ps_core_d->throttle_flag, 1);
            PS_PRINT_WARNING("bfgx no sepreted rx queue too large! subsys=%d\n", subsys);
        }
    }
...
int32 ps_decode_packet_func(struct ps_core_s *ps_core_d, uint8 *buf_ptr)
{
    ...
    {
        if ((SYS_INF_DEV_AGREE_HOST_SLP != *ptr) &&
             (SYS_INF_BFG_HEART_BEAT != *ptr))
        {
    ...
    /* if some data comes from device which is not the dev_ack msg,
        ...
        (SYS_INF_DEV_AGREE_HOST_SLP != *ptr))
    {
        ...
        PS_PRINT_INFO("recv data before dev_ack\n");
    }
    ...
    {   /* if count less then sys packet lenth and continue */
        ...
        if (MSG_BUTT > ps_core_d->rx_pkt_type)
        {
            ...
            switch (ps_core_d->rx_pkt_type)
	        {
                ...
                case NFC_Last_MSG:
                case OML_MSG:
                    if (MAX_NOBT_FRAME_LEN < ps_core_d->rx_pkt_total_len)
                    {
...
int32 ps_core_recv_uart_test(void *disc_data, const uint8 *data, int32 count)
{
	...
	PS_PRINT_INFO("ps_core_recv enter!\n");

	PS_PRINT_INFO("buffer = 0x%p, count = %d\n", data, count);

	ps_get_core_reference(&ps_core_d);
    if (unlikely(NULL == data) || (NULL == disc_data) || (NULL == ps_core_d))
    {
        PS_PRINT_ERR(" received null from TTY \n");
        return 0;
    }
	...
	PS_PRINT_INFO("before rx_debug quenen is %d\n", ps_core_d->rx_dbg_seq.qlen);

	ret = ps_push_skb_debug_queue(ps_core_d, data, count, RX_DBG_QUEUE);
	if (ret < 0)
	{
		PS_PRINT_ERR("ps_push_skb_debug_queue fail\n");
		return -EINVAL;
	}

	PS_PRINT_INFO("end rx_debug quenen is %d\n", ps_core_d->rx_dbg_seq.qlen);
    ...
    if (ps_core_d->rx_dbg_seq.qlen > RX_DBG_QUE_MAX_NUM)
    {
        tty_throttle(ps_core_d->tty);
        atomic_set(&ps_core_d->throttle_flag, 1);
        PS_PRINT_WARNING(" rx dbg queue too large!\n");
    }
...
int32 ps_core_init(struct ps_core_s **core_data)
{
    ...
    for(i = 0; i < BFGX_BUTT; i++)
    {
        ...
        atomic_set(&ps_core_d->bfgx_info[i].subsys_state, SUBSYS_STATE_SHUTDOWN);
    }

    /*bfgx打开关闭和异常处理同步*/
    mutex_init(&ps_core_d->bfgx_mutex);

    //init_completion(&ps_core_d->bfgx_init_ok);
