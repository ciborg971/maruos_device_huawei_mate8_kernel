...
#else
#define MAC_RES_VAP_SIZE        1392     /* DMAC VAP的大小目前为1208，预留40字节余量 */
#endif
...
#define MAC_RES_USER_SIZE       1400
    ...
    /* mac_vap_stru, hmac_vap_stru私有部分, dmac_vap_stru私有部分总规格 */
#ifdef _PRE_WLAN_FEATURE_CUSTOM_SECURITY
#define MAC_RES_VAP_SIZE        5728
#else
#define MAC_RES_VAP_SIZE        5728 /*适配ARM64 都对齐到8B*/
#endif
#ifdef CONFIG_ARM64
/*when arm64, c pointer change to 8 bytes*/
#define MAC_RES_USER_SIZE       4008
#else
    /* mac_user_stru, hmac_user_stru私有部分, dmac_user_stru私有部分总规格，考虑到ARM64都对齐到8B */
#define MAC_RES_USER_SIZE       2992
#endif
...
#endif
    /* mac_user_stru, hmac_user_stru私有部分, dmac_user_stru私有部分总规格，必须四字节对齐 */
#define MAC_RES_USER_SIZE       3700 /* 原来为3560,后增大结构体成员后为3640,预留60（原因：51双核加载dmac出现改结构体过大）*/
...
OAL_STATIC OAL_INLINE mac_device_stru*  mac_res_get_dev(oal_uint32 ul_dev_idx)
{
    ...
    {
        OAM_ERROR_LOG1(0, OAM_SF_ANY, "{mac_res_get_dev::invalid ul_dev_idx[%d].}", ul_dev_idx);
        return OAL_PTR_NULL;
    }
...
*****************************************************************************/
OAL_STATIC OAL_INLINE oal_void*  mac_res_get_hmac_user(oal_uint16 us_idx)
{
    if (OAL_UNLIKELY(us_idx >= g_us_max_asoc_user * MAC_RES_MAX_DEV_NUM + WLAN_SERVICE_VAP_MAX_NUM_PER_DEVICE * MAC_RES_MAX_DEV_NUM))
    {
        return OAL_PTR_NULL;
...
*****************************************************************************/
OAL_STATIC OAL_INLINE oal_void*  mac_res_get_mac_user(oal_uint16 us_idx)
{
    if (OAL_UNLIKELY(us_idx >= (g_us_max_asoc_user * MAC_RES_MAX_DEV_NUM + WLAN_SERVICE_VAP_MAX_NUM_PER_DEVICE * MAC_RES_MAX_DEV_NUM)))
    {
        return OAL_PTR_NULL;
...
*****************************************************************************/
OAL_STATIC OAL_INLINE oal_void*  mac_res_get_dmac_user(oal_uint16 us_idx)
{
...
{
    return (g_us_max_asoc_user * MAC_RES_MAX_DEV_NUM + WLAN_SERVICE_VAP_MAX_NUM_PER_DEVICE * MAC_RES_MAX_DEV_NUM);
}
