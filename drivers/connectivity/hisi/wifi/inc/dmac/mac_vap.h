...
#define MAC_11I_ASSERT_PRINTERR1(_cond, _errcode, _pst_mac_vap, _err_info, _errparam) do{\
if(!(_cond))\
{\
    if(OAL_PTR_NULL != _pst_mac_vap)\
    {\
        OAM_ERROR_LOG1(_pst_mac_vap->uc_vap_id, OAM_SF_WPA, _err_info, _errparam);\
        return _errcode;\
    }\
    OAM_ERROR_LOG1(0, OAM_SF_WPA, _err_info, _errparam);\
    return _errcode;\
}\
}while(0);
...
typedef enum
{
    ...
    MAC_VAP_STATE_AP_PAUSE           = 4,       /* 此状态遗弃，用MAC_VAP_STATE_PAUSE */

    /* sta独有状态 */
    MAC_VAP_STATE_STA_FAKE_UP       = 5,
    MAC_VAP_STATE_STA_WAIT_SCAN     = 6,
    MAC_VAP_STATE_STA_SCAN_COMP     = 7,
    MAC_VAP_STATE_STA_WAIT_JOIN     = 8,
    MAC_VAP_STATE_STA_JOIN_COMP     = 9,
    MAC_VAP_STATE_STA_WAIT_AUTH_SEQ2 = 10,
    MAC_VAP_STATE_STA_WAIT_AUTH_SEQ4 = 11,
    MAC_VAP_STATE_STA_AUTH_COMP      = 12,
    MAC_VAP_STATE_STA_WAIT_ASOC      = 13,
    MAC_VAP_STATE_STA_OBSS_SCAN      = 14,
    MAC_VAP_STATE_STA_BG_SCAN        = 15,

//#ifdef _PRE_WLAN_FEATURE_P2P
    MAC_VAP_STATE_P2P_LISTEN         = 16,/* p2p0 监听 */
//#endif /* _PRE_WLAN_FEATURE_P2P */
#ifdef _PRE_WLAN_FEATURE_ROAM
    MAC_VAP_STATE_ROAMING            = 17,/* 漫游 */
...
typedef enum
{
    ...
    MAC_ALG_CFG_ANTI_INTF_RSSI_STAT_MODE,
...
#define MAC_VAP_AP_STATE_BUTT       (MAC_VAP_STATE_AP_PAUSE + 1)
...
typedef struct
{
    ...
    oal_uint8   uc_cfg_pm_mode_manual;    //手工配置节能模式
...
typedef struct
{
    oal_uint8   uc_psm_info_enable      :1;  //开启psm的统计维测输出
    oal_uint8   uc_psm_debug_mode       :1;	 //开启psm的debug打印模式
    oal_uint8   uc_psm_resd             :6;
...
typedef struct
{
    oal_uint8    uc_sta0_vote      :4;
    oal_uint8    uc_sta1_vote      :4;
    oal_uint8    uc_sta2_vote      :4;
    oal_uint8    uc_ap_vote        :4;
}mac_cfg_ps_vote_stru;

typedef enum {
    MAC_STA_OFF_PM     = 0,                /* 关闭低功耗 */
    MAC_STA_HOST_OPEN_PM,                  /* host下发开低功耗 */
    MAC_STA_DBAC_OFF_PM,                   /* DBAC关闭低功耗 */
    MAC_STA_DBAC_OPEN_PM,                  /* DBAC下发开低功耗 */
    MAC_STA_PM_OPEN_BUTT                   /*最大状态*/
}mac_pm_open_mode;
...
extern oal_void  mac_vap_state_change_to_sdt(mac_vap_stru *pst_mac_vap, mac_vap_state_enum_uint8 en_vap_state);
