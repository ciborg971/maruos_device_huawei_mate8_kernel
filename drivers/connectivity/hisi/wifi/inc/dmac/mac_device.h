...
typedef struct
{
    mac_scan_cb_fn  p_fn_cb;                    /* PNO扫描完成，如果有结果上报时的回调函数指针 */
...
typedef struct
{
    oal_uint32                          ul_keepalive_limit;                    /* keepalive保活门限(总时间)  */
    oal_uint16                          us_keepalive_interval;                 /* keepalive保活间隔时间 */
    oal_uint8                           uc_keepalive_num;                      /* ap侧keepalive操作容忍的失败次数门限 */
    oal_uint8                           uc_aging_to_keepalive_num;             /* ap侧多少次agingtimrt触发一次keepalive操作 */
}mac_keepalive_stru;
...
/* end: P2P */
    ...
    oal_uint8                           auc_resv23[3];

    mac_keepalive_stru                  st_keepalive_dev;                      /* device结构体下keepalive特性结构体 */
    ...
    frw_timeout_stru                    bt_coex_abort_timer;                     /* 读取寄存器周期定时器 */
    oal_uint32                           ul_bar_size;
#endif
    ...
    oal_wiphy_mgmt_stru                 st_wiphy_mgmt;                         /* 用于管理Device级别，与内核交互的控制信息，如是否在扫描等  */
...
#define OAL_GET_KEEPALIVE_INTERVAL(_pst_dev)       ((_pst_dev)->st_keepalive_dev.us_keepalive_interval)
#define OAL_GET_KEEPALIVE_LIMIT(_pst_dev)          ((_pst_dev)->st_keepalive_dev.ul_keepalive_limit)
...
/*****************************************************************************
 函 数 名  : mac_need_enqueue_fake_queue_for_dbac
 功能描述  : 关联帧是否需要直接如fake队列
 输入参数  : mac_device_stru *pst_device
             mac_vap_stru *pst_vap
 输出参数  : 无
 返 回 值  : OAL_STATIC  OAL_INLINE  oal_bool_enum_uint8
...
*****************************************************************************/
OAL_STATIC  OAL_INLINE  oal_bool_enum_uint8 mac_need_enqueue_fake_queue_for_dbac(hal_to_dmac_device_stru *pst_hal_device, mac_vap_stru *pst_vap)
{
    return  pst_hal_device->uc_current_chan_number != pst_vap->st_channel.uc_chan_number ? OAL_TRUE : OAL_FALSE;
}
#endif
#ifdef _PRE_SUPPORT_ACS
/*****************************************************************************
