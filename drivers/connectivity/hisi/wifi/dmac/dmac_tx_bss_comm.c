...
{
    oal_uint8           auc_bcast_addr[WLAN_MAC_ADDR_LEN] = {0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF};
    mac_user_stru      *pst_mac_user;

    if (OAL_TRUE == MAC_GET_CB_IS_MCAST(pst_tx_cb))
    {
        oal_set_mac_addr(auc_user_macaddr, auc_bcast_addr);
    }
    else
    {
...
oal_void  dmac_tx_pause_info(hal_to_dmac_device_stru *pst_hal_device, dmac_vap_stru *pst_dmac_vap, oal_netbuf_stru *pst_netbuf)
{
    ...
    OAM_WARNING_LOG4(0, OAM_SF_TX, "{dmac_tx_pause_info::uc_is_paused = %d, uc_num_dq = %d, uc_retry_num = %d, us_mpdu_num = %d",
    ...
    {
        OAM_WARNING_LOG4(0, OAM_SF_TX, "{dmac_tx_pause_info::start = %d, last_seq_num = %d, size = %d, baw_head = %d",
    ...
    {
        OAM_WARNING_LOG1(0, OAM_SF_TX, "{dmac_tx_pause_info::warning!!! retry num:%d != retry q element num.}", ul_count);
    }
    ...
    {
        OAM_WARNING_LOG1(0, OAM_SF_TX, "{dmac_tx_pause_info::warning!!! mpdu num:%d != retry q and netbuf q element num.", ul_count);
    }
#endif /* _PRE_WLAN_FEATURE_TX_DSCR_OPT */
...
OAL_STATIC OAL_INLINE oal_void mac_check_cb_tid(mac_tx_ctl_stru *pst_tx_ctrl)
{
    ...
    {
        oam_report_netbuf_cb(auc_user_macaddr,(oal_uint8 *)pst_tx_ctrl,OAM_OTA_TYPE_TX_CB);
                ...
                hal_tx_txop_alg_stru     *pst_txop_alg)
{
    ...
    static oal_uint8  s_cipher_protocol_type = WLAN_80211_CIPHER_SUITE_NO_ENCRYP;
    ...
    if(s_cipher_protocol_type != st_txop_feature.pst_security->en_cipher_protocol_type)
    {
        OAM_WARNING_LOG3(pst_dmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_TX, "{dmac_tx_data: en_cipher_key_type = %d, en_cipher_protocol_type = %d, uc_cipher_key_id = %d}",
                        st_txop_feature.pst_security->en_cipher_key_type,st_txop_feature.pst_security->en_cipher_protocol_type, st_txop_feature.pst_security->uc_cipher_key_id);
        s_cipher_protocol_type = st_txop_feature.pst_security->en_cipher_protocol_type;
    }
    ...
    if (pst_bt_status->bit_bt_on)
    {
        OAL_DLIST_SEARCH_FOR_EACH(pst_dscr_entry, pst_tx_dscr_list_hdr)
        {
            ...
            if (OAL_TRUE == MAC_GET_CB_IS_VIPFRAME(pst_cb))
            {
                ...
                /* 软件拉高priority */
                dmac_btcoex_wlan_priority_set(1);
                ...
                /*为提高p2p配对成功率，在非BT音乐和电话场景下，保护EAPOL帧收发*/
                if((uc_data_type == MAC_DATA_EAPOL) && !(pst_bt_status->bit_bt_sco) && !(pst_bt_status->bit_bt_a2dp))
                {
    ...
    if (0 != ul_tx_q_status)
    {
        ...
        OAM_WARNING_LOG2(pst_dmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_TX, "dmac_tx_data::111 ul_tx_q_status = [%d], ppdu_cnt[%d]",
                         ul_tx_q_status, pst_hal_device->ast_tx_dscr_queue[uc_q_num].uc_ppdu_cnt);
    }
    else
    {
...
#endif /* _PRE_WLAN_FEATURE_TX_DSCR_OPT */
{
    ...
    {
        OAM_ERROR_LOG0(pst_tid_queue->uc_vap_id, OAM_SF_TX, "{dmac_tid_tx_queue_enqueue::tid_stats is null.}");
                                                      ...
                                                      oal_netbuf_stru *pst_netbuf)
{
    ...
    {
        OAM_ERROR_LOG0(pst_dmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_TX, "{dmac_tx_dscr_alloc::tx pkt is dropped, pst_tx_dscr alloc failed.}");
...
OAL_INLINE oal_uint32  dmac_tx_process_data(hal_to_dmac_device_stru *pst_hal_device, dmac_vap_stru *pst_dmac_vap, oal_netbuf_stru *pst_netbuf)
{
...
#if 0// def _PRE_WLAN_FEATURE_STA_PM
    /* sta mode下睡眠异常检查 */
    if (WLAN_VAP_MODE_BSS_STA == pst_dmac_vap->st_vap_base_info.en_vap_mode)
    {
        if((STA_PWR_SAVE_STATE_DOZE == dmac_psm_get_state(pst_dmac_vap))&& PM_WLAN_IsHostSleep())
        {
            OAL_IO_PRINT("dmac_tx_process_data in psm doze state\r\n");
            return OAL_FAIL;
        }
    }
#endif
    ...
    if (OAL_UNLIKELY(OAL_PTR_NULL == pst_tx_dscr))
    {
        OAM_ERROR_LOG0(pst_dmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_TX, "{dmac_tx_process_data::pst_tx_dscr alloc failed.}");
...
oal_uint32  dmac_tx_process_data_event(frw_event_mem_stru *pst_event_mem)
{
    ...
    while (pst_netbuf != OAL_PTR_NULL)
    {
        ...
        #if (_PRE_MULTI_CORE_MODE_OFFLOAD_DMAC == _PRE_MULTI_CORE_MODE)
                (MAC_DATA_ARP == uc_data_type) ||
        #endif
                (MAC_DATA_EAPOL == uc_data_type))
            {
                OAM_WARNING_LOG2(pst_event_hdr->uc_vap_id, OAM_SF_WPA, "{dmac_tx_process_data_event::datatype==%u, qos==%u}[0:dhcp 1:arp 2:eapol]", uc_data_type, mac_get_cb_is_qosdata(pst_tx_ctl));
        ...
        if (OAL_UNLIKELY(OAL_SUCC != ul_ret))
        {
            OAM_WARNING_LOG1(pst_event_hdr->uc_vap_id, OAM_SF_TX, "{dmac_tx_process_data_event::dmac_tx_process_data fail[%u].}", ul_ret);
            dmac_tx_excp_free_netbuf(pst_netbuf);
        }
...
oal_bool_enum_uint8 dmac_tx_dscr_need_dbac_buffer(hal_to_dmac_device_stru *pst_hal_device, dmac_vap_stru *pst_dmac_vap)
{
#ifdef _PRE_WLAN_FEATURE_DBAC
    mac_device_stru *pst_mac_device = mac_res_get_dev((oal_uint32)pst_hal_device->uc_mac_device_id);
    if (OAL_PTR_NULL == pst_mac_device)
    {
        OAM_ERROR_LOG1(0, OAM_SF_TX, "{dmac_tx_dscr_need_dbac_buffer::pst_mac_device null. dev id:%d}", pst_hal_device->uc_mac_device_id);
        return OAL_FALSE;
    }

    if (mac_is_dbac_running(pst_mac_device) && mac_need_enqueue_fake_queue_for_dbac(pst_hal_device, &pst_dmac_vap->st_vap_base_info))
    {
        return OAL_TRUE;
    }
#endif
...
OAL_STATIC oal_uint32  dmac_tx_mgmt_hw_send(hal_to_dmac_device_stru *pst_hal_device, dmac_vap_stru *pst_dmac_vap, hal_tx_dscr_stru *pst_mgmt_dscr, mac_tx_ctl_stru *pst_tx_ctl)
{
    ...
    *****************************************************************************/
    ...
    if (0 != ul_tx_q_status)
    {
        ...
        OAM_WARNING_LOG2(0, OAM_SF_TX, "dmac_tx_mgmt_hw_send::111 ul_tx_q_status = [%d], ppdu_cnt[%d]",
                         ul_tx_q_status, pst_hal_device->ast_tx_dscr_queue[uc_q_num].uc_ppdu_cnt);
    }
    else
    {
...
/*****************************************************************************
 函 数 名  : dmac_get_mac_user_by_macaddr
...
*****************************************************************************/
OAL_STATIC OAL_INLINE mac_user_stru* dmac_get_mac_user_by_macaddr(mac_vap_stru *pst_vap, oal_uint8 *puc_sta_mac_addr)
{
    oal_uint32                  ul_ret;
    oal_uint16                  us_user_idx;

    ul_ret = mac_vap_find_user_by_macaddr(pst_vap, puc_sta_mac_addr, &us_user_idx);
    if (OAL_SUCC != ul_ret)
    {
        return OAL_PTR_NULL;
    }
    return (mac_user_stru *)mac_res_get_mac_user(us_user_idx);
}
#ifdef _PRE_WLAN_FEATURE_ROAM
...
oal_uint32  dmac_tx_force(dmac_vap_stru *pst_dmac_vap, oal_netbuf_stru *pst_netbuf, oal_uint16 us_len, oal_uint8 uc_mgmt)
{
    ...
    {
       OAM_ERROR_LOG1(uc_vapid, OAM_SF_TX, "{dmac_tx_force::netbuf num[%d] != 1}", MAC_GET_CB_NETBUF_NUM(pst_tx_ctl));
       return OAL_FAIL;
    }
    ...
    {
       OAM_ERROR_LOG0(uc_vapid, OAM_SF_TX, "{dmac_tx_force::protectedframe detected}");
       return OAL_FAIL;
    }
    ...
    if (OAL_UNLIKELY(OAL_PTR_NULL == pst_dmac_user))
    {
        OAM_ERROR_LOG0(uc_vapid, OAM_SF_TX, "{dmac_tx_force::pst_dmac_user null}");
        return OAL_ERR_CODE_PTR_NULL;
    }

    pst_tx_dscr = (hal_tx_dscr_stru *)OAL_MEM_ALLOC(OAL_MEM_POOL_ID_SHARED_DSCR, WLAN_MEM_SHARED_TX_DSCR_SIZE1, OAL_TRUE);
    if (OAL_UNLIKELY(OAL_PTR_NULL == pst_tx_dscr))
    {
...
#ifdef _PRE_WLAN_FEATURE_DBAC
    /* 先判断此管理帧是否要缓存 */
    if (dmac_tx_dscr_need_dbac_buffer(pst_hal_device, pst_dmac_vap))
    {
...
*****************************************************************************/
OAL_STATIC oal_void dmac_tx_mgmt_buffer_proc(
                mac_device_stru     *pst_mac_device,
                dmac_vap_stru       *pst_dmac_vap,
                hal_tx_dscr_stru    *pst_mgmt_dscr,
                oal_netbuf_stru     *pst_netbuf_mgmt,
                oal_bool_enum_uint8 *pen_buffer)
{
    hal_to_dmac_device_stru        *pst_hal_device;
    oal_uint8                       uc_q_id = 0;
    ...
    if (dmac_tx_dscr_need_dbac_buffer(pst_hal_device, pst_dmac_vap))
    {
        ...
        OAM_WARNING_LOG1(0, OAM_SF_DBAC, "dmac_tx_mgmt: buffered by dbac. fake q id:%d", uc_q_id);
...
oal_uint32  dmac_tx_mgmt(dmac_vap_stru *pst_dmac_vap, oal_netbuf_stru *pst_netbuf_mgmt, oal_uint16 us_len)
{
    ...
    */
    ...
    if (WLAN_PROBE_RSP == pst_mac_header->st_frame_control.bit_sub_type)
    {
        /* 已关联用户，不允许频繁发送扫描 */
        pst_mac_user = dmac_get_mac_user_by_macaddr(&(pst_dmac_vap->st_vap_base_info), pst_mac_header->auc_address1);
        if ((pst_mac_user) && (pst_mac_user->en_user_asoc_state == MAC_USER_STATE_ASSOC))
        {
            pst_dmac_user = (dmac_user_stru *)pst_mac_user;
            ul_timestamp = (oal_uint32)OAL_TIME_GET_STAMP_MS();
            if (OAL_TIME_GET_RUNTIME(pst_dmac_user->ul_probe_req_ts, ul_timestamp) < PROBE_REQ_THREHOD_MS)
            {
            ...
            /* 更新用户最后活跃时刻 */
            pst_dmac_user->ul_probe_req_ts = ul_timestamp;
        }
    }
    ...
    *****************************************************************************/
    ...
    {
        dmac_tx_mgmt_buffer_proc(pst_mac_device, pst_dmac_vap, pst_mgmt_dscr, pst_netbuf_mgmt, &en_need_buffer);
                                      ...
                                      oal_dlist_head_stru *pst_dscr_list)
{
    ...
    /* 申请bar帧体 */
    pst_bar_buf = OAL_MEM_NETBUF_ALLOC(OAL_NORMAL_NETBUF, WLAN_SHORT_NETBUF_SIZE, OAL_TRUE);
    OAL_MEM_NETBUF_TRACE(pst_bar_buf, OAL_FALSE);
    if (OAL_PTR_NULL == pst_bar_buf)
    {
        OAM_ERROR_LOG0(pst_dmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_TX, "{dmac_tx_rifs_process::pst_bar_buf null.}");
                ...
                oal_uint8                  uc_mpdu_num)
{
    ...
    while (uc_mpdu_idx != uc_mpdu_num)
    {
        ...
        else if (OAL_FALSE == oal_netbuf_list_empty(&pst_tid_queue->st_buff_head))
        {
            ...
            if (OAL_PTR_NULL == pst_tx_dscr)
            {
                OAM_WARNING_LOG0(uc_vap_id, OAM_SF_TX, "{dmac_tid_tx_queue_remove::put the pkt back into tid netbuf Q, alloc tx dscr failed.}");
                dmac_tx_queue_mpdu(pst_netbuf, &pst_tid_queue->st_buff_head);
                                     ...
                                     oal_uint8                  uc_dscr_num)
{
    ...
    MAC_GET_CB_MPDU_NUM(pst_cb) = uc_dscr_num;
                                                                ...
                                                                oal_netbuf_stru *pst_buf)
{
    ...
    if (OAL_PTR_NULL == gst_alg_main.p_downlink_flowctl_func)
    {
        ...
        {
            OAM_INFO_LOG1(pst_vap->uc_vap_id, OAM_SF_ACS, "tx buffer is full,the coming buffer drop %d", pst_mac_device->us_total_mpdu_num);
            return OAL_FAIL;
        }
        else
        {
