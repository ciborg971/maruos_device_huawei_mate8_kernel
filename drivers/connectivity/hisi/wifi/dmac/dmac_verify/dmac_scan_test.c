        ...
        const char __user *buffer, size_t count, loff_t *ppos)
{
    ...
    while(1)
    {
        ...
        if (!uc_bidx)
        {
            ...
            OAM_INFO_LOG4(0, OAM_SF_ANY,"%-12s:%s\n", "scan type", st_scan_req.en_scan_type==WLAN_SCAN_TYPE_PASSIVE?"passive":"active");
            OAM_INFO_LOG4(0, OAM_SF_ANY,"%-12s:%s\n", "scan mode", (st_scan_req.en_scan_mode==WLAN_SCAN_MODE_FOREGROUND_AP_STA)?"foreground":"background");
            OAM_INFO_LOG4(0, OAM_SF_ANY,"%-12s:%d\n", "channels ", st_scan_req.uc_channel_nums);
            OAM_INFO_LOG4(0, OAM_SF_ANY,"%-12s:%s\n", "funcs ", g_apc_func_name_map[st_scan_req.uc_scan_func]);
