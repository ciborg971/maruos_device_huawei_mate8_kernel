...
oal_uint32 dmac_cfg_vap_init_event(frw_event_mem_stru *pst_event_mem)
{
    ...
    /* 初始化cfg_vap信息 */
    mac_vap_init(&(pst_dmac_vap->st_vap_base_info), pst_device->uc_chip_id, pst_device->uc_device_id, pst_device->uc_cfg_vap_id, WLAN_VAP_MODE_CONFIG);
#if 0
    /* 统计信息清零 */
	...
	if (OAL_UNLIKELY(OAL_PTR_NULL == pst_event_mem_rate))
	{
		OAL_IO_PRINT("hi1102_device_main_init: dmac_init_event_process FRW_EVENT_ALLOC result = OAL_PTR_NULL.\n");
		return OAL_FAIL;
	}

	l_return = dmac_init_event_process(pst_event_mem_rate);
...
OAL_STATIC oal_uint32  dmac_rf_realtime_cali_timeout(oal_void * p_arg)
{
    ...
    hal_rf_cali_realtime(pst_hal_device, pst_mac_device->en_max_band, uc_channel_idx, pst_mac_device->en_max_bandwidth, pst_dmac_vap->st_vap_base_info.en_protocol);
...
OAL_STATIC oal_uint32  dmac_init_hardware(mac_board_stru *pst_board)
{
    ...
    while (0 != uc_chip_id_bitmap)
    {
        ...
        {
            MAC_ERR_LOG2(0, "dmac_init_hardware:uc_chip_idx is exceeded support spec.", uc_chip_idx, pst_board->uc_chip_id_bitmap);
        ...
        for (uc_device = 0; uc_device < pst_chip->uc_device_nums; uc_device++)
        {
            ...
            {
                OAM_WARNING_LOG0(0, OAM_SF_ANY, "{dmac_init_hardware::array overflow,uc_device count is exceeded.}");
                return OAL_ERR_CODE_ARRAY_OVERFLOW;
            }
...
oal_uint32 dmac_init_event_process(frw_event_mem_stru *pst_event_mem)
{
    ...
    /* 申请netbuf，存放速率级信息 */
    pst_netbuf = OAL_MEM_NETBUF_ALLOC(OAL_NORMAL_NETBUF, sizeof(mac_data_rate_stru) * MAC_DATARATES_PHY_80211G_NUM, OAL_TRUE);
...
OAL_STATIC oal_void dmac_event_fsm_tx_adapt_subtable_register(oal_void)
{
    ...
    /* 注册TBTT事件子表 */
    g_ast_hmac_tbtt_event_sub_table[DMAC_TBTT_EVENT_SUB_TYPE].p_tx_adapt_func = dmac_proc_tbtt_event_tx_adapt;

    /* 注册MISC事件子表 */
    g_ast_hmac_wlan_misc_event_sub_table[DMAC_MISC_SUB_TYPE_RADAR_DETECT].p_tx_adapt_func = dmac_dfs_radar_detect_event_tx_adapt;
    ...
    g_ast_hmac_hcc_test_event_sub_table[DMAC_HCC_TEST_RX_SUB_TYPE].p_tx_adapt_func = dmac_hcc_test_event_rxsub_tx_adapt;
    g_ast_hmac_hcc_test_event_sub_table[HMAC_HCC_RX_TEST_SUB_TYPE].p_tx_adapt_func = dmac_hcc_test_event_rxsub_tx_adapt;
...
OAL_STATIC oal_void dmac_event_fsm_rx_adapt_subtable_register(oal_void)
{
    ...
    /* 注册DMAC模块HOST_CRX事件 */
    g_ast_dmac_host_crx_table[HMAC_TO_DMAC_SYN_INIT].p_rx_adapt_func = dmac_init_event_process_rx_adapt;
    g_ast_dmac_host_crx_table[HMAC_TO_DMAC_SYN_CREATE_CFG_VAP].p_rx_adapt_func = dmac_init_event_create_cfg_vap_rx_adapt;
    g_ast_dmac_host_crx_table[HMAC_TO_DMAC_SYN_CFG].p_rx_adapt_func = dmac_event_config_syn_rx_adapt;
    g_ast_dmac_host_crx_table[HMAC_TO_DMAC_SYN_ALG].p_rx_adapt_func = dmac_event_config_syn_rx_adapt;

    /* 注册DMAC模块HOST_DRX事件处理函数表 */
    g_ast_dmac_tx_host_drx[DMAC_TX_HOST_DRX].p_rx_adapt_func = dmac_tx_process_data_event_rx_adapt;

    g_ast_hmac_hcc_test_event_sub_table[DMAC_HCC_TEST_TX_SUB_TYPE].p_rx_adapt_func = dmac_hcc_test_event_rx_adapt;
    g_ast_hmac_hcc_test_event_sub_table[DMAC_HCC_TEST_LOOP_SUB_TYPE].p_rx_adapt_func = dmac_hcc_test_event_rx_adapt;
    g_ast_hmac_hcc_test_event_sub_table[DMAC_HCC_TEST_RX_SUB_TYPE].p_rx_adapt_func = dmac_hcc_test_event_rx_adapt;
    //g_ast_hmac_hcc_test_event_sub_table[HMAC_HCC_RX_TEST_SUB_TYPE].p_rx_adapt_func = dmac_hcc_test_event_rx_adapt;


    /* 注册DMAC模块WLAN_CTX事件处理函数表 */
    g_ast_dmac_wlan_ctx_event_sub_table[DMAC_WLAN_CTX_EVENT_SUB_TYPE_ACTION].p_rx_adapt_func        = dmac_tx_process_action_event_rx_adapt;
    g_ast_dmac_wlan_ctx_event_sub_table[DMAC_WLAN_CTX_EVENT_SUB_TYPE_ADD_USER].p_rx_adapt_func      = dmac_user_add_rx_adapt;
    g_ast_dmac_wlan_ctx_event_sub_table[DMAC_WLAN_CTX_EVENT_SUB_TYPE_DEL_USER].p_rx_adapt_func      = dmac_user_del_rx_adapt;
    g_ast_dmac_wlan_ctx_event_sub_table[DMAC_WLAN_CTX_EVENT_SUB_TYPE_MGMT].p_rx_adapt_func          = dmac_mgmt_tx_event_process_rx_adapt;
    g_ast_dmac_wlan_ctx_event_sub_table[DMAC_WLAN_CTX_EVENT_SUB_TYPE_STA_SET_EDCA_REG].p_rx_adapt_func  = dmac_mgmt_wmm_update_edca_machw_sta_rx_adapt;
    ...
    g_ast_dmac_wlan_ctx_event_sub_table[DMAC_WLAN_CTX_EVENT_SUB_TYPE_ASOC_WRITE_REG].p_rx_adapt_func = dmac_asoc_set_reg_event_process_rx_adapt;
    g_ast_dmac_wlan_ctx_event_sub_table[DMAC_WLAN_CTX_EVENT_SUB_TYPE_JOIN_SET_REG].p_rx_adapt_func      = dmac_join_set_reg_event_rx_adapt;
    g_ast_dmac_wlan_ctx_event_sub_table[DMAC_WLAN_CTX_EVENT_SUB_TYPE_JOIN_DTIM_TSF_REG].p_rx_adapt_func    = dmac_join_set_dtim_reg_event_rx_adapt;
    g_ast_dmac_wlan_ctx_event_sub_table[DMAC_WLAN_CTX_EVENT_SUB_TYPE_CONN_RESULT].p_rx_adapt_func    = dmac_mgmt_conn_result_event_rx_adapt;
#ifdef _PRE_WLAN_FEATURE_OPMODE_NOTIFY
    g_ast_dmac_wlan_ctx_event_sub_table[DMAC_WLAN_CTX_EVENT_SUB_TYPE_PSM_OPMODE_NOTIFY].p_rx_adapt_func  = dmac_psm_opmode_notify_process_rx_adapt;
#endif
    g_ast_dmac_wlan_ctx_event_sub_table[DMAC_WLAN_CTX_EVENT_SUB_TYPE_SET_RX_FILTER].p_rx_adapt_func  = dmac_set_rx_filter_value_rx_adapt;
    g_ast_dmac_wlan_ctx_event_sub_table[DMAC_WLAN_CTX_EVENT_SUB_TYPE_NOTIFY_ALG_ADD_USER].p_rx_adapt_func  = dmac_user_add_notify_alg_rx_adapt;
    g_ast_dmac_wlan_ctx_event_sub_table[DMAC_WLAN_CTX_EVENT_SUB_TYPE_BA_SYNC].p_rx_adapt_func           = dmac_rx_process_sync_event_rx_adapt;
    g_ast_dmac_wlan_ctx_event_sub_table[DMAC_WALN_CTX_EVENT_SUB_TYPR_SELECT_CHAN].p_rx_adapt_func = dmac_chan_select_channel_mac_rx_adapt;
    g_ast_dmac_wlan_ctx_event_sub_table[DMAC_WLAN_CTX_EVENT_SUB_TYPE_SWITCH_TO_NEW_CHAN].p_rx_adapt_func = dmac_chan_initiate_switch_to_new_channel_rx_adapt;
    g_ast_dmac_wlan_ctx_event_sub_table[DMAC_WLAN_CTX_EVENT_SUB_TYPR_RESTART_NETWORK].p_rx_adapt_func = dmac_chan_restart_network_after_switch_rx_adapt;
    g_ast_dmac_wlan_ctx_event_sub_table[DMAC_WALN_CTX_EVENT_SUB_TYPR_DISABLE_TX].p_rx_adapt_func = dmac_chan_disable_machw_rx_adapt;
    g_ast_dmac_wlan_ctx_event_sub_table[DMAC_WALN_CTX_EVENT_SUB_TYPR_ENABLE_TX].p_rx_adapt_func = dmac_chan_disable_machw_rx_adapt;
#ifdef _PRE_WLAN_FEATURE_DFS
#ifdef _PRE_WLAN_FEATURE_OFFCHAN_CAC
    g_ast_dmac_wlan_ctx_event_sub_table[DMAC_WLAN_CTX_EVENT_SUB_TYPR_SWITCH_TO_OFF_CHAN].p_rx_adapt_func = dmac_chan_disable_machw_rx_adapt;
    g_ast_dmac_wlan_ctx_event_sub_table[DMAC_WLAN_CTX_EVENT_SUB_TYPR_SWITCH_TO_HOME_CHAN].p_rx_adapt_func = dmac_chan_disable_machw_rx_adapt;
#endif
    g_ast_dmac_wlan_ctx_event_sub_table[DMAC_WLAN_CTX_EVENT_SUB_TYPR_DFS_TEST].p_rx_adapt_func = dmac_chan_disable_machw_rx_adapt;
#endif
    g_ast_dmac_wlan_ctx_event_sub_table[DMAC_WLAN_CTX_EVENT_SUB_TYPE_RESET_PSM].p_rx_adapt_func           = dmac_psm_reset_rx_adapt;

#ifdef _PRE_WLAN_FEATURE_EDCA_OPT_AP
    g_ast_dmac_wlan_ctx_event_sub_table[DMAC_WLAN_CTX_EVENT_SUB_TYPR_EDCA_OPT].p_rx_adapt_func  = dmac_edca_opt_stat_event_rx_adapt;
#endif
...
OAL_STATIC oal_void  dmac_event_fsm_action_subtable_register(oal_void)
{
    ...
    g_ast_hmac_hcc_test_event_sub_table[DMAC_HCC_TEST_TX_SUB_TYPE].p_func = dmac_hcc_test_event_handler;
    g_ast_hmac_hcc_test_event_sub_table[DMAC_HCC_TEST_LOOP_SUB_TYPE].p_func = dmac_hcc_test_event_handler;
    g_ast_hmac_hcc_test_event_sub_table[DMAC_HCC_TEST_RX_SUB_TYPE].p_func = dmac_hcc_test_rx_handler;
#endif
