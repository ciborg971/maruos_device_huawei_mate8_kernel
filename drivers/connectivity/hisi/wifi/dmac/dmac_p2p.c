...
oal_uint32  dmac_p2p_listen_rx_mgmt(dmac_vap_stru   *pst_dmac_vap,
                                    ...
                                    oal_uint8       *pen_go_on)
{
    ...
    {
        ul_ret = dmac_p2p_listen_filter_vap(pst_dmac_vap);
        if (OAL_SUCC != ul_ret)

        {
            OAM_INFO_LOG2(pst_dmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_P2P,
                          "{dmac_p2p_listen_rx_mgmt:: VAP p2p mode =%d VAP state=%d. skip processing probe req here}\r\n",
                          pst_dmac_vap->st_vap_base_info.en_p2p_mode,
                          pst_dmac_vap->st_vap_base_info.en_vap_state);
            return OAL_SUCC;
        }
        ul_ret = dmac_p2p_listen_filter_frame(pst_dmac_vap, puc_frame_body, us_frame_len);
        if (OAL_SUCC != ul_ret)
        {
            return OAL_SUCC;
        }
...
oal_uint32 dmac_p2p_noa_absent_end_event(frw_event_mem_stru *pst_event_mem)
{
    ...
    hal_p2p_pm_event_stru      *pst_p2p_pm_event;
    dmac_vap_stru              *pst_dmac_vap;
    ...
    pst_dmac_vap = (dmac_vap_stru *)mac_res_get_dmac_vap(pst_mac_vap->uc_vap_id);
    pst_p2p_pm_event      = (hal_p2p_pm_event_stru *)(pst_event->auc_event_data);
    //判断寄存器状态noa count是否结束，0:结束 1: 工作中; 更新P2P IE
    if(pst_p2p_pm_event->p2p_noa_status == OAL_FALSE)
    {
                                ...
                                mac_vap_stru     *pst_flw_vap)
{
    ...
    st_ps_open.uc_sta_pm_open = MAC_STA_DBAC_OFF_PM;

	/* 开启dbac前关闭低功耗 */
    if (WLAN_VAP_MODE_BSS_STA == pst_led_vap->en_vap_mode)
    {
        dmac_set_sta_pm_open(pst_led_vap, OAL_SIZEOF(mac_cfg_ps_open_stru), (oal_uint8 *)&st_ps_open);
    }

    if(WLAN_VAP_MODE_BSS_STA == pst_flw_vap->en_vap_mode)
    {
        dmac_set_sta_pm_open(pst_flw_vap, OAL_SIZEOF(mac_cfg_ps_open_stru), (oal_uint8 *)&st_ps_open);
    }
#endif
...
oal_void dmac_p2p_handle_ps(dmac_vap_stru * pst_dmac_vap, oal_bool_enum_uint8 en_pause)
{
    ...
    OAM_INFO_LOG3(pst_mac_vap->uc_vap_id, OAM_SF_P2P, "{dmac_p2p_handle_ps::en_pause[%d], hal_vap_id[%d], service_id[%d].}",
            en_pause, pst_dmac_vap->pst_hal_vap->uc_vap_id, pst_dmac_vap->pst_hal_vap->uc_service_id);
