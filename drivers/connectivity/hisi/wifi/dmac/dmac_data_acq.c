...
oal_uint32  dmac_data_acq_prepare(mac_vap_stru *pst_mac_vap,oal_uint8 *puc_param, mac_device_stru *pst_device)
{
    ...
    {
        /* 数据采集正在进行，不允许设置 */
        OAM_INFO_LOG0(pst_mac_vap->uc_vap_id, OAM_SF_ANY, "{dmac_data_acq_prepare::data acq is running.}\r\n");
...
oal_uint32  dmac_data_acq_condition(mac_vap_stru *pst_mac_vap,oal_uint8 *puc_param, mac_device_stru *pst_device)
{
    ...
    {
        /* 数据采集正在进行，不允许设置 */
        OAM_INFO_LOG1(pst_mac_vap->uc_vap_id, OAM_SF_CFG, "{dmac_data_acq_condition::data acq is in MAC REG status or is doing! daq_status=%d.}", g_st_data_acq.en_daq_status);
        return OAL_SUCC;
    }
...
oal_uint32 dmac_data_acq_report_data_info(oal_uint8 *puc_daq_addr, oal_uint32 ul_len, oal_netbuf_stru *pst_netbuf)
{
    ...
    {
        OAM_INFO_LOG0(0, OAM_SF_ANY, "{dmac_data_acq_report_data_info::wait app ack.}\r\n");
...
oal_uint32 dmac_data_acq_report_data_unit(oal_uint8 *puc_daq_addr, oal_uint32 ul_len, oal_netbuf_stru *pst_netbuf)
{
    ...
    {
        OAM_INFO_LOG0(0, OAM_SF_ANY, "{dmac_data_acq_report_data_unit::wait app ready.}\r\n");
...
oal_uint32 dmac_data_acq_report_data(oal_uint8 *puc_daq_addr, oal_uint32 ul_len, oal_netbuf_stru *pst_netbuf)
{
    ...
    while (ul_trans_remainder_len >= ul_unit_len)
    {
        if (OAL_FALSE == g_st_data_acq.en_daq_app_error)
        {
            ...
            {
                OAM_INFO_LOG0(0, OAM_SF_ANY, "{dmac_data_acq_report_data::wait app ready.}\r\n");
                oal_msleep(1000);
    ...
    {
        OAM_INFO_LOG0(0, OAM_SF_ANY, "{dmac_data_acq_report_data::last data.}\r\n");
    ...
    while(!g_st_data_acq.uc_ack_flag)
    {
        OAM_INFO_LOG0(0, OAM_SF_ANY, "{dmac_data_acq_report_data::wait app ack.}\r\n");
...
oal_void dmac_data_acq_workqueue(oal_work_stru *pst_work)
{
    ...
    {
        OAM_INFO_LOG0(0, OAM_SF_ANY, "{dmac_data_acq_workqueue::pst_netbuf null.}\r\n");
        ...
        OAM_INFO_LOG0(0, OAM_SF_ANY, "{dmac_data_acq_workqueue::start send data info.}\r\n");
        ul_ret = dmac_data_acq_report_data_info(puc_daq_addr, ul_len, pst_netbuf);
        if (OAL_SUCC == ul_ret)
        {
            OAM_INFO_LOG0(0, OAM_SF_ANY, "{dmac_data_acq_workqueue::start send data unit.}\r\n");
...
oal_uint32  dmac_data_acq_get_record(oal_void)
{
    ...
    if (DMAC_DATA_ACQ_STATUS_COMPLETE == g_st_data_acq.en_daq_status)
    {
        ...
        pst_queue_netbuf = OAL_MEM_NETBUF_ALLOC(OAL_NORMAL_NETBUF, DMAC_DATA_ACQ_UNIT_MAX_LEN, OAL_TRUE);
        ...
        OAM_INFO_LOG0(0, OAM_SF_ANY, "{dmac_data_acq_get_record::send data acq recordc.}\r\n");
...
oal_uint32  dmac_data_acq_recv_msg(oal_uint8 *puc_data, oal_uint32 ul_len)
{
    ...
    switch (pst_frag_hdr->en_send_type)
    {
        case DMAC_DATA_ACQ_SEND_TYPE_READY:
            /*  APP侧接收数据成功，继续发送  */
            dmac_data_acq_set_send_flag(OAL_TRUE);
            OAM_INFO_LOG0(0, OAM_SF_ANY, "{dmac_data_acq_report_data_info::DMAC_DATA_ACQ_SEND_TYPE_READY.}\r\n");
            break;
        ...
        case DMAC_DATA_ACQ_SEND_TYPE_ACK:
            ...
            OAM_INFO_LOG0(0, OAM_SF_ANY, "{dmac_data_acq_report_data_info::DMAC_DATA_ACQ_SEND_TYPE_ACK.}\r\n");
