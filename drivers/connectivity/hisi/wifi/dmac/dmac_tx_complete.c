    ...
    else
    {
        if (OAL_TRUE == MAC_GET_CB_IS_VIPFRAME(pst_tx_cb))
        {
                ...
                (MAC_DATA_EAPOL == uc_data_type))
            {
                ...
                OAM_WARNING_LOG2(0, OAM_SF_ANY, "{dmac_tx_complete_dump_dscr::datatype=%u, rst==%u}[0:dhcp 1:arp 2:eapol]",uc_data_type, uc_dscr_status);
            }
        }
...
OAL_STATIC OAL_INLINE oal_void dmac_keepalive_count_update(dmac_user_stru *pst_dmac_user,
                                                           oal_uint8     *puc_frame_hdr,
                                                           oal_uint8      uc_dscr_status,
                                                           oal_bool_enum_uint8 en_is_ampdu)
{
    if (OAL_TRUE == en_is_ampdu)
    {
#ifdef _PRE_WLAN_FEATURE_UAPSD
        if (OAL_FALSE == (pst_dmac_user->uc_uapsd_flag & MAC_USR_UAPSD_EN))
        {
            return;
        }
#endif
    ...
    else
    {
        if ((WLAN_DATA_BASICTYPE != mac_frame_get_type_value(puc_frame_hdr)) ||
            (WLAN_NULL_FRAME != mac_frame_get_subtype_value(puc_frame_hdr)))
        {
            return;
        }
    }
...
*****************************************************************************/
OAL_STATIC oal_uint32  dmac_tx_delete_ba(dmac_user_stru *pst_dmac_user)
{
    oal_uint8                    uc_tid;
    ...
    for (uc_tid = 0; uc_tid < WLAN_TID_MAX_NUM; uc_tid++)
    {
        if (OAL_PTR_NULL != pst_dmac_user->ast_tx_tid_queue[uc_tid].pst_ba_tx_hdl)
        {
            ...
            pst_del_ba_event->uc_tid            = uc_tid;
...
oal_uint32  dmac_tx_complete_event_handler(frw_event_mem_stru *pst_event_mem)
{
    ...
    if(OAL_UNLIKELY((uc_queue_num != pst_base_dscr->uc_q_num) || (uc_queue_num > HAL_TX_QUEUE_HI)))
    {
        ...
        oam_report_dscr(auc_user_macaddr, (oal_uint8 *)pst_base_dscr, WLAN_MEM_SHARED_TX_DSCR_SIZE1, OAM_OTA_TYPE_TX_DSCR);
        return OAL_FAIL;
    }
    ...
    if (OAL_UNLIKELY(OAL_FALSE == pst_base_dscr->bit_is_first))
    {
        ...
        OAL_DLIST_SEARCH_FOR_EACH(pst_dscr_entry, &(pst_hal_device->ast_tx_dscr_queue[uc_queue_num].st_header))
        {
            ...
            oam_report_dscr(auc_user_macaddr, (oal_uint8 *)pst_curr_dscr, WLAN_MEM_SHARED_TX_DSCR_SIZE1, OAM_OTA_TYPE_TX_DSCR);
        }
        return OAL_FAIL;
    }
    ...
    do
    {
        ...
        {
            OAM_WARNING_LOG0(0, OAM_SF_ANY, "{dmac_tx_complete_event_handler:: TX Q IS EMPTY.}");
            break;
        }
...
OAL_STATIC OAL_INLINE oal_void  dmac_tx_set_retry_dscr(hal_to_dmac_device_stru *pst_hal_device, hal_tx_dscr_stru *pst_dscr, oal_uint8 uc_tx_status, oal_uint8 uc_tx_chiper_type, oal_uint8 uc_chiper_key_id)
{
       ...
       || (uc_tx_status == DMAC_TX_SOFT_PSM_BACK))
    {
        ...
        hal_tx_get_dscr_iv_ls_word(pst_dscr, &ul_vi_value, uc_tx_chiper_type, uc_chiper_key_id);
        if (0 == ul_vi_value)
        {
            hal_tx_set_dscr_seqno_sw_generate(pst_hal_device, pst_dscr, 0);
        }
...
oal_void dmac_tx_sw_retry(dmac_vap_stru *pst_dmac_vap, oal_netbuf_stru *pst_buf, hal_tx_dscr_stru *pst_dscr)
{
    ...
    if (OAL_SUCC != OAL_MEM_FREE(pst_dscr, OAL_TRUE))
    {
        OAM_ERROR_LOG1(0, OAM_SF_TX, "{dmac_tx_sw_retry::tx dscr free fail, addr=0x%x.}", pst_dscr);
    }

    /* 调用管理报文接口发送 */
    dmac_tx_mgmt(pst_dmac_vap, pst_buf, pst_dscr->us_original_mpdu_len);
}
                ...
                hal_tx_dscr_ctrl_one_param  *pst_tx_dscr_one)
{
    ...
    if (OAL_PTR_NULL == pst_mac_hdr)
    {
        OAM_ERROR_LOG0(pst_dmac_user->st_user_base_info.uc_vap_id, OAM_SF_TX, "{dmac_tx_complete_mgmt_notify::pst_mac_header null.}");
...
#endif
    {
        if (MAC_GET_CB_RETRIED_NUM(pst_cb) < pst_dmac_vap->uc_sw_retry_limit)
        {
            ...
            /* 重传过程中若用户被删除，则不再重传 */
            ul_ret = mac_vap_find_user_by_macaddr(&(pst_dmac_vap->st_vap_base_info), pst_dmac_user->st_user_base_info.auc_user_mac_addr, &us_tx_user_idx);
            if((OAL_SUCC != ul_ret) || (0xffff == us_tx_user_idx))
            {
            ...
            //OAM_INFO_LOG1(pst_dmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_TX, "{dmac_tx_complete_ampdu_buffer::uc_retried_num=%d.}", MAC_GET_CB_RETRIED_NUM(pst_cb));
...
/* 由于02裸系统，需要使用multi_free释放大内存 */
    ...
    {
        dmac_keepalive_count_update(pst_dmac_user, OAL_NETBUF_HEADER(pst_buf), uc_dscr_status, OAL_FALSE);
    }
...
OAL_STATIC oal_uint32  dmac_tx_complete_ampdu_buffer(hal_to_dmac_device_stru *pst_hal_device, hal_tx_dscr_stru *pst_dscr)
{
...
#ifdef _PRE_DEBUG_MODE
    if (OAL_UNLIKELY(OAL_TRUE == en_is_vipframe))
    {
        ...
        oam_report_80211_frame(auc_macaddr,
    ...
    if (OAL_UNLIKELY(OAL_PTR_NULL == pst_dmac_vap))
    {
        OAM_INFO_LOG1(uc_vap_id, OAM_SF_TX, "{dmac_tx_complete_ampdu_buffer::pst_dmac_vap null!,and user->vap_id = [%d].}\r\n",
                    pst_dmac_user->st_user_base_info.uc_vap_id);
        dmac_tx_complete_free_dscr_list(pst_dscr, uc_dscr_num);
        return OAL_ERR_CODE_PTR_NULL;
    }
    ...
    if (WLAN_VAP_MODE_BSS_AP == pst_mac_vap->en_vap_mode)
    {
        dmac_keepalive_count_update(pst_dmac_user, OAL_NETBUF_HEADER(pst_buf), uc_dscr_status, OAL_TRUE);
    }
#endif
...
/*delete for alg integrate,TBD*/
    ...
    else  /*没有收到ba*/
    {
        ...
        hal_tx_get_dscr_tx_cnt(pst_hal_device, pst_dscr, &uc_data_tx_cnt);
        OAM_INFO_LOG4(uc_vap_id, OAM_SF_TX, "{dmac_tx_complete_ampdu_buffer::tx_dscr_status=%d,tx cnt=%d dscr_num=%d, seq_num=%d.}", uc_dscr_status, uc_data_tx_cnt,uc_dscr_num, us_seq_num);
        ...
        else if ((!DMAC_BA_ISSET(aul_ba_bitmap, DMAC_BA_INDEX(us_ssn, us_seq_num))) || (uc_dscr_status != DMAC_TX_SUCC))
        {
            ...
            if ((MAC_GET_CB_RETRIED_NUM(pst_cb) < pst_dmac_vap->uc_sw_retry_limit) && (OAL_FALSE == OAL_GET_THRUPUT_BYPASS_ENABLE(OAL_TX_HAL_HARDWARE_BYPASS)))
            {
                ...
                if ((uc_dscr_index == OAL_SUB(uc_dscr_num, 1)) && (OAL_FALSE == en_is_vipframe))
                {
                    ...
                    MAC_GET_CB_ALG_PKTNO((mac_tx_ctl_stru *)oal_netbuf_cb(pst_buf)) = ul_pktno;
