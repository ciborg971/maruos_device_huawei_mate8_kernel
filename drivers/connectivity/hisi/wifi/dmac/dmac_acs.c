                            ...
                            oal_uint8 *puc_buffer, oal_uint32 ul_len)
{
    ...
    OAM_INFO_LOG3(0, OAM_SF_ACS, "{try reply, type=%d len=%d cmd_id=%d}", en_cmd, ul_len, ul_cmd_id);
...
oal_uint32 dmac_acs_add_assoc_user_handler(mac_vap_stru *pst_vap, mac_user_stru *pst_user)
{
    ...
    OAM_INFO_LOG2(pst_vap->uc_vap_id, OAM_SF_ACS, "dmac_acs_add_assoc_user_handler:: assoc_id=%d, en_is_multi_user=%d.",
                   (oal_uint32)pst_user->us_assoc_id,
                    pst_user->en_is_multi_user);

    OAM_INFO_LOG3(pst_vap->uc_vap_id, OAM_SF_ACS, "dmac_acs_add_assoc_user_handler:: mac[0:2] = %02X:%02X:%02X",
                   pst_user->auc_user_mac_addr[0]&0xff,
                   pst_user->auc_user_mac_addr[1]&0xff,
                   pst_user->auc_user_mac_addr[2]&0xff);

    OAM_INFO_LOG3(pst_vap->uc_vap_id, OAM_SF_ACS, "dmac_acs_add_assoc_user_handler:: mac[3:5] = %02X:%02X:%02X",
                   pst_user->auc_user_mac_addr[3]&0xff,
        ...
        case WLAN_LEGACY_11B_MODE:
            ...
            if (pst_device->st_mac_rates_11g[uc_rate_idx].uc_phy_rate == uc_mcs)
            {
                *pul_rate_kbps = pst_device->st_mac_rates_11g[uc_rate_idx].uc_mbps * 1000;
                OAM_INFO_LOG1(0, OAM_SF_ACS, "{dmac_acs_judge_rate_condition:: (11a/b/g)rate_kbps=%d.", *pul_rate_kbps);
                en_found = OAL_TRUE;
            }
            break;
        ...
        case WLAN_HT_MODE:
                ...
                (en_bandwidth == g_ast_11n_table[uc_rate_idx].en_bandwidth))
            {
                *pul_rate_kbps = g_ast_11n_table[uc_rate_idx].ul_rate_kbps;
                OAM_INFO_LOG1(0, OAM_SF_ACS, "{dmac_acs_judge_rate_condition:: (11n)rate_kbps=%d.}", *pul_rate_kbps);
                en_found = OAL_TRUE;
            }
            break;
        ...
        case WLAN_VHT_MODE:
                ...
                (en_bandwidth == g_ast_11ac_table[uc_rate_idx].en_bandwidth))
            {
                *pul_rate_kbps = g_ast_11ac_table[uc_rate_idx].ul_rate_kbps;
                OAM_INFO_LOG1(0, OAM_SF_ACS, "{dmac_acs_judge_rate_condition:: (11ac)rate_kbps=%d.", *pul_rate_kbps);
                en_found = OAL_TRUE;
            }
            break;
...
oal_void  dmac_acs_tx_notify(dmac_acs_stru *pst_acs, oal_uint8 uc_vap_id, oal_netbuf_stru *pst_buf)
{
    ...
    if ((OAL_PTR_NULL != pst_vap_internal) && (OAL_TRUE == pst_vap_internal->en_dfs_wait_for_csa_sent))
    {
            ...
            && (OAL_TRUE == mac_ieeee80211_is_action(oal_netbuf_header(pst_buf))))
        {
            ...
            {
                OAM_INFO_LOG0(0, OAM_SF_ACS, "{dmac_acs_tx_notify:: Channel Switch Announcement packet sent out!.");
...
#endif
    OAM_INFO_LOG0(0, OAM_SF_ACS, "dmac acs init complete!");
