...
oal_void  mac_get_regdomain_info(mac_regdomain_info_stru **ppst_rd_info)
{
    ...
    {
        MAC_ERR_LOG(0, "ppst_rd_info is null ptr!");
...
mac_regclass_info_stru*  mac_get_channel_num_rc_info(oal_uint8 uc_band, oal_uint8 uc_ch_num)
{
    ...
    ul_ret = mac_get_channel_idx_from_num(uc_band, uc_ch_num, &uc_channel_idx);
    if (OAL_SUCC != ul_ret)
    {
        MAC_WARNING_LOG(0, "uc_ch_num is invalid");
        OAM_WARNING_LOG0(0, OAM_SF_ANY, "{mac_get_channel_num_rc_info::mac_get_channel_idx_from_num failed.}");
...
oal_uint32  mac_regdomain_set_country(oal_uint16 us_len, oal_uint8 *puc_param)
{
    ...
    /* 更新管制域信息 */
    oal_memcopy(&g_st_mac_regdomain, pst_mac_regdom, ul_size);
