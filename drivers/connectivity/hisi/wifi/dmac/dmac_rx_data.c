...
#pragma arm section rwdata = "BTCM", code ="BTCM", zidata = "BTCM", rodata = "BTCM"
#endif
...
oal_uint32 dmac_rx_filter_wlan_security_11i(mac_vap_stru *pst_mac_vap, dmac_rx_ctl_stru *pst_cb_ctrl)
{
    ...
    {
        OAM_ERROR_LOG0(pst_mac_vap->uc_vap_id, OAM_SF_RX, "{dmac_rx_filter_wlan_security_11i::pst_dmac_user null.}");
    ...
    pst_frame_hdr = (mac_ieee80211_frame_stru *)mac_get_rx_cb_mac_hdr(&(pst_cb_ctrl->st_rx_info));
    en_cipher_protocol_type = hal_ctype_to_cipher_suite(pst_cb_ctrl->st_rx_status.bit_cipher_protocol_type);

    if (pst_mac_user->en_port_valid == OAL_TRUE)
    {
        /*帧为数据帧*/
        if (WLAN_QOS_DATA == pst_frame_hdr->st_frame_control.bit_sub_type || WLAN_DATA == pst_frame_hdr->st_frame_control.bit_sub_type)
        {
            if (mac_is_grp_addr(pst_frame_hdr->auc_address1))
            {
                uc_grp_policy = mac_get_rsnacfggroupcipher(pst_mac_vap);
                if (uc_grp_policy != en_cipher_protocol_type)
                {
                    OAM_WARNING_LOG2(pst_mac_vap->uc_vap_id, OAM_SF_WPA, "{dmac_rx_filter_wlan_security_11i::uc_grp_policy=%d en_cipher_protocol_type=%d.",
                                     uc_grp_policy, en_cipher_protocol_type);
                    return OAL_FAIL;
                }
            }
            else
            {
                uc_pairwise_policy = pst_mac_user->st_key_info.en_cipher_type;
                if (uc_pairwise_policy != en_cipher_protocol_type)
                {
                    OAM_WARNING_LOG2(pst_mac_vap->uc_vap_id, OAM_SF_WPA, "{dmac_rx_filter_wlan_security_11i::uc_pairwise_policy=%d en_cipher_protocol_type=%d.",
                                     uc_pairwise_policy, en_cipher_protocol_type);
                    /* 原因是前面已经判断了用户的端口认证en_port_valid 通过，可以认为用户密钥协商已经完成，用户合法 */
#if 0
                    return OAL_FAIL;
#endif
                }
...
oal_uint32 dmac_rx_filter_wlan_wep(mac_vap_stru *pst_mac_vap, dmac_rx_ctl_stru  *pst_cb_ctrl)
{
    mac_ieee80211_frame_stru            *pst_frame_hdr     = OAL_PTR_NULL;
    mac_user_stru                       *pst_multi_user;
    wlan_ciper_protocol_type_enum_uint8  en_cipher_protocol_type;

    if(OAL_FALSE == mac_is_wep_enabled(pst_mac_vap))
    {
       return OAL_SUCC;
    }

    pst_frame_hdr = (mac_ieee80211_frame_stru *)mac_get_rx_cb_mac_hdr(&(pst_cb_ctrl->st_rx_info));
    if(0 == pst_frame_hdr->st_frame_control.bit_protected_frame)
    {
        return OAL_SUCC;
    }

    en_cipher_protocol_type = pst_cb_ctrl->st_rx_status.bit_cipher_protocol_type;
    if(OAL_TRUE == mac_get_excludeunencrypted(pst_mac_vap))
    {
        pst_multi_user = mac_res_get_mac_user(pst_mac_vap->us_multi_user_idx);
        if (OAL_PTR_NULL == pst_multi_user)
        {
            OAM_ERROR_LOG0(pst_mac_vap->uc_vap_id, OAM_SF_WEP, "{dmac_rx_filter_wlan_wep::mac_res_get_mac_user=null.");
            return OAL_FAIL;
        }
        if (en_cipher_protocol_type != hal_cipher_suite_to_ctype(pst_multi_user->st_key_info.en_cipher_type))
        {
            OAM_WARNING_LOG2(pst_mac_vap->uc_vap_id, OAM_SF_WEP, "{dmac_rx_filter_wlan_wep::cipher_protocol_type=%d en_cipher_type=%d.",
                                     en_cipher_protocol_type, pst_multi_user->st_key_info.en_cipher_type);
            return OAL_FAIL;
        }
    }
...
oal_uint32 dmac_rx_filter_wlan_security(mac_vap_stru *pst_mac_vap, dmac_rx_ctl_stru   *pst_cb_ctrl)
{
    oal_uint32       ul_ret            = OAL_SUCC;
#ifdef _PRE_WLAN_DFT_STAT
    dmac_vap_stru    *pst_dmac_vap;
    ...
    ul_ret = dmac_rx_filter_wlan_security_11i(pst_mac_vap, pst_cb_ctrl);
    if(OAL_FAIL == ul_ret)
    {
        //OAM_INFO_LOG0(pst_mac_vap->uc_vap_id, OAM_SF_RX, "{dmac_rx_filter_wlan_security::data droped beacause of 11ic.}\r\n");
        OAM_STAT_VAP_INCR(pst_mac_vap->uc_vap_id, rx_11i_check_fail_dropped, 1);
        DMAC_VAP_DFT_STATS_PKT_INCR(pst_dmac_vap->st_query_stats.ul_rx_security_check_fail_dropped, 1);
        return OAL_FAIL;
    }
#endif
    ul_ret = dmac_rx_filter_wlan_wep(pst_mac_vap,pst_cb_ctrl);
    if(OAL_FAIL == ul_ret)
    {
        //OAM_INFO_LOG0(pst_mac_vap->uc_vap_id, OAM_SF_RX, "{dmac_rx_filter_wlan_security::data droped beacause of wep.}\r\n");
        OAM_STAT_VAP_INCR(pst_mac_vap->uc_vap_id, rx_wep_check_fail_dropped, 1);
        DMAC_VAP_DFT_STATS_PKT_INCR(pst_dmac_vap->st_query_stats.ul_rx_security_check_fail_dropped, 1);
        return OAL_FAIL;
    }
...
oal_void  dmac_rx_data_user_is_null(mac_vap_stru *pst_mac_vap, mac_ieee80211_frame_stru *pst_frame_hdr)
{
    ...
    ul_len = OAL_SIZEOF(st_deauth_evt);
    ...
    dmac_rx_send_event(pst_dmac_vap, DMAC_WLAN_CRX_EVENT_SUB_TYPE_DEAUTH, (oal_uint8 *)&st_deauth_evt, ul_len);
}
                ...
                dmac_user_stru             *pst_dmac_user)
{
    ...
    if (WLAN_DATA_BASICTYPE == pst_frame_hdr->st_frame_control.bit_type)
    {
        ...
        if (mac_mib_get_rsnaactivated(pst_vap) || mac_is_wep_enabled(pst_vap))
        {
            ul_ret = dmac_rx_filter_wlan_security(pst_vap, pst_cb_ctrl);
            if(OAL_FAIL == ul_ret)
            {
                //OAM_INFO_LOG0(pst_vap->uc_vap_id, OAM_SF_RX, "{dmac_rx_filter_frame_sta::data droped beacause of security.}\r\n");
                return OAL_FAIL;
            }
        }
                ...
                dmac_rx_ctl_stru       *pst_cb_ctrl)
{
    ...
    if (WLAN_DATA_BASICTYPE == pst_frame_hdr->st_frame_control.bit_type)
    {
        ...
        if(MAC_USER_STATE_ASSOC != pst_dmac_user->st_user_base_info.en_user_asoc_state)/* 用户未关联 */
        {
            ...
            OAM_WARNING_LOG3(pst_vap->uc_vap_id, OAM_SF_RX, "{dmac_rx_filter_frame_ap::user mac[0-2]:%.2x:%.2x:%.2x}",
                             pst_dmac_user->st_user_base_info.auc_user_mac_addr[0],
                             pst_dmac_user->st_user_base_info.auc_user_mac_addr[1],
                             pst_dmac_user->st_user_base_info.auc_user_mac_addr[2]);
            OAM_WARNING_LOG3(pst_vap->uc_vap_id, OAM_SF_RX, "{dmac_rx_filter_frame_ap::user mac[3-5]:%.2x:%.2x:%.2x}",
        ...
        if(OAL_FAIL == ul_ret)
        {
            //OAM_WARNING_LOG0(pst_vap->uc_vap_id, OAM_SF_RX, "{dmac_rx_filter_frame_ap::data droped beacause of security.}");
            return OAL_FAIL;
        }
...
#endif
{
    ...
    {
        oam_report_80211_frame(auc_mac_addr,
        ...
        ul_ret = oam_report_netbuf_cb(auc_mac_addr, (oal_uint8 *)pst_cb_ctrl, OAM_OTA_TYPE_RX_DMAC_CB);
        if (OAL_SUCC != ul_ret)
        {
...
oal_uint32  dmac_rx_process_data_event(frw_event_mem_stru *pst_event_mem)
{
    ...
    while (0 != us_dscr_num)
    {
        ...
        if (OAL_PTR_NULL == pst_vap)
        {
            ...
            OAM_WARNING_LOG4(pst_event_hdr->uc_vap_id, OAM_SF_RX, "{dmac_rx_process_data_event::user disconnect, rx:uc_frame_type[0x%x], uc_frame_sub_type[0x%x], da[%x:%x]}", pst_frame_hdr->st_frame_control.bit_type, pst_frame_hdr->st_frame_control.bit_sub_type, puc_da[4], puc_da[5]);
            continue;
        }

        /* 维测，输出一个关键帧打印. DBAC场景下标记收到VIP帧 */
        uc_data_type = mac_get_data_type(pst_curr_netbuf);
        if(uc_data_type <= MAC_DATA_VIP || MAC_DATA_WAPI == uc_data_type)
        {
            OAM_WARNING_LOG2(pst_vap->uc_vap_id, OAM_SF_ANY, "{dmac_rx_process_data_event::datatype=%u, len==%u}[0:dhcp 1:arp 2:eapol]",uc_data_type, pst_cb_ctrl->st_rx_info.us_frame_len);
#ifdef _PRE_WLAN_FEATURE_DBAC
        ...
        {
            /* 释放当前MPDU的内存 */
            OAM_INFO_LOG0(pst_event_hdr->uc_vap_id, OAM_SF_RX, "{dmac_rx_process_data_event:: DMAC_RX_FRAME_CTRL_DROP == en_frame_ctrl");
            us_dscr_num -= pst_rx_info->bit_buff_nums;
            dmac_rx_free_netbuf_list(&st_netbuf_header, &pst_curr_netbuf, pst_rx_info->bit_buff_nums);
        ...
        if (WLAN_DATA_BASICTYPE == pst_frame_hdr->st_frame_control.bit_type)
        {
            dmac_btcoex_get_rate(pst_vap, pst_device, pst_cb_ctrl);
        }
#endif
...
OAL_STATIC oal_void  dmac_log_txq_overrun(mac_device_stru *pst_device, oal_uint32 ul_param)
{
    ...
    pst_tx_queue = &pst_device->pst_device_stru->ast_tx_dscr_queue[en_queue];

    hal_get_tx_q_status(pst_device->pst_device_stru, &ul_tx_q_status, en_queue);
    OAM_ERROR_LOG1(0, OAM_SF_TX, "{dmac_log_txq_overrun::sw ppdu num of this tx queue is %d.}", pst_tx_queue->uc_ppdu_cnt);
    OAM_ERROR_LOG1(0, OAM_SF_TX, "{dmac_log_txq_overrun::hw queue state is %d.} ", ul_tx_q_status);
    /* zhangyu 1102 needdo */
    hal_get_all_tx_q_status(pst_device->pst_device_stru, &ul_value);
    OAM_ERROR_LOG3(0, OAM_SF_TX, "{dmac_log_txq_overrun::uc_ppdu_cnt=%d ul_tx_q_status=%d ul_value=%d.}",
                   pst_tx_queue->uc_ppdu_cnt, ul_tx_q_status, ul_value);
}
...
/*****************************************************************************
 函 数 名  : dmac_handle_reset_mac_error_delay
...
*****************************************************************************/
OAL_STATIC oal_uint32  dmac_handle_reset_mac_error_delay(
                mac_device_stru                 *pst_mac_device,
                hal_mac_error_type_enum_uint8    en_error_id)
{
    ...
    if (pst_dft_err->us_err_cnt < HAL_MAC_ERROR_THRESHOLD)
    {
        return OAL_SUCC;
    }

    /* 否则复位，清除所有异常统计，重新开始计数 */
    OAL_MEMZERO(pst_hal_device->st_dfr_err_opern, OAL_SIZEOF(pst_hal_device->st_dfr_err_opern));
...
oal_uint32  dmac_mac_error_process_event(frw_event_mem_stru *pst_event_mem)
{
    ...
    dmac_error_proc_stru                st_err_proc_func[HAL_MAC_ERROR_TYPE_BUTT] =
    {
      ...
      {dmac_null_fn,0},{dmac_null_fn,0},{dmac_null_fn,0}   /* OTHER */
    };
    ...
    /* TOO SMALL的错误也直接跳过不打印 */
    #if 0
    else if (pst_device->ul_rx_buf_too_small_show_counter % 1000 == 1)
    {
        hal_show_fsm_info(pst_device->pst_device_stru);
    }
    #endif
                ...
                //dmac_rx_frame_ctrl_enum_uint8  *pen_frame_ctrl)
{
    ...
    if (HAL_RX_SUCCESS != us_dscr_status)
    {
        ...
        {
            OAM_INFO_LOG0(pst_vap->uc_vap_id, OAM_SF_RX, "{dmac_rx_process_frame::bit_dscr_status=HAL_RX_KEY_SEARCH_FAILURE.");
...
oal_uint32 dmac_get_stat_rate(dmac_user_stru *pst_dmac_user, oal_uint32 *pul_tx_rate, oal_uint32 *pul_rx_rate)
{
    ...
    {
        *pul_tx_rate = (oal_uint32)(pst_user_info->st_dmac_rate_stat_info.ul_tx_rate >> DMAC_STAT_RX_PKT_COUNT_OFFSET);
    }

    /* 获取平均接收速率 */
    if (0 != pst_user_info->st_dmac_rate_stat_info.us_rx_rate_stat_count)
    {
        *pul_rx_rate = (oal_uint32)(pst_user_info->st_dmac_rate_stat_info.ul_rx_rate >> DMAC_STAT_RX_PKT_COUNT_OFFSET);
    }
...
{
    pst_dmac_user->st_user_rate_info.en_dmac_rate_stat_flag = OAL_FALSE;
