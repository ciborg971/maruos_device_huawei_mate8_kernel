...
OAL_STATIC oal_uint32 dmac_ap_check_ssid(dmac_vap_stru *pst_dmac_vap, oal_uint8 *puc_probe_req_ssid)
{
    ...
    l_rst = oal_memcmp(&puc_probe_req_ssid[MAC_IE_HDR_LEN], puc_ssid, uc_prb_req_ssid_len);

    return ((0 == l_rst) ? OAL_SUCC : OAL_FAIL);
    /* return OAL_SUCC; */
...
{
    oal_uint8           auc_bcast_addr[6] = {0xff, 0xff, 0xff, 0xff, 0xff, 0xff};
    ...
    if ((0 != oal_compare_mac_addr(auc_bcast_addr, puc_bssid)) &&
...
oal_uint32  dmac_ap_up_rx_probe_req(dmac_vap_stru *pst_dmac_vap, oal_netbuf_stru *pst_netbuf)
{
    ...
    en_ret = dmac_ap_check_probe_req(pst_dmac_vap, puc_probe_req, pst_frame_hdr->auc_address3);
    if (OAL_FALSE == en_ret)
    {
        return OAL_SUCC;
    }
    ...
    {
        OAM_ERROR_LOG0(pst_dmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_SCAN, "{dmac_ap_up_rx_probe_req::pst_mgmt_buf null.}");
                                                    ...
                                                    frw_event_mem_stru             *pst_event_mem)
{
    ...
    us_auth_type = mac_get_auth_algo_num(pst_netbuf);
