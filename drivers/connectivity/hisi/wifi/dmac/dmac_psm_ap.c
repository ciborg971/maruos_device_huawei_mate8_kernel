...
oal_uint32 dmac_psm_user_ps_structure_init(dmac_user_stru *pst_dmac_user)
{
    ...
    {
        OAM_ERROR_LOG0(0, OAM_SF_PWR, "{dmac_psm_user_ps_structure_init::alloc pst_ps_stats mem fail.}");
...
#if 0
    pst_mac_hdr   = (mac_ieee80211_frame_stru *)oal_netbuf_data(pst_net_buf);
    puc_addr      = pst_mac_hdr->auc_address1;
    if (ETHER_IS_MULTICAST(puc_addr))
    {
...
OAL_STATIC oal_uint32 dmac_psm_tid_mpdu_num(dmac_user_stru  *pst_dmac_user)
{
...
*****************************************************************************/
OAL_STATIC oal_bool_enum_uint8 dmac_psm_is_psm_empty(dmac_user_stru *pst_dmac_user)
{
                                                        ...
                                                        dmac_user_stru *pst_dmac_user)
{
    ...
    OAL_DLIST_SEARCH_FOR_EACH_SAFE(pst_dlist_pos, pst_dlist_n, &pst_hal_device->ast_tx_dscr_queue[HAL_TX_QUEUE_VO].st_header)
    {
        ...
        OAM_INFO_LOG0(0, OAM_SF_PWR, "{dmac_psm_flush_txq_to_tid:: HAL_TX_QUEUE_VO oal_dlist_add_head}");
                                                    ...
                                                    wlan_phy_protocol_enum_uint8 en_protocol_mode)
{
    ...
    ul_value = pst_dmac_vap->ast_tx_mgmt_ucast[en_band].ast_per_rate[0].ul_value;

    pst_dmac_vap->ast_tx_mgmt_ucast[en_band].ast_per_rate[1].ul_value = ul_value;
    pst_dmac_vap->ast_tx_mgmt_ucast[en_band].ast_per_rate[2].ul_value = ul_value;
    pst_dmac_vap->ast_tx_mgmt_ucast[en_band].ast_per_rate[3].ul_value = ul_value;
...
oal_uint32  dmac_psm_send_null_data(dmac_vap_stru *pst_dmac_vap, dmac_user_stru *pst_dmac_user, oal_bool_enum_uint8 en_ps)
{
    ...
    if (OAL_PTR_NULL == pst_net_buf)
    {
        OAM_ERROR_LOG0(pst_dmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_PWR, "{dmac_psm_send_null_data::pst_net_buf null.}");
    ...
    dmac_psm_set_ucast_mgmt_tx_rate(pst_dmac_vap, en_band, uc_null_legacy_rate, uc_null_protocol_mode);

    ul_ret = dmac_tx_mgmt(pst_dmac_vap, pst_net_buf, OAL_SIZEOF(mac_ieee80211_frame_stru));

    /* 发送完成后恢复默认值: 2.4G初始化为11b 1M, long preable */
    dmac_psm_set_ucast_mgmt_tx_rate(pst_dmac_vap, en_band, uc_legacy_rate, en_protocol_mode);
...
oal_void  dmac_change_null_data_rate(dmac_vap_stru *pst_dmac_vap,dmac_user_stru *pst_dmac_user,oal_uint8 *uc_protocol_mode,oal_uint8 *uc_legacy_rate)
{
    ...
    en_avail_protocol_mode = pst_dmac_user->st_user_base_info.en_avail_protocol_mode;

    switch (en_avail_protocol_mode)
    {
        /* 11b 1M */
        ...
        default:
            OAM_WARNING_LOG1(pst_dmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_TX,
                             "{dmac_change_null_data_rate::invalid en_protocol[%d].}", en_avail_protocol_mode);
            return;
    }
