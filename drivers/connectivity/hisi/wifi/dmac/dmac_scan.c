...
{
    oal_uint8        auc_bcst_addr[WLAN_MAC_ADDR_LEN] = {0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF}; /* 广播地址 */
    oal_uint8        uc_ie_len;
                                                                 ...
                                                                 mac_scan_req_stru  *pst_scan_req_params)
{
 ...
 /* 入网开始，通知BT */
    ...
    hal_set_btcoex_soc_gpreg0(OAL_TRUE, BIT9, 9);   // 入网流程开始
                                                                  ...
                                                                  oal_uint8           uc_vap_id)
{
    ...
    pst_scan_params->uc_max_scan_count_per_channel = WLAN_DEFAULT_FG_SCAN_COUNT_PER_CHANNEL;   /* 非关联下的扫描，每信道扫描2次 */
...
/*****************************************************************************
 函 数 名  : dmac_scan_after_p2pgo_send_noa
 功能描述  : p2p go背景扫描发送完NoA的处理
 输入参数  : 无
 输出参数  : 无
 返 回 值  :
 调用函数  :
...
oal_void dmac_scan_end(mac_device_stru *pst_mac_device)
{
    ...
    hal_set_btcoex_soc_gpreg0(OAL_FALSE, BIT9, 9);   // 入网流程结束
    hal_coex_sw_irq_set(BIT5);
#endif
