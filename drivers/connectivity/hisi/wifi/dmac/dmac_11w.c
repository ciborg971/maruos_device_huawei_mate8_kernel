...
OAL_STATIC oal_bool_enum_uint8  dmac_11w_robust_action(oal_netbuf_stru *pst_mgmt_buf)
{
    ...
    switch (puc_mac_payload[MAC_ACTION_OFFSET_CATEGORY])
    {
        ...
        {
            OAM_INFO_LOG1(0, OAM_SF_PMF, "dmac_11w_robust_action:: Robust action[%d]",
                          puc_mac_payload[MAC_ACTION_OFFSET_CATEGORY]);
            return OAL_TRUE;
        }
                                             ...
                                             oal_netbuf_stru *pst_netbuf)
{
    ...
    ul_relt = mac_vap_find_user_by_macaddr(&pst_dmac_vap->st_vap_base_info, auc_addr1, &us_user_idx);
    if (OAL_SUCC != ul_relt)
    {
        //OAM_INFO_LOG1(pst_dmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_TX,
        //               "{dmac_11w_set_protectframe::mac_vap_find_user_by_macaddr failed[%d].", ul_relt);
    ...
    {
        OAM_INFO_LOG1(pst_dmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_TX,
                      "{dmac_11w_set_protectframe::pst_dmac_user null,us_user_idx=%d.}", us_user_idx);
        return;
    }
    ...
    if (OAL_TRUE == dmac_11w_robust_frame(pst_netbuf))
    {
        ...
        OAM_INFO_LOG1(pst_dmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_TX, "{dmac_11w_set_protectframe::cipher_key_type=%d.}", pst_security->en_cipher_key_type);
...
#endif
        OAM_INFO_LOG2(pst_dmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_CRYPTO, "{dmac_bip_crypto::en_cipher_key_type=%d us_len=%d.}",
                      pst_security->en_cipher_key_type, *pus_len);
...
oal_void dmac_11w_update_users_status(dmac_vap_stru  *pst_dmac_vap, mac_user_stru *pst_mac_user, oal_bool_enum_uint8 en_user_pmf)
{
    ...
    OAM_INFO_LOG3(pst_dmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_PMF,
                            "{dmac_11w_update_users_status::users pmf status:old[%d], new[%d], user_id[%d].}",
                            ul_user_pmf_old, pst_dmac_vap->ul_user_pmf_status, pst_mac_user->us_assoc_id);
...
oal_uint32 dmac_11w_rx_filter(dmac_vap_stru *pst_dmac_vap, oal_netbuf_stru  *pst_netbuf)
{
    ...
    {
        OAM_ERROR_LOG1(pst_dmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_PMF,
                      "{dmac_11w_rx_filter::pst_user null, us_ta_user_idx=%d.}", us_ta_user_idx);
    ...
    {
        OAM_ERROR_LOG0(pst_dmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_PMF,
    ...
    {
        OAM_ERROR_LOG2(pst_dmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_PMF,
