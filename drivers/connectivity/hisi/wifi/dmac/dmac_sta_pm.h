...
#define WLAN_PS_KEEPALIVE_MAX_NUM   100 /* 100 DTIM interval*/
...
#define DMAC_STATE_DOZE_TRANS_FAIL_NUM        2
#define DMAC_BEACON_DOZE_TRANS_FAIL_NUM       2
#define DMAC_BEACON_TIMEOUT_MAX_NUM           4
...
typedef enum {
    ...
    STA_PWR_EVENT_BEACON_TIMEOUT  = 8,      /* AWAKE状态下睡眠事件 先保留后面可以删掉 liuzhengqi todo */
    ...
    STA_PWR_EVENT_PNO_SCHED_SCAN_COMP,      /* PNO扫描完成，未扫描到执行的bss，进入睡眠 */
...
typedef struct _mac_sta_pm_handler
{
    ...
    oal_uint8                       en_receive_dtim                 :1;         /* STA是否以DTIM间隔苏醒 */
...
extern oal_void dmac_pm_sta_vote(dmac_vap_stru *pst_dmac_vap, oal_uint8 uc_vote);
extern oal_void dmac_pm_enable_front_end(mac_device_stru   *pst_mac_device);
