                            ...
                            oal_switch_enum_uint8      *pen_dscr_switch)
{
    ...
    {
        MAC_ERR_LOG(pst_mac_vap->uc_vap_id, "rx report get switch frame hdr is null!");
                                            ...
                                            oal_uint8        auc_user_macaddr[])
{
    ...
    if (OAL_PTR_NULL == pst_frame_hdr)
    {
        MAC_ERR_LOG(0, "rx report get user macaddr frame hdr is null!");
                                             ...
                                             oam_ota_type_enum_uint8 en_ota_type)
{
    ...
    {
        MAC_ERR_LOG2(0, "pst_tx_cb or pst_mac_vap is null!", pst_vap, pst_rx_cb);
                                ...
                                oal_int32     l_len)
{
    ...
    while (puc_pos < puc_end)
    {
        ...
        if (pst_ie->uc_len >= sizeof(*pst_ie))
        {
...
oal_void  mac_set_supported_rates_ie(oal_void *pst_vap, oal_uint8 *puc_buffer, oal_uint8 *puc_ie_len)
{
    ...
    if (WLAN_VAP_MODE_BSS_STA == pst_mac_vap->en_vap_mode && WLAN_VHT_MODE == pst_mac_vap->en_protocol)
    {
        ...
        if (en_band < WLAN_BAND_BUTT)
        {
            pst_rates_set = &(pst_mac_vap->ast_sta_sup_rates_ie[en_band].st_rate);
        }
    }
...
oal_void mac_set_dsss_params(oal_void *pst_vap, oal_uint8 *puc_buffer, oal_uint8 *puc_ie_len)
{
    ...
    ***************************************************************************/
    ...
    puc_buffer[2] = pst_mac_vap->st_channel.uc_chan_number;
...
*****************************************************************************/
oal_void mac_set_vendor_ie(oal_void *pst_vap, oal_uint8 *puc_buffer, oal_uint8 *puc_ie_len)
{
    mac_vap_stru                *pst_mac_vap = (mac_vap_stru *)pst_vap;
    mac_vendor_ie_stru          *pst_vendor_ie;
    ...
    pst_vendor_ie = (mac_vendor_ie_stru *)puc_buffer;
    pst_vendor_ie->uc_id = MAC_EID_VENDOR;
    pst_vendor_ie->uc_len = sizeof(mac_vendor_ie_stru) - MAC_IE_HDR_LEN;
    /* 此值参照a公司所写 */
    pst_vendor_ie->uc_ouitype = MAC_EID_NEIGHBOR_REPORT;
    ...
    OAL_MEMZERO(&pst_vendor_ie->st_ie, OAL_SIZEOF(mac_vendor_ie_htinfo_stru));
    pst_vendor_ie->st_ie.bit_11ac2g = pst_mac_vap->st_cap_flag.bit_11ac2g;
    *puc_ie_len = OAL_SIZEOF(mac_vendor_ie_stru);

}

/*****************************************************************************
 函 数 名  : mac_get_vendor_ie
 功能描述  : 获取厂商自定义ie
 输入参数  : puc_frame_body: 指向帧体
             uc_tag_param_offset: 偏移量
 输出参数  : puc_ie_len: element的长度
 返 回 值  : 无
 调用函数  :
 被调函数  :
...
*****************************************************************************/
oal_uint8  *mac_get_vendor_ie(oal_uint8 *puc_frame_body, oal_uint16 us_rx_len, oal_uint16 uc_tag_param_offset)
{
    ...
    {
        /* 221对应的都是非标准，可能有多个 */
        if (MAC_EID_VENDOR == puc_frame_body[us_index])
        {
            pst_vendor_ie = (mac_vendor_ie_stru *)&puc_frame_body[us_index];
            /*lint -e572*/ /*lint -e778*/
            if (pst_vendor_ie->auc_oui[0] == (oal_uint8)((MAC_HUAWEI_VENDER_IE >> 16) & 0xff)
            && pst_vendor_ie->auc_oui[1] == (oal_uint8)((MAC_HUAWEI_VENDER_IE >> 8) & 0xff)
            && pst_vendor_ie->auc_oui[2] == (oal_uint8)(MAC_HUAWEI_VENDER_IE & 0xff))
            /*lint +e572*/ /*lint +e778*/
            {
                return (&puc_frame_body[us_index]);
            }
        }
                ...
                oal_uint8 *puc_ie_len)
{
    ...
    {
        *puc_ie_len = 0;
        MAC_ERR_VAR(pst_mac_vap->uc_vap_id, "mac_set_rsn_ie_authenticator: uc_authentication_suites = 0");
...
oal_uint8* mac_get_bssload_ie(oal_uint8 *puc_frame_body, oal_uint32 us_frame_len, oal_uint16 us_offset)
{
    /* 寻找 OBSS 信息元素 */
    while (us_offset < us_frame_len)
    {
        if (MAC_EID_QBSS_LOAD == puc_frame_body[us_offset])
        {
            return(&puc_frame_body[us_offset]);
        }
        else
        {
            us_offset += (MAC_IE_HDR_LEN + puc_frame_body[us_offset + 1]);
        }
    }
...
oal_uint16 mac_get_rsn_capability(const oal_uint8 *puc_rsn_ie)
{
    ...
    if (us_pairwise_count > WLAN_AUTHENTICATION_SUITES)
    {
...
oal_void mac_set_supported_channel_ie(oal_uint8 *pst_vap, oal_uint8 *puc_buffer, oal_uint8 *puc_ie_len)
{
    ...
    *puc_buffer = MAC_EID_SUPPCHAN;
    ...
    for (uc_channel_idx = 0; uc_channel_idx < uc_channel_max_num; uc_channel_idx++)
    {
        ...
        ul_ret = mac_is_channel_idx_valid(pst_mac_vap->st_channel.en_band, uc_channel_idx);
        if (OAL_SUCC == ul_ret)
        {
            uc_channel_idx_cnt++;
...
oal_module_symbol(mac_set_vendor_ie);
oal_module_symbol(mac_get_vendor_ie);
