...
OAL_STATIC dmac_rx_frame_ctrl_enum_uint8 dmac_ao_process_arp_offload(dmac_vap_stru *pst_dmac_vap, dmac_user_stru *pst_dmac_user, oal_netbuf_stru *pst_netbuf)
{
    ...
    /* 2.申请net_buff */
    pst_tx_buf = OAL_MEM_NETBUF_ALLOC(OAL_NORMAL_NETBUF, WLAN_SHORT_NETBUF_SIZE, OAL_TRUE);
    if (OAL_UNLIKELY(OAL_PTR_NULL == pst_tx_buf))
    {
        OAM_ERROR_LOG0(pst_mac_vap->uc_vap_id, OAM_SF_PWR, "{dmac_ao_process_arp_offload::Alloc MAC memory buf failed.}");
        return DMAC_RX_FRAME_CTRL_GOON;
    }
    ...
    if (OAL_SUCC != ul_ret)
    {
        OAM_ERROR_LOG1(pst_mac_vap->uc_vap_id, OAM_SF_PWR, "{dmac_ao_process_arp_offload::Dmac tx process data failed[%d].}", ul_ret);
        dmac_tx_excp_free_netbuf(pst_tx_buf);
        return DMAC_RX_FRAME_CTRL_GOON;
    }
                                                          ...
                                                          mac_llc_snap_stru *pst_rx_snap)
{
    ...
    if (OAL_UNLIKELY(OAL_PTR_NULL == pst_tx_buf))
    {
        OAM_ERROR_LOG0(pst_mac_vap->uc_vap_id, OAM_SF_PWR, "{dmac_ao_encap_na::Alloc memory buf failed.}");
        return DMAC_RX_FRAME_CTRL_GOON;
    }
    ...
    if (OAL_UNLIKELY(OAL_SUCC != dmac_ao_encap_cb(pst_dmac_vap,
                                                  pst_dmac_user,
                                                  pst_tx_buf,
                                                  us_payload_len,
                                                  puc_dst_mac_addr)))
    ...
    *(oal_uint32 *)pst_tx_ipv6_hdr = OAL_HOST2NET_LONG(0x60000000);
    ...
    {
        OAM_ERROR_LOG1(pst_mac_vap->uc_vap_id, OAM_SF_PWR, "{dmac_ao_encap_na::Dmac tx data failed[%d].}", ul_ret);
        dmac_tx_excp_free_netbuf(pst_tx_buf);
        return DMAC_RX_FRAME_CTRL_GOON;
    }
...
OAL_STATIC dmac_rx_frame_ctrl_enum_uint8 dmac_ao_sta_process_multicast_filter(dmac_vap_stru * pst_dmac_vap , oal_netbuf_stru *pst_netbuf)
{
    ...
    {
        oal_ip_header_stru *pst_ipv4 = (oal_ip_header_stru *)(pst_snap + 1);
    ...
    {
        oal_ipv6hdr_stru *pst_ipv6 = (oal_ipv6hdr_stru *)(pst_snap + 1);
