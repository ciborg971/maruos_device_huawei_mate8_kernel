...
oal_uint32  dmac_uapsd_user_init(dmac_user_stru * pst_dmac_usr)
{
    ...
    {
        OAM_ERROR_LOG0(0, OAM_SF_PWR, "{dmac_uapsd_user_init:: alloc uapsd_statis mem fail!}");
...
oal_void dmac_uapsd_rx_trigger_check (dmac_vap_stru *pst_dmac_vap,dmac_user_stru *pst_dmac_user,oal_netbuf_stru *pst_net_buf)
{
        ...
        (uc_uapsd_flag & MAC_USR_UAPSD_SP))
    {
        ...
        OAM_INFO_LOG1(pst_dmac_user->st_user_base_info.uc_vap_id, OAM_SF_PWR,
                      "dmac_uapsd_rx_trigger_check:UAPSD not enable or User in USP,uc_uapsd_flag = %x",uc_uapsd_flag);
        return ;
    }
...
oal_void dmac_uapsd_process_trigger (dmac_vap_stru *pst_dmac_vap,dmac_user_stru *pst_dmac_user,oal_uint8 uc_ac,oal_netbuf_stru *pst_net_buf)
{
...
#endif
    us_seq = pst_mac_header->bit_seq_num;
    if((OAL_TRUE == pst_mac_header->st_frame_control.bit_retry)&&
        (us_seq == pst_dmac_user->st_uapsd_stru.us_uapsd_trigseq[uc_ac]))
    {
    ...
    pst_dmac_user->st_uapsd_stru.us_uapsd_trigseq[uc_ac] = us_seq;
...
oal_uint32  dmac_config_uapsd_debug(mac_vap_stru *pst_mac_vap, oal_uint8 uc_len, oal_uint8 *puc_param)
{
    ...
    /*VAP下的UAPAD信息*/
    //OAM_INFO_LOG1(0, OAM_SF_ANY, "pst_mac_vap->st_cap_flag.bit_uapsd = %d\n", mac_vap_get_uapsd_en(pst_mac_vap));
    ...
    if(0 == uc_debug_all)
    {
        ...
        ul_ret = mac_vap_find_user_by_macaddr(pst_mac_vap, auc_user_addr, &us_user_idx);

        if (OAL_SUCC != ul_ret)
        {
            OAM_WARNING_LOG0(pst_mac_vap->uc_vap_id, OAM_SF_ANY, "{dmac_config_uapsd_debug::do not find the user.}");
            return ul_ret;
        }
    ...
    else
    {
        ...
        //OAM_INFO_LOG0(0, OAM_SF_ANY, "all uapsd statis cleared\n");
    }
...
#if 0
   OAL_IO_PRINT("uc_uapsd_flag = %d\n",pst_dmac_user->uc_uapsd_flag);
   OAL_IO_PRINT("uc_qos_info   = %d\n",pst_uapsd_status->uc_qos_info);
   OAL_IO_PRINT("max_sp_len    = %d\n",pst_uapsd_status->uc_max_sp_len);
#endif
