...
OAL_STATIC oal_uint32  dmac_dft_report_dbb_env_param(mac_device_stru *pst_macdev)
{
    ...
    /* 上报参数 */
    return oam_report_dft_params(auc_user_macaddr, (oal_uint8 *)&st_dbb_env_param,(oal_uint16)OAL_SIZEOF(oam_stats_dbb_env_param_stru), OAM_OTA_TYPE_DBB_ENV_PARAM);
...
oal_void  dmac_dft_print_all_para(dmac_vap_stru *pst_dmac_sta)
{
    mac_device_stru                 *pst_mac_dev;

#ifdef _PRE_WLAN_FEATURE_STA_PM
    mac_cfg_ps_info_stru             st_ps_info;
#endif
    ...
    /* 打印sta低功耗维测信息 */
#ifdef _PRE_WLAN_FEATURE_STA_PM
    st_ps_info.uc_psm_info_enable= 1;
    st_ps_info.uc_psm_debug_mode = 0;
	
	/* 协议的计数 */
    dmac_show_ps_info(&(pst_dmac_sta->st_vap_base_info),OAL_SIZEOF(mac_cfg_ps_info_stru), (oal_uint8 *)(&st_ps_info));
   
    /* 平台的计数*/
	PM_WLAN_Debug_DumpAll();
#endif
...
oal_void  dmac_dft_report_all_ota_state(dmac_vap_stru *pst_dmac_sta)
{
    mac_device_stru                 *pst_mac_dev;
    oal_uint8                        auc_user_macaddr[WLAN_MAC_ADDR_LEN] = {0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF};
    oal_uint32                       ul_ret     = OAL_SUCC;
    /*lint -e813*/
    oal_uint32                      ul_report_reg_data[DMAC_DFT_LINKLOSS_DATA_LEN];/*lint +e813*/
    ...
    /*上报phy寄存器的值*/
    for(us_bank_select = 1;us_bank_select < 5;us_bank_select ++)
    {
        /*将统计值清零*/
        OAL_MEMZERO(&ul_report_reg_data, DMAC_DFT_LINKLOSS_DATA_LEN * OAL_SIZEOF(oal_uint32));

        hal_dft_get_phyhw_stat_info(pst_mac_dev->pst_device_stru,(oal_uint32 *)&ul_report_reg_data,us_bank_select);

        /* 将获取到的统计值上报 */
        ul_ret = oam_report_dft_params(auc_user_macaddr, (oal_uint8 *)&ul_report_reg_data,(oal_uint16)(DMAC_DFT_LINKLOSS_DATA_LEN * OAL_SIZEOF(oal_uint32)), OAM_OTA_TYPE_PHY_STAT);

        /*判断phy寄存器的值是否上报成功*/
        if(OAL_UNLIKELY(OAL_SUCC != ul_ret))
        {
            OAM_WARNING_LOG1(0, OAM_SF_ANY, "dmac_dft_report_all_ota_state : report_phyhw state error[%d]!", ul_ret);
        }
    }

    /*上报MAC寄存器的值*/
    for(us_bank_select = 0;us_bank_select < 5;us_bank_select ++)
    {
        /*将统计值清零*/
        OAL_MEMZERO(&ul_report_reg_data, DMAC_DFT_LINKLOSS_DATA_LEN * OAL_SIZEOF(oal_uint32));
        /* 从MAC寄存器获取统计值 */
        hal_dft_get_machw_stat_info(pst_mac_dev->pst_device_stru, (oal_uint32 *)&ul_report_reg_data,us_bank_select);
        /*上报mac信息*/
        ul_ret = oam_report_dft_params(auc_user_macaddr, (oal_uint8 *)&ul_report_reg_data,(oal_uint16)(DMAC_DFT_LINKLOSS_DATA_LEN * OAL_SIZEOF(oal_uint32)), OAM_OTA_TYPE_MACHW_STAT);

        /*判断mac寄存器的值是否上报成功*/
        if(OAL_UNLIKELY(OAL_SUCC != ul_ret))
        {
            OAM_WARNING_LOG1(0, OAM_SF_ANY, "dmac_dft_report_all_ota_state : report_machw state error[%d]!", ul_ret);
        }
    }
    ...
    OAL_MEMZERO(&ul_report_reg_data, DMAC_DFT_LINKLOSS_DATA_LEN * OAL_SIZEOF(oal_uint32));
    /* 从RF寄存器获取统计值 */
    hal_dft_get_rfhw_stat_info(pst_mac_dev->pst_device_stru, (oal_uint32 *)&ul_report_reg_data);
    /*上报RF信息*/
    ul_ret = oam_report_dft_params(auc_user_macaddr, (oal_uint8 *)&ul_report_reg_data,(oal_uint16)(DMAC_DFT_LINKLOSS_DATA_LEN * OAL_SIZEOF(oal_uint32)), OAM_OTA_TYPE_MACHW_STAT);
    ...
    if(OAL_UNLIKELY(OAL_SUCC != ul_ret))
    {
        OAM_WARNING_LOG1(0, OAM_SF_ANY, "dmac_dft_report_all_ota_state : report_rfchw state error[%d]!", ul_ret);
    }
...
oal_void dmac_dft_report_all_para(dmac_vap_stru *pst_dmac_sta,oal_uint8 uc_ota_switch)
{
    ...
    /*打印维测信息*/
    dmac_dft_print_all_para(pst_dmac_sta);
    /*OTA上报维测信息*/
    if(OAL_TRUE == uc_ota_switch)
    {
        dmac_dft_report_all_ota_state(pst_dmac_sta);
    }
}
...
oal_void dmac_dft_report_dmac_vap_info(mac_vap_stru  *pst_mac_vap)
{
    ...
    /* 上报dmac vap信息 */
    oam_report_dft_params(auc_user_macaddr,
...
oal_uint32  dmac_dft_tid_stat_init(mac_user_stru  *pst_mac_user)
{
    ...
    for (ul_tid_idx = 0; ul_tid_idx < WLAN_TID_MAX_NUM; ul_tid_idx++)
    {
        ...
        pst_tid_queue->pst_tid_stats = (dmac_tid_stats_stru *)OAL_MEM_ALLOC(OAL_MEM_POOL_ID_LOCAL,
                                                                            ...
                                                                            OAL_TRUE);
        if (OAL_UNLIKELY(OAL_PTR_NULL == pst_tid_queue->pst_tid_stats))
        {
            OAM_ERROR_LOG0(0, OAM_SF_TX, "{dmac_dft_tid_stat_init::tid_stats is null.");
            return OAL_ERR_CODE_PTR_NULL;
        }
...
oal_uint32  dmac_dft_ampdu_stat_init(mac_user_stru  *pst_mac_user)
{
    oal_uint32      ul_tid_idx;
    dmac_tid_stru  *pst_tid_queue;
    dmac_user_stru *pst_dmac_user;
    ...
    {
        OAM_ERROR_LOG0(0, OAM_SF_ANY, "dmac_dft_ampdu_stat_init::pst_mac_user is null!");
        return OAL_ERR_CODE_PTR_NULL;
    }
    ...
    for (ul_tid_idx = 0; ul_tid_idx < WLAN_TID_MAX_NUM; ul_tid_idx++)
    {
        ...
        pst_tid_queue->pst_tid_ampdu_stat = (dmac_tid_ampdu_stat_stru *)OAL_MEM_ALLOC(OAL_MEM_POOL_ID_LOCAL,
                                                                                      ...
                                                                                      OAL_TRUE);
        if (OAL_UNLIKELY(OAL_PTR_NULL == pst_tid_queue->pst_tid_ampdu_stat))
        {
            OAM_ERROR_LOG0(0, OAM_SF_TX, "{dmac_dft_ampdu_stat_init::tid_stats is null.");
            return OAL_ERR_CODE_PTR_NULL;
        }
