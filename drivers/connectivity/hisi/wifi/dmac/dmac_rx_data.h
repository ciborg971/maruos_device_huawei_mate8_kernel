...
#define DMAC_STAT_RX_PKT_COUNT (8)
#define DMAC_STAT_RX_PKT_COUNT_OFFSET (3)
                ...
                dmac_rx_ctl_stru           *pst_cb_ctrl)
{
    ...
    if (OAL_UNLIKELY(MAC_GET_RX_CB_MAC_HEADER_LEN(&(pst_cb_ctrl->st_rx_info)) > pst_cb_ctrl->st_rx_info.us_frame_len))
    {
        ...
        oam_report_dscr(auc_user_macaddr, (oal_uint8 *)pul_dscr, (oal_uint16)ul_rx_dscr_len, OAM_OTA_TYPE_RX_DSCR);
    }
...
/*****************************************************************************
 函 数 名  : dmac_rx_process_data_sta
 功能描述  :
 输入参数  : 无
 输出参数  : 无
 返 回 值  :
...
*****************************************************************************/
OAL_STATIC OAL_INLINE oal_uint32  dmac_rx_process_data_sta(
...
/*****************************************************************************
 函 数 名  : dmac_rx_process_data_ap
 功能描述  : AP模式下，对接收到数据帧时，对一些特性的处理
 输入参数  : (1)DMAC模块对应的vap指针
             (2)DMAC模块对应的user指针
             (3)接收到帧的对应的netbuf指针
 输出参数  : 无
 返 回 值  : 成功或者失败原因
...
*****************************************************************************/
OAL_STATIC OAL_INLINE oal_uint32  dmac_rx_process_data_ap(
                dmac_vap_stru          *pst_dmac_vap,
                dmac_user_stru         *pst_dmac_user,
                oal_netbuf_stru        *pst_netbuf)
{
...
/*****************************************************************************
 函 数 名  : dmac_rx_rssi_stat
 功能描述  : 接收流程上，统计rssi值
 输入参数  : dmac_user_rate_info_stru *pst_user_info
             用户速率信息结构体指针
...
*****************************************************************************/
OAL_STATIC OAL_INLINE oal_void dmac_rx_rssi_stat(dmac_user_rate_info_stru *pst_user_info,
                                                 hal_rx_statistic_stru *pst_rx_stats)
{
    if (0 != pst_rx_stats->c_rssi_dbm)
    {
        pst_user_info->st_dmac_rssi_stat_info.l_rx_rssi += pst_rx_stats->c_rssi_dbm;
        pst_user_info->st_dmac_rssi_stat_info.us_rx_rssi_stat_count++;
    }
}

/*****************************************************************************
 函 数 名  : dmac_rx_rate_stat
 功能描述  : 接收流程上，统计rate值
 输入参数  : dmac_user_rate_info_stru *pst_user_info
             用户速率信息结构体指针
             hal_rx_statistic_stru *pst_rx_stats
...
*****************************************************************************/
OAL_STATIC OAL_INLINE oal_void dmac_rx_rate_stat(dmac_user_stru *pst_dmac_user,
                                                 hal_rx_statistic_stru *pst_rx_stats)
{
    ...
    if (0 != ul_rate_kbps)
    {
        pst_dmac_user->st_user_rate_info.st_dmac_rate_stat_info.ul_rx_rate +=  ul_rate_kbps;

        pst_dmac_user->st_user_rate_info.st_dmac_rate_stat_info.us_rx_rate_stat_count ++;
...
*****************************************************************************/
OAL_STATIC OAL_INLINE oal_void dmac_rx_rssi_rate_stat(dmac_user_stru *pst_dmac_user, hal_rx_statistic_stru *pst_rx_stats)
{
    dmac_user_rate_info_stru *pst_user_info = &pst_dmac_user->st_user_rate_info;

    if (OAL_TRUE == pst_user_info->en_dmac_rssi_stat_flag)
    {
        dmac_rx_rssi_stat(pst_user_info, pst_rx_stats);
    }

    if (OAL_TRUE == pst_user_info->en_dmac_rate_stat_flag)
    {
        dmac_rx_rate_stat(pst_dmac_user, pst_rx_stats);
    }
                ...
                oal_netbuf_stru                *pst_netbuf)
{
   ...
   // pst_frame_hdr = (mac_ieee80211_frame_stru *)(pst_cb_ctrl->st_rx_info.pul_mac_hdr_start_addr);
    ...
    dmac_rx_rssi_rate_stat(pst_dmac_user, &pst_cb_ctrl->st_rx_statistic);

    /* 过滤三:DMAC接收低功耗的过滤 */
    ul_ret = dmac_rx_data_ps_process(pst_dmac_vap, pst_dmac_user, pst_netbuf);
    if (OAL_SUCC != ul_ret)
    {
        return OAL_FAIL;
    }
...
#endif
extern oal_uint32 dmac_start_stat_rate(dmac_user_stru *pst_dmac_user);
extern oal_uint32 dmac_get_stat_rate(dmac_user_stru *pst_dmac_user, oal_uint32 *pul_tx_rate, oal_uint32 *pul_rx_rate);
extern oal_uint32 dmac_stop_stat_rate(dmac_user_stru *pst_dmac_user);
