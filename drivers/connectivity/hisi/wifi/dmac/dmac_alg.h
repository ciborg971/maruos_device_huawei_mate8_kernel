                ...
                hal_tx_dscr_ctrl_one_param     *pst_tx_dscr_param)
{
    ...
    for (ul_index = DMAC_ALG_TX_COMPLETE_START; ul_index < DMAC_ALG_TX_COMPLETE_NOTIFY_BUTT; ul_index++)
    {
        if (OAL_PTR_NULL != pst_alg_stru->pa_tx_complete_notify_func[ul_index])
        {
            ...
            #if 0
            ul_ret = pst_alg_stru->pa_tx_complete_notify_func[ul_index](pst_user, pst_buf, pst_tx_dscr_param);
            if (OAL_SUCC != ul_ret)
            {
                OAM_WARNING_LOG1(pst_user->uc_vap_id, OAM_SF_TX, "{dmac_alg_tx_complete_notify::pa_tx_complete_notify_func failed[%d].}", ul_ret);
            }
            #endif
        }
    }
