...
oal_uint32 dmac_reset_gtk_token(mac_vap_stru *pst_mac_vap)
{
    ...
    if (OAL_PTR_NULL == pst_multi_user)
    {
        OAM_ERROR_LOG0(pst_mac_vap->uc_vap_id, OAM_SF_WPA, "{dmac_11i_add_gtk_key::mac_res_get_mac_user null.}");
 ...
 oal_uint32  dmac_11i_add_ptk_key(mac_vap_stru *pst_mac_vap, oal_uint8 *puc_mac_addr, oal_uint8 uc_key_index)
{
    ...
    if (OAL_SUCC != ul_ret)
    {
        OAM_ERROR_LOG1(pst_mac_vap->uc_vap_id, OAM_SF_WPA,
                       "{dmac_11i_add_ptk_key::mac_vap_find_user_by_macaddr failed. mac[XX%8x]}", MAC_ADDR(puc_mac_addr));
        return OAL_ERR_CODE_SECURITY_USER_INVAILD;
    }
    /*1.2 根据索引找到user内存区域*/
    pst_current_dmac_user = (dmac_user_stru *)mac_res_get_dmac_user(us_user_idx);
    if (OAL_PTR_NULL == pst_current_dmac_user)
    {
        OAM_ERROR_LOG1(pst_mac_vap->uc_vap_id, OAM_SF_WPA,
                       "{dmac_11i_add_ptk_key::pst_current_dmac_user null. us_user_idx=%d.}", us_user_idx);
...
oal_uint32  dmac_11i_add_wep_key(mac_vap_stru *pst_mac_vap, oal_uint8 uc_key_index)
{
    ...
    OAM_INFO_LOG1(pst_mac_vap->uc_vap_id, OAM_SF_WPA,
                  "{dmac_11i_add_wep_key::multiuser ciphertype==%u}", pst_multi_user->st_key_info.en_cipher_type);
    ...
    {
        OAM_ERROR_LOG1(pst_mac_vap->uc_vap_id, OAM_SF_WPA,
                       "{dmac_11i_add_wep_key::dmac_11i_update_key_to_ce invalid[%d].}", ul_ret);
...
OAL_STATIC oal_uint32  dmac_11i_del_ptk_key(mac_vap_stru *pst_mac_vap, oal_uint8 uc_key_index, oal_uint8 *puc_mac_addr)
{
    ...
    if (OAL_SUCC != ul_ret)
    {
        OAM_WARNING_LOG1(pst_mac_vap->uc_vap_id, OAM_SF_WPA, "{dmac_11i_del_ptk_key::mac_vap_find_user_by_macaddr failed[%d]}", ul_ret);
        return ul_ret;
    }
...
oal_uint32  dmac_config_11i_add_key_set_reg(mac_vap_stru *pst_mac_vap, oal_uint8 uc_key_index, oal_uint8 *puc_mac_addr)
{
    ...
    OAM_INFO_LOG1(pst_mac_vap->uc_vap_id, OAM_SF_WPA,
                           "{dmac_config_11i_add_key_set_reg::key_index:%d OK.}",uc_key_index);
    return OAL_SUCC;
}
...
oal_uint32  dmac_config_11i_set_default_key(mac_vap_stru *pst_mac_vap, oal_uint8 uc_len, oal_uint8 *puc_param)
{
    ...
    if (OAL_PTR_NULL == pst_multi_user)
    {
        OAM_ERROR_LOG0(pst_mac_vap->uc_vap_id, OAM_SF_WPA, "{hmac_config_11i_set_default_key::multi_user null.}");
        return OAL_ERR_CODE_SECURITY_USER_INVAILD;
    }
    ...
    OAM_INFO_LOG3(pst_mac_vap->uc_vap_id, OAM_SF_WPA, "{dmac_config_11i_set_default_key::key_id[%d] un[%d] mu[%d] OK}",
