...
oal_uint8 * wal_find_wpa_ie(oal_uint8 *puc_frame_body, oal_int32 l_len)
{
    oal_int32 l_index = 0;
    ...
    while (l_index < l_len)
    {
        if ((MAC_EID_WPA == puc_frame_body[l_index])
            && (0 == oal_memcmp(puc_frame_body + l_index + 2, g_auc_wpa_oui, MAC_OUI_LEN))
            && (MAC_OUITYPE_WPA == puc_frame_body[l_index + 2 + MAC_OUI_LEN]))
        {
            return(&puc_frame_body[l_index]);
        }
        else
        {
                                 ...
                                 mac_beacon_param_stru *pst_beacon_param)
{
    ...
    if (WLAN_WITP_CAPABILITY_PRIVACY & us_capability_info)
    {
        ...
        /* 查找 RSN 信息元素 */
        puc_rsn_ie = oal_cfg80211_find_ie(MAC_EID_RSN, pst_beacon_info->tail, pst_beacon_info->tail_len);
        ...
        puc_wpa_ie = wal_find_wpa_ie(pst_beacon_info->tail, pst_beacon_info->tail_len);
...
oal_uint32 wal_cfg80211_add_vap(mac_cfg_add_vap_param_stru *pst_add_vap_param)
{
    ...
    {
        OAM_WARNING_LOG0(pst_cfg_mac_vap->uc_vap_id, OAM_SF_CFG, "{wal_cfg80211_add_vap::pst_mac_device is null ptr!}\r\n");
        return OAL_ERR_CODE_PTR_NULL;
    }
    ...
    ***************************************************************************/
    ...
    if(OAL_PTR_NULL != pst_rsp_msg)
    {
        ...
        {
            OAM_WARNING_LOG1(pst_cfg_mac_vap->uc_vap_id, OAM_SF_ANY, "{wal_cfg80211_add_vap::hmac add vap fail, ul_err_code[%d]!}\r\n",
...
oal_uint32 wal_cfg80211_del_vap(mac_cfg_del_vap_param_stru *pst_del_vap_param)
{
    ...
    ***************************************************************************/
    ...
    if (OAL_UNLIKELY(OAL_SUCC != l_ret))
    {
        OAM_WARNING_LOG1(0, OAM_SF_ANY, "{wal_cfg80211_del_vap::return err code %d!}\r\n", l_ret);
        /* 不能去注册net_device，需要保持net_device，在外部根据业务释放net_device */
...
#endif
{
    ...
    OAM_WARNING_LOG4(pst_mac_vap->uc_vap_id, OAM_SF_CFG, "{wal_cfg80211_scan::start a new normal scan. n_channels[%d], ie_len[%d], n_ssid[%d], vapid[%d]}",
      pst_request->n_channels, pst_request->ie_len, pst_request->n_ssids, pst_mac_vap->uc_vap_id);

    /* TBD: gong 不可以重叠扫描, 若上次未结束，需要返回busy  */
    /* 等待上次的扫描请求完成 */
    /*lint -e730*//* info, boolean argument to function */
    l_ret = OAL_WAIT_EVENT_INTERRUPTIBLE(pst_wiphy_mgmt->st_wait_queue,(OAL_PTR_NULL == pst_wiphy_mgmt->pst_request));
    /*lint +e730*/
    ...
    /*lint +e801*/
    /* 保存当前的扫描请求指针 */
    pst_wiphy_mgmt->pst_request = pst_request;

#if (LINUX_VERSION_CODE >= KERNEL_VERSION(3,10,44))
    /* 如果当前调度扫描在运行，先暂停调度扫描 */
    if (OAL_PTR_NULL != pst_wiphy_mgmt->pst_sched_scan_req)
    {
    ...
    /* 进入扫描 */
    wal_scan_work_func(pst_wiphy_mgmt, pst_netdev, pst_request);
...
#endif
{
    ...
    ***************************************************************************/
    ...
    if (OAL_UNLIKELY(OAL_SUCC != l_ret))
    {
        OAM_ERROR_LOG1(0, OAM_SF_ANY, "{wal_cfg80211_add_key::wal_send_cfg_event: return err code %d!}\r\n", l_ret);
        return -OAL_EFAIL;
    }
...

{
    ...
    ***************************************************************************/
    ...
    if (OAL_UNLIKELY(OAL_SUCC != l_ret))
    {
        OAM_ERROR_LOG1(0, OAM_SF_ANY, "{wal_cfg80211_get_key::return err code %d!}\r\n", l_ret);
        return -OAL_EINVAL;
    }
...

{
    ...
    ***************************************************************************/
    ...
    if (OAL_UNLIKELY(OAL_SUCC != l_ret))
    {
        OAM_WARNING_LOG1(0, OAM_SF_ANY, "{wal_cfg80211_remove_key::return err code %d!}\r\n", l_ret);
        return -OAL_EFAIL;
    }
                                                          ...
                                                          oal_net_device_stru *pst_netdev)
{
    ...
    if (l_ret != OAL_SUCC)
    {
        OAM_ERROR_LOG2(uc_vap_id, OAM_SF_ANY, "{wal_cfg80211_set_channel::channel num is invalid. band, ch num [%d] [%d]!}\r\n", pst_channel->band, l_channel);
        return -OAL_EINVAL;
    }
    ...
    ***************************************************************************/
    ...
    if(OAL_PTR_NULL != pst_rsp_msg)
    {
        ...
        if (OAL_SUCC != pst_write_rsp_msg->ul_err_code)
        {
            OAM_ERROR_LOG1(uc_vap_id, OAM_SF_ANY, "{wal_cfg80211_set_channel::wal_send_cfg_event return err code: [%d].}",
                           pst_write_rsp_msg->ul_err_code);
            return -OAL_EFAIL;
        }
    }
                                                ...
                                                oal_ap_settings_stru     *pst_ap_settings)
{
    ...
    *****************************************************************************/
    ...
    if (OAL_SUCC != l_ret)
    {
        OAM_ERROR_LOG1(uc_vap_id, OAM_SF_ANY, "{wal_cfg80211_start_ap::failed to set channel, return err code[%x]}", l_ret);
        return -OAL_EFAIL;
    }
                                                ...
                                                oal_net_device_stru *pst_netdev)
{
    ...
    OAM_INFO_LOG0(0, OAM_SF_ANY, "{wal_cfg80211_stop_ap::enter here.}");
    ...
    {
        OAM_WARNING_LOG0(uc_vap_id, OAM_SF_ANY, "{wal_cfg80211_stop_ap::vap is not in down state!}");
        return OAL_SUCC;
    }
...
OAL_STATIC oal_void  wal_cfg80211_print_sched_scan_req_info(oal_cfg80211_sched_scan_request_stru  *pst_request)
{
    ...
    OAL_SPRINTF(ac_tmp_buff, OAL_SIZEOF(ac_tmp_buff), "Sched scan info: n_channels[%d], interval[%d], flags[%d], rssi_thold[%d].\n",
                                                           ...
                                                           oal_cfg80211_sched_scan_request_stru  *pst_request)
{
    ...
    /* 保存当前的PNO调度扫描请求指针 */
    pst_wiphy_mgmt->pst_sched_scan_req     = pst_request;
    pst_wiphy_mgmt->en_sched_scan_complete = OAL_FALSE;

    /* 维测打印上层下发的调度扫描请求参数信息 */
    wal_cfg80211_print_sched_scan_req_info(pst_request);

    /* 下发pno扫描请求到hmac */
    wal_cfg80211_start_sched_scan(pst_netdev, &st_pno_scan_info);
                                                           ...
                                                           oal_net_device_stru     *pst_netdev)
{
    ...
    if (OAL_PTR_NULL == pst_mac_device)
    {
        OAM_ERROR_LOG0(0, OAM_SF_CFG, "{wal_cfg80211_sched_scan_stop:: pst_mac_device is null!}");
        return -OAL_EINVAL;
    }
    ...
    if ((OAL_PTR_NULL != pst_wiphy_mgmt->pst_sched_scan_req) &&
        (OAL_TRUE != pst_wiphy_mgmt->en_sched_scan_complete))
    {
        /* 如果正常扫描请求未执行，则上报调度扫描结果 */
        if (OAL_PTR_NULL == pst_wiphy_mgmt->pst_request)
        {
            oal_cfg80211_sched_scan_result(pst_mac_device->pst_wiphy);
        }

        pst_wiphy_mgmt->pst_sched_scan_req     = OAL_PTR_NULL;
        pst_wiphy_mgmt->en_sched_scan_complete = OAL_TRUE;
...
#endif
{
    ...
    switch (en_type) {
        ...
        case NL80211_IFTYPE_STATION:
        {
            ...
            if(NL80211_IFTYPE_AP == pst_net_dev->ieee80211_ptr->iftype)
            {
                ...
                l_ret = wal_stop_vap(pst_net_dev);
                if(OAL_SUCC == l_ret)
                {
    ...
    if(NL80211_IFTYPE_AP == en_type)
    {
        if (OAL_NETDEVICE_FLAGS(pst_net_dev) & OAL_IFF_RUNNING)
        {
            ...
            OAL_IO_PRINT("stop netdevice:%s", pst_net_dev->name);
...
oal_uint8 wal_cfg80211_get_station_filter(mac_vap_stru *pst_mac_vap, oal_uint8 *puc_mac)
{
    ...
    mac_vap_find_user_by_macaddr(pst_mac_vap, puc_mac, &us_user_idx);
    pst_hmac_user = (hmac_user_stru *)mac_res_get_hmac_user(us_user_idx);
    if(OAL_PTR_NULL == pst_hmac_user)
    {
        OAM_WARNING_LOG1(pst_mac_vap->uc_vap_id, OAM_SF_CFG, "{wal_cfg80211_get_station_filter::user %d is null.}", us_user_idx);
        return OAL_FALSE;
    }
...
*****************************************************************************/
oal_void wal_cfg80211_unregister_netdev(hmac_device_stru *pst_hmac_device, oal_net_device_stru *pst_net_dev)
{
...
//    OAL_NETDEVICE_TX_QUEUE_LEN(pst_net_dev) = 0;
    ...
    st_add_vap_param.en_vap_mode = en_vap_mode;
    st_add_vap_param.en_p2p_mode = en_p2p_mode;
    st_add_vap_param.uc_cfg_vap_indx = uc_cfg_vap_id;
    ...
    ***************************************************************************/
    /* 填写消息 */
    WAL_WRITE_MSG_HDR_INIT(&st_write_msg, WLAN_CFGID_ADD_VAP, OAL_SIZEOF(mac_cfg_add_vap_param_stru));
    ((mac_cfg_add_vap_param_stru *)st_write_msg.auc_value)->pst_net_dev     = st_add_vap_param.pst_net_dev;
    ((mac_cfg_add_vap_param_stru *)st_write_msg.auc_value)->en_vap_mode     = st_add_vap_param.en_vap_mode;
    ((mac_cfg_add_vap_param_stru *)st_write_msg.auc_value)->uc_cfg_vap_indx = st_add_vap_param.uc_cfg_vap_indx;
#ifdef _PRE_WLAN_FEATURE_P2P
    ((mac_cfg_add_vap_param_stru *)st_write_msg.auc_value)->en_p2p_mode     = st_add_vap_param.en_p2p_mode;
#endif
    /* 发送消息 */
    l_ret = wal_send_cfg_event(pst_cfg_net_dev,
                               WAL_MSG_TYPE_WRITE,
    ...
    if(OAL_PTR_NULL != pst_rsp_msg)
    {
        ...
        if (OAL_SUCC != pst_write_rsp_msg->ul_err_code)
        {
            OAM_WARNING_LOG1(pst_cfg_mac_vap->uc_vap_id, OAM_SF_ANY, "{wal_cfg80211_add_virtual_intf::ul_err_code=%d!}\r\n",
                                        pst_write_rsp_msg->ul_err_code);
            wal_cfg80211_unregister_netdev(pst_hmac_device, pst_net_dev);
            /* 异常处理，释放内存 */
            OAL_MEM_FREE(pst_wdev, OAL_FALSE);
            return ERR_PTR(-EAGAIN);
        }
                                                   ...
                                                   oal_wireless_dev_stru    *pst_wdev)
{
    ...
    {
        OAM_WARNING_LOG1(0, OAM_SF_ANY, "{wal_cfg80211_del_virtual_intf::hmac_vap is null. vap_id[%d]!}\r\n", pst_mac_vap->uc_vap_id);
        return -OAL_EINVAL;
    }
                                                    ...
                                                    oal_uint64               *pull_cookie)
{
    ...
    ***************************************************************************/
    ...
    if(OAL_PTR_NULL != pst_rsp_msg)
    {
        ...
        if (OAL_SUCC != pst_write_rsp_msg->ul_err_code)
        {
            OAM_ERROR_LOG1(0, OAM_SF_CFG, "{wal_cfg80211_remain_on_channel::wal_send_cfg_event return err code:[%d]!}\r\n",
                             pst_write_rsp_msg->ul_err_code);
            return -OAL_EFAIL;
        }
    }
                                                            ...
                                                            oal_uint64               ull_cookie)
{
    ...
    /***************************************************************************
        抛事件到wal层处理
    ***************************************************************************/
    OAM_WARNING_LOG0(0, OAM_SF_P2P, "wal_cfg80211_cancel_remain_on_channel.");
    ...
    if(OAL_PTR_NULL != pst_rsp_msg)
    {
        ...
        if (OAL_SUCC != pst_write_rsp_msg->ul_err_code)
        {
            OAM_ERROR_LOG1(0, OAM_SF_P2P, "{wal_cfg80211_cancel_remain_on_channel::wal_send_cfg_event return err code:[%d]!}\r\n",
                            pst_write_rsp_msg->ul_err_code);
            return -OAL_EFAIL;
        }
    }
                                                ...
                                                mac_mgmt_frame_stru *pst_mgmt_tx_param)
{
    ...
    ***************************************************************************/
    ...
    return (pst_mgmt_tx->mgmt_tx_status == OAL_SUCC)?OAL_SUCC:OAL_FAIL;
}
                                            ...
                                            oal_uint64                  *pull_cookie)
{
    ...
    hmac_vap_stru                   *pst_hmac_vap;
    oal_mgmt_tx_stru                *pst_mgmt_tx;
    oal_uint32                      ul_start_time_stamp;
    ...
    *pull_cookie = pst_p2p_info->ull_send_action_id++;   /* cookie值上层调用需要判断是否是这次的发送导致的callback */
    ...
    }while((ul_ret != OAL_SUCC) && (uc_retry++ < WAL_MGMT_TX_RETRY_CNT));
...
oal_uint32  wal_cfg80211_mgmt_tx_status(frw_event_mem_stru *pst_event_mem)
{
    ...
    if(OAL_SUCC == wal_check_cookie_from_array(&g_uc_cookie_array_bitmap, pst_mgmt_tx->mgmt_frame_id))
    {
        ...
        OAL_SMP_MB();
        OAL_WAIT_QUEUE_WAKE_UP(&pst_mgmt_tx->st_wait_queue);
    }
...
oal_void wal_cfg80211_mgmt_frame_register(struct wiphy *wiphy,
                                        ...
                                        bool reg)
{
    OAM_INFO_LOG0(0, OAM_SF_CFG, "{wal_cfg80211_mgmt_frame_register::enter 000.}");
				...
				bool enabled, oal_int32 timeout)
{
    ...
    if(g_wlan_pm_switch)
    {
        ...
        OAM_WARNING_LOG1(0, OAM_SF_PWR, "{wal_cfg80211_set_power_mgmt::pm:[%d]}", enabled);
        ...
        if (!IS_LEGACY_VAP(pst_mac_vap))
        {
            OAM_WARNING_LOG0(0, OAM_SF_PWR,"wal_cfg80211_set_power_mgmt:vap not legacy");
            return OAL_SUCC;
        }

        pst_sta_pm_open = (mac_cfg_ps_open_stru *)(st_write_msg.auc_value);
        pst_sta_pm_open->uc_sta_pm_open = enabled;
