...
oal_uint32  wal_scan_comp_proc_sta(frw_event_mem_stru *pst_event_mem)
{
    ...
    if (OAL_PTR_NULL == pst_mac_device)
    {
        OAM_WARNING_LOG0(pst_event->st_event_hdr.uc_vap_id, OAM_SF_SCAN,
                         "{wal_scan_comp_proc_sta::pst_mac_device is null ptr!}");
        return OAL_ERR_CODE_PTR_NULL;
    }
    ...
    {
        OAM_ERROR_LOG0(pst_event->st_event_hdr.uc_vap_id, OAM_SF_SCAN,
                       "{wal_scan_comp_proc_sta::pst_hmac_device is null ptr!}");
        return OAL_ERR_CODE_PTR_NULL;
    }
    ...
    /* 如果是上层下发的扫描结果，无论扫描结果成功与否，统一上报扫描结果，有几个上报几个 */
    if ((OAL_PTR_NULL != pst_wiphy_mgmt->pst_request) ||
        (OAL_PTR_NULL != pst_wiphy_mgmt->pst_sched_scan_req))
    {
        /* 上报所有扫描到的bss */
...
{
    frw_event_stru              *pst_event;
    oal_disconnet_result_stru    st_disconnet_result;
    oal_net_device_stru         *pst_net_device;
    ...
    oal_memset(&st_disconnet_result, 0, OAL_SIZEOF(oal_disconnet_result_stru));

    /* 准备上报内核的关联结果结构体 */
    st_disconnet_result.us_reason_code = *pus_disasoc_reason_code;

    /* 调用内核接口，上报去关联结果 */
    ul_ret = oal_cfg80211_disconnected(pst_net_device,
                              st_disconnet_result.us_reason_code,
                              st_disconnet_result.pus_disconn_ie,
                              st_disconnet_result.us_disconn_ie_len,
                              GFP_ATOMIC);
    if (OAL_SUCC != ul_ret)
    {
...
oal_uint32  wal_disconnet_sta_proc_ap(frw_event_mem_stru *pst_event_mem)
{
    frw_event_stru            *pst_event;
    oal_net_device_stru       *pst_net_device;
    ...
    if (OAL_UNLIKELY(OAL_PTR_NULL == pst_event_mem))
    {
        OAM_ERROR_LOG0(0, OAM_SF_ASSOC, "{wal_disconnet_sta_proc_ap::pst_event_mem is null!}\r\n");
        return OAL_ERR_CODE_PTR_NULL;
    }
    ...
    if (OAL_PTR_NULL == pst_net_device)
    {
        OAM_ERROR_LOG0(pst_event->st_event_hdr.uc_vap_id, OAM_SF_ASSOC, "{wal_disconnet_sta_proc_ap::get net device ptr is null!}\r\n");
        return OAL_ERR_CODE_PTR_NULL;
    }
    ...
    if (OAL_SUCC != ul_ret)
    {
        OAM_ERROR_LOG1(pst_event->st_event_hdr.uc_vap_id, OAM_SF_ASSOC, "{wal_disconnet_sta_proc_ap::oal_cfg80211_del_sta fail[%d]!}\r\n", ul_ret);
        return ul_ret;
    }

    OAM_WARNING_LOG3(pst_event->st_event_hdr.uc_vap_id, OAM_SF_ASSOC, "{wal_disconnet_sta_proc_ap mac[%x %x %x] OK!}\r\n", auc_disconn_user_addr[3], auc_disconn_user_addr[4], auc_disconn_user_addr[5]);
...
oal_uint32  wal_send_mgmt_to_host(frw_event_mem_stru *pst_event_mem)
{
    ...
    if (OAL_SUCC != ul_ret)
    {
        OAM_WARNING_LOG1(pst_event->st_event_hdr.uc_vap_id, OAM_SF_ASSOC, "{wal_send_mgmt_to_host::oal_cfg80211_rx_mgmt fail[%d]!}\r\n", ul_ret);
        return OAL_FAIL;
    }
#if 0
    /* 统计上报结果 */
...
#endif
#endif
    OAM_INFO_LOG2(pst_event->st_event_hdr.uc_vap_id, OAM_SF_ANY, "{wal_send_mgmt_to_host::freq = %d, len = %d OK!}\r\n", l_freq, us_len);
...
oal_uint32 wal_p2p_listen_timeout(frw_event_mem_stru *pst_event_mem)
{
    ...
    oal_cfg80211_remain_on_channel_expired(pst_wdev,
					                    ...
					                    &st_listen_channel,
					                    GFP_ATOMIC);
    OAM_INFO_LOG1(0, OAM_SF_P2P, "{wal_p2p_listen_timeout::END!cookie %d}\r\n", ull_cookie);
#endif
