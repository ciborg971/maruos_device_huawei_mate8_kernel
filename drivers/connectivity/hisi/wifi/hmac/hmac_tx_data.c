        ...
        case OAL_HOST2NET_SHORT(ETHER_TYPE_IP):
            ...
            if (MAC_UDP_PROTOCAL == pst_ip->uc_protocol)
            {
                ...
                if (OAL_TRUE == mac_is_dhcp_port(pst_udp_hdr))
                {
                    ...
                    OAM_WARNING_LOG2(0, OAM_SF_TX, "{hmac_tx_classify_lan_to_wlan::dhcp frame!.}", pst_ip->uc_tos, uc_tid);
                }
            }
...
OAL_STATIC oal_bool_enum_uint8  hmac_tx_is_need_frag(hmac_vap_stru *pst_hmac_vap, hmac_user_stru *pst_hmac_user, oal_netbuf_stru *pst_netbuf, mac_tx_ctl_stru *pst_tx_ctl)
{
    ...
    /* 5、wai帧不进行分片       */
#ifdef _PRE_WLAN_FEATURE_WAPI
    if (MAC_DATA_WAPI == mac_get_data_type(pst_netbuf))
    {
        return OAL_FALSE;
    }
#endif
                ...
                oal_netbuf_stru  *pst_buf)
{
    ...
    mac_tx_ctl_stru                          *pst_tx_ctl;
    oal_uint16                                us_ether_type = oal_byteorder_host_to_net_uint16(ETHER_TYPE_IP);
...
OAL_INLINE oal_uint32  hmac_tx_lan_to_wlan_no_tcp_opt(mac_vap_stru *pst_vap, oal_netbuf_stru *pst_buf)
{
    ...
    */
    ...
    uc_data_type =   mac_get_eth_type((mac_ether_header_stru *)oal_netbuf_data(pst_buf), &us_len);
    ...
    {
        OAM_WARNING_LOG2(pst_vap->uc_vap_id, OAM_SF_WPA, "{hmac_tx_lan_to_wlan_no_tcp_opt::send datatype==%u, len==%u}[0:dhcp 1:arp 2:eapol]", uc_data_type, us_len);
    }
