...
OAL_STATIC hmac_rx_buf_stru* hmac_ba_buffer_frame_in_reorder(hmac_ba_rx_stru* pst_ba_rx_hdl, oal_uint16 us_seq_num)
{
    ...
    if (1 == pst_rx_buf->in_use)
    {
        ...
        OAM_WARNING_LOG1(0, OAM_SF_BA, "{hmac_ba_buffer_frame_in_reorder::slot already used, seq[%d].}", us_seq_num);
    }
    else
    {
...
oal_uint32  hmac_ba_reset_rx_handle(mac_device_stru *pst_mac_device, hmac_ba_rx_stru **ppst_rx_ba, oal_uint8 uc_tid, oal_bool_enum_uint8 en_is_aging)
{
    ...
    *ppst_rx_ba = OAL_PTR_NULL;
    ...
    OAM_INFO_LOG0(0, OAM_SF_BA, "{hmac_ba_reset_rx_handle.}");
...
oal_void  hmac_up_rx_bar(hmac_vap_stru *pst_hmac_vap, dmac_rx_ctl_stru *pst_rx_ctl, oal_netbuf_stru *pst_netbuf)
{
    ...
    /*  获取用户指针 */
    ul_ret = mac_vap_find_user_by_macaddr(&(pst_hmac_vap->st_vap_base_info), puc_sa_addr, &us_user_idx);
    if (OAL_SUCC != ul_ret)
    {
        OAM_WARNING_LOG1(0, OAM_SF_ANY, "{hmac_up_rx_bar::mac_vap_find_user_by_macaddr failed.}", ul_ret);
        return;
    }
    ...
    pst_ta_user     = (hmac_user_stru *)mac_res_get_hmac_user(us_user_idx);
