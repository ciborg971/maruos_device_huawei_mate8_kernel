...
oal_uint32 hmac_blacklist_del(mac_vap_stru *pst_mac_vap, oal_uint8 *puc_mac_addr)
{
    ...
    if (hmac_blacklist_mac_is_bcast(puc_mac_addr))
    {
        OAM_ERROR_LOG0(pst_mac_vap->uc_vap_id, OAM_SF_ANY,
            "{broadcast addr; delete all hmac_blacklist}");
...
{
    cs_blacklist_type_enum_uint8    en_type;
    ...
    /* 2.1 获取参数 */
    en_type      = pst_blacklist_cfg->uc_type;
    en_mode      = pst_blacklist_cfg->uc_mode;
    puc_mac_addr = pst_blacklist_cfg->auc_sa;
...
oal_uint32 hmac_autoblacklist_enable(mac_vap_stru *pst_mac_vap, oal_uint8 uc_enabled)
{
    ...
    {
        /*OAM_INFO_LOG1(pst_mac_vap->uc_vap_id, OAM_SF_ANY, "{autoblacklist is already %s}",(uc_enabled == 0) ? "Disabled" : "Enabled");*/
        /*  warning: passing argument 7 of 'oam_log_print1' makes integer from pointer without a cast */
        OAM_ERROR_LOG1(pst_mac_vap->uc_vap_id, OAM_SF_ANY, "{autoblacklist is already %d}",uc_enabled);
        return OAL_SUCC;
    }
    ...
    OAM_INFO_LOG1(pst_mac_vap->uc_vap_id, OAM_SF_ANY, "{autoblacklist is %d}",uc_enabled);
    return OAL_SUCC;
}
...
oal_bool_enum_uint8 hmac_blacklist_filter(mac_vap_stru *pst_mac_vap, oal_uint8 *puc_mac_addr)
{
    ...
    if (CS_BLACKLIST_MODE_BLACK == pst_blacklist_info->uc_mode)
    {
        ...
        if (OAL_SUCC != ul_ret)
        {
            OAM_ERROR_LOG1(pst_mac_vap->uc_vap_id, OAM_SF_ANY, "{hmac_blacklist_get failed %d}",ul_ret);
            return OAL_FALSE;
        }
        /* 2.2 如果找不到表单，不需要过滤 */
        if (OAL_PTR_NULL == pst_blacklist)
        ...
        pst_blacklist->ul_drop_counter++;
        OAM_INFO_LOG4(pst_mac_vap->uc_vap_id, OAM_SF_ANY,
            "{>>>>>> blacklist_filter drop_counter = %d MAC:=x.x.x.%02x.%02x.%02x}",
    ...
    if (CS_BLACKLIST_MODE_WHITE == pst_blacklist_info->uc_mode)
    {
        ...
        {
            OAM_INFO_LOG4(pst_mac_vap->uc_vap_id, OAM_SF_ANY,
                "{>>>>>> whitelist_filter MAC:=%02X:XX:XX:%02X:%02X:%02X}",
                puc_mac_addr[0],puc_mac_addr[3],puc_mac_addr[4],puc_mac_addr[5]);
            return OAL_TRUE;
        }
        ...
        {
            OAM_INFO_LOG4(pst_mac_vap->uc_vap_id, OAM_SF_ANY,
                "{>>>>>> aging reach. go through=%02X:XX:XX:%02X:%02X:%02X}",
                puc_mac_addr[0],puc_mac_addr[3],puc_mac_addr[4],puc_mac_addr[5]);
            return OAL_FALSE;
        }
        ...
        {
            OAM_INFO_LOG4(pst_mac_vap->uc_vap_id, OAM_SF_ANY,
                "{>>>>>> aging not zero; whilte_filter MAC=%02X:XX:XX:%02X:%02X:%02X}",
                puc_mac_addr[0],puc_mac_addr[3],puc_mac_addr[4],puc_mac_addr[5]);
            return OAL_TRUE;
        }
    }
