...
oal_void  hmac_fsm_change_state(hmac_vap_stru *pst_hmac_vap, mac_vap_state_enum_uint8 en_vap_state)
{
    ...
    /*将vap状态改变信息上报*/
    mac_vap_state_change_to_sdt(&pst_hmac_vap->st_vap_base_info, en_vap_state);

    pst_hmac_vap->st_vap_base_info.en_vap_state = en_vap_state;
...
OAL_STATIC oal_void hmac_fsm_init_sta(oal_void)
{
    ...
    */
	...
	g_pa_hmac_sta_fsm_func[MAC_VAP_STATE_P2P_LISTEN][HMAC_FSM_INPUT_SCAN_REQ]    = hmac_scan_proc_scan_req_event;
    ...
    */
    ...
    g_pa_hmac_sta_fsm_func[MAC_VAP_STATE_P2P_LISTEN][HMAC_FSM_INPUT_RX_MGMT]         = hmac_sta_not_up_rx_mgmt;
#endif
    ...
    */
    ...
    g_pa_hmac_sta_fsm_func[MAC_VAP_STATE_P2P_LISTEN][HMAC_FSM_INPUT_LISTEN_REQ]    = hmac_p2p_remain_on_channel;//在监听状态接收到新的监听命令，则执行新的监听。
    ...
    /* input column. This input is valid in MAC_VAP_STATE_P2P_LISTEN state.  */
    /*                                                                       */
    /* +----------------------------+------------------------+               */
    /* | FSM State                  | FSM Function           |               */
    /* +----------------------------+------------------------+               */
    /* | MAC_VAP_STATE_P2P_LISTEN   | hmac_p2p_listen_timeout|               */
    /* | All other states           | null_fn                |               */
    /* +----------------------------+------------------------+               */
    ...
    g_pa_hmac_sta_fsm_func[MAC_VAP_STATE_P2P_LISTEN][HMAC_FSM_INPUT_LISTEN_TIMEOUT] = hmac_p2p_listen_timeout;
    ...
    for (ul_state = 0; ul_state < MAC_VAP_STA_STATE_BUTT; ul_state++)
    {
      g_pa_hmac_sta_fsm_func[ul_state][HMAC_FSM_INPUT_ROAMING_STOP] = hmac_fsm_null_fn;
    }
    g_pa_hmac_sta_fsm_func[MAC_VAP_STATE_ROAMING][HMAC_FSM_INPUT_ROAMING_STOP]   = hmac_roam_resume_user;
#endif //_PRE_WLAN_FEATURE_ROAM
}
