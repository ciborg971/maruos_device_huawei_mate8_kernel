...
extern oal_uint32 hmac_cfg_vap_send_event(mac_device_stru *pst_device);
...
OAL_STATIC oal_uint32 hmac_cfg_vap_init(mac_device_stru *pst_device)
{
    ...
    {
        ul_ret = hmac_cfg_vap_send_event(pst_device);
        if (OAL_SUCC != ul_ret)
        {
            OAM_ERROR_LOG1(pst_device->uc_cfg_vap_id, OAM_SF_ANY, "{hmac_cfg_vap_init::hmac_cfg_vap_send_event failed[%d].}",ul_ret);
            return ul_ret;
        }
    }
#endif
...
void hmac_suspend_action(hmac_device_stru    *pst_hmac_device)
{
    ...
    /* 此处不需要指定vap,使用配置vap抛事件 */
    uc_cfg_vap_id   = pst_hmac_device->pst_device_base_info->uc_cfg_vap_id;
    pst_cfg_mac_vap = (mac_vap_stru *)mac_res_get_mac_vap(uc_cfg_vap_id);

	/* 开了host低功耗才需要将亮暗屏状态同步到device */
    if (g_wlan_pm_switch)
    {
        uc_in_suspend = pst_hmac_device->pst_device_base_info->uc_in_suspend;

        /***************************************************************************
            抛事件到DMAC层, 同步屏幕最新状态到DMAC
        ***************************************************************************/
        ul_ret = hmac_config_send_event(pst_cfg_mac_vap, WLAN_CFGID_SUSPEND_ACTION_SYN, OAL_SIZEOF(oal_uint8), (oal_uint8 *)&uc_in_suspend);
        if (OAL_UNLIKELY(OAL_SUCC != ul_ret))
        {
    ...
    {
        OAM_INFO_LOG0(0,OAM_SF_PWR,"wlan powermgmt is off now ...");
    }
}
...
oal_void  hmac_do_suspend_action(hmac_device_stru    *pst_hmac_device, oal_uint8  uc_in_suspend)
{
    ...
    hmac_suspend_action(pst_hmac_device);
