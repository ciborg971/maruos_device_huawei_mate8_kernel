...
OAL_STATIC  oal_int32 hi1102_cali_div_round_closest(oal_int32 l_a, oal_int32 l_b)
{
    ...
    return (oal_int32)(c_flag * (((l_a << 1) + l_b) / (l_b << 1)));
}
...
oal_void hi1102_rf_cali_dpd_corr_calc(oal_uint32* hi1102_dpd_cali_data_read, oal_uint32* hi1102_dpd_cali_data_calc)
{
  ...
  // hi1102_complex_stru  DPD_Theta[DPD_CALI_LUT_LENGTH] = {0};
   ...
   for(idx = 0; idx <DPD_CALI_LUT_LENGTH ; idx++)
   {
     ...
     LUT_Gain[idx] = (oal_uint32)hi1102_cali_div_round_closest( (l_amp << 10) ,(DPD_CALI_AMP_REF_RMS * (idx+ 1)));
   }
   ...
   for(idx = 0; idx < DPD_CALI_LUT_LENGTH; idx++ )
   {
     LUT_Gain[idx] = (oal_uint32)hi1102_cali_div_round_closest(pa_gain_target<<9, LUT_Gain[idx]);
   }
