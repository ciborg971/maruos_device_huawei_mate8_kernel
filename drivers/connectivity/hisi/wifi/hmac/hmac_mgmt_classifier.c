...
oal_uint32  hmac_mgmt_rx_delba_event(frw_event_mem_stru *pst_event_mem)
{
    ...
    if (OAL_SUCC != ul_rslt)
    {
        OAM_WARNING_LOG1(pst_event_hdr->uc_vap_id, OAM_SF_BA, "{hmac_mgmt_rx_delba_event::mac_vap_find_user_by_macaddr failed[%d].}", ul_rslt);
        return OAL_FAIL;
    }

    pst_hmac_user = (hmac_user_stru *)mac_res_get_hmac_user(us_user_dix);
...
oal_uint32  hmac_rx_process_mgmt_event(frw_event_mem_stru *pst_event_mem)
{
    ...
    {
        hmac_fsm_call_func_ap(pst_vap, HMAC_FSM_INPUT_RX_MGMT, pst_crx_event);
    }
    else if (WLAN_VAP_MODE_BSS_STA == pst_vap->st_vap_base_info.en_vap_mode)
    {
        hmac_fsm_call_func_sta(pst_vap, HMAC_FSM_INPUT_RX_MGMT, pst_crx_event);
    }
...
oal_uint32  hmac_mgmt_tbtt_event(frw_event_mem_stru *pst_event_mem)
{
    ...
    hmac_fsm_call_func_sta(pst_hmac_vap, HMAC_FSM_INPUT_MISC, &st_misc_input);
...
oal_uint32  hmac_proc_disasoc_misc_event(frw_event_mem_stru *pst_event_mem)
{
    ...
    OAM_WARNING_LOG0(0, OAM_SF_ASSOC, "{hmac_proc_disasoc_misc_event::enter.}");
