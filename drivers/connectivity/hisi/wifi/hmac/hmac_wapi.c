...
oal_uint32 hmac_wapi_calc_mic_data(mac_ieee80211_frame_stru *pst_mac_hdr, oal_uint8 uc_mac_hdr_len, oal_uint8 uc_keyidx, oal_uint8 *puc_payload, oal_uint16 us_pdu_len, oal_uint8 *puc_mic, oal_uint16 us_mic_len)
{
    ...
    /* frame control */
    oal_memcopy(puc_mic, &(pst_mac_hdr->st_frame_control), OAL_SIZEOF(pst_mac_hdr->st_frame_control));
...
{
    return (0 == (*puc_pn & BIT0))?OAL_FALSE:OAL_TRUE;
}
...
//#endif /* WAPI_DEBUG_MODE */
    ...
    pst_netbuff_des = OAL_MEM_NETBUF_ALLOC(OAL_NORMAL_NETBUF, WLAN_MEM_NETBUF_SIZE2, OAL_TRUE);
