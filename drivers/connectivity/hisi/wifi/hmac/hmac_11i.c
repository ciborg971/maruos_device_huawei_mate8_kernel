                                        ...
                                        oal_bool_enum_uint8 en_pairwise,oal_uint8 uc_key_index)
{
    ...
    if (OAL_PTR_NULL != puc_mac_addr)
    {
        OAM_INFO_LOG4(pst_mac_vap->uc_vap_id, OAM_SF_WPA,
                      "{hmac_get_key_info::puc_mac_addr null, mac addr=%02X:XX:XX:%02X:%02X:%02X.}",
...
oal_uint32 hmac_config_wapi_add_key_and_sync(mac_vap_stru *pst_mac_vap, mac_addkey_param_stru *pst_payload_addkey_params)
{
    ...
    OAM_WARNING_LOG0(0, OAM_SF_WPA,"{hmac_config_wapi_add_key_and_sync:: wapi_is_work}");
...
oal_uint32 hmac_config_11i_add_key(mac_vap_stru *pst_mac_vap, oal_uint16 us_len, oal_uint8 *puc_param)
{
    ...
    if (OAL_PTR_NULL == puc_mac_addr)
    {
        OAM_ERROR_LOG0(pst_mac_vap->uc_vap_id, OAM_SF_WPA, "{hmac_config_11i_add_key::get_mac_user null.}");
...
#ifdef _PRE_WLAN_FEATURE_ROAM
    if ((WLAN_VAP_MODE_BSS_STA == pst_mac_vap->en_vap_mode) && (OAL_FALSE == en_pairwise))
    {
        ...
        else
        {
            ...
            if (OAL_PTR_NULL == pst_mac_user)
            {
                OAM_INFO_LOG1(pst_mac_vap->uc_vap_id, OAM_SF_WPA,
                              "{hmac_config_11i_remove_key::multi_user[%d] null.}", pst_mac_vap->us_multi_user_idx);
                return OAL_ERR_CODE_SECURITY_USER_INVAILD;
            }
        }
...
oal_uint8 hmac_get_pcip_policy_auth(oal_uint8 *puc_frame, oal_uint8 *puc_len, oal_uint8 *puc_oui)
{
    ...
    *puc_len = (oal_uint8)(puc_frame[0] * 4)+ 2;
    ...
    else
    {
        ...
        /* 比较单播套件OUI */
        if (0 != oal_memcmp(puc_frame + uc_indext, puc_oui, MAC_OUI_LEN))
        {
            uc_pcip_policy = 0xFF;
        }
...
oal_uint8 hmac_get_auth_policy_auth(oal_uint8 * puc_frame, oal_uint8 *uc_len, oal_uint8 *puc_oui)
{
    ...
    *uc_len = (oal_uint8)(puc_frame[0] * 4) + 2;
    ...
    else
    {
        ...
        /* 检测AKM OUI 数据 */
        if (0 != oal_memcmp(puc_frame + uc_index, puc_oui, MAC_OUI_LEN))
        {
            uc_auth_policy = 0xFF;
        }
                                                               ...
                                                               mac_bss_dscr_stru  *pst_bss_dscr)
{
    ...
    ul_ret = mac_vap_check_bss_cap_info_phy_ap(pst_bss_dscr->us_cap_info, pst_mac_vap);
    if (ul_ret != OAL_TRUE)
    {
        OAM_WARNING_LOG1(pst_mac_vap->uc_vap_id, OAM_SF_WPA,
                         "{hmac_check_capability_mac_phy_supplicant::mac_vap_check_bss_cap_info_phy_ap failed[%d].}", ul_ret);
        return OAL_ERR_CODE_SECURITY_CAP_PHY;
    }
...
oal_uint32 hmac_rx_tkip_mic_failure_process(frw_event_mem_stru *pst_event_mem)
{
    ...
    /* 去关联的STA mac地址 */
    oal_memcopy((oal_uint8 *)pst_event->auc_event_data, pst_mic_event, sizeof(dmac_to_hmac_mic_event_stru));
...
oal_uint32 hmac_11i_ether_type_filter(hmac_vap_stru *pst_vap, oal_uint8 *puc_mac_addr, oal_uint16 us_ether_type)
{
    ...
    {
        ul_ret = mac_vap_find_user_by_macaddr(&(pst_vap->st_vap_base_info), puc_mac_addr, &us_user_idx);
        if (OAL_SUCC != ul_ret)
        {
            OAM_WARNING_LOG4(pst_mac_vap->uc_vap_id, OAM_SF_WPA,
                             "{hmac_11i_ether_type_filter::mac addr=%02X:XX:XX:%02X:%02X:%02X.}",
                             puc_mac_addr[0], puc_mac_addr[3], puc_mac_addr[4], puc_mac_addr[5]);
            return OAL_ERR_CODE_SECURITY_USER_INVAILD;
        }
        ...
        if (OAL_PTR_NULL == pst_mac_user)
        {
            OAM_ERROR_LOG0(pst_mac_vap->uc_vap_id, OAM_SF_WPA, "{hmac_11i_ether_type_filter::pst_mac_user null.}");
            return OAL_ERR_CODE_SECURITY_USER_INVAILD;
        }
