...
OAL_STATIC OAL_INLINE oal_uint32 hmac_protection_set_lsig_txop_mechanism(mac_vap_stru *pst_mac_vap, oal_switch_enum_uint8 en_flag)
{
...
OAL_STATIC oal_bool_enum hmac_protection_lsigtxop_check(mac_vap_stru *pst_mac_vap)
{
    ...
    {
        pst_mac_user = mac_res_get_mac_user(pst_mac_vap->uc_assoc_vap_id); /*user保存的是AP的信息*/
...
OAL_STATIC oal_uint32 hmac_protection_update_ht_protection(mac_vap_stru *pst_mac_vap)
{
    ...
    if (OAL_TRUE == en_lsigtxop)
    {
        ...
        if (OAL_SWITCH_ON == pst_mac_vap->st_protection.bit_rts_cts_protect_mode)
        {
            ...
            HMAC_CHECK_RET(ul_ret);
            ul_ret = hmac_protection_set_lsig_txop_mechanism(pst_mac_vap, OAL_SWITCH_ON);
            HMAC_CHECK_RET(ul_ret);
        }
    }
    else/*其余情况需要设置ht保护方式为rts cts protection 机制*/
    {
        ...
        {
            ul_ret = hmac_protection_set_lsig_txop_mechanism(pst_mac_vap, OAL_SWITCH_OFF);
            HMAC_CHECK_RET(ul_ret);
...
OAL_STATIC oal_uint32 hmac_protection_set_ht_protection(mac_vap_stru *pst_mac_vap, oal_switch_enum_uint8 en_flag)
{
    ...
    if (OAL_TRUE == en_lsigtxop)
    {
        ul_ret = hmac_protection_set_lsig_txop_mechanism(pst_mac_vap, en_flag);
        HMAC_CHECK_RET(ul_ret);
    }
    else
    {
...
OAL_STATIC oal_uint32  hmac_protection_del_user_stat_ap(mac_vap_stru *pst_mac_vap, mac_user_stru *pst_mac_user)
{
...
oal_uint32 hmac_protection_del_user(mac_vap_stru *pst_mac_vap, mac_user_stru *pst_mac_user)
{
    ...
    {
        /*删除保护模式相关user统计*/
        ul_ret = hmac_protection_del_user_stat_ap(pst_mac_vap, pst_mac_user);
        HMAC_CHECK_RET(ul_ret);
