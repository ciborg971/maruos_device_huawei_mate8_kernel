...

{
    ...
    FRW_EVENT_HDR_INIT(&(pst_event->st_event_hdr),
                       ...
                       HMAC_HOST_CTX_EVENT_SUB_TYPE_STA_DISCONNET_AP,
                       WLAN_MAC_ADDR_LEN,
...
OAL_STATIC oal_void  hmac_ap_rx_auth_req(hmac_vap_stru *pst_hmac_vap, oal_netbuf_stru *pst_auth_req, dmac_wlan_crx_event_stru   *pst_mgmt_rx_event)
{
    ...
    pst_auth_rsp = (oal_netbuf_stru *)OAL_MEM_NETBUF_ALLOC(OAL_NORMAL_NETBUF, WLAN_MEM_NETBUF_SIZE2, OAL_TRUE);
...
OAL_STATIC oal_uint32 hmac_ap_rx_deauth_req(hmac_vap_stru *pst_hmac_vap, oal_uint8 *puc_mac_hdr, oal_bool_enum_uint8 en_is_protected)
{
    ...
    OAM_WARNING_LOG4(0, OAM_SF_AUTH,
                     "{hmac_ap_rx_deauth_req::Because of err_code[%d], received deauth frame frome source addr, sa[0-2] %2x:%2x:%2x.}",
                     us_err_code, puc_sa[0], puc_sa[1], puc_sa[2]);
    OAM_WARNING_LOG3(0, OAM_SF_AUTH,
                     "{hmac_ap_rx_deauth_req::sa[3-5] %2x:%2x:%2x.}",
                     puc_sa[3], puc_sa[4], puc_sa[5]);

    ul_ret = mac_vap_find_user_by_macaddr(&pst_hmac_vap->st_vap_base_info, puc_sa, &us_user_idx);
    if (OAL_SUCC != ul_ret)
    {
        OAM_WARNING_LOG1(pst_hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_AUTH, "{hmac_ap_rx_deauth_req::mac_vap_find_user_by_macaddr failed[%d].}", ul_ret);
        /* 没有查到对应的USER,发送去认证消息 */
        if (us_err_code != MAC_DEAUTH_LV_SS)
        {
            hmac_mgmt_send_deauth_frame(&(pst_hmac_vap->st_vap_base_info), puc_sa, MAC_NOT_AUTHED, OAL_FALSE);
            return ul_ret;
        }
        else
        {
    ...
    {
        OAM_ERROR_LOG1(pst_hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_AUTH, "{hmac_ap_rx_deauth_req::pst_hmac_user null, us_user_idx=%d.}", us_user_idx);
        /* 没有查到对应的USER,发送去认证消息 */
        hmac_mgmt_send_deauth_frame(&(pst_hmac_vap->st_vap_base_info), puc_sa, MAC_NOT_AUTHED, OAL_FALSE);
        return OAL_FAIL;
    }
    OAM_INFO_LOG0(pst_hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_AUTH, "{hmac_ap_rx_deauth_req::ap rx a deauth req.}");
#if (_PRE_WLAN_FEATURE_PMF != _PRE_PMF_NOT_SUPPORT)
                ...
                mac_status_code_enum_uint16    *pen_status_code)
{
    ...
    ul_ret = (oal_uint32)mac_vap_check_bss_cap_info_phy_ap(us_cap_info, pst_mac_vap);
                ...
                mac_status_code_enum_uint16    *pen_status_code)
{
    ...
    ***************************************************************************/
    ...
    {
        OAM_WARNING_LOG1(pst_mac_vap->uc_vap_id, OAM_SF_ASSOC,
                         "{hmac_ap_up_update_sta_user::AP refuse STA assoc, update support rates failed, status_code=%d.}", *pen_status_code);
        return *pen_status_code;
    }
...
#ifdef _PRE_WLAN_FEATURE_11AC2G
    /* 更新11ac2g能力 */
    puc_vendor_ie = mac_get_vendor_ie((oal_uint8*)puc_payload, (oal_uint16)ul_msg_len, MAC_LISTEN_INT_LEN + MAC_CAP_INFO_LEN);
    ...
    {
        OAM_ERROR_LOG1(pst_mac_user->uc_vap_id, OAM_SF_ASSOC,
                       "{hmac_ap_up_update_sta_user::usr_info_syn failed[%d].}", ul_rslt);
    }
                ...
                oal_uint32                      ul_payload_len)
{
    ...
    if (OAL_SUCC != ul_rslt)
    {
        ...
        OAM_WARNING_LOG4(pst_hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_ASSOC,
                         "{hmac_ap_up_rx_asoc_req::user mac[0,3,4,5]:%.2x:%.2x:%.2x:%.2x.}",
    ...
    if (MAC_REJECT_TEMP != en_status_code)
    {
 ...
 #if defined (_PRE_WLAN_FEATURE_WPA) || defined(_PRE_WLAN_FEATURE_WPA2)
        /* 如果AP 正在处理 mic faile， 则不能关联 */
        if (OAL_TRUE == pst_hmac_vap->en_mic_faile_in_progress)
        {
            OAM_ERROR_LOG1(pst_hmac_user->st_user_base_info.uc_vap_id, OAM_SF_ASSOC,
                          "{hmac_ap_up_rx_asoc_req::MIC faile in progress.useR_id=%d].}", pst_hmac_user->st_user_base_info.us_assoc_id);
            en_status_code = MAC_UNSPEC_FAIL;
            OAM_INFO_LOG1(pst_hmac_user->st_user_base_info.uc_vap_id, OAM_SF_ASSOC,
                          "{hmac_ap_up_rx_asoc_req::AP refuse STA assoc,en_status_code=%d.}", en_status_code);
        }
 #endif
        ...
        if (IS_P2P_GO(&pst_hmac_vap->st_vap_base_info) &&
            ...
            (OAL_PTR_NULL == mac_find_vendor_ie(MAC_WLAN_OUI_WFA, MAC_WLAN_OUI_TYPE_WFA_P2P, puc_payload + MAC_CAP_INFO_LEN + MAC_LISTEN_INT_LEN, (oal_int32)(ul_payload_len - (MAC_CAP_INFO_LEN + MAC_LISTEN_INT_LEN)))))
        {
            OAM_INFO_LOG1(pst_hmac_user->st_user_base_info.uc_vap_id, OAM_SF_ASSOC,
    ...
    pst_asoc_rsp = (oal_netbuf_stru *)OAL_MEM_NETBUF_ALLOC(OAL_NORMAL_NETBUF, WLAN_MEM_NETBUF_SIZE2, OAL_TRUE);
                ...
                oal_bool_enum_uint8             en_is_protected)
{
    ...
    OAM_WARNING_LOG4(0, OAM_SF_ASSOC,
                     "{hmac_ap_up_rx_disasoc::Because of err_code[%d], received disassoc frame frome source addr, sa[0-2] %2x:%2x:%2x.}",
                     *((oal_uint16 *)(puc_mac_hdr + MAC_80211_FRAME_LEN)), puc_sa[0], puc_sa[1], puc_sa[2]);
    OAM_WARNING_LOG3(0, OAM_SF_ASSOC, "{hmac_ap_up_rx_disasoc::sa[3-5] %2x:%2x:%2x.}",
                     puc_sa[3], puc_sa[4], puc_sa[5]);
    ...
    {
        OAM_WARNING_LOG1(pst_hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_ASSOC,
                         "{hmac_ap_up_rx_disasoc::mac_vap_find_user_by_macaddr failed[%d].}", ul_rslt);
        /* 没有查到对应的USER,发送去认证消息 */
...
OAL_STATIC oal_void  hmac_ap_up_rx_2040_coext(hmac_vap_stru *pst_hmac_vap, oal_netbuf_stru *pst_netbuf)
{
        ...
        (OAL_FALSE    == mac_mib_get_2040BSSCoexistenceManagementSupport(pst_mac_vap)))
    {
        OAM_INFO_LOG0(pst_hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_2040,
                         "{hmac_ap_up_rx_2040_coext::hmac_user_del failed.}");
        return;
    }
...
OAL_STATIC oal_void  hmac_ap_up_rx_smps_frame(hmac_vap_stru *pst_hmac_vap, oal_netbuf_stru *pst_netbuf)
{
    ...
    if (OAL_SUCC != ul_ret)
    {
        OAM_WARNING_LOG1(pst_hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_SMPS,
                      "{hmac_ap_up_rx_smps_frame::mac_vap_find_user_by_macaddr failed[%d].}", ul_ret);
        return;
    }
    ...
    {
        OAM_ERROR_LOG0(pst_hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_SMPS, "{hmac_ap_up_rx_smps_frame::pst_mac_user null.}");
        return;
    }
...
OAL_STATIC oal_void  hmac_ap_up_rx_action(hmac_vap_stru *pst_hmac_vap, oal_netbuf_stru *pst_netbuf, oal_bool_enum_uint8 en_is_protected)
{
    ...
    if (OAL_SUCC != ul_rslt)
    {
        OAM_WARNING_LOG3(pst_hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_RX, "{hmac_ap_up_rx_action::mac_vap_find_user_by_macaddr failed[%d],bit_type = %d,bit_sub_type = %d.}",
                         ul_rslt,pst_frame_hdr->st_frame_control.bit_type,pst_frame_hdr->st_frame_control.bit_sub_type);
        OAM_WARNING_LOG3(pst_hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_RX, "{hmac_ap_up_rx_action::auc_address:[0-2]%.2x:%.2x:%.2x.}",
                         pst_frame_hdr->auc_address2[0],
                         pst_frame_hdr->auc_address2[1],
                         pst_frame_hdr->auc_address2[2]);
        OAM_WARNING_LOG3(pst_hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_RX, "{hmac_ap_up_rx_action::[3-5]%.2x:%.2x:%.2x.}",
...
OAL_STATIC oal_void  hmac_ap_store_network(mac_device_stru *pst_mac_device, oal_netbuf_stru *pst_netbuf)
{
    ...
    dmac_rx_ctl_stru       *pst_rx_ctrl = (dmac_rx_ctl_stru *)oal_netbuf_cb(pst_netbuf);
    oal_uint8               auc_bcast_bssid[WLAN_MAC_ADDR_LEN]   = {0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF};
    ...
    /* 忽略广播BSSID */
    if (0 == oal_compare_mac_addr(auc_bcast_bssid, auc_network_bssid))
    {
        return;
    }
    ...
    {
        oal_set_mac_addr(pst_bss_id_list->auc_bssid_array[uc_loop], auc_network_bssid);
