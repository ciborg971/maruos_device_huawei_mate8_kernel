...
OAL_STATIC oal_void hmac_update_join_req_params_prot_sta(hmac_vap_stru * pst_hmac_vap, hmac_join_req_stru * pst_join_req)
{
    if (WLAN_MIB_DESIRED_BSSTYPE_INFRA  == pst_hmac_vap->st_vap_base_info.pst_mib_info->st_wlan_mib_sta_config.en_dot11DesiredBSSType)
    {
        ...
        if ((OAL_TRUE == pst_join_req->st_bss_dscr.uc_wmm_cap) && (OAL_TRUE == pst_hmac_vap->st_vap_base_info.pst_mib_info->st_wlan_mib_vht_txbf_config.en_dot11VHTSUBeamformeeOptionImplemented))
        {
            pst_hmac_vap->st_vap_base_info.pst_mib_info->st_wlan_mib_txbf_config.en_dot11ReceiveStaggerSoundingOptionImplemented = OAL_TRUE;
            pst_hmac_vap->st_vap_base_info.pst_mib_info->st_wlan_mib_txbf_config.uc_dot11ExplicitCompressedBeamformingFeedbackOptionImplemented = 1;
            pst_hmac_vap->st_vap_base_info.pst_mib_info->st_wlan_mib_txbf_config.ul_dot11NumberCompressedBeamformingMatrixSupportAntenna        = 1;
        }
        else
        {
            pst_hmac_vap->st_vap_base_info.pst_mib_info->st_wlan_mib_txbf_config.en_dot11ReceiveStaggerSoundingOptionImplemented = OAL_FALSE;
            pst_hmac_vap->st_vap_base_info.pst_mib_info->st_wlan_mib_txbf_config.uc_dot11ExplicitCompressedBeamformingFeedbackOptionImplemented = 0;
            pst_hmac_vap->st_vap_base_info.pst_mib_info->st_wlan_mib_txbf_config.ul_dot11NumberCompressedBeamformingMatrixSupportAntenna        = 0;
        }
...
oal_uint32  hmac_sta_wait_join(hmac_vap_stru *pst_hmac_sta, oal_void *pst_msg)
{
    ...
    {
        OAM_ERROR_LOG0(pst_hmac_sta->st_vap_base_info.uc_vap_id, OAM_SF_ASSOC, "{hmac_sta_wait_join::pst_event_mem null.}");
        return OAL_ERR_CODE_PTR_NULL;
    }
    ...
    switch (mac_get_frame_sub_type(puc_mac_hdr))
    {
        case WLAN_FC0_SUBTYPE_BEACON:
        {
            ...
            if (0 == oal_memcmp(auc_bssid, pst_hmac_sta->st_vap_base_info.auc_bssid, WLAN_MAC_ADDR_LEN))
            {
                ...
                /* 拷贝参数 */
                oal_memcopy(pst_event->auc_event_data, (oal_void *)&st_set_dtim_tsf_reg_params, OAL_SIZEOF(dmac_ctx_set_dtim_tsf_reg_stru));
...
oal_uint32  hmac_sta_wait_auth(hmac_vap_stru *pst_hmac_sta, oal_void *pst_msg)
{
    ...
    /* 申请认证帧空间 */
    pst_auth_frame = OAL_MEM_NETBUF_ALLOC(OAL_NORMAL_NETBUF, WLAN_MEM_NETBUF_SIZE2, OAL_TRUE);
...
oal_uint32  hmac_sta_wait_asoc(hmac_vap_stru *pst_sta, oal_void *pst_msg)
{
    ...
    pst_asoc_req_frame = OAL_MEM_NETBUF_ALLOC(OAL_NORMAL_NETBUF, WLAN_MEM_NETBUF_SIZE2, OAL_TRUE);
    ...
    if (OAL_PTR_NULL == pst_sta->puc_asoc_req_ie_buff)
    {
        if(OAL_UNLIKELY(ul_asoc_frame_len < OAL_ASSOC_REQ_IE_OFFSET))
        {
            ...
            oam_report_dft_params(auc_user_macaddr, (oal_uint8 *)oal_netbuf_header(pst_asoc_req_frame),(oal_uint16)ul_asoc_frame_len, OAM_OTA_TYPE_80211_FRAME);
        }
        else
        {
            /*Should we change the ie buff from local mem to netbuf ?*/
            /* 此处申请的内存，只在上报给内核后释放 */
            pst_sta->puc_asoc_req_ie_buff = OAL_MEM_ALLOC(OAL_MEM_POOL_ID_LOCAL, (oal_uint16)(ul_asoc_frame_len - OAL_ASSOC_REQ_IE_OFFSET), OAL_TRUE);
            if (OAL_PTR_NULL == pst_sta->puc_asoc_req_ie_buff)
            {
                OAM_ERROR_LOG1(pst_sta->st_vap_base_info.uc_vap_id, OAM_SF_ASSOC, "{hmac_sta_wait_asoc::puc_asoc_req_ie_buff null,alloc %u bytes failed}",
								(oal_uint16)(ul_asoc_frame_len - OAL_ASSOC_REQ_IE_OFFSET));
                oal_netbuf_free(pst_asoc_req_frame);
                return OAL_FAIL;
            }
            pst_sta->ul_asoc_req_ie_len = ul_asoc_frame_len - OAL_ASSOC_REQ_IE_OFFSET;
...
oal_void hmac_p2p_listen_comp_cb(void *p_arg)
{
    ...
    hmac_fsm_call_func_sta(pst_hmac_vap, HMAC_FSM_INPUT_LISTEN_TIMEOUT, &(pst_hmac_vap->st_vap_base_info));
...
OAL_STATIC oal_void  hmac_cfg80211_prepare_listen_req_param(mac_scan_req_stru *pst_scan_params, oal_int8 *puc_param)
{
    ...
    mac_get_channel_idx_from_num(pst_channel_tmp->en_band, pst_channel_tmp->uc_chan_number, &(pst_channel_tmp->uc_idx));
...
oal_uint32  hmac_p2p_listen_timeout(hmac_vap_stru *pst_hmac_vap_sta, oal_void *p_param)
{
    ...
    mac_vap_state_change_to_sdt(&pst_hmac_vap->st_vap_base_info, pst_mac_device->st_p2p_info.en_last_vap_state);
    pst_hmac_vap->st_vap_base_info.en_vap_state = pst_mac_device->st_p2p_info.en_last_vap_state;
...
oal_uint32 hmac_p2p_remain_on_channel(hmac_vap_stru *pst_hmac_vap_sta, oal_void *p_param)
{
    ...
    if (MAC_VAP_STATE_P2P_LISTEN == pst_hmac_vap_sta->st_vap_base_info.en_vap_state)
    {
    ...
    mac_vap_state_change_to_sdt(&pst_hmac_vap_sta->st_vap_base_info, MAC_VAP_STATE_P2P_LISTEN);
    pst_hmac_vap_sta->st_vap_base_info.en_vap_state = MAC_VAP_STATE_P2P_LISTEN;
    hmac_set_rx_filter_value(&pst_hmac_vap_sta->st_vap_base_info);
    ...
    /* 调用扫描入口，准备进行监听动作 */
    hmac_scan_proc_scan_req_event(pst_hmac_vap_sta, (oal_void *)&st_scan_params);
                      ...
                      mac_wmm_set_param_type_enum_uint8      en_type)
{
    ...
    if (OAL_PTR_NULL == pst_event_mem)
    {
        OAM_ERROR_LOG0(pst_hmac_sta->st_vap_base_info.uc_vap_id, OAM_SF_ASSOC, "{hmac_update_vht_opern_ie_sta::pst_event_mem null.}");
        return OAL_ERR_CODE_PTR_NULL;
    }
    ...
    /* 拷贝参数 */
    oal_memcopy(pst_event->auc_event_data, (oal_void *)&st_asoc_set_edca_reg_param, OAL_SIZEOF(dmac_ctx_sta_asoc_set_edca_reg_stru));
...
/* #ifdef _PRE_WLAN_FEATURE_20_40_80_COEXIST */
    ...
    if (0 == pst_mac_user->en_avail_bandwidth)
    {
        st_ht_hdl.bit_secondary_chan_offset = MAC_SCN;
    }

    ul_change = hmac_ie_proc_sec_chan_offset_2040(pst_mac_vap, st_ht_hdl.bit_secondary_chan_offset);
...
/*    us_cap_info     = puc_payload[us_offset] | (puc_payload[(us_offset + 1)] << 8);
    ...
    if (OAL_PTR_NULL == pst_mac_device)
    {
        OAM_ERROR_LOG0(pst_hmac_sta->st_vap_base_info.uc_vap_id, OAM_SF_ASSOC,
                        "{hmac_sta_wait_asoc_rx::pst_mac_device null.}");
        return OAL_ERR_CODE_PTR_NULL;
    }
...
/* TBD 1102临时注释STA TXOPPS处理,后续调试再打开*/
    ...
    /* 如果STA 正在处理 mic faile， 则不能关联 */
    if (OAL_TRUE == pst_hmac_sta->en_mic_faile_in_progress)
    {
        OAM_ERROR_LOG1(pst_hmac_sta->st_vap_base_info.uc_vap_id, OAM_SF_ASSOC,
                        "{hmac_sta_wait_asoc_rx::MIC faile, user id=%d.}", pst_hmac_user_ap->st_user_base_info.us_assoc_id);
        st_asoc_rsp.en_result_code = HMAC_MGMT_INVALID; /* MIC faile  */
    ...
    if(OAL_TRUE == pst_mac_user_ap->st_ht_hdl.en_ht_capable)
    {
        /* 支持HT, 默认初始化 */
        hmac_amsdu_init_user(pst_hmac_user_ap);
    }
...
#ifdef _PRE_WLAN_FEATURE_11AC2G
    /* 更新11ac2g能力 */
    puc_vendor_ie = mac_get_vendor_ie((oal_uint8*)puc_payload, us_msg_len, us_offset);
    hmac_mgmt_update_11ac2g_cap(puc_vendor_ie, pst_hmac_user_ap);
#endif
    ...
    OAM_WARNING_LOG3(pst_hmac_sta->st_vap_base_info.uc_vap_id, OAM_SF_ASSOC,
                       "{hmac_sta_wait_asoc_rx::uc_chan_number:%d,en_band:%d,en_bandwidth:%d.}",
                       pst_mac_vap->st_channel.uc_chan_number,
...
//    pst_mac_vap->en_protocol = pst_hmac_user_ap->st_user_base_info.en_protocol_mode;
    ...
    OAM_WARNING_LOG2(pst_hmac_sta->st_vap_base_info.uc_vap_id, OAM_SF_ASSOC,
                       "{hmac_sta_wait_asoc_rx::mac user en_bandwidth_cap:%d,en_avail_bandwidth:%d}",
                       en_bandwidth_cap,
...
OAL_STATIC oal_uint32 hmac_sta_update_join_req_params(hmac_vap_stru *pst_hmac_vap,hmac_join_req_stru *pst_join_req)
{
    ...
    if (OAL_PTR_NULL == pst_mac_device)
    {
        OAM_ERROR_LOG0(pst_hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_ASSOC,
                       "{hmac_sta_update_join_req_params::get mac_device_stru null.}");
        return OAL_ERR_CODE_PTR_NULL;
    }
...
OAL_STATIC oal_uint32 hmac_sta_rx_deauth_req(hmac_vap_stru *pst_hmac_vap, oal_uint8 *puc_mac_hdr, oal_bool_enum_uint8 en_is_protected)
{
    ...
    OAM_WARNING_LOG4(0, OAM_SF_AUTH,
                     "{hmac_sta_rx_deauth_req::Because of err_code[%d], received deauth or disassoc frame frome source addr, sa[0-2] %2x:%2x:%2x.}",
                     *((oal_uint16 *)(puc_mac_hdr + MAC_80211_FRAME_LEN)), puc_sa[0], puc_sa[1], puc_sa[2]);
    OAM_WARNING_LOG3(0, OAM_SF_AUTH,
                     "{hmac_sta_rx_deauth_req::sa[3-5] %2x:%2x:%2x.}",
                     puc_sa[3], puc_sa[4], puc_sa[5]);
...
OAL_STATIC oal_uint32  hmac_sta_up_rx_beacon(hmac_vap_stru *pst_hmac_vap_sta, oal_netbuf_stru *pst_netbuf)
{
    ...
    puc_tim_elm = mac_find_ie(MAC_EID_TIM, (oal_uint8 *)pst_mac_hdr + MAC_TAG_PARAM_OFFSET, us_frame_len - MAC_TAG_PARAM_OFFSET);
    if (OAL_PTR_NULL != puc_tim_elm)
    {
...
#ifdef _PRE_WLAN_FEATURE_11AC2G
    /* 更新11ac2g能力 */
    puc_vendor_ie = mac_get_vendor_ie((oal_uint8*)pst_mac_hdr, us_frame_len, us_frame_offset);
    hmac_mgmt_update_11ac2g_cap(puc_vendor_ie, pst_hmac_user);
#endif
    ...
    //puc_dsss_ie = mac_get_dsss_param_ie(puc_frame_body, us_frame_len, us_frame_offset);
    puc_dsss_ie = mac_find_ie(MAC_EID_DSPARMS, puc_frame_body + us_frame_offset, us_frame_len - us_frame_offset);
    if (OAL_PTR_NULL != puc_dsss_ie)
    {
        uc_channel = puc_dsss_ie[MAC_IE_HDR_LEN];
        if (uc_channel != pst_hmac_vap_sta->st_vap_base_info.st_channel.uc_chan_number)
        {
            OAM_WARNING_LOG2(pst_hmac_vap_sta->st_vap_base_info.uc_vap_id, OAM_SF_ANY,
                            "hmac_sta_up_rx_beacon::CHANNEL CHANGED!old_channel [%d], new_channel [%d]",
                            pst_hmac_vap_sta->st_vap_base_info.st_channel.uc_chan_number,
                            uc_channel);

            /* 发送去认证到ap */
            hmac_mgmt_send_deauth_frame(&pst_hmac_vap_sta->st_vap_base_info,
                                        auc_addr_sa,
                                        MAC_UNSPEC_REASON,
                                        pst_hmac_user->st_user_base_info.st_cap_info.bit_pmf_active);

            /* 删除用户 */
            ul_ret = hmac_user_del(&pst_hmac_vap_sta->st_vap_base_info, pst_hmac_user);
            if (OAL_SUCC != ul_ret)
            {
                OAM_WARNING_LOG1(pst_hmac_vap_sta->st_vap_base_info.uc_vap_id, OAM_SF_ANY,
                                "{hmac_sta_up_rx_beacon::hmac_user_del failed[%d].}", ul_ret);
            }

            /* 设置状态为FAKE UP */
            hmac_fsm_change_state(pst_hmac_vap_sta, MAC_VAP_STATE_STA_FAKE_UP);

            /* 抛事件上报内核，已经去关联 */
            hmac_sta_handle_disassoc_rsp(pst_hmac_vap_sta, HMAC_REPORT_DEAUTH);
        }
    }
...
OAL_STATIC oal_void  hmac_sta_up_rx_action(hmac_vap_stru *pst_hmac_vap, oal_netbuf_stru *pst_netbuf, oal_bool_enum_uint8 en_is_protected)
{
    ...
    /* 获取发送端的用户指针 */
    ul_rslt = mac_vap_find_user_by_macaddr(&pst_hmac_vap->st_vap_base_info, pst_frame_hdr->auc_address2, &us_user_idx);
    if (OAL_SUCC != ul_rslt)
    {
        OAM_WARNING_LOG1(pst_hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_RX, "{hmac_sta_up_rx_action::mac_vap_find_user_by_macaddr failed[%d].}", ul_rslt);
        return;
    }
