...
oal_uint32 hmac_roam_connect_set_join_reg(mac_vap_stru *pst_mac_vap)
{
    ...
    /* 设置dtim period信息 */
    pst_reg_params->us_beacon_period = pst_mac_vap->pst_mib_info->st_wlan_mib_sta_config.ul_dot11DTIMPeriod;
...
OAL_STATIC oal_uint32  hmac_roam_connect_notify_wpas(hmac_vap_stru *pst_hmac_vap, oal_uint8 *puc_mac_hdr, oal_uint16 us_msg_len)
{
    ...
    oal_memcopy(pst_event->auc_event_data, &st_roam_rsp, OAL_SIZEOF(hmac_roam_rsp_stru));
...
OAL_STATIC oal_uint32 hmac_roam_send_ft_req(hmac_roam_info_stru *pst_roam_info, oal_void *p_param)
{
    ...
    pst_ft_frame = OAL_MEM_NETBUF_ALLOC(OAL_NORMAL_NETBUF, WLAN_MEM_NETBUF_SIZE2, OAL_TRUE);
...
OAL_STATIC oal_uint32 hmac_roam_send_reassoc_req(hmac_roam_info_stru *pst_roam_info)
{
    ...
    pst_assoc_req_frame = OAL_MEM_NETBUF_ALLOC(OAL_NORMAL_NETBUF, WLAN_MEM_NETBUF_SIZE2, OAL_TRUE);
    ...
    mac_set_vendor_ie(pst_mac_vap, puc_assoc_buff, &uc_ie_len);
    us_assoc_len      += uc_ie_len;
    puc_assoc_buff    += uc_ie_len;
#endif
...
OAL_STATIC oal_uint32 hmac_roam_process_auth_seq2(hmac_roam_info_stru *pst_roam_info, oal_void *p_param)
{
    ...
    OAM_WARNING_LOG3(pst_hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_ROAM,
                       "{hmac_roam_process_auth_seq2::frame_sub_type = 0x%x, seq_num = 0x%x, auth_alg = 0x%x.}", 
                       uc_frame_sub_type,us_auth_seq_num,mac_get_auth_alg(puc_mac_hdr));
...
OAL_STATIC oal_uint32 hmac_roam_process_assoc_rsp(hmac_roam_info_stru *pst_roam_info, oal_void *p_param)
{
    ...
    if (us_msg_len <= OAL_ASSOC_RSP_IE_OFFSET)
    {
        OAM_WARNING_LOG1(pst_hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_ROAM,
                               "{hmac_roam_process_assoc_rsp::us_msg_len[%d].}", us_msg_len);
    ...
    /* assoc帧校验，错误帧不处理，在超时中统一处理 */
    uc_frame_sub_type = mac_get_frame_sub_type(puc_mac_hdr);

    OAM_WARNING_LOG1(pst_hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_ROAM,
                       "{hmac_roam_process_assoc_rsp::frame_sub_type = 0x%x}", uc_frame_sub_type);
    ...
    /* 处理vendor信息元素，更新用户11ac2g信息 */
#ifdef _PRE_WLAN_FEATURE_11AC2G
    puc_tmp_ie = mac_get_vendor_ie((oal_uint8*)puc_payload, us_msg_len, us_ie_offset);
    if (OAL_PTR_NULL != puc_tmp_ie)
    {
        hmac_mgmt_update_11ac2g_cap(puc_tmp_ie, pst_hmac_user);
    }
#endif
    ...
    if (OAL_SUCC != ul_ret)
    {
        hmac_ie_proc_opmode_field(&(pst_hmac_vap->st_vap_base_info), &pst_hmac_user->st_user_base_info, (mac_opmode_notify_stru *)puc_tmp_ie, mac_get_frame_sub_type(puc_mac_hdr));
        return ul_ret;
    }
#endif
    ...
    {
        OAM_ERROR_LOG1(pst_hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_ROAM,
                       "{hmac_roam_process_assoc_rsp::hmac_syn_vap_state failed[%d].}", ul_ret);
        return ul_ret;
    }
