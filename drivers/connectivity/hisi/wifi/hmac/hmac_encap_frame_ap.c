                ...
                oal_uint16                      us_type)
{
    ...
    if (OAL_TRUE == pst_mac_user->st_vht_hdl.en_vht_capable)
    {
        ...
        mac_set_vendor_ie((oal_void *)pst_mac_ap, puc_asoc_rsp, &uc_ie_len);
        puc_asoc_rsp += uc_ie_len;
#endif
                                                ...
                                                oal_uint16  *pus_user_index)
{
    ...
    *puc_auth_resend = OAL_FALSE;
    ...
    if(!uc_is_seq1)
    {
        OAM_ERROR_LOG0(pst_mac_vap->uc_vap_id, OAM_SF_ANY, "{hmac_encap_auth_rsp_get_user_idx::user should have been added at seq1!}");
        return OAL_FAIL;
    }
...
oal_void hmac_tid_clear(mac_vap_stru *pst_mac_vap, hmac_user_stru *pst_hmac_user)
{
    ...
    for (uc_loop = 0; uc_loop < WLAN_WME_MAX_TID_NUM; uc_loop++)
    {
        ...
        {
            FRW_TIMER_IMMEDIATE_DESTROY_TIMER(&(pst_hmac_user->ast_hmac_amsdu[uc_loop].st_amsdu_timer));
        }

        if (pst_hmac_user->ast_tid_info[uc_loop].st_ba_tx_info.st_addba_timer.en_is_registerd == OAL_TRUE)
        {
            FRW_TIMER_IMMEDIATE_DESTROY_TIMER(&(pst_hmac_user->ast_tid_info[uc_loop].st_ba_tx_info.st_addba_timer));
        }

        pst_hmac_user->ast_tid_info[uc_loop].uc_tid_no      = (oal_uint8)uc_loop;

        //pst_hmac_user->ast_tid_info[uc_tid_loop].pst_hmac_user  = (oal_void *)pst_hmac_user;
        pst_hmac_user->ast_tid_info[uc_loop].us_hmac_user_idx = pst_hmac_user->st_user_base_info.us_assoc_id;

        /* 清除接收方向会话句柄 */
        if (OAL_PTR_NULL != pst_hmac_user->ast_tid_info[uc_loop].pst_ba_rx_info)
        {
            hmac_ba_reset_rx_handle(pst_device, &pst_hmac_user->ast_tid_info[uc_loop].pst_ba_rx_info, uc_loop, OAL_TRUE);
        }

        if (DMAC_BA_INIT != pst_hmac_user->ast_tid_info[uc_loop].st_ba_tx_info.en_ba_status)
        {
            /* 存在TX BA会话句柄，要-- */
#ifdef _PRE_WLAN_FEATURE_AMPDU_VAP
        ...
        /* 初始化ba tx操作句柄 */
        pst_hmac_user->ast_tid_info[uc_loop].st_ba_tx_info.en_ba_status         = DMAC_BA_INIT;
        pst_hmac_user->ast_tid_info[uc_loop].st_ba_tx_info.uc_addba_attemps     = 0;
        pst_hmac_user->ast_tid_info[uc_loop].st_ba_tx_info.uc_dialog_token      = 0;
        pst_hmac_user->ast_tid_info[uc_loop].st_ba_tx_info.en_ba_switch         = OAL_TRUE;
...
oal_uint16  hmac_encap_auth_rsp(mac_vap_stru *pst_mac_vap, oal_netbuf_stru *pst_auth_rsp, oal_netbuf_stru *pst_auth_req, oal_uint8 *puc_chtxt)
{
    ...
    OAM_WARNING_LOG3(pst_mac_vap->uc_vap_id, OAM_SF_AUTH, "{hmac_encap_auth_rsp::user mac[0-2]:%.2x:%.2x:%.2x}",
                                auc_addr2[0],auc_addr2[1],auc_addr2[2]);
    OAM_WARNING_LOG3(pst_mac_vap->uc_vap_id, OAM_SF_AUTH, "{hmac_encap_auth_rsp::user mac[3-5]:%.2x:%.2x:%.2x}",
    ...
    if(OAL_SUCC != ul_ret)
    {
        ...
        {
            OAM_ERROR_LOG0(pst_mac_vap->uc_vap_id, OAM_SF_AUTH,"{hmac_encap_auth_rsp::hmac_ap_get_user_idx Err!}");
        }
