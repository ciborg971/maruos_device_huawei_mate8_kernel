...
OAL_STATIC oal_void hmac_scan_print_scan_params(mac_scan_req_stru *pst_scan_params, mac_vap_stru *pst_mac_vap)
{
    ...
    oal_int32                l_remainder_len  = OAM_REPORT_MAX_STRING_LEN;
    oal_uint8                uc_loop;
    ...
    {
        OAM_WARNING_LOG0(0, OAM_SF_SCAN, "{hmac_scan_print_scan_params::pc_print_buff null}");
        return;
    }
    OAL_MEMZERO(pc_print_buff, OAM_REPORT_MAX_STRING_LEN);

    /* 打印扫描结果 */
    OAL_SPRINTF(pc_print_buff, OAM_REPORT_MAX_STRING_LEN, "SCAN PARAM:\nssid_num = %d. ie_len = %d\n", pst_scan_params->uc_ssid_num, pst_mac_vap->ast_app_ie[OAL_APP_PROBE_REQ_IE].ul_ie_len);
    l_remainder_len = (oal_int32)(OAM_REPORT_MAX_STRING_LEN - OAL_STRLEN(pc_print_buff));

    /* 遍历扫描到的bss信息 */
    for (uc_loop = 0; uc_loop < pst_scan_params->uc_ssid_num; uc_loop++)
    {
        OAL_SPRINTF(ac_tmp_buff, sizeof(ac_tmp_buff), "NO.%d, SSID:%s.\n",
                    uc_loop,
                    pst_scan_params->ac_ssid[uc_loop]);
    ...
    OAL_SPRINTF(ac_tmp_buff, sizeof(ac_tmp_buff),
                "bss_type:%d.\nscan_type: %d.\nbssid_num: %d.\nuc_ssid_num: %d.\n",
                pst_scan_params->en_bss_type, pst_scan_params->en_scan_type,
                (oal_int32)pst_scan_params->uc_bssid_num, (oal_int32)pst_scan_params->uc_ssid_num);
    oal_strncat(pc_print_buff, ac_tmp_buff, l_remainder_len-1);
    ...
    OAL_SPRINTF(ac_tmp_buff, sizeof(ac_tmp_buff),
                "mac_scan_count_per_channel: %d.\nuc_max_send_probe_req_count_per_channel: %d.\n"
                "need_switch_back_home_channel: %d.\np2p0_scan: %d.\n",
                pst_scan_params->uc_max_scan_count_per_channel, pst_scan_params->uc_max_send_probe_req_count_per_channel,
                pst_scan_params->en_need_switch_back_home_channel, pst_scan_params->bit_is_p2p0_scan);
    oal_strncat(pc_print_buff, ac_tmp_buff, l_remainder_len-1);
    ...
    OAL_SPRINTF(ac_tmp_buff, sizeof(ac_tmp_buff),
                "channel_nums: %d.\nscan_time: %d.\n"
                "en_scan_mode: %d.\nuc_scan_func: %d.\nuc_vap_id: %d.\n",
                pst_scan_params->uc_channel_nums, pst_scan_params->us_scan_time,
                pst_scan_params->en_scan_mode, pst_scan_params->uc_scan_func,
    ...
    OAL_SPRINTF(ac_tmp_buff, sizeof(ac_tmp_buff),
                "source_addr[0-5]: %02X:%02X:%02X:%02X:%02X:%02X\n",
                pst_scan_params->auc_sour_mac_addr[0], pst_scan_params->auc_sour_mac_addr[1],
                pst_scan_params->auc_sour_mac_addr[2], pst_scan_params->auc_sour_mac_addr[3],
                pst_scan_params->auc_sour_mac_addr[4], pst_scan_params->auc_sour_mac_addr[5]);
    oal_strncat(pc_print_buff, ac_tmp_buff, l_remainder_len-1);

    /* 添加信息元素打印 */
    l_remainder_len = (oal_int32)(OAM_REPORT_MAX_STRING_LEN - OAL_STRLEN(pc_print_buff));
    if (l_remainder_len > (oal_int32)pst_mac_vap->ast_app_ie[OAL_APP_PROBE_REQ_IE].ul_ie_len)
    {
        hmac_snprintf_hex((oal_uint8 *)(pc_print_buff + OAL_STRLEN(pc_print_buff)), l_remainder_len, pst_mac_vap->ast_app_ie[OAL_APP_PROBE_REQ_IE].puc_ie, (oal_int32)pst_mac_vap->ast_app_ie[OAL_APP_PROBE_REQ_IE].ul_ie_len);
    }

    OAL_MEMZERO(ac_tmp_buff, sizeof(ac_tmp_buff));
    l_remainder_len = (oal_int32)(OAM_REPORT_MAX_STRING_LEN - OAL_STRLEN(pc_print_buff));
    OAL_SPRINTF(ac_tmp_buff, sizeof(ac_tmp_buff), "\nie len = %d, msg len = %d\n", pst_mac_vap->ast_app_ie[OAL_APP_PROBE_REQ_IE].ul_ie_len, (oal_uint32)OAL_STRLEN(pc_print_buff));
    oal_strncat(pc_print_buff, ac_tmp_buff, l_remainder_len-1);
...
oal_void hmac_scan_print_scanned_bss_info(oal_uint8 uc_device_id)
{
    ...
    oal_uint8                   auc_broadcast_mac_addr[WLAN_MAC_ADDR_LEN] = {0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF};
    ...
    OAL_DLIST_SEARCH_FOR_EACH(pst_entry, &(pst_bss_mgmt->st_bss_list_head))
    {
        ...
        oal_memcopy(auc_sdt_parse_hdr, (void *)pst_frame_hdr, MAC_80211_FRAME_LEN);
        ...
        /*lint -e416*/
        oam_report_80211_frame(auc_broadcast_mac_addr,
                               auc_sdt_parse_hdr,
                               MAC_80211_FRAME_LEN,
...
OAL_STATIC oal_void hmac_scan_clean_expire_scanned_bss(hmac_scan_record_stru  *pst_scan_record)
{
    ...
    OAM_WARNING_LOG1(0, OAM_SF_SCAN,
                     "{hmac_scan_clean_expire_scanned_bss::cleaned expire scanned BSS success, remaining bss num is %d.}",
                     pst_bss_mgmt->ul_bss_num);
                                                                          ...
                                                                          oal_uint16         us_frame_len)
{
    ...
    if (OAL_PTR_NULL != puc_ie)
    {
        ...
        oal_memcopy(&(pst_bss_dscr->auc_supp_rates[uc_num_rates]), puc_ie + MAC_IE_HDR_LEN, uc_num_ex_rates);
    }
...
OAL_STATIC oal_void  hmac_scan_print_scan_record_info(hmac_scan_record_stru   *pst_scan_record)
{
    oal_time_us_stru  st_curr_time;

    /* 打印此次扫描耗时 */
    oal_time_get_stamp_us(&st_curr_time);
    OAM_WARNING_LOG3(pst_scan_record->uc_vap_id, OAM_SF_SCAN,
                     "{hmac_scan_print_scan_record_info::scan comp, scan_status[%d], duration time is: [%d]s[%d]ms.}",
                     pst_scan_record->en_scan_rsp_status,
                     (st_curr_time.i_sec - pst_scan_record->st_scan_start_timestamp.i_sec),
                     (st_curr_time.i_usec - pst_scan_record->st_scan_start_timestamp.i_usec)/1000);
...
oal_uint32  hmac_scan_proc_scan_comp_event(frw_event_mem_stru *pst_event_mem)
{
    ...
    /* 删除扫描超时保护定时器 */
    if (OAL_TRUE == pst_hmac_device->st_scan_mgmt.st_scan_timeout.en_is_registerd)
    {
        FRW_TIMER_IMMEDIATE_DESTROY_TIMER(&(pst_hmac_device->st_scan_mgmt.st_scan_timeout));
    }
    ...
    if ((WLAN_VAP_MODE_BSS_STA == pst_hmac_vap->st_vap_base_info.en_vap_mode) &&
        (MAC_VAP_STATE_STA_WAIT_SCAN == pst_hmac_vap->st_vap_base_info.en_vap_state))
    {
        /* 改变vap状态到SCAN_COMP */
    ...
    /* 打印扫描结果 测试使用 */
    OAM_WARNING_LOG2(pst_event_hdr->uc_vap_id, OAM_SF_SCAN, "{hmac_scan_proc_scan_comp_event::scan completed, status:%d, vap channel num:%d}",
                        pst_scan_mgmt->st_scan_record_mgmt.en_scan_rsp_status,
                        pst_hmac_vap->st_vap_base_info.st_channel.uc_chan_number);

    hmac_scan_print_scan_record_info(&(pst_scan_mgmt->st_scan_record_mgmt));
...
OAL_STATIC oal_void  hmac_scan_set_sour_mac_addr_in_probe_req(hmac_vap_stru        *pst_hmac_vap,
                                                              ...
                                                              oal_bool_enum_uint8   en_is_p2p0_scan)
{
    ...
    {
        /* 如果随机mac addr扫描特性开启，则设置随机mac addr到probe req帧中 */
        if (OAL_TRUE == en_is_rand_mac_addr_scan)
        {
                                                               ...
                                                               oal_bool_enum_uint8   en_is_random_mac_addr_scan)
{
    ...
    if ((OAL_SUCC == ul_ret) && (OAL_PTR_NULL != pst_mac_vap_temp))
    {
        ...
        /* 背景扫描，每信道发送probe req的次数为1 */
        pst_scan_params->uc_max_scan_count_per_channel    = WLAN_DEFAULT_BG_SCAN_COUNT_PER_CHANNEL;
        pst_scan_params->en_need_switch_back_home_channel = OAL_TRUE; /* 默认背景扫描，扫描过程中需要切回工作信道工作 */
    }
    else
    {
        /* 没有找到up状态下的vap，说明是前景扫描 */
        pst_scan_params->en_scan_mode = WLAN_SCAN_MODE_FOREGROUND;

        /* 前景扫描，每个信道上总共发送probe req的次数为1 */
        if (MAC_SCAN_FUNC_P2P_LISTEN != pst_scan_params->uc_scan_func)
        {
            pst_scan_params->uc_max_scan_count_per_channel = WLAN_DEFAULT_FG_SCAN_COUNT_PER_CHANNEL;
        }
    }
...
oal_uint32  hmac_scan_proc_scan_req_event(hmac_vap_stru *pst_hmac_vap, oal_void *p_params)
{
    ...
    if (OAL_PTR_NULL == pst_hmac_device)
    {
        OAM_WARNING_LOG0(pst_hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_SCAN, "{hmac_scan_proc_scan_req_event::pst_hmac_device null.}");
        return OAL_ERR_CODE_MAC_DEVICE_NULL;
    }
    ...
    /* 记录扫描开始时间 */
    oal_time_get_stamp_us(&(pst_scan_record->st_scan_start_timestamp));
...
oal_uint32  hmac_scan_proc_sched_scan_req_event(hmac_vap_stru *pst_hmac_vap, oal_void *p_params)
{
    ...
    if (OAL_PTR_NULL == pst_hmac_device)
    {
        OAM_WARNING_LOG0(pst_hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_SCAN, "{hmac_scan_proc_sched_scan_req_event::pst_hmac_device null.}");
        return OAL_ERR_CODE_MAC_DEVICE_NULL;
    }
    ...
    /* 事件data域内携带PNO扫描请求参数 */
    oal_memcopy(pst_event->auc_event_data, &pst_pno_scan_params, OAL_SIZEOF(mac_pno_scan_stru *));
...
oal_uint32  hmac_scan_test(mac_vap_stru *pst_mac_vap, oal_uint16 us_len, oal_uint8 *puc_param)
{
    ...
    oal_set_mac_addr(st_scan_req.auc_bssid[0], auc_bcst_addr);
                                                            ...
                                                            hmac_scan_record_stru *pst_scan_record)
{
    ...
    OAL_DLIST_SEARCH_FOR_EACH(pst_entry, &(pst_bss_mgmt->st_bss_list_head))
    {
        ...
        OAM_INFO_LOG3(pst_mac_vap->uc_vap_id, OAM_SF_2040, "pst_bss_dscr->st_channel.uc_chan_number = %d, ul_pri = %d, SSID=%s\n",
                        pst_bss_dscr->st_channel.uc_chan_number, ul_pri, pst_bss_dscr->ac_ssid);
                                                            ...
                                                            hmac_scan_record_stru *pst_scan_record)
{
    ...
    OAL_DLIST_SEARCH_FOR_EACH(pst_entry, &(pst_bss_mgmt->st_bss_list_head))
    {
        ...
        OAM_INFO_LOG2(pst_mac_vap->uc_vap_id, OAM_SF_2040, "hmac_obss_check_40mhz_5g:bss uc_channel_number = %d, ssid:%s\n", pst_bss_dscr->st_channel.uc_chan_number, pst_bss_dscr->ac_ssid);
...
mac_need_init_scan_res_enum_uint8 hmac_need_init_scan(hmac_device_stru *pst_hmac_device, mac_vap_stru *pst_in_mac_vap, mac_try_init_scan_type en_type)
{
    ...
    OAM_WARNING_LOG1(pst_in_mac_vap->uc_vap_id, OAM_SF_ACS, "{hmac_need_init_scan:res=%d", en_result);
...
oal_uint32 hmac_init_scan_do(hmac_device_stru *pst_hmac_dev, mac_vap_stru *pst_mac_vap, mac_init_scan_req_stru *pst_cmd)
{
    ...
    // hmac_scan_update_scan_params中自动判断扫描类型和扫描次数
    // 前景扫描次数加倍，因此设置45ms相当于90ms。
    ...
    st_scan_req.us_scan_time        = st_scan_req.en_scan_mode == WLAN_SCAN_MODE_FOREGROUND ? 45 : 30;
    ...
    for (uc_idx = 0; uc_idx < pst_cmd->ul_cmd_len; uc_idx += 2)
    {
        ...
        mac_get_channel_idx_from_num(st_scan_req.ast_channel_list[uc_cnt].en_band,
...
oal_uint32  hmac_init_scan_try(mac_device_stru *pst_mac_device, mac_vap_stru *pst_in_mac_vap, mac_try_init_scan_type en_type)
{
    ...
    oal_uint8   en_scan_type = hmac_need_init_scan(pst_hmac_device, pst_in_mac_vap, en_type);
