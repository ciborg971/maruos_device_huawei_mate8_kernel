...
OAL_STATIC OAL_INLINE oal_uint32 hmac_normal_check_legacy_vap_num(mac_device_stru *pst_mac_device, wlan_vap_mode_enum_uint8   en_vap_mode)
{
    ...
    if (WLAN_VAP_MODE_BSS_AP == en_vap_mode)
    {
        ...
        {
            /* 已创建的AP个数达到最大值4 */
            OAM_WARNING_LOG1(0, OAM_SF_CFG, "{hmac_normal_check_legacy_vap_num::ap num exceeds the supported spec[%d].}",  (pst_mac_device->uc_vap_num - pst_mac_device->uc_sta_num));
            return OAL_ERR_CODE_CONFIG_EXCEED_SPEC;
        }
    }
    else if (WLAN_VAP_MODE_BSS_STA == en_vap_mode)
...
oal_uint32  hmac_config_add_vap(mac_vap_stru *pst_vap, oal_uint16 us_len, oal_uint8 *puc_param)
{
    ...
    /* 初始化HMAC VAP */
    ul_ret = hmac_vap_init(pst_hmac_vap, pst_dev->uc_chip_id, pst_dev->uc_device_id, uc_vap_id, pst_param->en_vap_mode);
    if (OAL_UNLIKELY(OAL_SUCC != ul_ret))
    {
    ...
    ***************************************************************************/
    ...
    {
        for (uc_vap_idx = 0; uc_vap_idx < pst_dev->uc_vap_num; uc_vap_idx++)
        {
            pst_tmp_vap = (mac_vap_stru *)mac_res_get_mac_vap(pst_dev->auc_vap_id[uc_vap_idx]);
            if (OAL_PTR_NULL == pst_tmp_vap)
            {
                OAM_ERROR_LOG1(0, OAM_SF_SCAN, "{hmac_calc_up_ap_num::pst_mac_vap null,vap_idx=%d.}",
                               pst_dev->auc_vap_id[uc_vap_idx]);
                continue;
            }

            if ((MAC_VAP_STATE_UP == pst_tmp_vap->en_vap_state) && (pst_tmp_vap != pst_vap))
            {
                pst_hmac_vap->st_vap_base_info.st_channel.en_band        = pst_tmp_vap->st_channel.en_band;
                pst_hmac_vap->st_vap_base_info.st_channel.en_bandwidth   = pst_tmp_vap->st_channel.en_bandwidth;
                pst_hmac_vap->st_vap_base_info.st_channel.uc_chan_number = pst_tmp_vap->st_channel.uc_chan_number;
                pst_hmac_vap->st_vap_base_info.st_channel.uc_idx         = pst_tmp_vap->st_channel.uc_idx;
                break;
            }
        }
    }
    ...
    OAM_WARNING_LOG4(pst_vap->uc_vap_id, OAM_SF_ANY, "{hmac_config_add_vap::SUCCESS!!vap_mode[%d], p2p_mode[%d}, vap_id[%d], multi user idx[%d]",
                    pst_param->en_vap_mode, pst_param->en_p2p_mode, pst_param->uc_vap_id, pst_vap->us_multi_user_idx);
...
oal_uint32  hmac_config_del_vap(mac_vap_stru *pst_vap, oal_uint16 us_len, oal_uint8 *puc_param)
{
    ...
    {
        OAM_WARNING_LOG1(pst_vap->uc_vap_id, OAM_SF_CFG, "{hmac_config_del_vap::deleting vap failed. vap state is not INIT, en_vap_state=%d.}", pst_vap->en_vap_state);
        return OAL_FAIL;
    }
    ...
    {
        /* 去注册 */
        oal_net_unregister_netdev(pst_hmac_vap->pst_net_device);
    ...
    ***************************************************************************/
    ...
    OAM_WARNING_LOG3(pst_vap->uc_vap_id, OAM_SF_ANY, "{hmac_config_del_vap::Del succ.vap_mode[%d], p2p_mode[%d], multi user idx[%d]}",
                pst_del_vap_param->en_vap_mode, pst_del_vap_param->en_p2p_mode, pst_vap->us_multi_user_idx);
...
oal_uint32 hmac_config_def_chan(mac_vap_stru *pst_mac_vap)
{
    ...
    if ((0 == pst_mac_vap->st_channel.uc_chan_number) && (!IS_P2P_GO(pst_mac_vap)))
    {
        ...
        ul_chan_num = pst_mac_vap->st_channel.uc_chan_number;
        hmac_config_set_freq(pst_mac_vap, OAL_SIZEOF(oal_uint32), (oal_uint8*)&ul_chan_num);
    }
...
oal_uint32  hmac_config_start_vap(mac_vap_stru *pst_mac_vap, oal_uint16 us_len, oal_uint8 *puc_param)
{
    ...
    {
        /* TBD 其它分支 暂不支持 待开发 */
        OAM_WARNING_LOG0(pst_mac_vap->uc_vap_id, OAM_SF_CFG, "{hmac_config_start_vap::Do not surport other mode.}");
    }
    ...
    OAM_WARNING_LOG1(pst_mac_vap->uc_vap_id, OAM_SF_CFG, "{hmac_config_start_vap::host start vap ok. now post event to dmac. vap mode[%d]}",
...
oal_uint32  hmac_config_sta_update_rates(mac_vap_stru *pst_mac_vap, mac_cfg_mode_param_stru *pst_cfg_mode, mac_bss_dscr_stru *pst_bss_dscr)
{
    ...
    OAM_INFO_LOG0(pst_mac_vap->uc_vap_id, OAM_SF_CFG, "{hmac_config_sta_update_rates::now post event to dmac.}");
...
oal_uint32 hmac_config_down_vap(mac_vap_stru *pst_mac_vap, oal_uint16 us_len, oal_uint8 *puc_param)
{
    ...
    {
        OAM_INFO_LOG0(pst_mac_vap->uc_vap_id, OAM_SF_CFG, "{hmac_config_down_vap::vap already down.}");
        return OAL_SUCC;
    }
    ...
    OAM_WARNING_LOG0(pst_mac_vap->uc_vap_id, OAM_SF_CFG, "{hmac_config_down_vap::now post event to dmac.}");
...
oal_uint32  hmac_config_set_shpreamble(mac_vap_stru *pst_mac_vap, oal_uint16 us_len, oal_uint8 *puc_param)
{
    ...
    ***************************************************************************/
    ...
    if (OAL_UNLIKELY(OAL_SUCC != ul_ret))
    {
        OAM_INFO_LOG1(pst_mac_vap->uc_vap_id, OAM_SF_CFG, "{hmac_config_set_concurrent::hmac_config_send_event failed[%d].}", ul_ret);
    }
...
oal_uint32  hmac_config_set_shortgi20(mac_vap_stru *pst_mac_vap, oal_uint16 us_len, oal_uint8 *puc_param)
{
    ...
    hmac_config_send_event(pst_mac_vap, WLAN_CFGID_SHORTGI, SHORTGI_CFG_STRU_LEN, (oal_uint8 *)&shortgi_cfg);
    #endif
    /*========================================================================*/
...
oal_uint32  hmac_config_set_freq(mac_vap_stru *pst_mac_vap, oal_uint16 us_len, oal_uint8 *puc_param)
{
    ...
    ul_ret = mac_is_channel_num_valid(pst_mac_vap->st_channel.en_band, (oal_uint8)ul_freq);
    if (OAL_SUCC != ul_ret)
    {
        OAM_ERROR_LOG2(pst_mac_vap->uc_vap_id, OAM_SF_CFG, "{hmac_config_set_freq::mac_is_channel_num_valid[%d] failed[%d].}", ul_freq, ul_ret);
        return OAL_ERR_CODE_INVALID_CONFIG;
    }

#ifdef _PRE_WLAN_FEATURE_11D
    /* 信道14特殊处理，只在11b协议模式下有效 */
    if ((14 == ul_freq) && (WLAN_LEGACY_11B_MODE != pst_mac_vap->en_protocol))
    {
...
/* gaolin: CAC在初始检查时统一执行 */
    ...
    pst_mac_vap->st_channel.uc_chan_number = (oal_uint8)ul_freq;
    mac_get_channel_idx_from_num(pst_mac_vap->st_channel.en_band, (oal_uint8)ul_freq, &(pst_mac_vap->st_channel.uc_idx));
    ...
    if (1 == pst_mac_device->uc_vap_num || 0 == pst_mac_device->uc_max_channel)
    {
        ...
        l_channel_param.uc_channel = (oal_uint8)ul_freq;
...
oal_uint32  hmac_config_set_reset_state(mac_vap_stru *pst_mac_vap, oal_uint16 us_len, oal_uint8 *puc_param)
{
    ...
    {
        OAM_ERROR_LOG0(0, OAM_SF_CFG, "{hmac_scan_find_scanned_bss_by_index::pst_hmac_device is null.}");
        return OAL_PTR_NULL;
    }
...
oal_uint32  hmac_config_vap_info(mac_vap_stru *pst_mac_vap, oal_uint16 us_len, oal_uint8 *puc_param)
{
    ...
    hmac_vap_member_size_stru    st_hmac_vap_member_size;
    mac_user_stru               *pst_multi_user;
    mac_user_stru               *pst_mac_user;
    oal_int32                    l_remainder_len = OAM_PRINT_FORMAT_LENGTH;
    ...
    oal_int8        *pac_key_type2string[] = {"TX_GTK", "PTK", "RX_GTK", "error"};

    oal_int8         ac_tmp_buff[OAM_PRINT_FORMAT_LENGTH] = {0};
    oal_int8         ac_protocol_buff[OAM_PRINT_FORMAT_LENGTH]={0};
    ...
    if ((WLAN_VAP_MODE_BSS_STA == pst_mac_vap->en_vap_mode)&& (NULL != (pst_mac_user = mac_res_get_mac_user(pst_mac_vap->uc_assoc_vap_id))))
    {
        ...
        OAL_SPRINTF(ac_protocol_buff, sizeof(ac_protocol_buff), "sta|ap[%s|%s]",
        pac_protocol2string[pst_mac_vap->en_protocol], pac_protocol2string[en_disp_protocol]);
    }
    else
    {
        en_disp_protocol = pst_mac_vap->en_protocol;
        OAL_SPRINTF(ac_protocol_buff, sizeof(ac_protocol_buff), "%s",
                    pac_protocol2string[en_disp_protocol]);
    }

    OAL_SPRINTF(pc_print_buff,
                ...
                pst_mac_vap->en_vap_state, pst_mac_vap->pst_mib_info->st_wlan_mib_sta_config.auc_dot11DesiredSSID,
                pst_mac_vap->st_cap_flag.bit_hide_ssid,
                ac_protocol_buff,
                pac_band2string[pst_mac_vap->st_channel.en_band],
                pac_bw2string[pst_mac_vap->en_protocol <= WLAN_MIXED_TWO_11G_MODE ? WLAN_BAND_WIDTH_20M : pst_mac_vap->st_channel.en_bandwidth],
    ...
    if(OAL_TRUE == pst_mac_vap->pst_mib_info->st_wlan_mib_privacy.en_dot11PrivacyInvoked)
    {
        ...
        if (OAL_TRUE == pst_mac_vap->pst_mib_info->st_wlan_mib_privacy.en_dot11RSNAActivated)
        {
            if(1 == pst_mac_vap->st_cap_flag.bit_wpa)
            {
                ...
                OAL_SPRINTF(pc_print_buff, OAM_REPORT_MAX_STRING_LEN, "Privacy Invoked: \nRSNA-WPA \n "
                        "GRUOP     WPA PAIREWISE0[Actived]    WPA PAIRWISE1[Actived]     AUTH1[Active]     AUTH2[Active]\n "
                        "%s        %s[%s]          %s[%s]             %s[%s]             %s[%s]\n",
                        pac_ciper2string[uc_group_suit],
                        pac_ciper2string[uc_pairwise_suit[0]],
                        ...
                        pac_ciper2string[uc_pairwise_suit[1]],
            ...
            if(1 == pst_mac_vap->st_cap_flag.bit_wpa2)
            {
                ...
                OAL_SPRINTF(ac_tmp_buff, sizeof(ac_tmp_buff), "Privacy Invoked: \nRSNA-WPA2 \n"
                ...
                OAL_MEMZERO(ac_tmp_buff, sizeof(ac_tmp_buff));
                l_remainder_len = (oal_int32)(OAM_REPORT_MAX_STRING_LEN - OAL_STRLEN(pc_print_buff));
            }
        }
        else
        {
            OAL_SPRINTF(pc_print_buff, OAM_REPORT_MAX_STRING_LEN, "Privacy Invoked: \nWEP: %s\n", pac_ciper2string[pst_mac_vap->pst_mib_info->st_wlan_mib_rsna_cfg.uc_dot11RSNAConfigGroupCipher]);
            l_remainder_len = (oal_int32)(OAM_REPORT_MAX_STRING_LEN - OAL_STRLEN(pc_print_buff));
        }

        OAL_SPRINTF(ac_tmp_buff, OAL_SIZEOF(ac_tmp_buff), "MULTI_USER: cipher_type:%s, key_type:%s \n",
                                                        pac_ciper2string[pst_multi_user->st_key_info.en_cipher_type],
                                                        pac_key_type2string[pst_multi_user->st_user_tx_info.st_security.en_cipher_key_type]);
    ...
    {
        oal_strncat(pc_print_buff, "Privacy NOT Invoked\n", l_remainder_len-1);
    }

    /* APP IE 信息 */
    for (uc_loop = 0; uc_loop < OAL_APP_IE_NUM; uc_loop++)
    {
        OAL_SPRINTF(ac_tmp_buff, sizeof(ac_tmp_buff), "APP IE:type= %d, addr = %p, len = %d, max_len = %d\n",
                    uc_loop,
        ...
        oal_strncat(pc_print_buff, ac_tmp_buff, l_remainder_len-1);
        oal_memset(ac_tmp_buff, 0, sizeof(ac_tmp_buff));
        l_remainder_len = (oal_int32)(OAM_REPORT_MAX_STRING_LEN - OAL_STRLEN(pc_print_buff));
    }
    ...
    /* SDT连接同时上报打印字符串与上报结构体*/
    oam_get_output_type(&en_output_type);
    if (OAM_OUTPUT_TYPE_SDT == en_output_type)
    {
        /* 计算并上报hmac vap中一部分成员的大小 */
        st_hmac_vap_member_size.ul_hmac_vap_cfg_priv_stru_size = OAL_SIZEOF(hmac_vap_cfg_priv_stru);
        st_hmac_vap_member_size.ul_frw_timeout_stru_size       = OAL_SIZEOF(frw_timeout_stru);
        st_hmac_vap_member_size.ul_oal_spin_lock_stru_size     = OAL_SIZEOF(oal_spin_lock_stru);
        st_hmac_vap_member_size.ul_mac_key_mgmt_stru_size      = OAL_SIZEOF(mac_key_mgmt_stru);
        st_hmac_vap_member_size.ul_mac_pmkid_cache_stru_size   = OAL_SIZEOF(mac_pmkid_cache_stru);
        st_hmac_vap_member_size.ul_mac_curr_rateset_stru_size  = OAL_SIZEOF(mac_curr_rateset_stru);
        st_hmac_vap_member_size.ul_hmac_vap_stru_size          = OAL_SIZEOF(hmac_vap_stru);

        oam_ota_report((oal_uint8 *)&st_hmac_vap_member_size,
                       (oal_uint16)sizeof(hmac_vap_member_size_stru),
                       0, 0, OAM_OTA_TYPE_HMAC_VAP_MEMBER_SIZE);

        /* 上报hmac vap结构体信息 */
        oam_ota_report((oal_uint8 *)pst_hmac_vap,
                       (oal_uint16)sizeof(hmac_vap_stru),
                       0, 0, OAM_OTA_TYPE_HMAC_VAP);
    }
...
oal_uint32  hmac_dpd_data_processed_send(mac_vap_stru *pst_mac_vap, oal_void *p_param)
{
    ...
    pst_netbuf_dpd_data = OAL_MEM_NETBUF_ALLOC(OAL_NORMAL_NETBUF,WLAN_LARGE_NETBUF_SIZE, OAL_TRUE);
...
oal_uint32 hmac_dpd_cali_data_recv(frw_event_mem_stru *pst_event_mem)
{
   ...
   if (OAL_UNLIKELY(OAL_PTR_NULL == pst_event_mem))
   {
    ...
    pst_vap = mac_res_get_hmac_vap(pst_event->st_event_hdr.uc_vap_id);

    pst_dpd_cali_data_read = (dpd_cali_data_stru *)OAL_NETBUF_DATA(pst_dpd_data_netbuf);

    for (index = 0; index < DPD_CALI_LUT_LENGTH; index++)
    {
        //OAM_ERROR_LOG1(0, OAM_SF_CALIBRATE, "{hmac_dpd_cali_data_recv::[%x]!}\r\n",
        //pst_dpd_cali_data_read->us_dpd_data[index]);
        dpd_cali_data_calc.us_dpd_data[index] = 0;
    }

    hi1102_rf_cali_dpd_corr_calc(pst_dpd_cali_data_read->us_dpd_data, dpd_cali_data_calc.us_dpd_data);
...
oal_uint32 hmac_config_set_random_mac_addr_scan(mac_vap_stru *pst_mac_vap, oal_uint16 us_len, oal_uint8 *puc_param)
{
    ...
    OAM_INFO_LOG1(0, OAM_SF_CFG, "{hmac_config_set_random_mac_addr_scan::set rand mac addr scan succ, switch_status[%d].}",
...
oal_uint32  hmac_config_ampdu_start(mac_vap_stru *pst_mac_vap, oal_uint16 us_len, oal_uint8 *puc_param)
{
    ...
    /* 获取用户对应的索引 */
    ul_ret = mac_vap_find_user_by_macaddr(pst_mac_vap, pst_ampdu_start->auc_mac_addr, &us_user_index);
    if (OAL_SUCC != ul_ret)
    {
        OAM_WARNING_LOG1(pst_mac_vap->uc_vap_id, OAM_SF_CFG, "{hmac_config_ampdu_start::mac_vap_find_user_by_macaddr failed[%d].}", ul_ret);
        return ul_ret;
    }
    ...
    if (OAL_TRUE == hmac_tid_need_ba_session(pst_hmac_vap, pst_hmac_user, pst_ampdu_start->uc_tidno, OAL_PTR_NULL))
    {
        /*
            ...
            (6)ul_arg4:TIMEOUT时间
        */
        OAM_ERROR_LOG1(pst_mac_vap->uc_vap_id, OAM_SF_CFG, "{hmac_config_ampdu_start::uc_tidno=%d.}", pst_ampdu_start->uc_tidno);
        st_action_args.uc_category = MAC_ACTION_CATEGORY_BA;
        st_action_args.uc_action   = MAC_BA_ACTION_ADDBA_REQ;
        st_action_args.ul_arg1     = pst_ampdu_start->uc_tidno;    /* 该数据帧对应的TID号 */
        st_action_args.ul_arg2     = WLAN_AMPDU_TX_MAX_NUM;    /* ADDBA_REQ中，buffer_size的默认大小 */
...
oal_uint32  hmac_config_ampdu_end(mac_vap_stru *pst_mac_vap, oal_uint16 us_len, oal_uint8 *puc_param)
{
    ...
    /* 获取用户对应的索引 */
    ul_ret = mac_vap_find_user_by_macaddr(pst_mac_vap, pst_ampdu_end->auc_mac_addr, &us_user_index);
    if (OAL_SUCC != ul_ret)
    {
        OAM_WARNING_LOG1(pst_mac_vap->uc_vap_id, OAM_SF_CFG, "{hmac_config_ampdu_end::mac_vap_find_user_by_macaddr failed[%d].}",ul_ret);
        return ul_ret;
    }
...
oal_uint32  hmac_config_addba_req(mac_vap_stru *pst_mac_vap, oal_uint16 us_len, oal_uint8 *puc_param)
{
    ...
    /* 获取用户对应的索引 */
    ul_ret = mac_vap_find_user_by_macaddr(pst_mac_vap, pst_addba_req->auc_mac_addr, &us_user_index);
    if (OAL_SUCC != ul_ret)
    {
        OAM_WARNING_LOG1(pst_mac_vap->uc_vap_id, OAM_SF_CFG, "{hmac_config_addba_req::mac_vap_find_user_by_macaddr failed[%d].}",ul_ret);
        return ul_ret;
    }
...
oal_uint32  hmac_config_delba_req(mac_vap_stru *pst_mac_vap, oal_uint16 us_len, oal_uint8 *puc_param)
{
    ...
    /* 获取用户对应的索引 */
    ul_ret = mac_vap_find_user_by_macaddr(pst_mac_vap, pst_delba_req->auc_mac_addr, &us_user_index);
    if (OAL_SUCC != ul_ret)
    {
        OAM_WARNING_LOG1(pst_mac_vap->uc_vap_id, OAM_SF_CFG, "{hmac_config_delba_req::mac_vap_find_user_by_macaddr failed[%d].}", ul_ret);
        return ul_ret;
    }
...
oal_uint32  hmac_config_amsdu_start(mac_vap_stru *pst_mac_vap, oal_uint16 us_len, oal_uint8 *puc_param)
{
    ...
    hmac_user_stru                  *pst_hamc_user;
    oal_uint8                        uc_tid_index;
    oal_uint16                       us_user_index = ALL;
    oal_uint32                       ul_ret;
    ...
    /* 获取用户对应的索引 */
    ul_ret = mac_vap_find_user_by_macaddr(pst_mac_vap, pst_amsdu_param->auc_mac_addr, &us_user_index);
    if (OAL_SUCC != ul_ret)
    {
        OAM_WARNING_LOG1(pst_mac_vap->uc_vap_id, OAM_SF_CFG, "{hmac_config_amsdu_start::mac_vap_find_user_by_macaddr failed[%d].}", ul_ret);
        return ul_ret;
    }

    /* 获取hmac用户 */
    pst_hamc_user = (hmac_user_stru *)mac_res_get_hmac_user(us_user_index);
    if (OAL_PTR_NULL == pst_hamc_user)
    {
    ...
    {
        hmac_amsdu_set_maxnum(&pst_hamc_user->ast_hmac_amsdu[uc_tid_index], pst_amsdu_param->uc_amsdu_max_num);
        hmac_amsdu_set_maxsize(&pst_hamc_user->ast_hmac_amsdu[uc_tid_index], pst_hamc_user, pst_amsdu_param->us_amsdu_max_size);
    }
...
oal_uint32  hmac_config_user_info(mac_vap_stru *pst_mac_vap, oal_uint16 us_len, oal_uint8 *puc_param)
{
    ...
    if (OAM_OUTPUT_TYPE_SDT != en_output_type)
    {
        ...
        OAL_IO_PRINT("    user_tx_info.security.cipher_key_type:      %s \r\n"
                    ...
                    pac_keytype2string[pst_hmac_user->st_user_base_info.st_user_tx_info.st_security.en_cipher_key_type],
                    pac_cipher2string[pst_hmac_user->st_user_base_info.st_key_info.en_cipher_type]);
...
oal_uint32  hmac_config_kick_user(mac_vap_stru *pst_mac_vap, oal_uint16 us_len, oal_uint8 *puc_param)
{
    ...
    OAM_WARNING_LOG3(pst_mac_vap->uc_vap_id, OAM_SF_CFG, "{hmac_config_kick_user::kick user mac[0-2]:%.2x:%.2x:%.2x}",
                                pst_kick_user_param->auc_mac_addr[0],
                                pst_kick_user_param->auc_mac_addr[1],
                                pst_kick_user_param->auc_mac_addr[2]);
    OAM_WARNING_LOG3(pst_mac_vap->uc_vap_id, OAM_SF_CFG, "{hmac_config_kick_user::kick user mac[3-5]:%.2x:%.2x:%.2x}",
                                pst_kick_user_param->auc_mac_addr[3],
                                pst_kick_user_param->auc_mac_addr[4],
                                pst_kick_user_param->auc_mac_addr[5]);
    OAM_WARNING_LOG1(pst_mac_vap->uc_vap_id, OAM_SF_CFG, "{hmac_config_kick_user::kick user reacon code:%d}",
...
oal_uint32  hmac_config_list_sta(mac_vap_stru *pst_mac_vap, oal_uint16 us_len, oal_uint8 *puc_param)
{
    ...
    oal_strcat(pc_print_buff, "      ADDR         Protocol Type \n");
    l_remainder_len = (oal_int32)(OAM_REPORT_MAX_STRING_LEN - OAL_STRLEN(pc_print_buff));
    ...
    for (uc_user_idx = 0; uc_user_idx < MAC_VAP_USER_HASH_MAX_VALUE; uc_user_idx++)
    {
        ...
        while ((&pst_mac_vap->ast_user_hash[uc_user_idx]) != pst_head)
        {
            ...
            OAL_SPRINTF(ac_tmp_buff, OAL_SIZEOF(ac_tmp_buff), "%02X:XX:XX:%02X:%02X:%02X       %s \n",
...
oal_uint32  hmac_config_get_sta_list(mac_vap_stru *pst_mac_vap, oal_uint16 *us_len, oal_uint8 *puc_param)
{
    ...
    pst_netbuf = OAL_MEM_NETBUF_ALLOC(OAL_NORMAL_NETBUF,ul_netbuf_len, OAL_TRUE);
    if(NULL != pst_netbuf)
    {
	...
	*us_len = (oal_uint16)OAL_SIZEOF(pst_netbuf);
	...
	OAM_WARNING_LOG1(pst_mac_vap->uc_vap_id, OAM_SF_CFG,"hmac_config_get_sta_list, us_len:%d\n", *us_len);
...
oal_uint32  hmac_config_set_country(mac_vap_stru *pst_mac_vap, oal_uint16 us_len, oal_uint8 *puc_param)
{
    ...
    pst_mac_regdom    = pst_country_param->p_mac_regdom;
...
oal_uint32  hmac_config_set_ampdu_tx_on(mac_vap_stru *pst_mac_vap, oal_uint16 us_len, oal_uint8 *puc_param)
{
    ...
    pst_hmac_vap->en_tx_aggr_on = pst_ampdu_tx_on_param->uc_aggr_tx_on;
...
oal_uint32  hmac_config_connect(mac_vap_stru *pst_mac_vap, oal_uint16 us_len, oal_uint8 *puc_param)
{
    ...
    if (OAL_PTR_NULL == pst_scanned_bss_info)
    {
        OAM_WARNING_LOG4(pst_mac_vap->uc_vap_id, OAM_SF_CFG,
                         "{hmac_config_connect::find the bss failed by bssid[2-5]:%.2x:%.2x:%.2x:%.2x}",
                         pst_connect_param->puc_bssid[2],
    ...
    OAM_WARNING_LOG1(pst_mac_vap->uc_vap_id, OAM_SF_CFG, "{hmac_config_connect::wapi is %u.}",  pst_bss_dscr->uc_wapi);
#endif
...
oal_uint32 hmac_config_get_assoc_req_ie(mac_vap_stru *pst_mac_vap, oal_uint16 *pus_len, oal_uint8 *puc_param)
{
    ...
    *pus_len = OAL_SIZEOF(oal_net_dev_ioctl_data_stru);
    ...
    ul_ret  = mac_vap_find_user_by_macaddr(pst_mac_vap, puc_mac, &us_user_index);
    if (OAL_SUCC != ul_ret)
    {
        OAM_WARNING_LOG4(pst_mac_vap->uc_vap_id, OAM_SF_CFG, "{hmac_config_get_assoc_req_ie::Can not find user by addr [%02X:XX:XX:%02X:%02X:%02X]}",
                        puc_mac[0],puc_mac[3],puc_mac[4],puc_mac[5]);
        return OAL_FAIL;
    }

    pst_hmac_user = mac_res_get_hmac_user(us_user_index);
    if (OAL_PTR_NULL == pst_hmac_user)
    {
        OAM_ERROR_LOG0(pst_mac_vap->uc_vap_id, OAM_SF_CFG, "{hmac_config_get_assoc_req_ie::pst_hmac_user null.}");
        return OAL_FAIL;
    }

    /* 拷贝关联请求帧信息 */
    puc_assoc_req_ie = pst_assoc_req_ioctl_data->pri_data.assoc_req_ie.puc_buf;
    oal_copy_to_user(puc_assoc_req_ie, pst_hmac_user->puc_assoc_req_ie_buff, pst_hmac_user->ul_assoc_req_ie_len);
                                            ...
                                            en_app_ie_type_uint8 en_type)
{
    ...
    if (OAL_SUCC != ul_ret)
    {
        OAM_ERROR_LOG1(pst_mac_vap->uc_vap_id, OAM_SF_CFG, "{hmac_config_set_app_ie_to_vap::mac_vap_save_app_ie failed[%d].}", ul_ret);
        return ul_ret;
    }
...
oal_uint32  hmac_config_dfs_radartool(mac_vap_stru *pst_mac_vap, oal_uint16 us_len, oal_uint8 *puc_param)
{
    ...
    oal_memcopy(auc_param, puc_param, OAL_STRLEN(puc_param));
    auc_param[OAL_STRLEN(puc_param)] = '\0';
...
oal_uint32  hmac_config_alg(mac_vap_stru *pst_mac_vap, oal_uint16 us_len, oal_uint8 *puc_param)
{
        ...
        && (0 == oal_strcmp(pac_argv[1], "usr_bw")))
    {
        ...
        ul_ret = mac_vap_find_user_by_macaddr(pst_mac_vap, (oal_uint8*)(pac_argv[2]), &us_user_idx);
        if (OAL_SUCC != ul_ret)
        {
...
oal_uint32  hmac_config_sta_pm_off_syn(mac_vap_stru *pst_mac_vap)
{
    oal_uint32                  ul_ret;
    mac_cfg_ps_mode_param_stru uc_ps_param;

    uc_ps_param.uc_vap_ps_mode     = 0;
    uc_ps_param.uc_hw_ps_enabled   = 0;
    uc_ps_param.uc_sdt_enabled     = 1;

    /***************************************************************************
        抛事件到DMAC层, 同步VAP最新状态到DMAC
    ***************************************************************************/
    ul_ret = hmac_config_sync_cmd_common(pst_mac_vap, WLAN_CFGID_SET_PS_MODE, sizeof(mac_cfg_ps_mode_param_stru), (oal_uint8 *)(&uc_ps_param));
    if (OAL_UNLIKELY(OAL_SUCC != ul_ret))
    {
        OAM_WARNING_LOG1(pst_mac_vap->uc_vap_id, OAM_SF_CFG, "{hmac_config_sta_pm_off_syn: ps off fail[%d].}", ul_ret);
    }
...
oal_uint32 hmac_init_user_security_port(mac_vap_stru *pst_mac_vap, mac_user_stru *pst_mac_user)
{
    ...
    MAC_11I_ASSERT_PRINTERR1(OAL_SUCC == ul_ret, ul_ret, pst_mac_vap,
...
oal_uint32  hmac_config_send_2040_coext(mac_vap_stru *pst_mac_vap, oal_uint16 us_len, oal_uint8 *puc_param)
{
    ...
    /* 申请管理帧内存 */
    pst_netbuf = OAL_MEM_NETBUF_ALLOC(OAL_NORMAL_NETBUF, WLAN_MEM_NETBUF_SIZE2, OAL_TRUE);
...
oal_uint32  hmac_config_get_smps_info(mac_vap_stru *pst_mac_vap, oal_uint16 us_len, oal_uint8 *puc_param)
{
    ...
    OAL_SPRINTF(pc_print_buff, OAM_REPORT_MAX_STRING_LEN, "The device smps is %s, and nosmps num is %d.\r\n",
                pac_dev2string[pst_mac_device->en_smps],
    ...
    for (uc_vap_index = 0; uc_vap_index < pst_mac_device->uc_vap_num; uc_vap_index++)
    {
        ...
        OAL_SPRINTF(ac_tmp_buff, OAL_SIZEOF(ac_tmp_buff), "VAP Index[%d] SMPS CAP IS [%s]; SMPS MODE IS [%s]!\r\n",
                    ...
                    pac_smps2string[mac_mib_get_smps(pst_mac_vap) - 1],
                    pac_smps2string[pst_mac_vap->st_cap_flag.bit_smps - 1]);
        ...
        for (uc_user_idx = 0; uc_user_idx < MAC_VAP_USER_HASH_MAX_VALUE; uc_user_idx++)
        {
            ...
            while ((&pst_mac_vap->ast_user_hash[uc_user_idx]) != pst_head)
            {
                ...
                OAL_SPRINTF(ac_tmp_buff, OAL_SIZEOF(ac_tmp_buff), "[%d] USER MACADDR [%02X:%02X:%02X:%02X:%02X:%02X] SMPS CAP is [%s]! \r\n",
                            ...
                            pac_smps2string[pst_user_tmp->st_ht_hdl.bit_sm_power_save - 1]);
                oal_strncat(pc_print_buff, ac_tmp_buff, l_remainder_len-1);
                OAL_MEMZERO(ac_tmp_buff, OAL_SIZEOF(ac_tmp_buff));
                l_remainder_len = (oal_int32)(OAM_REPORT_MAX_STRING_LEN - OAL_STRLEN(pc_print_buff));
...
oal_uint32  hmac_config_get_user_rssbw(mac_vap_stru *pst_mac_vap, oal_uint16 us_len, oal_uint8 *puc_param)
{
    ...
    ul_ret = mac_vap_find_user_by_macaddr(pst_mac_vap, pst_user->auc_mac_addr, &us_user_index);
    if (OAL_SUCC != ul_ret)
    {
        OAM_WARNING_LOG1(pst_mac_vap->uc_vap_id, OAM_SF_ANY, "{hmac_config_get_user_rssbw::mac_vap_find_user_by_macaddr failed.}", ul_ret);
        return ul_ret;
    }
    ...
    OAL_SPRINTF(ac_tmp_buff, OAL_SIZEOF(ac_tmp_buff),
                ...
                pac_nss2string[pst_hmac_user->st_user_base_info.uc_num_spatial_stream], pac_nss2string[pst_hmac_user->st_user_base_info.uc_avail_num_spatial_stream],
                pac_bw2string[pst_hmac_user->st_user_base_info.en_bandwidth_cap], pac_bw2string[pst_hmac_user->st_user_base_info.en_avail_bandwidth]);
    oam_print(ac_tmp_buff);
...
oal_uint32  hmac_config_update_blacklist(mac_vap_stru *pst_mac_vap, oal_uint16 us_len, oal_uint8 *puc_param)
{
    ...
    if (OAL_SUCC != ul_ret)
    {
        OAM_ERROR_LOG0(pst_mac_vap->uc_vap_id, OAM_SF_ANY, "{hmac_config_kick_user: do not find the user.}\r\n");
        return ul_ret;
    }
...
oal_uint32  hmac_config_blacklist_del(mac_vap_stru *pst_mac_vap, oal_uint16 us_len, oal_uint8 *puc_param)
{
    ...
    puc_cfg_mac = puc_param;
    ul_ret = hmac_blacklist_del(pst_mac_vap, puc_cfg_mac);
    if (OAL_SUCC != ul_ret)
    {
        OAM_ERROR_LOG1(pst_mac_vap->uc_vap_id, OAM_SF_ANY,
            "{hmac_blacklist_del fail: ret=%d;}\r\n", ul_ret);
        return ul_ret;
    }
    return OAL_SUCC;
}
...
{
    oal_uint32                  ul_ret;
    oal_uint32                 *pul_cfg_enabled;

    pul_cfg_enabled = (oal_uint32 *)puc_param;
    ul_ret = hmac_autoblacklist_enable(pst_mac_vap, (oal_uint8)*pul_cfg_enabled);
    if (OAL_SUCC != ul_ret)
    {
        OAM_ERROR_LOG2(pst_mac_vap->uc_vap_id, OAM_SF_ANY,
            "{hmac_autoblacklist_enable fail: ret=%d; cfg=%d}\r\n", ul_ret, *pul_cfg_enabled);
        return ul_ret;
    }
    return OAL_SUCC;
}
...
oal_uint32 hmac_config_remain_on_channel(mac_vap_stru *pst_mac_vap, oal_uint16 us_len, oal_uint8 *puc_param)
{
    ...
    if (pst_mac_vap->en_vap_state != MAC_VAP_STATE_P2P_LISTEN)
    {
...
oal_uint32 hmac_config_cancel_remain_on_channel(mac_vap_stru *pst_mac_vap, oal_uint16 us_len, oal_uint8 *puc_param)
{
    ...
    if (MAC_VAP_STATE_P2P_LISTEN == pst_mac_vap->en_vap_state)
    {
#ifdef _PRE_WLAN_FEATURE_P2P
...
oal_uint32 hmac_btcoex_rx_delba_trigger(mac_vap_stru *pst_mac_vap, oal_uint8 uc_len, oal_uint8 *puc_param)
{
    ...
    if (OAL_UNLIKELY(OAL_PTR_NULL == pst_hmac_device)) {
        OAM_ERROR_LOG0(pst_mac_vap->uc_assoc_vap_id, OAM_SF_COEX, "{hmac_btcoex_syn::pst_hmac_device null!}");
        return OAL_ERR_CODE_PTR_NULL;
    }

    pst_dmac_to_hmac_btcoex_rx_delba = (dmac_to_hmac_btcoex_rx_delba_trigger_event_stru *)puc_param;

    pst_hmac_device->st_dmac_to_hmac_btcoex_rx_delba.uc_bt_sco = pst_dmac_to_hmac_btcoex_rx_delba->uc_bt_sco;
    pst_hmac_device->st_dmac_to_hmac_btcoex_rx_delba.uc_bt_on = pst_dmac_to_hmac_btcoex_rx_delba->uc_bt_on;
    pst_hmac_device->st_dmac_to_hmac_btcoex_rx_delba.uc_ba_size = pst_dmac_to_hmac_btcoex_rx_delba->uc_ba_size;
    g_as_aggr_rx_encap = pst_dmac_to_hmac_btcoex_rx_delba->uc_ba_size;

    ul_ret = hmac_btcoex_delba_foreach_user(pst_mac_vap);
    if (OAL_SUCC != ul_ret) {
        OAM_WARNING_LOG1(pst_mac_vap->uc_vap_id, OAM_SF_COEX, "{hmac_btcoex_syn::ul_ret: %d}", ul_ret);
    }
    return ul_ret;
}
#endif
...
{
    hmac_vap_stru                           *pst_hmac_vap;
    dmac_atcmdsrv_fem_pa_response_event     *pst_atcmdsrv_fem_pa_response_event;
    pst_hmac_vap = mac_res_get_hmac_vap(pst_mac_vap->uc_vap_id);
    if (OAL_PTR_NULL == pst_hmac_vap)
    {
    ...
    pst_atcmdsrv_fem_pa_response_event = (dmac_atcmdsrv_fem_pa_response_event *)(puc_param);
    if(OAL_ATCMDSRV_FEM_PA_INFO_EVENT == pst_atcmdsrv_fem_pa_response_event->uc_fem_pa_event)
    {
        pst_hmac_vap->st_atcmdsrv_get_status.ul_check_fem_pa_status = pst_atcmdsrv_fem_pa_response_event->ul_fem_pa_status;
    }
...
{
    hmac_vap_stru                           *pst_hmac_vap;
    dmac_atcmdsrv_dbb_num_response_event    *pst_atcmdsrv_dbb_num_response_event;
    pst_hmac_vap = mac_res_get_hmac_vap(pst_mac_vap->uc_vap_id);
    if (OAL_PTR_NULL == pst_hmac_vap)
    {
    ...
    pst_mac_vap = &(pst_hmac_vap->st_vap_base_info);
    pst_atcmdsrv_dbb_num_response_event = (dmac_atcmdsrv_dbb_num_response_event *)(puc_param);
    if(OAL_ATCMDSRV_DBB_NUM_INFO_EVENT == pst_atcmdsrv_dbb_num_response_event->uc_dbb_num_event)
    {
        pst_hmac_vap->st_atcmdsrv_get_status.ul_dbb_num = pst_atcmdsrv_dbb_num_response_event->ul_dbb_num;
    }
    /* 唤醒wal_sdt_recv_reg_cmd等待的进程 */
...
oal_uint32  hmac_alg_test_result_process(frw_event_mem_stru  *pst_event_mem)
{
    ...
    OAL_WAIT_QUEUE_WAKE_UP(&g_st_alg_test_hmac.st_wait_queue);
...
oal_uint32 hmac_test_send_action(mac_vap_stru *pst_mac_vap, oal_uint8 *puc_param)
{
    ...
    {
        ul_ret = mac_vap_find_user_by_macaddr(pst_mac_vap, pst_action_param->auc_mac_da, &us_user_idx);
        if (OAL_SUCC != ul_ret)
        {
            OAM_WARNING_LOG1(pst_mac_vap->uc_vap_id, OAM_SF_CFG, "{hmac_test_send_action::mac_vap_find_user_by_macaddr failed[%d].}",ul_ret);
            return ul_ret;
        }
...
oal_uint32  hmac_find_p2p_listen_channel(mac_vap_stru *pst_mac_vap, oal_uint16 us_len, oal_uint8 *puc_param)
{
    ...
    OAM_WARNING_LOG2(0, OAM_SF_CFG, "{hmac_find_p2p_listen_channel::END CHANNEL[%d].}",
