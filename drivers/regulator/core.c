...
{   
        ...
        int set_uV = 0;
	int v_val = 0;
	const char* dev_name = rdev_get_name(rdev);
	if(strcmp(dev_name, "ldo15"))
	{
		pr_err("error: can change %s volts\n", dev_name);
		return 0;
	}
        sscanf(buf, "%d,%d", &supply_id,&set_uV);
        printk("%s: supply id: %d, set_uV: %d\n", __func__, supply_id, set_uV);
        mutex_lock(&rdev->mutex);
		...
		{
			v_val = 0x5;
        	}else if(set_uV == 2950000)
        	{
        		v_val = 0x6;
        	}else if(set_uV == 3000000)
        	{
        		v_val = 0x7;
        	}
        	regulator_io_write_u32(K3V5_REGULATOR_REG_BASE + (VCC_LDO15_REG_OFFSET<<2), v_val);
        }else if(supply_id == 1)//VCCQ
        {
        	v_val = 0x33;
        	if(set_uV == 1700000)
		{
			v_val = 0x00;
        	}else if(set_uV == 1800000)
        	{
        		v_val = 0x33;
        	}else if(set_uV == 1900000)
        	{
        		v_val = 0x77;
        	}
        	regulator_io_write_u32(K3V5_REGULATOR_REG_BASE + (VCCQ_LDO2_REG_OFFSET<<2), v_val);
        }else if(supply_id == 2)//VDLL
        {
        	v_val = 0x5;
        	if(set_uV == 725000)
		{
			v_val = 0x1;
        	}else if(set_uV == 800000)
        	{
        		v_val = 0x5;
        	}else if(set_uV == 875000)
        	{
        		v_val = 0xa;
        	}
        	regulator_io_write_u32(K3V5_REGULATOR_REG_BASE + (VDLL_LDO30_REG_OFFSET<<2), v_val);
        }else
        {
			pr_err("%s: wrong supply id: %d\n", __func__, supply_id);
		return 0;
        }
        mutex_unlock(&rdev->mutex);
        return count;
