...
static void hi3630_ppll0_disable(struct clk_hw *hw)
{
	...
	if (!ppll0_clk->ref_cnt) {
		ret = hisi_clkmbox_send_msg(&ppll0_clk->cmd[0], NULL, ASYNC_MSG);
...
static int hi3630_mclk_enable(struct clk_hw *hw)
{
	...
	if (mclk->ref_cnt < 2) {
		ret = hisi_clkmbox_send_msg(&mclk->en_cmd[0], NULL, SYNC_MSG);
		if (ret)
...
static void hi3630_mclk_disable(struct clk_hw *hw)
{
	...
	if (!mclk->ref_cnt) {
		ret = hisi_clkmbox_send_msg(&mclk->dis_cmd[0], NULL, SYNC_MSG);
		if (ret)
