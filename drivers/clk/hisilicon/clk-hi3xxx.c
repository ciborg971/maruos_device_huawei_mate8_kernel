				  ...
				  unsigned long parent_rate)
{
	...
	ret = hisi_clkmbox_send_msg(xfreq_clk->set_rate_cmd, NULL, ASYNC_MSG);
	if (ret < 0) {
...
static int hi3xxx_mclk_prepare(struct clk_hw *hw)
{
	...
	if (mclk->ref_cnt < 2) {
		ret = hisi_clkmbox_send_msg_sync(&mclk->en_cmd[0], NULL, SYNC_MSG);
		if (ret)
...
static void hi3xxx_mclk_unprepare(struct clk_hw *hw)
{
	...
	if (!mclk->ref_cnt) {
		ret = hisi_clkmbox_send_msg_sync(&mclk->dis_cmd[0], NULL, SYNC_MSG);
		if (ret)
