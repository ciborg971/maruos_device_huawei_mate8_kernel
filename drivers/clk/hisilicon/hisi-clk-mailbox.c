...
static void hisi_clkmbox_default_complete(rproc_msg_t *ack_buffer,
...
static int hisi_clkmbox_send_ipc(mbox_msg_t *msg, void *context, int autoack)
{
	...
	do {
		ret = RPROC_ASYNC_SEND(rproc_id, (mbox_msg_t *)msg, LPM3_CMD_LEN, autoack, hisi_clkmbox_default_complete, context);
		loop--;
...
static int hisi_clkmbox_send_ipc_sync(mbox_msg_t *msg, void *context, int autoack)
{
	int loop = MAX_SEND_IPC_TRY;
	int ret, rproc_id;
	...
	/* try again if failing to send */
	do {
		ret = RPROC_SYNC_SEND(rproc_id, (mbox_msg_t *)msg, LPM3_CMD_LEN, autoack, NULL, 0);
		loop--;
	} while (ret == -ENOMEM && loop > 0);
	if (ret) {
		pr_err("[%s] fail to send mbox msg, ret = %d!\n",
...
int hisi_clkmbox_send_msg_sync(mbox_msg_t *msg, void *context, int autoack)
{
	int ret = 0;
	int loop = MAX_SEND_MSG_TRY;
	...
	/* try again when the mail box is full */
	do {
		ret = hisi_clkmbox_send_ipc_sync(msg, context, autoack);
		loop--;
	} while (ret != 0 && loop > 0);
