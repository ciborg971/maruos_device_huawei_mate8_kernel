 ...
 ******************************************************************************
  文 件 名   : soc_seceng_s_interface.h
  版 本 号   : 初稿
  作    者   : Excel2Code
  生成日期   : 2015-03-10 11:22:56
  最近修改   :
  功能描述   : 接口头文件
  函数列表   :
  修改历史   :
  1.日    期   : 2015年3月10日
    作    者   : fWX220878
    修改内容   : 从《Hi3650V100 SOC寄存器手册_SECENG_S.xml》自动生成
...
typedef union
{
    ...
    struct
    {
        ...
        unsigned int  secure_CPU_spniden_en    : 1;  /* bit[3]    : 该bit控制debug等级为证书认证方式（efuse bit2025为0，efuse bit2026为1）下MCPU的spniden信号和M7的niden信号。
                                                                    ...
                                                                    同时写该寄存器的bit3和bit1为1，则M7的niden信号被配置为0x1； */
        unsigned int  switch_CPU_dbgen_en      : 1;  /* bit[4]    : 该bit控制debug等级为证书认证方式（efuse bit2025为0，efuse bit2026为1）下A53和MAIA的dbgen信号：
                                                                    写该bit为1则A53和MAIA的dbgen被配置为0x1；
                                                                    写该bit为0则A53和MAIA的dbgen被配置为0x0。 */
        unsigned int  switch_CPU_niden_en      : 1;  /* bit[5]    : 该bit控制debug等级为证书认证方式（efuse bit2025为0，efuse bit2026为1）下A53和MAIA以及CCI400的niden信号：
                                                                    写该bit为1则A53和MAIA以及CCI400的niden被配置为0x1；
                                                                    写该bit为0则A53和MAIA以及CCI400的niden被配置为0x0。 */
        unsigned int  switch_CPU_spiden_en     : 1;  /* bit[6]    : 该bit控制debug等级为证书认证方式（efuse bit2025为0，efuse bit2026为1）下A53和MAIA的spiden信号：
                                                                    写该bit为1则A53和MAIA的spiden被配置为0x1；
                                                                    写该bit为0则A53和MAIA的spiden被配置为0x0。 */
        unsigned int  switch_CPU_spniden_en    : 1;  /* bit[7]    : 该bit控制debug等级为证书认证方式（efuse bit2025为0，efuse bit2026为1）下A53和MAIA以及CCI400的spniden信号：
                                                                    写该bit为1则A53和MAIA以及CCI400的spniden被配置为0x1；
                                                                    写该bit为0则A53和MAIA以及CCI400的spniden被配置为0x0。 */
        ...
        unsigned int  reserved_0               : 5;  /* bit[12-16]: 保留 */
        unsigned int  ivp_dsp_dbgen_en         : 1;  /* bit[17]   : 该bit控制debug等级为证书认证方式（efuse bit2025为0，efuse bit2026为1）下IVP DSP的dbgen信号：
        ...
        unsigned int  reserved_1               : 6;  /* bit[25-30]: 保留 */
