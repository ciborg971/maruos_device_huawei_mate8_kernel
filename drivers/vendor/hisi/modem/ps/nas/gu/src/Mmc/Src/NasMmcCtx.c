...
VOS_UINT32 NAS_MMC_GetNextHistoryTimerLen(VOS_VOID)
{
    ...
    /* 建议available时长是history时长的整数倍，如果不是整数倍时，最后一次history定时器时长取剩余时长
       例: history第二阶段定时器时长为20s, available第二阶段定时器时长为65s
           在65s周期内启动定时器总次数为(65/20 = 3)次
           第一次定时器时长为history第二阶段定时器时长:20s
           第二次定时器时长为history第二阶段定时器时长:20s
           第三次定时器时长为剩余时长:(65-2*20) = 25s
    */

    /* History全局变量中存的以s为单位，将其转换成以毫秒为单位 */
...
VOS_UINT8 NAS_MMC_IsNeedStartHistoryTimer(VOS_VOID)
{
    ...
    NAS_MML_AVAIL_TIMER_CFG_STRU       *pstAvailTimerCfg    = VOS_NULL_PTR;
    /* Deleted by c00318887 for 预置频点搜网优化, 2015-9-10, begin */
    /* Deleted by c00318887 for 预置频点搜网优化, 2015-9-10, end */
    VOS_UINT32                          ulCount;
    VOS_UINT32                          ulAvailableSearchTimeLen;
    VOS_UINT32                          ulHistoryTimerLen;
    ...
    /* history搜 active标志为false，不需要启动history搜定时器 */
    ...
    if (ulCount < pstAvailTimerCfg->ulFirstSearchTimeCount)
    {
        /* 获取第一阶段available时间间隔和history时间间隔 */
        ulAvailableSearchTimeLen = (pstAvailTimerCfg->ulFirstSearchTimeLen);
        ulHistoryTimerLen        = pstHistoryinfo->ulFirstTimerLen;
    }
    else
    {
        /* 获取第二阶段available时间间隔和history时间间隔 */
        ulAvailableSearchTimeLen = (pstAvailTimerCfg->ulDeepSearchTimeLen);
        ulHistoryTimerLen        = pstHistoryinfo->ulSecondTimerLen;
    }

    /* 定时器时长为0，不需要启动history搜定时器 */
    if (0 == ulHistoryTimerLen)
    {
        return VOS_FALSE;
    }

    /* 该阶段available时长 >= 2*history搜时长 */
