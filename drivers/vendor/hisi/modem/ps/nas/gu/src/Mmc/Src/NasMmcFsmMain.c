...
VOS_UINT32 NAS_MMC_ProcTiAvailableTimerExpired_NormalSearch_L1Main(VOS_VOID)
{
    ...
    /* 如果available定时器启动时是ncell类型或history类型，
       available超时时不满足ncell搜或不满足history搜做普通指定搜时需要把普通available timer计数器加1 */
...
)
{
    ...
    /* 产品线问题单 AnyCampon转Suitable campon时，不停止available timer */
...
VOS_UINT8  NAS_MMC_IsNeedHistorySearch_L1Main(VOS_VOID)
{
    ...
    /* history搜 active标志为false，不需要history搜 */
    ...
    /* 当前hitory启动次数 < (available时长/history时长)，需要history搜 */
