...
)
{
    ...
    ulDeepTimerLen = (pstAvailTimerCfg->ulDeepSearchTimeLen) * NAS_MML_ONE_THOUSAND_MILLISECOND;
...
/*****************************************************************************
Function Name   :   NAS_MMC_SndMsccBgSearchCnf
Description     :   Send message MMC_MSCC_BG_SEARCH_CNF to MSCC.
Input parameters:   MMC_LMM_BG_SRCH_RLT_ENUM_UINT32 enRlst
...
/*****************************************************************************
Function Name   :   NAS_MMC_SndMsccStopBgSearchCnf
Description     :   Send message MMC_MSCC_STOP_BG_SEARCH_CNF to MSCC.
...
{
    MMC_MSCC_REG_REJ_PLMN_SELE_IND      *pstRegRejPlmnSeleInd = VOS_NULL_PTR;

    pstRegRejPlmnSeleInd = (MMC_MSCC_REG_REJ_PLMN_SELE_IND *)PS_ALLOC_MSG(WUEPS_PID_MMC,
                         sizeof(MMC_MSCC_REG_REJ_PLMN_SELE_IND) - VOS_MSG_HEAD_LENGTH);

    if( VOS_NULL_PTR == pstRegRejPlmnSeleInd )
    {
        NAS_WARNING_LOG(WUEPS_PID_MMC, "NAS_MMC_SndMsccRegRejPlmnSeleInd():ERROR:Memory Alloc Error for pstRegRejPlmnSeleInd ");
        return;
    }

    PS_MEM_SET(((VOS_UINT8*)pstRegRejPlmnSeleInd) + VOS_MSG_HEAD_LENGTH, 0, sizeof(MMC_MSCC_REG_REJ_PLMN_SELE_IND) - VOS_MSG_HEAD_LENGTH);

    /* 填充消息头 */
    pstRegRejPlmnSeleInd->stMsgHeader.ulMsgName         = ID_MMC_MSCC_REG_REJ_PLMN_SELE_IND;
    pstRegRejPlmnSeleInd->stMsgHeader.ulSenderCpuId     = VOS_LOCAL_CPUID;
    pstRegRejPlmnSeleInd->stMsgHeader.ulSenderPid       = WUEPS_PID_MMC;
    pstRegRejPlmnSeleInd->stMsgHeader.ulReceiverCpuId   = VOS_LOCAL_CPUID;
    pstRegRejPlmnSeleInd->stMsgHeader.ulReceiverPid     = UEPS_PID_MSCC;
    pstRegRejPlmnSeleInd->stMsgHeader.ulLength          = sizeof(MMC_MSCC_REG_REJ_PLMN_SELE_IND) - VOS_MSG_HEAD_LENGTH;
    pstRegRejPlmnSeleInd->ucPlmnSeleFlg               = ucPlmnSeleFlg;

    PS_SEND_MSG(WUEPS_PID_MMC, pstRegRejPlmnSeleInd);
