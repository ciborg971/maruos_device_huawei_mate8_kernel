...
typedef struct tagCKP_BACKUP_PPPALLINFO_S
{
    ...
    USHORT usDynPeerId;                /* 保存第一次发送服务器id */
    UCHAR ucLcpPktId;
    UCHAR ucIpcpPktId;
    UCHAR ucAAAType;
    UCHAR aucRvs;
...
extern ULONG g_ulDmpuMppcUser[MAX_DMPU_NUM]; /* 子卡MPPC用户数 */
extern ULONG g_ulDmpuVjUser[MAX_DMPU_NUM];   /* 子卡VJ用户数 */
extern ULONG g_ulPppSubBordStat[MAX_DMPU_NUM]; /* 子卡状态 */
