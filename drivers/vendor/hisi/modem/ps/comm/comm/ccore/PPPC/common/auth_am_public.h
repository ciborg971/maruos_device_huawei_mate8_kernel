...
typedef struct
{
    ...
    ULONG ulAAAindex;
    ULONG ulInputPeakRate;      /* 上行峰值速率, 单位为bps*/
    ULONG ulOutputPeakRate;     /* 下行峰值速率, 单位为bps*/
    ULONG ulGuarantBitRateUp;   /* PDSN用户上行保证速率 (bps)*/
    ULONG ulGuarantBitRateDown; /* PDSN用户下行保证速率(bps) */
    UCHAR ucRateFlag;
...
typedef struct tagRD_MIPAGENT_VSA_S
{
    ...
    USHORT                 usMipAgentLifetime;   /* LNS地址 */
...
typedef struct tagAaaAuthRspMsg
{
    ...
    ULONG ulIdleTimeout;            /* 会话超时时间 */
    ULONG ulIPMTU;                  /* MTU参数 */
    ULONG ulIp;                     /* IP地址 */
    ULONG ulIpMask;                 /* IP地址子网掩码 */
    ULONG ulPIMARY_DNS_SERVER;      /* 主DNS SERVER */
    ULONG ulSECOND_DNS_SERVER;      /* 从DNS SERVER */
    ULONG ulAlwaysOn;               /*r002*/
    ULONG ulMNAAAFlag;
    ULONG ulAcctInterimInterval;
     /* L2TP */
    USHORT usTunnelType[AAA_L2TP_LNS_NUM];                 /* 隧道类型，目前只支持L2TP */
    USHORT usTunnelMediaType[AAA_L2TP_LNS_NUM];            /* 公共网类型，目前只支持IP */
    ULONG  ulTunnelServer[AAA_L2TP_LNS_NUM];                   /* LNS地址 */
    ...
    ULONG  ulMipAgentHaAddr;
    ULONG  ulMipAgentHomeAddr;
    ULONG  ulMipAgentSpi;                       /* PMIPv4用户与HA间安全联盟SPI */
    ULONG  ulPmipV6Spi;                         /* PMIPv6用户与LMA间安全联盟SPI */
