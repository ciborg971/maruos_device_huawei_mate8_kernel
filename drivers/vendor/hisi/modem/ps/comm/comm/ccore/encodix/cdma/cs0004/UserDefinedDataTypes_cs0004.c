...
#else
		int i=0, max=0;
		ED_WARNING_REMOVER (i);
		ED_WARNING_REMOVER (max);
#ifdef ED_THIS
#undef ED_THIS
#endif
#define ED_THIS Source
		...
		for (i=0; i<Source->SDUs.items; i++) {
			CurrOfs += ENCODE_c_FCSCH_PDU_SDUs_data (Buffer, CurrOfs, Source->SDUs.data [i], pp_CS0004E_Details);
		}
		/* 3.1.2.3.1.4.1 */
...
#define ED_THIS Destin
		...
		/* C.S0004 3.1.2.3.1.1 */
		bits = 2;
		if (LastOfs - (int)CurrOfs < bits) {
			bits = ED_MESSAGE_TOO_SHORT;
		} 
		else {
			ED_LOCATOR_SET (&(Destin->PD___LOCATOR), CurrOfs, bits);
			Destin->PD = (ED_OCTET)EDBitsToInt (Buffer, CURPOS, (ED_EXLONG)bits);
			/* Set the exported tag */
			pp_CS0004E_Details->userTagPresent[1] = 1;
		...
		if (LastOfs - (int)CurrOfs < bits) {
			...
			ED_LOCATOR_SET (&(Destin->MSG_ID___LOCATOR), CurrOfs, bits);
			Destin->MSG_ID = (ED_OCTET)EDBitsToInt (Buffer, CURPOS, (ED_EXLONG)bits);
			/* Set the exported tag */
			pp_CS0004E_Details->userTagPresent[2] = 1;
		...
		for (i=0; ; i++) {
			...
			bits = DECODE_c_FCSCH_PDU_SDUs_data (Buffer, CurrOfs, Destin->SDUs.data [i], LastOfs-CurrOfs , pp_CS0004E_Details);
			if (bits < 0) {
				/* Decoding failed - Remove the unused array items */
				SETITEMS_c_FCSCH_PDU_SDUs (&Destin->SDUs, i);
				goto exit_0;
			}
			CurrOfs += bits;
		}
		...
		ED_SET_DECODE_BITS (CurrOfs-BitOffset);
#endif
		return (ED_EXLONG)(CurrOfs-BitOffset);
...
#define ED_THIS Destin
		...
		if (LastOfs - (int)CurrOfs < bits) {
			...
			ED_LOCATOR_SET (&(Destin->ACK_SEQ___LOCATOR), CurrOfs, bits);
			Destin->ACK_SEQ = (ED_OCTET)EDBitsToInt (Buffer, CURPOS, (ED_EXLONG)bits);
		}
		if (bits < 0) {
			/* Decoding failed - Go to the exit point */
			goto exitOnFail;
		...
		if (LastOfs - (int)CurrOfs < bits) {
			...
			ED_LOCATOR_SET (&(Destin->MSG_SEQ___LOCATOR), CurrOfs, bits);
			Destin->MSG_SEQ = (ED_OCTET)EDBitsToInt (Buffer, CURPOS, (ED_EXLONG)bits);
		}
		if (bits < 0) {
			/* Decoding failed - Go to the exit point */
			goto exitOnFail;
		...
		if (LastOfs - (int)CurrOfs < bits) {
			...
			ED_LOCATOR_SET (&(Destin->ACK_REQ___LOCATOR), CurrOfs, bits);
			Destin->ACK_REQ = (ED_BOOLEAN)EDBitsToInt (Buffer, CURPOS, (ED_EXLONG)bits);
		}
		if (bits < 0) {
			/* Decoding failed - Go to the exit point */
			goto exitOnFail;
		...
		if (LastOfs - (int)CurrOfs < bits) {
			...
			ED_LOCATOR_SET (&(Destin->VALID_ACK___LOCATOR), CurrOfs, bits);
			Destin->VALID_ACK = (ED_BOOLEAN)EDBitsToInt (Buffer, CURPOS, (ED_EXLONG)bits);
		}
		if (bits < 0) {
			/* Decoding failed - Go to the exit point */
			goto exitOnFail;
		...
		if (LastOfs - (int)CurrOfs < bits) {
			...
			ED_LOCATOR_SET (&(Destin->ADDR_TYPE___LOCATOR), CurrOfs, bits);
			Destin->ADDR_TYPE = (ED_OCTET)EDBitsToInt (Buffer, CURPOS, (ED_EXLONG)bits);
		}
		if (bits < 0) {
			/* Decoding failed - Go to the exit point */
			goto exitOnFail;
		...
		if (LastOfs - (int)CurrOfs < bits) {
			...
			ED_LOCATOR_SET (&(Destin->ADDR_LEN___LOCATOR), CurrOfs, bits);
			Destin->ADDR_LEN = (ED_OCTET)EDBitsToInt (Buffer, CURPOS, (ED_EXLONG)bits);
			/* Set the exported tag */
			pp_CS0004E_Details->userTagPresent[0] = 1;
		...
		/* BEGIN OF PAD ZONE - DECODE inclusive=0 bits=4 octet(=0)/bit(=1)=0 */
		{
			...
			if ((Destin->ADDR_TYPE==4)) {
				...
				if (LastOfs - (int)CurrOfs < bits) {
					...
					ED_LOCATOR_SET (&(Destin->EXT_ADDR_TYPE___LOCATOR), CurrOfs, bits);
					Destin->EXT_ADDR_TYPE = (ED_OCTET)EDBitsToInt (Buffer, CURPOS, (ED_EXLONG)bits);
				}
				if (bits < 0) {
			...
			if ((Destin->ADDR_TYPE==0)) {
				...
				bits = DECODE_c_FCSCH_PDU_SDUs_data_IMSI_S (Buffer, CurrOfs, Destin->IMSI_S, LastOfs-CurrOfs , pp_CS0004E_Details);
				if (bits < 0) {
					/* If decoding failed, set the failed field to "not present" */
			...
			if ((Destin->ADDR_TYPE==1)) {
				...
				bits = DECODE_c_FCSCH_PDU_SDUs_data_ESN (Buffer, CurrOfs, Destin->ESN, LastOfs-CurrOfs , pp_CS0004E_Details);
				if (bits < 0) {
					/* If decoding failed, set the failed field to "not present" */
			...
			if ((Destin->ADDR_TYPE==2)) {
				...
				bits = DECODE_c_FCSCH_PDU_SDUs_data_IMSI (Buffer, CurrOfs, Destin->IMSI, LastOfs-CurrOfs , pp_CS0004E_Details);
				if (bits < 0) {
					/* If decoding failed, set the failed field to "not present" */
			...
			if ((Destin->ADDR_TYPE==3)) {
				...
				bits = DECODE_c_FCSCH_PDU_SDUs_data_TMSI (Buffer, CurrOfs, Destin->TMSI, LastOfs-CurrOfs , pp_CS0004E_Details);
				if (bits < 0) {
					/* If decoding failed, set the failed field to "not present" */
			...
			if ((Destin->ADDR_TYPE==4)) {
				...
				bits = DECODE_c_FCSCH_PDU_SDUs_data_Extended_Address (Buffer, CurrOfs, Destin->Extended_Address, LastOfs-CurrOfs , pp_CS0004E_Details);
				if (bits < 0) {
					/* If decoding failed, set the failed field to "not present" */
			...
			if ((Destin->ADDR_TYPE==5)) {
				...
				bits = DECODE_c_FCSCH_PDU_SDUs_data_BROADCAST (Buffer, CurrOfs, Destin->BROADCAST, LastOfs-CurrOfs , pp_CS0004E_Details);
				if (bits < 0) {
					/* If decoding failed, set the failed field to "not present" */
		...
		if (((pp_CS0004E_Details->n_P_REV_IN_USEs>=9)&&((pp_CS0004E_Details->userTag[1])==2))) {
			...
			if (LastOfs - (int)CurrOfs < bits) {
				...
				ED_LOCATOR_SET (&(Destin->MACI_INCL___LOCATOR), CurrOfs, bits);
				Destin->MACI_INCL = (ED_BOOLEAN)EDBitsToInt (Buffer, CURPOS, (ED_EXLONG)bits);
			}
			if (bits < 0) {
				/* If decoding failed, set the failed field to "not present" */
		...
		if (((pp_CS0004E_Details->n_P_REV_IN_USEs>=9)&&(Destin->MACI_INCL_Present ? Destin->MACI_INCL : 0))) {
			...
			if (LastOfs - (int)CurrOfs < bits) {
				...
				ED_LOCATOR_SET (&(Destin->SDU_KEY_ID___LOCATOR), CurrOfs, bits);
				Destin->SDU_KEY_ID = (ED_OCTET)EDBitsToInt (Buffer, CURPOS, (ED_EXLONG)bits);
			}
			if (bits < 0) {
				/* If decoding failed, set the failed field to "not present" */
		...
		if (((pp_CS0004E_Details->n_P_REV_IN_USEs>=9)&&(Destin->MACI_INCL_Present ? Destin->MACI_INCL : 0))) {
			...
			if (LastOfs - (int)CurrOfs < bits) {
				...
				ED_LOCATOR_SET (&(Destin->SDU_INTEGRITY_ALGO___LOCATOR), CurrOfs, bits);
				Destin->SDU_INTEGRITY_ALGO = (ED_OCTET)EDBitsToInt (Buffer, CURPOS, (ED_EXLONG)bits);
			}
			if (bits < 0) {
				/* If decoding failed, set the failed field to "not present" */
		...
		if (((pp_CS0004E_Details->n_P_REV_IN_USEs>=9)&&(Destin->MACI_INCL_Present ? Destin->MACI_INCL : 0))) {
			...
			if (LastOfs - (int)CurrOfs < bits) {
				...
				ED_LOCATOR_SET (&(Destin->SDU_SSEQ_OR_SSEQH___LOCATOR), CurrOfs, bits);
				Destin->SDU_SSEQ_OR_SSEQH = (ED_BOOLEAN)EDBitsToInt (Buffer, CURPOS, (ED_EXLONG)bits);
			}
			if (bits < 0) {
				/* If decoding failed, set the failed field to "not present" */
		...
		if ((((pp_CS0004E_Details->n_P_REV_IN_USEs>=9)&&(Destin->MACI_INCL_Present ? Destin->MACI_INCL : 0))&&!((Destin->SDU_SSEQ_OR_SSEQH_Present ? Destin->SDU_SSEQ_OR_SSEQH : 0)))) {
			...
			if (LastOfs - (int)CurrOfs < bits) {
				...
				ED_LOCATOR_SET (&(Destin->SDU_SSEQ___LOCATOR), CurrOfs, bits);
				Destin->SDU_SSEQ = (ED_OCTET)EDBitsToInt (Buffer, CURPOS, (ED_EXLONG)bits);
			}
			if (bits < 0) {
				/* If decoding failed, set the failed field to "not present" */
		...
		if ((((pp_CS0004E_Details->n_P_REV_IN_USEs>=9)&&(Destin->MACI_INCL_Present ? Destin->MACI_INCL : 0))&&(Destin->SDU_SSEQ_OR_SSEQH_Present ? Destin->SDU_SSEQ_OR_SSEQH : 0))) {
			...
			if (LastOfs - (int)CurrOfs < bits) {
				...
				ED_LOCATOR_SET (&(Destin->SDU_SSEQ_H___LOCATOR), CurrOfs, bits);
				Destin->SDU_SSEQ_H = (ED_LONG)EDBitsToInt (Buffer, CURPOS, (ED_EXLONG)bits);
			}
			if (bits < 0) {
				/* If decoding failed, set the failed field to "not present" */
		...
		if (((pp_CS0004E_Details->n_P_REV_IN_USEs>=7)&&((pp_CS0004E_Details->userTag[1])!=0))) {
			...
			if (LastOfs - (int)CurrOfs < bits) {
				...
				ED_LOCATOR_SET (&(Destin->ENC_FIELDS_INCL___LOCATOR), CurrOfs, bits);
				Destin->ENC_FIELDS_INCL = (ED_BOOLEAN)EDBitsToInt (Buffer, CURPOS, (ED_EXLONG)bits);
			}
			if (bits < 0) {
				/* If decoding failed, set the failed field to "not present" */
		...
		if ((Destin->ENC_FIELDS_INCL_Present ? Destin->ENC_FIELDS_INCL : 0)) {
			...
			if (LastOfs - (int)CurrOfs < bits) {
				...
				ED_LOCATOR_SET (&(Destin->SDU_ENCRYPT_MODE___LOCATOR), CurrOfs, bits);
				Destin->SDU_ENCRYPT_MODE = (ED_OCTET)EDBitsToInt (Buffer, CURPOS, (ED_EXLONG)bits);
			}
			if (bits < 0) {
				/* If decoding failed, set the failed field to "not present" */
		...
		if (((Destin->SDU_ENCRYPT_MODE_Present ? Destin->SDU_ENCRYPT_MODE : 0)==1)) {
			...
			if (LastOfs - (int)CurrOfs < bits) {
				...
				ED_LOCATOR_SET (&(Destin->ENC_SEQ___LOCATOR), CurrOfs, bits);
				Destin->ENC_SEQ = (ED_OCTET)EDBitsToInt (Buffer, CURPOS, (ED_EXLONG)bits);
			}
			if (bits < 0) {
				/* If decoding failed, set the failed field to "not present" */
		...
		if (LastOfs - (int)CurrOfs < bits) {
			...
			ED_LOCATOR_SET (&(Destin->SDU___LOCATOR), CurrOfs, bits);
			EDAllocBinary (&(((&(Destin->SDU)))->value), (unsigned)(bits), &(((&(Destin->SDU)))->usedBits));
			EDCopyBits (((&(Destin->SDU))->value), 0, Buffer, CURPOS, (ED_EXLONG)bits);
		}
		if (bits < 0) {
			/* Decoding failed - Go to the exit point */
			goto exitOnFail;
		...
		if ((((SDU_LENGTH >= 0))&&(Destin->MACI_INCL_Present ? Destin->MACI_INCL : 0))) {
			...
			if (LastOfs - (int)CurrOfs < bits) {
				...
				ED_LOCATOR_SET (&(Destin->MACI___LOCATOR), CurrOfs, bits);
				Destin->MACI = (ED_LONG)EDBitsToInt (Buffer, CURPOS, (ED_EXLONG)bits);
			}
			if (bits < 0) {
				/* If decoding failed, set the failed field to "not present" */
		...
		if ((SDU_PROCESSED==1)) {
			...
			if (LastOfs - (int)CurrOfs < bits) {
				...
				ED_LOCATOR_SET (&(Destin->ADD_RECORD_LEN___LOCATOR), CurrOfs, bits);
				Destin->ADD_RECORD_LEN = (ED_OCTET)EDBitsToInt (Buffer, CURPOS, (ED_EXLONG)bits);
			}
			if (bits < 0) {
				/* If decoding failed, set the failed field to "not present" */
		...
		bits = (((SDU_PROCESSED==1)) ? ((8*(Destin->ADD_RECORD_LEN_Present ? Destin->ADD_RECORD_LEN : 0))) : (0));
		if (LastOfs - (int)CurrOfs < bits) {
			bits = ED_MESSAGE_TOO_SHORT;
		} 
		else {
...
#endif
#define ED_THIS Destin
		bits = (8*((pp_CS0004E_Details->userTag[0])-1));
		if (LastOfs - (int)CurrOfs < bits) {
			bits = ED_MESSAGE_TOO_SHORT;
		} 
		else {
			ED_LOCATOR_SET (&(Destin->MEID___LOCATOR), CurrOfs, bits);
			EDAllocBinary (&(((&(Destin->MEID)))->value), (unsigned)(bits), &(((&(Destin->MEID)))->usedBits));
			EDCopyBits (((&(Destin->MEID))->value), 0, Buffer, CURPOS, (ED_EXLONG)bits);
		}
		if (bits < 0) {
			/* Decoding failed - Go to the exit point */
			goto exitOnFail;
...
#define ED_THIS Destin
		...
		if (LastOfs - (int)CurrOfs < bits) {
			...
			ED_LOCATOR_SET (&(Destin->IMSI_M_S1___LOCATOR), CurrOfs, bits);
			Destin->IMSI_M_S1 = (ED_LONG)EDBitsToInt (Buffer, CURPOS, (ED_EXLONG)bits);
		}
		if (bits < 0) {
			/* Decoding failed - Go to the exit point */
			goto exitOnFail;
		...
		if (LastOfs - (int)CurrOfs < bits) {
			...
			ED_LOCATOR_SET (&(Destin->IMSI_M_S2___LOCATOR), CurrOfs, bits);
			Destin->IMSI_M_S2 = (ED_LONG)EDBitsToInt (Buffer, CURPOS, (ED_EXLONG)bits);
		}
		if (bits < 0) {
			/* Decoding failed - Go to the exit point */
			goto exitOnFail;
...
#endif
#define ED_THIS Destin
		bits = (8*(pp_CS0004E_Details->userTag[0]));
		if (LastOfs - (int)CurrOfs < bits) {
			bits = ED_MESSAGE_TOO_SHORT;
		} 
		else {
			ED_LOCATOR_SET (&(Destin->ESN___LOCATOR), CurrOfs, bits);
			EDAllocBinary (&(((&(Destin->ESN)))->value), (unsigned)(bits), &(((&(Destin->ESN)))->usedBits));
			EDCopyBits (((&(Destin->ESN))->value), 0, Buffer, CURPOS, (ED_EXLONG)bits);
		}
		if (bits < 0) {
			/* Decoding failed - Go to the exit point */
			goto exitOnFail;
...
#define ED_THIS Destin
		...
		if (LastOfs - (int)CurrOfs < bits) {
			...
			ED_LOCATOR_SET (&(Destin->IMSI_CLASS___LOCATOR), CurrOfs, bits);
			Destin->IMSI_CLASS = (ED_BOOLEAN)EDBitsToInt (Buffer, CURPOS, (ED_EXLONG)bits);
		}
		if (bits < 0) {
			/* Decoding failed - Go to the exit point */
			goto exitOnFail;
		...
		if ((Destin->IMSI_CLASS==0)) {
			...
			bits = DECODE_c_FCSCH_PDU_SDUs_data_IMSI_IMSI_CLASS_0 (Buffer, CurrOfs, Destin->IMSI_CLASS_0, LastOfs-CurrOfs , pp_CS0004E_Details);
			if (bits < 0) {
				/* If decoding failed, set the failed field to "not present" */
		...
		if ((Destin->IMSI_CLASS==1)) {
			...
			bits = DECODE_c_FCSCH_PDU_SDUs_data_IMSI_IMSI_CLASS_1 (Buffer, CurrOfs, Destin->IMSI_CLASS_1, LastOfs-CurrOfs , pp_CS0004E_Details);
			if (bits < 0) {
				/* If decoding failed, set the failed field to "not present" */
...
#define ED_THIS Destin
		...
		if (LastOfs - (int)CurrOfs < bits) {
			...
			ED_LOCATOR_SET (&(Destin->IMSI_CLASS_0_TYPE___LOCATOR), CurrOfs, bits);
			Destin->IMSI_CLASS_0_TYPE = (ED_OCTET)EDBitsToInt (Buffer, CURPOS, (ED_EXLONG)bits);
		}
		if (bits < 0) {
			/* Decoding failed - Go to the exit point */
			goto exitOnFail;
		...
		CurrOfs += bits;
		/* Decode spare bit(s) */
		bits = (((Destin->IMSI_CLASS_0_TYPE==0)) ? (3) : ((((Destin->IMSI_CLASS_0_TYPE==1)) ? (4) : ((((Destin->IMSI_CLASS_0_TYPE==2)) ? (1) : (2))))));
		if (LastOfs-CurrOfs < bits) {
			bits = ED_MESSAGE_TOO_SHORT;
			goto exitOnFail;
		}
		...
		if (((Destin->IMSI_CLASS_0_TYPE==2)||(Destin->IMSI_CLASS_0_TYPE==3))) {
			...
			if (LastOfs - (int)CurrOfs < bits) {
				...
				ED_LOCATOR_SET (&(Destin->MCC10___LOCATOR), CurrOfs, bits);
				Destin->MCC10 = (ED_LONG)EDBitsToInt (Buffer, CURPOS, (ED_EXLONG)bits);
			}
			if (bits < 0) {
				/* If decoding failed, set the failed field to "not present" */
		...
		if (((Destin->IMSI_CLASS_0_TYPE==1)||(Destin->IMSI_CLASS_0_TYPE==3))) {
			...
			if (LastOfs - (int)CurrOfs < bits) {
				...
				ED_LOCATOR_SET (&(Destin->IMSI_11_12___LOCATOR), CurrOfs, bits);
				Destin->IMSI_11_12 = (ED_OCTET)EDBitsToInt (Buffer, CURPOS, (ED_EXLONG)bits);
			}
			if (bits < 0) {
				/* If decoding failed, set the failed field to "not present" */
		...
		if (LastOfs - (int)CurrOfs < bits) {
			...
			ED_LOCATOR_SET (&(Destin->IMSI_S___LOCATOR), CurrOfs, bits);
			EDCopyBits (Destin->IMSI_S, 0, Buffer, CURPOS, (ED_EXLONG)bits);
		}
		if (bits < 0) {
			/* Decoding failed - Go to the exit point */
			goto exitOnFail;
...
#define ED_THIS Destin
		...
		if (LastOfs - (int)CurrOfs < bits) {
			...
			ED_LOCATOR_SET (&(Destin->IMSI_CLASS_1_TYPE___LOCATOR), CurrOfs, bits);
			Destin->IMSI_CLASS_1_TYPE = (ED_BOOLEAN)EDBitsToInt (Buffer, CURPOS, (ED_EXLONG)bits);
		}
		if (bits < 0) {
			/* Decoding failed - Go to the exit point */
			goto exitOnFail;
		...
		if (LastOfs - (int)CurrOfs < bits) {
			...
			ED_LOCATOR_SET (&(Destin->IMSI_ADDR_NUM___LOCATOR), CurrOfs, bits);
			Destin->IMSI_ADDR_NUM = (ED_OCTET)EDBitsToInt (Buffer, CURPOS, (ED_EXLONG)bits);
		}
		if (bits < 0) {
			/* Decoding failed - Go to the exit point */
			goto exitOnFail;
		...
		if ((Destin->IMSI_CLASS_1_TYPE==1)) {
			...
			if (LastOfs - (int)CurrOfs < bits) {
				...
				ED_LOCATOR_SET (&(Destin->MCC___LOCATOR), CurrOfs, bits);
				Destin->MCC = (ED_LONG)EDBitsToInt (Buffer, CURPOS, (ED_EXLONG)bits);
			}
			if (bits < 0) {
				/* If decoding failed, set the failed field to "not present" */
...
#define ED_THIS Destin
		/* If ADDR_LEN is greater than four, 8 * (ADDR_LEN - 4); otherwise, 0.		 */
		bits = ((((pp_CS0004E_Details->userTag[0])>4)) ? ((8*((pp_CS0004E_Details->userTag[0])-4))) : (0));
		if (LastOfs - (int)CurrOfs < bits) {
			bits = ED_MESSAGE_TOO_SHORT;
		} 
		else {
			ED_LOCATOR_SET (&(Destin->TMSI_ZONE___LOCATOR), CurrOfs, bits);
			EDAllocBinary (&(((&(Destin->TMSI_ZONE)))->value), (unsigned)(bits), &(((&(Destin->TMSI_ZONE)))->usedBits));
			EDCopyBits (((&(Destin->TMSI_ZONE))->value), 0, Buffer, CURPOS, (ED_EXLONG)bits);
		}
		if (bits < 0) {
			/* Decoding failed - Go to the exit point */
			goto exitOnFail;
		...
		CurrOfs += bits;
		/* If ADDR_LEN is greater than four, 32; otherwise, 8 * ADDR_LEN. */
		bits = ((((pp_CS0004E_Details->userTag[0])>4)) ? (32) : ((8*(pp_CS0004E_Details->userTag[0]))));
		if (LastOfs - (int)CurrOfs < bits) {
			bits = ED_MESSAGE_TOO_SHORT;
		} 
		else {
			ED_LOCATOR_SET (&(Destin->TMSI_CODE_ADDR___LOCATOR), CurrOfs, bits);
			EDAllocBinary (&(((&(Destin->TMSI_CODE_ADDR)))->value), (unsigned)(bits), &(((&(Destin->TMSI_CODE_ADDR)))->usedBits));
			EDCopyBits (((&(Destin->TMSI_CODE_ADDR))->value), 0, Buffer, CURPOS, (ED_EXLONG)bits);
		}
		if (bits < 0) {
			/* Decoding failed - Go to the exit point */
			goto exitOnFail;
...
#define ED_THIS Destin
		...
		if (LastOfs - (int)CurrOfs < bits) {
			...
			ED_LOCATOR_SET (&(Destin->BC_ADDR___LOCATOR), CurrOfs, bits);
			EDAllocBinary (&(((&(Destin->BC_ADDR)))->value), (unsigned)(bits), &(((&(Destin->BC_ADDR)))->usedBits));
			EDCopyBits (((&(Destin->BC_ADDR))->value), 0, Buffer, CURPOS, (ED_EXLONG)bits);
		}
		if (bits < 0) {
			/* Decoding failed - Go to the exit point */
			goto exitOnFail;
...
#define ED_THIS Destin
		...
		/* BEGIN OF PAD ZONE - DECODE inclusive=0 bits=4 octet(=0)/bit(=1)=0 */
		{
			...
			if ((Destin->ADDR_TYPE==0)) {
				...
				bits = DECODE_c_FCSCH_NON_PRIMARY_BROADCAST_PDU_IMSI_S (Buffer, CurrOfs, Destin->IMSI_S, LastOfs-CurrOfs , pp_CS0004E_Details);
				if (bits < 0) {
					/* If decoding failed, set the failed field to "not present" */
			...
			if ((Destin->ADDR_TYPE==1)) {
				...
				bits = DECODE_c_FCSCH_NON_PRIMARY_BROADCAST_PDU_ESN (Buffer, CurrOfs, Destin->ESN, LastOfs-CurrOfs , pp_CS0004E_Details);
				if (bits < 0) {
					/* If decoding failed, set the failed field to "not present" */
			...
			if ((Destin->ADDR_TYPE==2)) {
				...
				bits = DECODE_c_FCSCH_NON_PRIMARY_BROADCAST_PDU_IMSI (Buffer, CurrOfs, Destin->IMSI, LastOfs-CurrOfs , pp_CS0004E_Details);
				if (bits < 0) {
					/* If decoding failed, set the failed field to "not present" */
			...
			if ((Destin->ADDR_TYPE==3)) {
				...
				bits = DECODE_c_FCSCH_NON_PRIMARY_BROADCAST_PDU_TMSI (Buffer, CurrOfs, Destin->TMSI, LastOfs-CurrOfs , pp_CS0004E_Details);
				if (bits < 0) {
					/* If decoding failed, set the failed field to "not present" */
			...
			if ((Destin->ADDR_TYPE==4)) {
				...
				bits = DECODE_c_FCSCH_NON_PRIMARY_BROADCAST_PDU_Extended_Address (Buffer, CurrOfs, Destin->Extended_Address, LastOfs-CurrOfs , pp_CS0004E_Details);
				if (bits < 0) {
					/* If decoding failed, set the failed field to "not present" */
			...
			if ((Destin->ADDR_TYPE==5)) {
				...
				bits = DECODE_c_FCSCH_NON_PRIMARY_BROADCAST_PDU_BROADCAST (Buffer, CurrOfs, Destin->BROADCAST, LastOfs-CurrOfs , pp_CS0004E_Details);
				if (bits < 0) {
					/* If decoding failed, set the failed field to "not present" */
...
#endif
#define ED_THIS Destin
		bits = (8*((pp_CS0004E_Details->userTag[1])-1));
		if (LastOfs - (int)CurrOfs < bits) {
			bits = ED_MESSAGE_TOO_SHORT;
		} 
		else {
...
#endif
#define ED_THIS Destin
		bits = (8*(pp_CS0004E_Details->userTag[1]));
		if (LastOfs - (int)CurrOfs < bits) {
			bits = ED_MESSAGE_TOO_SHORT;
		} 
		else {
...
#define ED_THIS Destin
		...
		if ((Destin->IMSI_CLASS==0)) {
			...
			bits = DECODE_c_FCSCH_NON_PRIMARY_BROADCAST_PDU_IMSI_IMSI_CLASS_0 (Buffer, CurrOfs, Destin->IMSI_CLASS_0, LastOfs-CurrOfs , pp_CS0004E_Details);
			if (bits < 0) {
				/* If decoding failed, set the failed field to "not present" */
		...
		if ((Destin->IMSI_CLASS==1)) {
			...
			bits = DECODE_c_FCSCH_NON_PRIMARY_BROADCAST_PDU_IMSI_IMSI_CLASS_1 (Buffer, CurrOfs, Destin->IMSI_CLASS_1, LastOfs-CurrOfs , pp_CS0004E_Details);
			if (bits < 0) {
				/* If decoding failed, set the failed field to "not present" */
...
#define ED_THIS Destin
		/* If ADDR_LEN is greater than four, 8 * (ADDR_LEN - 4); otherwise, 0.		 */
		bits = ((((pp_CS0004E_Details->userTag[1])>4)) ? ((8*((pp_CS0004E_Details->userTag[1])-4))) : (0));
		if (LastOfs - (int)CurrOfs < bits) {
			bits = ED_MESSAGE_TOO_SHORT;
		} 
		else {
		...
		CurrOfs += bits;
		/* If ADDR_LEN is greater than four, 32; otherwise, 8 * ADDR_LEN. */
		bits = ((((pp_CS0004E_Details->userTag[1])>4)) ? (32) : ((8*(pp_CS0004E_Details->userTag[1]))));
		if (LastOfs - (int)CurrOfs < bits) {
			bits = ED_MESSAGE_TOO_SHORT;
		} 
		else {
...
#define ED_THIS Source
		...
		for (i=0; i<Source->page_records.items; i++) {
			CurrOfs += ENCODE_c_GPM_PDU_page_records_data (Buffer, CurrOfs, Source->page_records.data [i], pp_CS0004E_Details);
		}
...
#define ED_THIS Destin
		...
		if (LastOfs - (int)CurrOfs < bits) {
			...
			ED_LOCATOR_SET (&(Destin->CONFIG_MSG_SEQ___LOCATOR), CurrOfs, bits);
			Destin->CONFIG_MSG_SEQ = (ED_OCTET)EDBitsToInt (Buffer, CURPOS, (ED_EXLONG)bits);
		}
		if (bits < 0) {
			/* Decoding failed - Go to the exit point */
			goto exitOnFail;
		...
		if (LastOfs - (int)CurrOfs < bits) {
			...
			ED_LOCATOR_SET (&(Destin->ACC_MSG_SEQ___LOCATOR), CurrOfs, bits);
			Destin->ACC_MSG_SEQ = (ED_OCTET)EDBitsToInt (Buffer, CURPOS, (ED_EXLONG)bits);
		}
		if (bits < 0) {
			/* Decoding failed - Go to the exit point */
			goto exitOnFail;
		...
		if (LastOfs - (int)CurrOfs < bits) {
			...
			ED_LOCATOR_SET (&(Destin->CLASS_0_DONE___LOCATOR), CurrOfs, bits);
			Destin->CLASS_0_DONE = (ED_BOOLEAN)EDBitsToInt (Buffer, CURPOS, (ED_EXLONG)bits);
		}
		if (bits < 0) {
			/* Decoding failed - Go to the exit point */
			goto exitOnFail;
		...
		if (LastOfs - (int)CurrOfs < bits) {
			...
			ED_LOCATOR_SET (&(Destin->CLASS_1_DONE___LOCATOR), CurrOfs, bits);
			Destin->CLASS_1_DONE = (ED_BOOLEAN)EDBitsToInt (Buffer, CURPOS, (ED_EXLONG)bits);
		}
		if (bits < 0) {
			/* Decoding failed - Go to the exit point */
			goto exitOnFail;
		...
		if (LastOfs - (int)CurrOfs < bits) {
			...
			ED_LOCATOR_SET (&(Destin->TMSI_DONE___LOCATOR), CurrOfs, bits);
			Destin->TMSI_DONE = (ED_BOOLEAN)EDBitsToInt (Buffer, CURPOS, (ED_EXLONG)bits);
		}
		if (bits < 0) {
			/* Decoding failed - Go to the exit point */
			goto exitOnFail;
		...
		if (LastOfs - (int)CurrOfs < bits) {
			...
			ED_LOCATOR_SET (&(Destin->ORDERED_TMSIS___LOCATOR), CurrOfs, bits);
			Destin->ORDERED_TMSIS = (ED_BOOLEAN)EDBitsToInt (Buffer, CURPOS, (ED_EXLONG)bits);
		}
		if (bits < 0) {
			/* Decoding failed - Go to the exit point */
			goto exitOnFail;
		...
		if (LastOfs - (int)CurrOfs < bits) {
			...
			ED_LOCATOR_SET (&(Destin->BROADCAST_DONE___LOCATOR), CurrOfs, bits);
			Destin->BROADCAST_DONE = (ED_BOOLEAN)EDBitsToInt (Buffer, CURPOS, (ED_EXLONG)bits);
		}
		if (bits < 0) {
			/* Decoding failed - Go to the exit point */
			goto exitOnFail;
		...
		if (LastOfs - (int)CurrOfs < bits) {
			...
			ED_LOCATOR_SET (&(Destin->ADD_LENGTH___LOCATOR), CurrOfs, bits);
			Destin->ADD_LENGTH = (ED_OCTET)EDBitsToInt (Buffer, CURPOS, (ED_EXLONG)bits);
		}
		if (bits < 0) {
			/* Decoding failed - Go to the exit point */
			goto exitOnFail;
		...
		CurrOfs += bits;
		bits = (8*Destin->ADD_LENGTH);
		if (LastOfs - (int)CurrOfs < bits) {
			bits = ED_MESSAGE_TOO_SHORT;
		} 
		else {
			ED_LOCATOR_SET (&(Destin->ADD_PFIELD___LOCATOR), CurrOfs, bits);
			EDAllocBinary (&(((&(Destin->ADD_PFIELD)))->value), (unsigned)(bits), &(((&(Destin->ADD_PFIELD)))->usedBits));
			EDCopyBits (((&(Destin->ADD_PFIELD))->value), 0, Buffer, CURPOS, (ED_EXLONG)bits);
		}
		if (bits < 0) {
			/* Decoding failed - Go to the exit point */
			goto exitOnFail;
		...
		for (i=0; ; i++) {
			...
			bits = DECODE_c_GPM_PDU_page_records_data (Buffer, CurrOfs, Destin->page_records.data [i], LastOfs-CurrOfs , pp_CS0004E_Details);
			if (bits < 0) {
				/* Decoding failed - Remove the unused array items */
				SETITEMS_c_GPM_PDU_page_records (&Destin->page_records, i);
				goto exit_0;
			}
			CurrOfs += bits;
		}
...
#define ED_THIS Destin
		...
		if (LastOfs - (int)CurrOfs < bits) {
			...
			ED_LOCATOR_SET (&(Destin->PAGE_CLASS___LOCATOR), CurrOfs, bits);
			Destin->PAGE_CLASS = (ED_OCTET)EDBitsToInt (Buffer, CURPOS, (ED_EXLONG)bits);
		}
		if (bits < 0) {
			/* Decoding failed - Go to the exit point */
			goto exitOnFail;
		...
		if (LastOfs - (int)CurrOfs < bits) {
			...
			ED_LOCATOR_SET (&(Destin->PAGE_SUBCLASS___LOCATOR), CurrOfs, bits);
			Destin->PAGE_SUBCLASS = (ED_OCTET)EDBitsToInt (Buffer, CURPOS, (ED_EXLONG)bits);
		}
		if (bits < 0) {
			/* Decoding failed - Go to the exit point */
			goto exitOnFail;
		...
		if (((Destin->PAGE_CLASS==3)&&(Destin->PAGE_SUBCLASS!=0))) {
			...
			if (LastOfs - (int)CurrOfs < bits) {
				...
				ED_LOCATOR_SET (&(Destin->PAGE_SUBCLASS_EXT___LOCATOR), CurrOfs, bits);
				Destin->PAGE_SUBCLASS_EXT = (ED_OCTET)EDBitsToInt (Buffer, CURPOS, (ED_EXLONG)bits);
			}
			if (bits < 0) {
				/* If decoding failed, set the failed field to "not present" */
		...
		if (((Destin->PAGE_CLASS==2)&&(Destin->PAGE_SUBCLASS==3))) {
			...
			if (LastOfs - (int)CurrOfs < bits) {
				...
				ED_LOCATOR_SET (&(Destin->TMSI_ZONE_LEN___LOCATOR), CurrOfs, bits);
				Destin->TMSI_ZONE_LEN = (ED_OCTET)EDBitsToInt (Buffer, CURPOS, (ED_EXLONG)bits);
			}
			if (bits < 0) {
				/* If decoding failed, set the failed field to "not present" */
		...
		bits = ((((Destin->PAGE_CLASS==2)&&(Destin->PAGE_SUBCLASS==3))) ? ((8*(Destin->TMSI_ZONE_LEN_Present ? Destin->TMSI_ZONE_LEN : 0))) : (0));
		if (LastOfs - (int)CurrOfs < bits) {
			bits = ED_MESSAGE_TOO_SHORT;
		} 
		else {
		...
		if ((((Destin->PAGE_CLASS==2)&&(Destin->PAGE_SUBCLASS==0))||((Destin->PAGE_CLASS==2)&&(Destin->PAGE_SUBCLASS==3)))) {
			...
			if (LastOfs - (int)CurrOfs < bits) {
				...
				ED_LOCATOR_SET (&(Destin->TMSI_CODE_ADDR_32___LOCATOR), CurrOfs, bits);
				Destin->TMSI_CODE_ADDR_32 = (ED_LONG)EDBitsToInt (Buffer, CURPOS, (ED_EXLONG)bits);
			}
			if (bits < 0) {
				/* If decoding failed, set the failed field to "not present" */
		...
		if (((Destin->PAGE_CLASS==2)&&(Destin->PAGE_SUBCLASS==1))) {
			...
			if (LastOfs - (int)CurrOfs < bits) {
				...
				ED_LOCATOR_SET (&(Destin->TMSI_CODE_ADDR_24___LOCATOR), CurrOfs, bits);
				Destin->TMSI_CODE_ADDR_24 = (ED_LONG)EDBitsToInt (Buffer, CURPOS, (ED_EXLONG)bits);
			}
			if (bits < 0) {
				/* If decoding failed, set the failed field to "not present" */
		...
		if (((Destin->PAGE_CLASS==2)&&(Destin->PAGE_SUBCLASS==2))) {
			...
			if (LastOfs - (int)CurrOfs < bits) {
				...
				ED_LOCATOR_SET (&(Destin->TMSI_CODE_ADDR_16___LOCATOR), CurrOfs, bits);
				Destin->TMSI_CODE_ADDR_16 = (ED_LONG)EDBitsToInt (Buffer, CURPOS, (ED_EXLONG)bits);
			}
			if (bits < 0) {
				/* If decoding failed, set the failed field to "not present" */
		...
		if ((((Destin->PAGE_CLASS==3)&&(Destin->PAGE_SUBCLASS==0))||(((Destin->PAGE_CLASS==3)&&(Destin->PAGE_SUBCLASS==3))&&((Destin->PAGE_SUBCLASS_EXT_Present ? Destin->PAGE_SUBCLASS_EXT : 0)==0)))) {
			...
			if (LastOfs - (int)CurrOfs < bits) {
				...
				ED_LOCATOR_SET (&(Destin->BURST_TYPE___LOCATOR), CurrOfs, bits);
				Destin->BURST_TYPE = (ED_OCTET)EDBitsToInt (Buffer, CURPOS, (ED_EXLONG)bits);
			}
			if (bits < 0) {
				/* If decoding failed, set the failed field to "not present" */
		...
		bits = (((((Destin->PAGE_CLASS==3)&&(Destin->PAGE_SUBCLASS==0))||(((Destin->PAGE_CLASS==3)&&(Destin->PAGE_SUBCLASS==3))&&((Destin->PAGE_SUBCLASS_EXT_Present ? Destin->PAGE_SUBCLASS_EXT : 0)==0)))) ? ((8*(Destin->ADDR_LEN_Present ? Destin->ADDR_LEN : 0))) : (0));
		if (LastOfs - (int)CurrOfs < bits) {
			bits = ED_MESSAGE_TOO_SHORT;
		} 
		else {
		...
		if ((((Destin->PAGE_CLASS==3)&&(Destin->PAGE_SUBCLASS==1))&&((Destin->PAGE_SUBCLASS_EXT_Present ? Destin->PAGE_SUBCLASS_EXT : 0)==3))) {
			...
			if (LastOfs - (int)CurrOfs < bits) {
				...
				ED_LOCATOR_SET (&(Destin->LENGTH___LOCATOR), CurrOfs, bits);
				Destin->LENGTH = (ED_OCTET)EDBitsToInt (Buffer, CURPOS, (ED_EXLONG)bits);
			}
			if (bits < 0) {
				/* If decoding failed, set the failed field to "not present" */
		...
		if ((Destin->PAGE_CLASS!=3)) {
			...
			if (LastOfs - (int)CurrOfs < bits) {
				...
				ED_LOCATOR_SET (&(Destin->SDU_INCLUDED___LOCATOR), CurrOfs, bits);
				Destin->SDU_INCLUDED = (ED_BOOLEAN)EDBitsToInt (Buffer, CURPOS, (ED_EXLONG)bits);
			}
			if (bits < 0) {
				/* If decoding failed, set the failed field to "not present" */
		...
		if ((((Destin->PAGE_CLASS==3)&&(Destin->PAGE_SUBCLASS==3))&&((Destin->PAGE_SUBCLASS_EXT_Present ? Destin->PAGE_SUBCLASS_EXT : 0)==0))) {
			...
			if (LastOfs - (int)CurrOfs < bits) {
				...
				ED_LOCATOR_SET (&(Destin->EXT_BCAST_SDU_LENGTH_IND___LOCATOR), CurrOfs, bits);
				Destin->EXT_BCAST_SDU_LENGTH_IND = (ED_OCTET)EDBitsToInt (Buffer, CURPOS, (ED_EXLONG)bits);
			}
			if (bits < 0) {
				/* If decoding failed, set the failed field to "not present" */
		...
		if ((Destin->EXT_BCAST_SDU_LENGTH_IND_Present&&(((Destin->EXT_BCAST_SDU_LENGTH_IND_Present ? Destin->EXT_BCAST_SDU_LENGTH_IND : 0)==0)||((Destin->EXT_BCAST_SDU_LENGTH_IND_Present ? Destin->EXT_BCAST_SDU_LENGTH_IND : 0)==1)))) {
			...
			if (LastOfs - (int)CurrOfs < bits) {
				...
				ED_LOCATOR_SET (&(Destin->EXT_BCAST_SDU_LENGTH___LOCATOR), CurrOfs, bits);
				Destin->EXT_BCAST_SDU_LENGTH = (ED_OCTET)EDBitsToInt (Buffer, CURPOS, (ED_EXLONG)bits);
			}
			if (bits < 0) {
				/* If decoding failed, set the failed field to "not present" */
...
#define ED_THIS Destin
		...
		if (((Destin->MSG_ID==37)||(Destin->MSG_ID==38))) {
			...
			if (LastOfs - (int)CurrOfs < bits) {
				...
				ED_LOCATOR_SET (&(Destin->UPM_SEGMENT_SEQ___LOCATOR), CurrOfs, bits);
				Destin->UPM_SEGMENT_SEQ = (ED_OCTET)EDBitsToInt (Buffer, CURPOS, (ED_EXLONG)bits);
			}
			if (bits < 0) {
				/* If decoding failed, set the failed field to "not present" */
		...
		bits = (int)(LastOfs-CurrOfs);
		ED_LOCATOR_SET (&(Destin->Universal_Page_Block___LOCATOR), CurrOfs, bits);
		EDAllocBinary (&(((&(Destin->Universal_Page_Block)))->value), (unsigned)bits, &(((&(Destin->Universal_Page_Block)))->usedBits));
		EDCopyBits (((&(Destin->Universal_Page_Block))->value), 0, Buffer, CURPOS, (ED_EXLONG)bits);
		if (bits < 0) {
			/* Decoding failed - Go to the exit point */
			goto exitOnFail;
		}
...
#define ED_THIS Source
		...
		for (i=0; i<max; i++) {
			PUSH_INT (Source->BURST_TYPE.data [i], 6);
		}
		max = 16;
		ED_ASSERT (max == Source->SUBRECORD.items);
		if (Source->SUBRECORD.items < max) max = Source->SUBRECORD.items;
		for (i=0; i<max; i++) {
			CurrOfs += ENCODE_c_f_csch_Universal_Page_Block_SUBRECORD_data (Buffer, CurrOfs, Source->SUBRECORD.data [i], pp_CS0004E_Details);
		}
		for (i=0; i<Source->page_records.items; i++) {
			CurrOfs += ENCODE_c_f_csch_Universal_Page_Block_page_records_data (Buffer, CurrOfs, Source->page_records.data [i], pp_CS0004E_Details);
		}
#ifdef NOCSN_DEBUG
...
#define ED_THIS Destin
		...
		if (LastOfs - (int)CurrOfs < bits) {
			...
			ED_LOCATOR_SET (&(Destin->READ_NEXT_SLOT___LOCATOR), CurrOfs, bits);
			Destin->READ_NEXT_SLOT = (ED_BOOLEAN)EDBitsToInt (Buffer, CURPOS, (ED_EXLONG)bits);
		}
		if (bits < 0) {
			/* Decoding failed - Go to the exit point */
			goto exitOnFail;
		...
		if (LastOfs - (int)CurrOfs < bits) {
			...
			ED_LOCATOR_SET (&(Destin->READ_NEXT_SLOT_BCAST___LOCATOR), CurrOfs, bits);
			Destin->READ_NEXT_SLOT_BCAST = (ED_BOOLEAN)EDBitsToInt (Buffer, CURPOS, (ED_EXLONG)bits);
		}
		if (bits < 0) {
			/* Decoding failed - Go to the exit point */
			goto exitOnFail;
		...
		if (LastOfs - (int)CurrOfs < bits) {
			...
			ED_LOCATOR_SET (&(Destin->BCAST_INCLUDED___LOCATOR), CurrOfs, bits);
			Destin->BCAST_INCLUDED = (ED_BOOLEAN)EDBitsToInt (Buffer, CURPOS, (ED_EXLONG)bits);
		}
		if (bits < 0) {
			/* Decoding failed - Go to the exit point */
			goto exitOnFail;
		...
		if (Destin->BCAST_INCLUDED) {
			...
			if (LastOfs - (int)CurrOfs < bits) {
				...
				ED_LOCATOR_SET (&(Destin->NUM_BCAST___LOCATOR), CurrOfs, bits);
				Destin->NUM_BCAST = (ED_OCTET)EDBitsToInt (Buffer, CURPOS, (ED_EXLONG)bits);
				/* Set the exported tag */
				pp_CS0004E_Details->userTagPresent[0] = 1;
		...
		if (LastOfs - (int)CurrOfs < bits) {
			...
			ED_LOCATOR_SET (&(Destin->IMSI_INCLUDED___LOCATOR), CurrOfs, bits);
			Destin->IMSI_INCLUDED = (ED_BOOLEAN)EDBitsToInt (Buffer, CURPOS, (ED_EXLONG)bits);
		}
		if (bits < 0) {
			/* Decoding failed - Go to the exit point */
			goto exitOnFail;
		...
		if (Destin->IMSI_INCLUDED) {
			...
			if (LastOfs - (int)CurrOfs < bits) {
				...
				ED_LOCATOR_SET (&(Destin->NUM_IMSI___LOCATOR), CurrOfs, bits);
				Destin->NUM_IMSI = (ED_OCTET)EDBitsToInt (Buffer, CURPOS, (ED_EXLONG)bits);
				/* Set the exported tag */
				pp_CS0004E_Details->userTagPresent[1] = 1;
		...
		if (LastOfs - (int)CurrOfs < bits) {
			...
			ED_LOCATOR_SET (&(Destin->TMSI_INCLUDED___LOCATOR), CurrOfs, bits);
			Destin->TMSI_INCLUDED = (ED_BOOLEAN)EDBitsToInt (Buffer, CURPOS, (ED_EXLONG)bits);
		}
		if (bits < 0) {
			/* Decoding failed - Go to the exit point */
			goto exitOnFail;
		...
		if (Destin->TMSI_INCLUDED) {
			...
			if (LastOfs - (int)CurrOfs < bits) {
				...
				ED_LOCATOR_SET (&(Destin->NUM_TMSI___LOCATOR), CurrOfs, bits);
				Destin->NUM_TMSI = (ED_OCTET)EDBitsToInt (Buffer, CURPOS, (ED_EXLONG)bits);
				/* Set the exported tag */
				pp_CS0004E_Details->userTagPresent[2] = 1;
		...
		if (LastOfs - (int)CurrOfs < bits) {
			...
			ED_LOCATOR_SET (&(Destin->RESERVED_TYPE_INCLUDED___LOCATOR), CurrOfs, bits);
			Destin->RESERVED_TYPE_INCLUDED = (ED_BOOLEAN)EDBitsToInt (Buffer, CURPOS, (ED_EXLONG)bits);
		}
		if (bits < 0) {
			/* Decoding failed - Go to the exit point */
			goto exitOnFail;
		...
		if (Destin->RESERVED_TYPE_INCLUDED) {
			...
			if (LastOfs - (int)CurrOfs < bits) {
				...
				ED_LOCATOR_SET (&(Destin->NUM_RESERVED_TYPE___LOCATOR), CurrOfs, bits);
				Destin->NUM_RESERVED_TYPE = (ED_OCTET)EDBitsToInt (Buffer, CURPOS, (ED_EXLONG)bits);
				/* Set the exported tag */
				pp_CS0004E_Details->userTagPresent[3] = 1;
		...
		for (i=0; i<Destin->BURST_TYPE.items; i++) {
			bits = 6;
			if (LastOfs - (int)CurrOfs < bits) {
				bits = ED_MESSAGE_TOO_SHORT;
			} 
			else {
				ED_LOCATOR_SET (&(Destin->BURST_TYPE.data___LOCATOR[i]), CurrOfs, bits);
				Destin->BURST_TYPE.data [i] = (ED_OCTET)EDBitsToInt (Buffer, CURPOS, (ED_EXLONG)bits);
			}
			if (bits < 0) {
				/* Decoding failed - Remove the unused array items */
				SETITEMS_c_f_csch_Universal_Page_Block_BURST_TYPE (&Destin->BURST_TYPE, i);
				goto exitOnFail;
			}
			CurrOfs += bits;
		}
		...
		for (i=0; i<Destin->SUBRECORD.items; i++) {
			bits = DECODE_c_f_csch_Universal_Page_Block_SUBRECORD_data (Buffer, CurrOfs, Destin->SUBRECORD.data [i], LastOfs-CurrOfs , pp_CS0004E_Details);
			if (bits < 0) {
				/* Decoding failed - Remove the unused array items */
				SETITEMS_c_f_csch_Universal_Page_Block_SUBRECORD (&Destin->SUBRECORD, i);
				goto exitOnFail;
			}
			CurrOfs += bits;
		}
		...
		for (i=0; ; i++) {
			...
			bits = DECODE_c_f_csch_Universal_Page_Block_page_records_data (Buffer, CurrOfs, Destin->page_records.data [i], LastOfs-CurrOfs , pp_CS0004E_Details);
			if (bits < 0) {
				/* Decoding failed - Remove the unused array items */
				SETITEMS_c_f_csch_Universal_Page_Block_page_records (&Destin->page_records, i);
				goto exit_0;
			}
			CurrOfs += bits;
		}
...
#endif
#define ED_THIS Destin
		bits = (((pp_CS0004E_Details->userTagPresent[0])) ? (((pp_CS0004E_Details->userTag[0])+1)) : (0));
		if (LastOfs - (int)CurrOfs < bits) {
			bits = ED_MESSAGE_TOO_SHORT;
		} 
		else {
			ED_LOCATOR_SET (&(Destin->BC_ADDR_BLOCK___LOCATOR), CurrOfs, bits);
			EDAllocBinary (&(((&(Destin->BC_ADDR_BLOCK)))->value), (unsigned)(bits), &(((&(Destin->BC_ADDR_BLOCK)))->usedBits));
			EDCopyBits (((&(Destin->BC_ADDR_BLOCK))->value), 0, Buffer, CURPOS, (ED_EXLONG)bits);
		}
		if (bits < 0) {
			/* Decoding failed - Go to the exit point */
			goto exitOnFail;
		...
		CurrOfs += bits;
		bits = (((pp_CS0004E_Details->userTagPresent[1])) ? (((pp_CS0004E_Details->userTag[1])+1)) : (0));
		if (LastOfs - (int)CurrOfs < bits) {
			bits = ED_MESSAGE_TOO_SHORT;
		} 
		else {
			ED_LOCATOR_SET (&(Destin->IMSI_ADDR_BLOCK___LOCATOR), CurrOfs, bits);
			EDAllocBinary (&(((&(Destin->IMSI_ADDR_BLOCK)))->value), (unsigned)(bits), &(((&(Destin->IMSI_ADDR_BLOCK)))->usedBits));
			EDCopyBits (((&(Destin->IMSI_ADDR_BLOCK))->value), 0, Buffer, CURPOS, (ED_EXLONG)bits);
		}
		if (bits < 0) {
			/* Decoding failed - Go to the exit point */
			goto exitOnFail;
		...
		CurrOfs += bits;
		bits = (((pp_CS0004E_Details->userTagPresent[2])) ? (((pp_CS0004E_Details->userTag[2])+1)) : (0));
		if (LastOfs - (int)CurrOfs < bits) {
			bits = ED_MESSAGE_TOO_SHORT;
		} 
		else {
			ED_LOCATOR_SET (&(Destin->TMSI_ADDR_BLOCK___LOCATOR), CurrOfs, bits);
			EDAllocBinary (&(((&(Destin->TMSI_ADDR_BLOCK)))->value), (unsigned)(bits), &(((&(Destin->TMSI_ADDR_BLOCK)))->usedBits));
			EDCopyBits (((&(Destin->TMSI_ADDR_BLOCK))->value), 0, Buffer, CURPOS, (ED_EXLONG)bits);
		}
		if (bits < 0) {
			/* Decoding failed - Go to the exit point */
			goto exitOnFail;
		...
		CurrOfs += bits;
		bits = (((pp_CS0004E_Details->userTagPresent[3])) ? (((pp_CS0004E_Details->userTag[3])+1)) : (0));
		if (LastOfs - (int)CurrOfs < bits) {
			bits = ED_MESSAGE_TOO_SHORT;
		} 
		else {
			ED_LOCATOR_SET (&(Destin->RESERVED_ADDR_BLOCK___LOCATOR), CurrOfs, bits);
			EDAllocBinary (&(((&(Destin->RESERVED_ADDR_BLOCK)))->value), (unsigned)(bits), &(((&(Destin->RESERVED_ADDR_BLOCK)))->usedBits));
			EDCopyBits (((&(Destin->RESERVED_ADDR_BLOCK))->value), 0, Buffer, CURPOS, (ED_EXLONG)bits);
		}
		if (bits < 0) {
			/* Decoding failed - Go to the exit point */
			goto exitOnFail;
...
#define ED_THIS Destin
		...
		if ((((((((Destin->PAGE_CLASS==0)&&(Destin->PAGE_SUBCLASS==0))||((Destin->PAGE_CLASS==0)&&(Destin->PAGE_SUBCLASS==1)))||((Destin->PAGE_CLASS==0)&&(Destin->PAGE_SUBCLASS==2)))||((Destin->PAGE_CLASS==0)&&(Destin->PAGE_SUBCLASS==3)))||((Destin->PAGE_CLASS==1)&&(Destin->PAGE_SUBCLASS==0)))||((Destin->PAGE_CLASS==1)&&(Destin->PAGE_SUBCLASS==1)))) {
			...
			if (LastOfs - (int)CurrOfs < bits) {
				...
				ED_LOCATOR_SET (&(Destin->IMSI_S_33_16___LOCATOR), CurrOfs, bits);
				Destin->IMSI_S_33_16 = (ED_LONG)EDBitsToInt (Buffer, CURPOS, (ED_EXLONG)bits);
			}
			if (bits < 0) {
				/* If decoding failed, set the failed field to "not present" */
		...
		if ((((Destin->PAGE_CLASS==2)&&(Destin->PAGE_SUBCLASS==0))||((Destin->PAGE_CLASS==2)&&(Destin->PAGE_SUBCLASS==3)))) {
			...
			if (LastOfs - (int)CurrOfs < bits) {
				...
				ED_LOCATOR_SET (&(Destin->TMSI_CODE_ADDR_31_16___LOCATOR), CurrOfs, bits);
				Destin->TMSI_CODE_ADDR_31_16 = (ED_LONG)EDBitsToInt (Buffer, CURPOS, (ED_EXLONG)bits);
			}
			if (bits < 0) {
				/* If decoding failed, set the failed field to "not present" */
		...
		if (((Destin->PAGE_CLASS==2)&&(Destin->PAGE_SUBCLASS==1))) {
			...
			if (LastOfs - (int)CurrOfs < bits) {
				...
				ED_LOCATOR_SET (&(Destin->TMSI_CODE_ADDR_23_16___LOCATOR), CurrOfs, bits);
				Destin->TMSI_CODE_ADDR_23_16 = (ED_OCTET)EDBitsToInt (Buffer, CURPOS, (ED_EXLONG)bits);
			}
			if (bits < 0) {
				/* If decoding failed, set the failed field to "not present" */
		...
		if ((((Destin->PAGE_CLASS==3)&&(Destin->PAGE_SUBCLASS==3))&&((Destin->PAGE_SUBCLASS_EXT_Present ? Destin->PAGE_SUBCLASS_EXT : 0)==0))) {
			...
			bits = (8*((Destin->ADDR_LEN_Present ? Destin->ADDR_LEN : 0)-2));
			if (LastOfs - (int)CurrOfs < bits) {
				bits = ED_MESSAGE_TOO_SHORT;
			} 
			else {
				ED_LOCATOR_SET (&(Destin->BC_ADDR_REMAINDER___LOCATOR), CurrOfs, bits);
				EDAllocBinary (&((Destin->BC_ADDR_REMAINDER)->value), (unsigned)(bits), &((Destin->BC_ADDR_REMAINDER)->usedBits));
				EDCopyBits ((Destin->BC_ADDR_REMAINDER->value), 0, Buffer, CURPOS, (ED_EXLONG)bits);
			}
			if (bits < 0) {
				/* If decoding failed, set the failed field to "not present" */
		...
		bits = (((((((Destin->PAGE_CLASS==1)&&(Destin->PAGE_SUBCLASS==2))||((Destin->PAGE_CLASS==1)&&(Destin->PAGE_SUBCLASS==3)))||(((Destin->PAGE_CLASS==3)&&(Destin->PAGE_SUBCLASS==2))&&((Destin->PAGE_SUBCLASS_EXT_Present ? Destin->PAGE_SUBCLASS_EXT : 0)==3)))||(((Destin->PAGE_CLASS==3)&&(Destin->PAGE_SUBCLASS==3))&&((Destin->PAGE_SUBCLASS_EXT_Present ? Destin->PAGE_SUBCLASS_EXT : 0)==3)))) ? (5) : ((((((Destin->PAGE_CLASS==3)&&(Destin->PAGE_SUBCLASS==1))&&((Destin->PAGE_SUBCLASS_EXT_Present ? Destin->PAGE_SUBCLASS_EXT : 0)==3))) ? (4) : (0))));
		if (LastOfs - (int)CurrOfs < bits) {
			bits = ED_MESSAGE_TOO_SHORT;
		} 
		else {
			ED_LOCATOR_SET (&(Destin->RESERVED_LEN___LOCATOR), CurrOfs, bits);
			EDAllocBinary (&(((&(Destin->RESERVED_LEN)))->value), (unsigned)(bits), &(((&(Destin->RESERVED_LEN)))->usedBits));
			EDCopyBits (((&(Destin->RESERVED_LEN))->value), 0, Buffer, CURPOS, (ED_EXLONG)bits);
		}
		if (bits < 0) {
			/* Decoding failed - Go to the exit point */
			goto exitOnFail;
		...
		if (LastOfs - (int)CurrOfs < bits) {
			...
			ED_LOCATOR_SET (&(Destin->RESERVED___LOCATOR), CurrOfs, bits);
			EDAllocBinary (&(((&(Destin->RESERVED)))->value), (unsigned)(bits), &(((&(Destin->RESERVED)))->usedBits));
			EDCopyBits (((&(Destin->RESERVED))->value), 0, Buffer, CURPOS, (ED_EXLONG)bits);
		}
		if (bits < 0) {
			/* Decoding failed - Go to the exit point */
			goto exitOnFail;
		...
		if ((Destin->PAGE_CLASS!=3)) {
			...
			if (LastOfs - (int)CurrOfs < bits) {
				...
				ED_LOCATOR_SET (&(Destin->EXT_MS_SDU_LENGTH_INCL___LOCATOR), CurrOfs, bits);
				Destin->EXT_MS_SDU_LENGTH_INCL = (ED_BOOLEAN)EDBitsToInt (Buffer, CURPOS, (ED_EXLONG)bits);
			}
			if (bits < 0) {
				/* If decoding failed, set the failed field to "not present" */
		...
		if ((Destin->EXT_MS_SDU_LENGTH_INCL_Present ? Destin->EXT_MS_SDU_LENGTH_INCL : 0)) {
			...
			if (LastOfs - (int)CurrOfs < bits) {
				...
				ED_LOCATOR_SET (&(Destin->EXT_MS_SDU_LENGTH___LOCATOR), CurrOfs, bits);
				Destin->EXT_MS_SDU_LENGTH = (ED_OCTET)EDBitsToInt (Buffer, CURPOS, (ED_EXLONG)bits);
			}
			if (bits < 0) {
				/* If decoding failed, set the failed field to "not present" */
...
#define ED_THIS Destin
		...
		pp_CS0004E_Details->userTagBusy[3] = 1;
#endif
		bits = DECODE_c_r_csch_PDU_Message_Type (Buffer, CurrOfs, (&(Destin->Message_Type)), LastOfs-CurrOfs , pp_CS0004E_Details);
		if (bits < 0) {
			/* Decoding failed - Go to the exit point */
			goto exitOnFail;
		}
		...
		if ((pp_CS0004E_Details->n_P_REV_IN_USEs==6)) {
			...
			bits = DECODE_c_r_csch_PDU_LAC_Length (Buffer, CurrOfs, Destin->LAC_Length, LastOfs-CurrOfs , pp_CS0004E_Details);
			if (bits < 0) {
				/* If decoding failed, set the failed field to "not present" */
		...
		bits = DECODE_c_r_csch_PDU_ARQ (Buffer, CurrOfs, (&(Destin->ARQ)), LastOfs-CurrOfs , pp_CS0004E_Details);
		if (bits < 0) {
			/* Decoding failed - Go to the exit point */
			goto exitOnFail;
		}
		CurrOfs += bits;
		bits = DECODE_c_r_csch_PDU_Addressing (Buffer, CurrOfs, (&(Destin->Addressing)), LastOfs-CurrOfs , pp_CS0004E_Details);
		if (bits < 0) {
			/* Decoding failed - Go to the exit point */
			goto exitOnFail;
		}
		CurrOfs += bits;
		bits = DECODE_c_r_csch_PDU_Authentication (Buffer, CurrOfs, (&(Destin->Authentication)), LastOfs-CurrOfs , pp_CS0004E_Details);
		if (bits < 0) {
			/* Decoding failed - Go to the exit point */
			goto exitOnFail;
		}
		...
		if ((pp_CS0004E_Details->n_P_REV_IN_USEs>=7)) {
			...
			bits = DECODE_c_r_csch_PDU_Extended_Encryption (Buffer, CurrOfs, Destin->Extended_Encryption, LastOfs-CurrOfs , pp_CS0004E_Details);
			if (bits < 0) {
				/* If decoding failed, set the failed field to "not present" */
		...
		/* Decode spare bit(s) */
		bits = ((int)CurrOfs < EDBitsToInt (Buffer, pp_CS0004E_Details->userTag[0]/*LAC_LENGTH_OFFSET*/, 5)*8+pp_CS0004E_Details->userTag[0]/*LAC_LENGTH_OFFSET*/ ? EDBitsToInt (Buffer, pp_CS0004E_Details->userTag[0]/*LAC_LENGTH_OFFSET*/, 5)*8+pp_CS0004E_Details->userTag[0]/*LAC_LENGTH_OFFSET*/ - CurrOfs : 0);
		if (LastOfs-CurrOfs < bits) {
			bits = ED_MESSAGE_TOO_SHORT;
			goto exitOnFail;
		}
		...
		if ((pp_CS0004E_Details->n_P_REV_IN_USEs>=6)) {
			...
			bits = DECODE_c_r_csch_Radio_Environment_Report (Buffer, CurrOfs, Destin->Radio_Environment_Report, LastOfs-CurrOfs , pp_CS0004E_Details);
			if (bits < 0) {
				/* If decoding failed, set the failed field to "not present" */
...
#define ED_THIS Destin
		...
		if (LastOfs - (int)CurrOfs < bits) {
			...
			ED_LOCATOR_SET (&(Destin->LAC_LENGTH___LOCATOR), CurrOfs, bits);
			Destin->LAC_LENGTH = (ED_OCTET)EDBitsToInt (Buffer, CURPOS, (ED_EXLONG)bits);
		}
		if (bits < 0) {
			/* Decoding failed - Go to the exit point */
			goto exitOnFail;
...
#define ED_THIS Destin
		...
		if (LastOfs - (int)CurrOfs < bits) {
			...
			ED_LOCATOR_SET (&(Destin->ACK_TYPE___LOCATOR), CurrOfs, bits);
			Destin->ACK_TYPE = (ED_OCTET)EDBitsToInt (Buffer, CURPOS, (ED_EXLONG)bits);
		}
		if (bits < 0) {
			/* Decoding failed - Go to the exit point */
			goto exitOnFail;
		...
		if ((Destin->ACK_TYPE==4)) {
			...
			if (LastOfs - (int)CurrOfs < bits) {
				...
				ED_LOCATOR_SET (&(Destin->EXT_ACK_TYPE___LOCATOR), CurrOfs, bits);
				Destin->EXT_ACK_TYPE = (ED_OCTET)EDBitsToInt (Buffer, CURPOS, (ED_EXLONG)bits);
			}
			if (bits < 0) {
				/* If decoding failed, set the failed field to "not present" */
...
#define ED_THIS Destin
		...
		if (LastOfs - (int)CurrOfs < bits) {
			...
			ED_LOCATOR_SET (&(Destin->MSID_TYPE___LOCATOR), CurrOfs, bits);
			Destin->MSID_TYPE = (ED_OCTET)EDBitsToInt (Buffer, CURPOS, (ED_EXLONG)bits);
		}
		if (bits < 0) {
			/* Decoding failed - Go to the exit point */
			goto exitOnFail;
		...
		if ((Destin->MSID_TYPE==4)) {
			...
			if (LastOfs - (int)CurrOfs < bits) {
				...
				ED_LOCATOR_SET (&(Destin->EXT_MSID_TYPE___LOCATOR), CurrOfs, bits);
				Destin->EXT_MSID_TYPE = (ED_OCTET)EDBitsToInt (Buffer, CURPOS, (ED_EXLONG)bits);
			}
			if (bits < 0) {
				/* If decoding failed, set the failed field to "not present" */
		...
		if (LastOfs - (int)CurrOfs < bits) {
			...
			ED_LOCATOR_SET (&(Destin->MSID_LEN___LOCATOR), CurrOfs, bits);
			Destin->MSID_LEN = (ED_OCTET)EDBitsToInt (Buffer, CURPOS, (ED_EXLONG)bits);
		}
		if (bits < 0) {
			/* Decoding failed - Go to the exit point */
			goto exitOnFail;
		...
		/* BEGIN OF PAD ZONE - DECODE inclusive=0 bits=4 octet(=0)/bit(=1)=0 */
		{
			...
			if (((((Destin->MSID_TYPE==0)||(Destin->MSID_TYPE==1))||(Destin->MSID_TYPE==3))||((Destin->MSID_TYPE==4)&&((Destin->EXT_MSID_TYPE_Present ? Destin->EXT_MSID_TYPE : 0)==2)))) {
				...
				if (LastOfs - (int)CurrOfs < bits) {
					...
					ED_LOCATOR_SET (&(Destin->ESN___LOCATOR), CurrOfs, bits);
					Destin->ESN = (ED_LONG)EDBitsToInt (Buffer, CURPOS, (ED_EXLONG)bits);
				}
				if (bits < 0) {
					/* If decoding failed, set the failed field to "not present" */
			...
			if (((((Destin->MSID_TYPE==4)&&((Destin->EXT_MSID_TYPE_Present ? Destin->EXT_MSID_TYPE : 0)==0))||((Destin->MSID_TYPE==4)&&((Destin->EXT_MSID_TYPE_Present ? Destin->EXT_MSID_TYPE : 0)==1)))||((Destin->MSID_TYPE==4)&&((Destin->EXT_MSID_TYPE_Present ? Destin->EXT_MSID_TYPE : 0)==2)))) {
				...
				if (LastOfs - (int)CurrOfs < bits) {
					...
					ED_LOCATOR_SET (&(Destin->MEID___LOCATOR), CurrOfs, bits);
					EDCopyBits (Destin->MEID, 0, Buffer, CURPOS, (ED_EXLONG)bits);
				}
				if (bits < 0) {
					/* If decoding failed, set the failed field to "not present" */
			...
			if (((((Destin->MSID_TYPE==2)||(Destin->MSID_TYPE==3))||((Destin->MSID_TYPE==4)&&((Destin->EXT_MSID_TYPE_Present ? Destin->EXT_MSID_TYPE : 0)==1)))||((Destin->MSID_TYPE==4)&&((Destin->EXT_MSID_TYPE_Present ? Destin->EXT_MSID_TYPE : 0)==2)))) {
				...
				bits = DECODE_c_r_csch_PDU_Addressing_IMSI_class_specific_subfields (Buffer, CurrOfs, Destin->IMSI_class_specific_subfields, LastOfs-CurrOfs , pp_CS0004E_Details);
				if (bits < 0) {
					/* If decoding failed, set the failed field to "not present" */
			...
			if ((Destin->MSID_TYPE==5)) {
				...
				bits = (((Destin->MSID_LEN>4)) ? ((8*(Destin->MSID_LEN-4))) : (0));
				if (LastOfs - (int)CurrOfs < bits) {
					bits = ED_MESSAGE_TOO_SHORT;
				} 
				else {
			...
			if ((Destin->MSID_TYPE==5)) {
				...
				bits = (((Destin->MSID_LEN>4)) ? (32) : ((8*Destin->MSID_LEN)));
				if (LastOfs - (int)CurrOfs < bits) {
					bits = ED_MESSAGE_TOO_SHORT;
				} 
				else {
...
#define ED_THIS Destin
		if (((pp_CS0004E_Details->userTag[3])==0)) {
			...
			bits = DECODE_c_r_csch_PDU_Addressing_IMSI_class_specific_subfields_IMSI_CLASS_0 (Buffer, CurrOfs, Destin->IMSI_CLASS_0, LastOfs-CurrOfs , pp_CS0004E_Details);
			if (bits < 0) {
				/* If decoding failed, set the failed field to "not present" */
		...
		if (((pp_CS0004E_Details->userTag[3])==1)) {
			...
			bits = DECODE_c_r_csch_PDU_Addressing_IMSI_class_specific_subfields_IMSI_CLASS_1 (Buffer, CurrOfs, Destin->IMSI_CLASS_1, LastOfs-CurrOfs , pp_CS0004E_Details);
			if (bits < 0) {
				/* If decoding failed, set the failed field to "not present" */
...
#define ED_THIS Destin
		...
		if (LastOfs - (int)CurrOfs < bits) {
			...
			ED_LOCATOR_SET (&(Destin->AUTH_MODE_INCL___LOCATOR), CurrOfs, bits);
			Destin->AUTH_MODE_INCL = (ED_BOOLEAN)EDBitsToInt (Buffer, CURPOS, (ED_EXLONG)bits);
		}
		if (bits < 0) {
			/* Decoding failed - Go to the exit point */
			goto exitOnFail;
		...
		if ((Destin->AUTH_MODE_INCL==1)) {
			...
			if (LastOfs - (int)CurrOfs < bits) {
				...
				ED_LOCATOR_SET (&(Destin->AUTHR___LOCATOR), CurrOfs, bits);
				Destin->AUTHR = (ED_LONG)EDBitsToInt (Buffer, CURPOS, (ED_EXLONG)bits);
			}
			if (bits < 0) {
				/* If decoding failed, set the failed field to "not present" */
		...
		if ((Destin->AUTH_MODE_INCL==1)) {
			...
			if (LastOfs - (int)CurrOfs < bits) {
				...
				ED_LOCATOR_SET (&(Destin->RANDC___LOCATOR), CurrOfs, bits);
				Destin->RANDC = (ED_OCTET)EDBitsToInt (Buffer, CURPOS, (ED_EXLONG)bits);
			}
			if (bits < 0) {
				/* If decoding failed, set the failed field to "not present" */
		...
		if ((Destin->AUTH_MODE_INCL==1)) {
			...
			if (LastOfs - (int)CurrOfs < bits) {
				...
				ED_LOCATOR_SET (&(Destin->COUNT___LOCATOR), CurrOfs, bits);
				Destin->COUNT = (ED_OCTET)EDBitsToInt (Buffer, CURPOS, (ED_EXLONG)bits);
			}
			if (bits < 0) {
				/* If decoding failed, set the failed field to "not present" */
...
#define ED_THIS Source
		...
		for (i=0; i<max; i++) {
			CurrOfs += ENCODE_c_r_csch_PDU_Radio_Environment_Report_ADD_PILOTS_data (Buffer, CurrOfs, Source->ADD_PILOTS.data [i], pp_CS0004E_Details);
		}
		if ((pp_CS0004E_Details->n_P_REV_IN_USEs>=7)) {
...
#define ED_THIS Destin
		...
		if (LastOfs - (int)CurrOfs < bits) {
			...
			ED_LOCATOR_SET (&(Destin->ACTIVE_PILOT_STRENGTH___LOCATOR), CurrOfs, bits);
			Destin->ACTIVE_PILOT_STRENGTH = (ED_OCTET)EDBitsToInt (Buffer, CURPOS, (ED_EXLONG)bits);
		}
		if (bits < 0) {
			/* Decoding failed - Go to the exit point */
			goto exitOnFail;
		...
		if (LastOfs - (int)CurrOfs < bits) {
			...
			ED_LOCATOR_SET (&(Destin->FIRST_IS_ACTIVE___LOCATOR), CurrOfs, bits);
			Destin->FIRST_IS_ACTIVE = (ED_BOOLEAN)EDBitsToInt (Buffer, CURPOS, (ED_EXLONG)bits);
		}
		if (bits < 0) {
			/* Decoding failed - Go to the exit point */
			goto exitOnFail;
		...
		if (LastOfs - (int)CurrOfs < bits) {
			...
			ED_LOCATOR_SET (&(Destin->FIRST_IS_PTA___LOCATOR), CurrOfs, bits);
			Destin->FIRST_IS_PTA = (ED_BOOLEAN)EDBitsToInt (Buffer, CURPOS, (ED_EXLONG)bits);
		}
		if (bits < 0) {
			/* Decoding failed - Go to the exit point */
			goto exitOnFail;
		...
		if (LastOfs - (int)CurrOfs < bits) {
			...
			ED_LOCATOR_SET (&(Destin->NUM_ADD_PILOTS___LOCATOR), CurrOfs, bits);
			Destin->NUM_ADD_PILOTS = (ED_OCTET)EDBitsToInt (Buffer, CURPOS, (ED_EXLONG)bits);
		}
		if (bits < 0) {
			/* Decoding failed - Go to the exit point */
			goto exitOnFail;
		...
		for (i=0; i<Destin->ADD_PILOTS.items; i++) {
			bits = DECODE_c_r_csch_PDU_Radio_Environment_Report_ADD_PILOTS_data (Buffer, CurrOfs, Destin->ADD_PILOTS.data [i], LastOfs-CurrOfs , pp_CS0004E_Details);
			if (bits < 0) {
				/* Decoding failed - Remove the unused array items */
				SETITEMS_c_r_csch_Radio_Environment_Report_ADD_PILOTS (&Destin->ADD_PILOTS, i);
				goto exitOnFail;
			}
			CurrOfs += bits;
		}
		if ((pp_CS0004E_Details->n_P_REV_IN_USEs>=7)) {
			...
			bits = DECODE_c_r_csch_PDU_Radio_Environment_Report_Aux_Radio_Environment_Report (Buffer, CurrOfs, Destin->Aux_Radio_Environment_Report, LastOfs-CurrOfs , pp_CS0004E_Details);
			if (bits < 0) {
				/* If decoding failed, set the failed field to "not present" */
...
#define ED_THIS Destin
		...
		if (LastOfs - (int)CurrOfs < bits) {
			...
			ED_LOCATOR_SET (&(Destin->NUM_AUX_PILOTS___LOCATOR), CurrOfs, bits);
			Destin->NUM_AUX_PILOTS = (ED_OCTET)EDBitsToInt (Buffer, CURPOS, (ED_EXLONG)bits);
		}
		if (bits < 0) {
			/* Decoding failed - Go to the exit point */
			goto exitOnFail;
		...
		if (Destin->NUM_AUX_PILOTS) {
			...
			bits = DECODE_c_r_csch_PDU_Radio_Environment_Report_Aux_Radio_Environment_Report_AUX_PILOTS (Buffer, CurrOfs, Destin->AUX_PILOTS, LastOfs-CurrOfs , pp_CS0004E_Details);
			if (bits < 0) {
				/* If decoding failed, set the failed field to "not present" */
...
#define ED_THIS Destin
		...
		if (LastOfs - (int)CurrOfs < bits) {
			...
			ED_LOCATOR_SET (&(Destin->PILOT_PN_PHASE___LOCATOR), CurrOfs, bits);
			Destin->PILOT_PN_PHASE = (ED_LONG)EDBitsToInt (Buffer, CURPOS, (ED_EXLONG)bits);
		}
		if (bits < 0) {
			/* Decoding failed - Go to the exit point */
			goto exitOnFail;
		...
		if (LastOfs - (int)CurrOfs < bits) {
			...
			ED_LOCATOR_SET (&(Destin->PILOT_STRENGTH___LOCATOR), CurrOfs, bits);
			Destin->PILOT_STRENGTH = (ED_OCTET)EDBitsToInt (Buffer, CURPOS, (ED_EXLONG)bits);
		}
		if (bits < 0) {
			/* Decoding failed - Go to the exit point */
			goto exitOnFail;
		...
		if (LastOfs - (int)CurrOfs < bits) {
			...
			ED_LOCATOR_SET (&(Destin->ACCESS_HO_EN___LOCATOR), CurrOfs, bits);
			Destin->ACCESS_HO_EN = (ED_BOOLEAN)EDBitsToInt (Buffer, CURPOS, (ED_EXLONG)bits);
		}
		if (bits < 0) {
			/* Decoding failed - Go to the exit point */
			goto exitOnFail;
		...
		if (LastOfs - (int)CurrOfs < bits) {
			...
			ED_LOCATOR_SET (&(Destin->ACCESS_ATTEMPTED___LOCATOR), CurrOfs, bits);
			Destin->ACCESS_ATTEMPTED = (ED_BOOLEAN)EDBitsToInt (Buffer, CURPOS, (ED_EXLONG)bits);
		}
		if (bits < 0) {
			/* Decoding failed - Go to the exit point */
			goto exitOnFail;
...
#define ED_THIS Destin
		...
		if (LastOfs - (int)CurrOfs < bits) {
			...
			ED_LOCATOR_SET (&(Destin->PILOT_REC_TYPE___LOCATOR), CurrOfs, bits);
			Destin->PILOT_REC_TYPE = (ED_OCTET)EDBitsToInt (Buffer, CURPOS, (ED_EXLONG)bits);
			/* Set the exported tag */
			pp_CS0004E_Details->userTagPresent[4] = 1;
		...
		if (LastOfs - (int)CurrOfs < bits) {
			...
			ED_LOCATOR_SET (&(Destin->RECORD_LEN___LOCATOR), CurrOfs, bits);
			Destin->RECORD_LEN = (ED_OCTET)EDBitsToInt (Buffer, CURPOS, (ED_EXLONG)bits);
		}
		if (bits < 0) {
			/* Decoding failed - Go to the exit point */
			goto exitOnFail;
		...
		/* BEGIN OF PAD ZONE - DECODE inclusive=0 bits=3 octet(=0)/bit(=1)=0 */
		{
			...
			/* Execute the normal decoding */
			/* Type-specific fields */
			bits = DECODE_c_r_csch_PDU_Radio_Environment_Report_Aux_Radio_Environment_Report_AUX_PILOTS_Type_specific_fields (Buffer, CurrOfs, (&(Destin->Type_specific_fields)), LastOfs-CurrOfs , pp_CS0004E_Details);
			if (bits < 0) {
				/* Decoding failed - Go to the exit point */
				goto exitOnFail;
			}
...
#define ED_THIS Destin
		if (((pp_CS0004E_Details->userTag[4])==0)) {
			...
			bits = DECODE_c_r_csch_PDU_Radio_Environment_Report_Aux_Radio_Environment_Report_AUX_PILOTS_Type_specific_fields_Walsh (Buffer, CurrOfs, Destin->Walsh, LastOfs-CurrOfs , pp_CS0004E_Details);
			if (bits < 0) {
				/* If decoding failed, set the failed field to "not present" */
...
#define ED_THIS Destin
		...
		if (LastOfs - (int)CurrOfs < bits) {
			...
			ED_LOCATOR_SET (&(Destin->QOF___LOCATOR), CurrOfs, bits);
			Destin->QOF = (ED_OCTET)EDBitsToInt (Buffer, CURPOS, (ED_EXLONG)bits);
		}
		if (bits < 0) {
			/* Decoding failed - Go to the exit point */
			goto exitOnFail;
		...
		if (LastOfs - (int)CurrOfs < bits) {
			...
			ED_LOCATOR_SET (&(Destin->WALSH_LENGTH___LOCATOR), CurrOfs, bits);
			Destin->WALSH_LENGTH = (ED_OCTET)EDBitsToInt (Buffer, CURPOS, (ED_EXLONG)bits);
		}
		if (bits < 0) {
			/* Decoding failed - Go to the exit point */
			goto exitOnFail;
		...
		CurrOfs += bits;
		bits = (Destin->WALSH_LENGTH+6);
		if (LastOfs - (int)CurrOfs < bits) {
			bits = ED_MESSAGE_TOO_SHORT;
		} 
		else {
			ED_LOCATOR_SET (&(Destin->PILOT_WALSH___LOCATOR), CurrOfs, bits);
			EDAllocBinary (&(((&(Destin->PILOT_WALSH)))->value), (unsigned)(bits), &(((&(Destin->PILOT_WALSH)))->usedBits));
			EDCopyBits (((&(Destin->PILOT_WALSH))->value), 0, Buffer, CURPOS, (ED_EXLONG)bits);
		}
		if (bits < 0) {
			/* Decoding failed - Go to the exit point */
			goto exitOnFail;
...
#define ED_THIS Destin
		...
		bits = DECODE_c_f_dsch_PDU_Regular_Message_Type (Buffer, CurrOfs, (&(Destin->Message_Type)), LastOfs-CurrOfs , pp_CS0004E_Details);
		if (bits < 0) {
			/* Decoding failed - Go to the exit point */
			goto exitOnFail;
		}
		CurrOfs += bits;
		bits = DECODE_c_f_dsch_PDU_Regular_ARQ (Buffer, CurrOfs, (&(Destin->ARQ)), LastOfs-CurrOfs , pp_CS0004E_Details);
		if (bits < 0) {
			/* Decoding failed - Go to the exit point */
			goto exitOnFail;
		}
		CurrOfs += bits;
		bits = DECODE_c_f_dsch_PDU_Regular_Encryption (Buffer, CurrOfs, (&(Destin->Encryption)), LastOfs-CurrOfs , pp_CS0004E_Details);
		if (bits < 0) {
			/* Decoding failed - Go to the exit point */
			goto exitOnFail;
		}
		CurrOfs += bits;
		bits = DECODE_c_f_dsch_PDU_Regular_Integrity_Fields (Buffer, CurrOfs, (&(Destin->Integrity_Fields)), LastOfs-CurrOfs , pp_CS0004E_Details);
		if (bits < 0) {
			/* Decoding failed - Go to the exit point */
			goto exitOnFail;
		}
		CurrOfs += bits;
		/* Available from P_REV_IN_USE >= 9 */
		bits = DECODE_c_f_dsch_PDU_Regular_Extended_Encryption (Buffer, CurrOfs, (&(Destin->Extended_Encryption)), LastOfs-CurrOfs , pp_CS0004E_Details);
		if (bits < 0) {
			/* Decoding failed - Go to the exit point */
			goto exitOnFail;
		}
...
#define ED_THIS Destin
		...
		if (LastOfs - (int)CurrOfs < bits) {
			...
			ED_LOCATOR_SET (&(Destin->MSG_TYPE___LOCATOR), CurrOfs, bits);
			Destin->MSG_TYPE = (ED_OCTET)EDBitsToInt (Buffer, CURPOS, (ED_EXLONG)bits);
		}
		if (bits < 0) {
			/* Decoding failed - Go to the exit point */
			goto exitOnFail;
...
#define ED_THIS Destin
		...
		if (LastOfs - (int)CurrOfs < bits) {
			...
			ED_LOCATOR_SET (&(Destin->ENCRYPTION___LOCATOR), CurrOfs, bits);
			Destin->ENCRYPTION = (ED_OCTET)EDBitsToInt (Buffer, CURPOS, (ED_EXLONG)bits);
			/* Set the exported tag */
			pp_CS0004E_Details->userTagPresent[1] = 1;
...
#define ED_THIS Destin
		...
		bits = DECODE_c_f_dsch_PDU_Mini_ARQ (Buffer, CurrOfs, (&(Destin->ARQ)), LastOfs-CurrOfs , pp_CS0004E_Details);
		if (bits < 0) {
			/* Decoding failed - Go to the exit point */
			goto exitOnFail;
		}
		CurrOfs += bits;
		bits = DECODE_c_f_dsch_PDU_Mini_Message_Type (Buffer, CurrOfs, (&(Destin->Message_Type)), LastOfs-CurrOfs , pp_CS0004E_Details);
		if (bits < 0) {
			/* Decoding failed - Go to the exit point */
			goto exitOnFail;
		}
...
#define ED_THIS Destin
		...
		bits = DECODE_c_r_dsch_PDU_Regular_Message_Type (Buffer, CurrOfs, (&(Destin->Message_Type)), LastOfs-CurrOfs , pp_CS0004E_Details);
		if (bits < 0) {
			/* Decoding failed - Go to the exit point */
			goto exitOnFail;
		}
		CurrOfs += bits;
		bits = DECODE_c_r_dsch_PDU_Regular_ARQ (Buffer, CurrOfs, (&(Destin->ARQ)), LastOfs-CurrOfs , pp_CS0004E_Details);
		if (bits < 0) {
			/* Decoding failed - Go to the exit point */
			goto exitOnFail;
		}
		CurrOfs += bits;
		bits = DECODE_c_r_dsch_PDU_Regular_Encryption (Buffer, CurrOfs, (&(Destin->Encryption)), LastOfs-CurrOfs , pp_CS0004E_Details);
		if (bits < 0) {
			/* Decoding failed - Go to the exit point */
			goto exitOnFail;
		}
		CurrOfs += bits;
		bits = DECODE_c_r_dsch_PDU_Regular_Integrity_Fields (Buffer, CurrOfs, (&(Destin->Integrity_Fields)), LastOfs-CurrOfs , pp_CS0004E_Details);
		if (bits < 0) {
			/* Decoding failed - Go to the exit point */
			goto exitOnFail;
		}
		CurrOfs += bits;
		/* Available from P_REV_IN_USE >= 9 */
		bits = DECODE_c_r_dsch_PDU_Regular_Extended_Encryption (Buffer, CurrOfs, (&(Destin->Extended_Encryption)), LastOfs-CurrOfs , pp_CS0004E_Details);
		if (bits < 0) {
			/* Decoding failed - Go to the exit point */
			goto exitOnFail;
		}
...
#define ED_THIS Destin
		...
		bits = DECODE_c_r_dsch_PDU_Mini_ARQ (Buffer, CurrOfs, (&(Destin->ARQ)), LastOfs-CurrOfs , pp_CS0004E_Details);
		if (bits < 0) {
			/* Decoding failed - Go to the exit point */
			goto exitOnFail;
		}
		CurrOfs += bits;
		bits = DECODE_c_r_dsch_PDU_Mini_Message_Type (Buffer, CurrOfs, (&(Destin->Message_Type)), LastOfs-CurrOfs , pp_CS0004E_Details);
		if (bits < 0) {
			/* Decoding failed - Go to the exit point */
			goto exitOnFail;
		}
...
#define ED_THIS Destin
		...
		if (LastOfs - (int)CurrOfs < bits) {
			...
			ED_LOCATOR_SET (&(Destin->SERVICE_OPTION___LOCATOR), CurrOfs, bits);
			Destin->SERVICE_OPTION = (ED_LONG)EDBitsToInt (Buffer, CURPOS, (ED_EXLONG)bits);
		}
		if (bits < 0) {
			/* Decoding failed - Go to the exit point */
			goto exitOnFail;
		...
		CurrOfs += bits;
		bits = (int)(LastOfs-CurrOfs);
		ED_LOCATOR_SET (&(Destin->ADD_MS_RECORD___LOCATOR), CurrOfs, bits);
		EDAllocBinary (&(((&(Destin->ADD_MS_RECORD)))->value), (unsigned)bits, &(((&(Destin->ADD_MS_RECORD)))->usedBits));
		EDCopyBits (((&(Destin->ADD_MS_RECORD))->value), 0, Buffer, CURPOS, (ED_EXLONG)bits);
		if (bits < 0) {
			/* Decoding failed - Go to the exit point */
			goto exitOnFail;
		}
...
#define ED_THIS Destin
		...
		if (LastOfs - (int)CurrOfs < bits) {
			...
			ED_LOCATOR_SET (&(Destin->BCN___LOCATOR), CurrOfs, bits);
			Destin->BCN = (ED_OCTET)EDBitsToInt (Buffer, CURPOS, (ED_EXLONG)bits);
		}
		if (bits < 0) {
			/* Decoding failed - Go to the exit point */
			goto exitOnFail;
		...
		if (LastOfs - (int)CurrOfs < bits) {
			...
			ED_LOCATOR_SET (&(Destin->TIME_OFFSET___LOCATOR), CurrOfs, bits);
			Destin->TIME_OFFSET = (ED_LONG)EDBitsToInt (Buffer, CURPOS, (ED_EXLONG)bits);
		}
		if (bits < 0) {
			/* Decoding failed - Go to the exit point */
			goto exitOnFail;
		...
		if (LastOfs - (int)CurrOfs < bits) {
			...
			ED_LOCATOR_SET (&(Destin->REPEAT_TIME_OFFSET___LOCATOR), CurrOfs, bits);
			Destin->REPEAT_TIME_OFFSET = (ED_OCTET)EDBitsToInt (Buffer, CURPOS, (ED_EXLONG)bits);
		}
		if (bits < 0) {
			/* Decoding failed - Go to the exit point */
			goto exitOnFail;
		...
		CurrOfs += bits;
		bits = (int)(LastOfs-CurrOfs);
		ED_LOCATOR_SET (&(Destin->ADD_BCAST_RECORD___LOCATOR), CurrOfs, bits);
		EDAllocBinary (&(((&(Destin->ADD_BCAST_RECORD)))->value), (unsigned)bits, &(((&(Destin->ADD_BCAST_RECORD)))->usedBits));
		EDCopyBits (((&(Destin->ADD_BCAST_RECORD))->value), 0, Buffer, CURPOS, (ED_EXLONG)bits);
		if (bits < 0) {
			/* Decoding failed - Go to the exit point */
			goto exitOnFail;
		}
