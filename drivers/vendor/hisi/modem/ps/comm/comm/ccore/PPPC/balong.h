...
{
    ULONG  stMainDNSServerIpv6[IPV6_DNS_LEN];    /* ?¯車?IPv6 DNS﹞t???¯ */
    ULONG  stBackupDNSServerIpv6[IPV6_DNS_LEN];  /* ㊣?車?IPv6 DNS﹞t???¯ */
    ULONG    ulMainDNSServer;     /* ?¯DNS﹞t???¯ */
    ULONG    ulBackupDNSServer;   /* ㊣?DNS﹞t???¯ */
    USHORT   usApnIndex ;         /* APN?¯辰y   */
...
{
    ULONG ulDataLength;         /* 那y?Y?谷?D那y?Y米?3∟?豕㏒?豕??米﹞??∫[0, ulDataBlockLength]*/
    UCHAR * pucData;            /* 那y?Y?谷?D那y?Y米??e那?米??﹞, [pucDataBlock + 512, pucDataBlock + ulDataBlockLength]*/
    ULONG ulDataBlockLength;    /* 那y?Y?谷米?℅邦3∟?豕,*/
...
typedef struct
 {
     ULONG   ulUserTotalNum;            /* HRPD車??∫那y*/
     ULONG   ulA10RpNum;                /* HRPD車??∫A10米?谷?????那y*/
     ULONG   ulL2tpNum;                 /* rp谷?????那y*/
     ULONG   ulSIPUserNum;              /* ?辰米ㄓIP車??∫那y */
     ULONG   ulCMIPUserNum;             /* CMIP車??∫那y */
     ULONG   ulPMIPUserNum;             /* PMIPv4車??∫那y */
     ULONG   ulPMIPv6UserNum;           /* PMIPv6車??∫那y */
     ULONG   ulPpcUserNum;              /* PPC車??∫那y */
     ULONG   ulOnlineUserNum;           /* ?迆????﹞?車??∫那y */
     ULONG   ulContentUserNum;          /* ?迆豕Y??﹞?車??∫那y */
     ULONG   ulVjUserNum;               /* VJ ?1??車??∫ */
     ULONG   ulMppcUserNum;             /* MPPC?1??車??∫ */
     ULONG   ulMppVjUserNum;            /* MPPC + VJ?1??車??∫ */
     ULONG   ulPccUserNum;              /* PCC車??∫*/
     ULONG   ulDoUserNum;               /* DO車??∫那y*/
     ULONG   ul1xUserNum;               /* 1X車??∫那y*/
     ULONG   ulWifiUserNum;             /* Wifi車??∫那y*/
     ULONG   ulSipv6UserNum;            /* IPv6米ㄓ??車??∫那y*/
     ULONG   ulSipDualStackUserNum;     /* IPv6????車??∫那y*/
     ULONG   ulPMIPv6S6UserNum;         /* PMIPv6 IPv6米ㄓ??車??∫那y*/
     ULONG   ulPMIPv6DualStackUserNum;  /* PMIPv6????車??∫那y*/
     ULONG   ulEhrpdUserTotalNum;       /* eHRPD車??∫那y */
     ULONG   ulEhrpdUserA10Num;         /* eHRPD車??∫A10谷?????那y */
     ULONG   ulEhrpdPdnNumber;          /* eHRPD車??∫米?pdn芍??車那y */
     ULONG   ulEhrpdPccSessionNum;      /*PCC session芍??車那y*/
     ULONG   ulEhrpdDualStackPdnNumber;  /*eHRPD車??∫????PDN芍??車那y*/
     ULONG   ulEhrpdIpv6StackPdnNumber;  /*eHRPD車??∫IPV6-PDN芍??車那y*/
 }stDomainNumInfo;

 typedef  struct tagCPU_STATUS_CHG
{
    ULONG ulSlotId;         /**< ℅∩足?﹞⊿谷迆㊣??‘米?CPU米????-2???o? */
    ULONG ulCpuId;          /**< ℅∩足?﹞⊿谷迆㊣??‘米?CPU米?CPUo? */

    ULONG ulStatus_flg;     /**< ℅∩足?㊣??‘㊣那??㏒?豕??米﹞??∫: STATUS_CHG_FLAG_E */
    ULONG ulPreStatus;      /**< CPU?-角∩米?℅∩足? */
    ULONG ulNowStatus;      /**< CPU???迆米?℅∩足? */

    ULONG ulBoardType;      /**< ℅∩足?﹞⊿谷迆㊣??‘米?CPU米?米ㄓ～?角角D赤 */
    ULONG ulProductType;    /**< 2迆?﹞角角D赤 GGSN?⊿PDSN?⊿DPI 米豕 */
    ULONG ulBoardExtType;   /**< 米ㄓ～?角??1角角D赤 */
    /* Added start by majun 52643 at 2013-02-25 PGP V2R6C01 for 谷豕㊣?1邦角赤???? */
    ULONG ulBoardRawType;   /**< 米ㄓ～?角??1角角D赤 */
    ULONG ulBoardVirtualType;   /**< 米ㄓ～?角??1角角D赤 */
    /* Added end by majun 52643 at 2013-02-25 PGP V2R6C01 for 谷豕㊣?1邦角赤???? */

    ULONG ulSGIndex;    /**< 米ㄓSG?∩??那㊣米?芍¯??o? */
    ULONG ulSGType;     /**< 米ㄓSG?∩??那㊣米?角角D赤 */
    ULONG ulMemSize;    /**< CPU?迆∩?角角D赤 */
    ULONG ulModeType;   /**< ???‘?㏒那? */
...
{
    ULONG ulCpuChgNum;/**< ℅∩足?﹞⊿谷迆米?CPU㊣??‘米?米ㄓ～?那y芍? */
    CPU_STATUS_CHG_S   *pstStatusChg; /**< 米ㄓ～?米?CPU℅∩足?㊣??‘那y?Y */
} CPU_STATUS_PKG_S;

typedef struct tagA11_S_IFTRC_FILTER
{
    ULONG ulFlag;                                       /* ㊣赤那??迆℅迄﹞?那?*/
    ULONG ulLogicIp;                                    /* ㊣???IP米??﹞ */
    ULONG ulVpnIndex;                                   /* VPN?¯辰y*/
    ULONG ulPeerIp;                                     /* ????IP米??﹞ */
    ULONG ulMsgType;                                    /* ???⊿角角D赤 */
}A11_S_IFTRC_FILTER;
...
typedef struct
{
    ULONG   ulPdpindex;
    ULONG   ulTeidc;
    USHORT  usLocalTID;   /*㊣????赤米角id*/
    USHORT  usLocalSID;   /*㊣????芍?～id*/
    ULONG   ulGiifIndex ; /*piif???-?車?迆?¯辰y*/
    UCHAR   ucScid;         /*ppp?迄?迆sc???-id*/
    UCHAR   aucRev[3];    /*㊣㏒芍?℅???,?Y?T那1車?*/
}L2TP_DOWNPARA_S;
...
typedef struct
{
    ...
    PF_ULONG  ulModuleId;
    PF_ULONG  ulMsgCode;
    PF_ULONG  ulPdpIndex;   /* GSDBindex,PF谷??赤??????那㊣㏒??迄?Ygtpuindex∩車GTPU㊣赤?D??
...
typedef struct tagHSGW_APN_INFO_S
{
    ...
    USHORT usIdleLen;              /* idle-timerout 3∟?豕 米ㄓ??:minute */

    ULONG ulIpv6RaInterval;       /* ﹞⊿?赤?﹞車谷赤“??℅?∩車那㊣3∟ */
...
{
    ULONG   ulType;        /*D豕辰a㊣?﹞??芍11米?type*/
    ULONG   ulSdbIndex;    /* ﹞a℅～米?TLV那y?Y芍ㄓ那?車迆????芍㏒?豕米?sdb谷??????¯辰y */
    ULONG   ulSessionIndex;
    UCHAR   ucNum;         /*?a?迄米??芍11D豕辰a㊣?﹞??角谷迄??*/
    UCHAR   ucReserved;    /*㊣㏒芍?℅???*/
    USHORT  usStrLen;      /*?a???芍11米?3∟?豕 2?～邦角“TLV赤﹞米?3∟?豕??那?㊣?﹞Y那y?Y?迆豕Y3∟?豕*/
    UCHAR   ucValue[0];    /* ㊣?﹞Y米?那y?Y */
...
typedef struct
{
    ULONG ulPdpIndex;       /* RP 谷??????¯辰y */
    ULONG ulTeidc;
    ULONG ulPcfKey;         /* PCF㊣那那? */
    ULONG ulHaAddr;
    ULONG ulCoAAddr;
    ...
    USHORT usFailureCause;    /* ?∟???辰?邦D?那∫～邦?-辰辰?米 */
    USHORT usReleaseCause;    /* 豕ㄓ???-辰辰?米 */

    USHORT usIriTargetMap;    /*車?車迆足?3?米?谷??????D米?bitmap ?米*/
    USHORT usIdpTargetMap;    /*車?車迆足?3?米?谷??????D米?bitmap ?米*/
...
typedef struct tagA11_QOS_INFO
{
    ...
    ULONG ulInputPeakRate;  /*谷?DD﹞??米?迄?那㏒? 米ㄓ???abps*/
    ULONG ulOutputPeakRate; /*??DD﹞??米?迄?那㏒? 米ㄓ???abps*/
    ULONG ulGuarantBitRateUp; /*PDSN車??∫谷?DD㊣㏒?∟?迄?那 (bps)*/
    ULONG ulGuarantBitRateDown; /* PDSN車??∫??DD㊣㏒?∟?迄?那(bps) */
...
typedef struct tagPTM_COMPONENT_INFO
{
    ...
    USHORT usOsID;               /* ℅⊿辰a:2?辰??“?迄車D?㏒?谷??3?那??‘芍? */
    USHORT usReserve990;
...
typedef struct
{
    ULONG ulDmpuDpeNum;                                     /* ℅車?“谷?DPE??3足那y㏒????～1足?“?a14 */
    ULONG ulMppcUserNum[MAX_DMPU_NUM][MAX_DPE_NUM_PDSN];    /* ㊣??迄～??迆?¯℅車?“DPE谷??∟??米?MPPC車??∫那y */
    ULONG ulVjUserNum[MAX_DMPU_NUM][MAX_DPE_NUM_PDSN];      /* ㊣??迄～??迆?¯℅車?“DPE谷??∟??米?VJ車??∫那y */
...
{
    USHORT   usMessageType;              /*車?車迆??那?﹞⊿?赤?辰?車那?*/
    USHORT   usPririty;                          /*?赤車??豕??㏒“車??∫℅??o?“辰?㏒?,?∫3?4??車??豕?? */
    ULONG    ulSrcModuleID;                 /*?∩?㏒?谷o?*/
    ULONG    ulDstModuleID;                 /*??㊣那?㏒?谷o?*/
    ULONG    ulSequence;                      /*﹞⊿?赤D辰o?,RPC℅“車?*/

    ULONG    ulSrcSlotID;                    /* ?∩2???o? */
    ULONG    ulDstSlotID;                       /* ??米?2???o? */
    UCHAR    ucMsgType;                     /* ???⊿角角D赤㏒?豕?PDU, TIMER, REQUEST, NOTIFY... */
    UCHAR    ucMsgBodyStyle;            /*車?車迆???¯???⊿赤﹞o赤???⊿足?那?﹞?那?辰?足?米?,?迆FSU?D?¯辰a車?車迆??﹞?那?﹞?MBuf﹞?那?*/
    USHORT   usMsgCode;                    /* ???⊿??㏒???足????⊿角角D赤米???﹞? */

    USHORT   usResponseType;             /* 那?﹞?辰a?車?車那?﹞?車??∫車|∩e㏒oACK/ NOACK/NEGACK*/
    USHORT   usFrameLen;                     /* frame content 米?3∟?豕 ㏒?車??∫那y?Y??3∟?豕   */
/* Modified start by jiangyongliang 50758 at 2012-03-09 PGPV2R6 for 1∟3足那那??linux */
#ifndef __X86_PLATFORM__
    VOID   * ptrMsgBody;                   /*???⊿足?????*/
#else
    ULONG    ulMsgBodyOffset;                   /*???⊿足?????*/
#endif
/* Modified end by jiangyongliang 50758 at 2012-03-09 PGPV2R6 for 1∟3足那那??linux */
    USHORT   usAppSeqNum;                /*車|車?2??米芍Do?㏒?車?車迆﹞⊿?赤﹞?o赤?車那?﹞????車車|∩e?????“芍⊿1?芍a*/
    USHORT   usReserved;                     /*㊣㏒芍? ㏒?辰?o車角?3?那1車?*/
}  MSG_HDR_S, SYS_MSG_S;
...
typedef struct tagUSM_LAP2_MSG
{
    ...
    ULONG ulPdpIndex;        /*PDP谷??????¯辰y*/
    ULONG ulTeidc;

    ULONG aulPdpAddr[LAP_IP_ULONG_NUM];     /*米??﹞*/
    ...
    USHORT usPoolIndex;      /*AAA﹞米??米??﹞3??¯辰y*/
    USHORT usApnIndex;       /*APN?¯辰y*/
    USHORT usCheckSum;       /*?足?谷o赤*/
...
typedef struct tagAaaAuthRspMsg
{
    ...
    ULONG ulMsgType;                /* ???⊿角角D赤 */
    ULONG ulSDBIndex;               /* context?¯辰y ?∩SDB?¯辰y */
    ...
    ULONG ulIdleTimeout;            /* ?芍?～3?那㊣那㊣?? */
    ULONG ulIPMTU;                  /* MTU2?那y */
    ULONG ulIp;                     /* IP米??﹞ */
    ULONG ulIpMask;                 /* IP米??﹞℅車赤??迆?? */
    ULONG ulPIMARY_DNS_SERVER;      /* ?¯DNS SERVER */
    ULONG ulSECOND_DNS_SERVER;      /* ∩車DNS SERVER */
    ULONG ulAlwaysOn;               /*r002*/
    ULONG ulMNAAAFlag;
    ULONG ulAcctInterimInterval;
     /* L2TP */
    USHORT usTunnelType[AAA_L2TP_LNS_NUM];                 /* ?赤米角角角D赤㏒????～???∫3?L2TP */
    USHORT usTunnelMediaType[AAA_L2TP_LNS_NUM];            /* 1?12赤?角角D赤㏒????～???∫3?IP */
    ULONG  ulTunnelServer[AAA_L2TP_LNS_NUM];                   /* LNS米??﹞ */
    ...
    ULONG  ulMipAgentSpi;                       /* PMIPv4車??∫車?HA??～2豕?芍a??SPI */
    ULONG  ulPmipV6Spi;                         /* PMIPv6車??∫車?LMA??～2豕?芍a??SPI */
...
typedef struct
{
    ...
    ULONG       ulUsmStateTimerId;          /* USM℅∩足??迆?“那㊣?¯ID */
    ULONG       ulA11StateTimerId;          /* A11℅∩足??迆?“那㊣?¯ID */

    ULONG       ulSessionTimeOut;           /* Session Time Out */
    ULONG       ulReleaseCode;              /* 車??∫豕ㄓ???迆2??-辰辰??㏒?AM_RELEASE_CODE_E㏒??迄車D???邦谷?㊣“豕ㄓ???-辰辰??車谷∩?℅a????米? */
    ...
    ULONG       ulLifeTimerId;              /* LifeTime?“那㊣?¯ID */
    ULONG       ulSessionUpdateTimerId;     /* SessionUpdateReq??﹞⊿?“那㊣?¯ID */

    ULONG       ulRegUpdateTimerId;         /* RegUpdateReq??﹞⊿?“那㊣?¯ID */
    ULONG       ulSessionTimeId;            /* 車??∫米?session time?“那㊣?¯ID */
    ULONG       ulReAuthTimeId;             /* 車??∫米?reauth time?“那㊣?¯ID */
    /* BEGIN: Added for PN: PS10.1 HSGW by lihairong, 2013/3/4 */
    ULONG       ulConMainTimerId;          /* A11,LCP,EAP℅那?∩㊣㏒芍??“那㊣?¯ID */
    /* END:   Added for PN: PS10.1 HSGW by lihairong, 2013/3/4 */

    ULONG       ulWaitPcfReleaseTimeId;     /* lifetime ㏒? 0米?rrq?“那㊣?¯id */
    ...
    USHORT      usLifetime;                 /* Lifetime?“那㊣?¯那㊣3∟㏒?米ㄓ??:s */
    USHORT      usProtocolType;

    USHORT      usTokenId;
    USHORT      bAlawysOn           : 1,            /* 那?﹞??aAlwaysOn車??∫ */
    ...
    ULONG       ulBackupPreRpIndex;     /*㊣?﹞Y?角1?*/
    ULONG       ulBackupNextRpIndex;    /*㊣?﹞Y?角1?*/
    ULONG       ulBackupLifetimeExpire; /*㊣?﹞Y?角1?,?迆㊣?～?那?米??¯～?米?lifetime?“那㊣?¯㊣?﹞Y那y?Yo車㏒?㊣?～?????米?3?米?  */
    ULONG       ulBackupSesstimeExpire; /*㊣?﹞Y?角1?,?迆㊣?～?那?米??¯～?米?session timeout?“那㊣?¯㊣?﹞Y那y?Yo車㏒?㊣?～?????米?3?米?  */
...
typedef struct tagDiamAuthApnInfo
{
    ...
    ULONG ulULMaxBw;                                                    /* 谷?DD℅?∩車???車∩??赤 */
    ULONG ulDLMaxBw;                                                    /* ??DD℅?∩車???車∩??赤 */
...
{
    ULONG  ulRpIndex;   /* ?TD∫那㊣足?0 */
    USHORT usPcfIndex;  /* ?TD∫那㊣足?0xffff */
    USHORT usReserved;  /* 車?車迆辰?o車角??1, ?TD∫那㊣足?0 */
}MEAOBJINDEX_STRU;
...
typedef struct tagPPP_CFG_INFO_REC_S
{
    ...
    UCHAR          ucAuthoption;                                            /* ??豕“﹞?那?谷豕?“*/    /* Added by LHT04029 at 2006-06-15 R2C02B025 for BYHD10294 */
...
{
    ULONG   ulTeidc;               /*pdp谷?????㊣那那?*/
    ULONG   ulPdpindex;            /*pdp谷??????¯辰y*/
    ULONG   ulGtpuIndex;           /*??﹞⊿??℅a﹞⊿??米?gtpuindex,??辰?o車*/
    ULONG   ulFilterTableIndex;    /*那y?Y??1y??足??t芍∩㊣赤米??¯辰yn㏒???辰?o車*/
    ULONG   ulSendACCM;            /*ACCM applied to send message */
    ULONG   ulRecvACCM;            /*ACCM applied to receive message*/
    ULONG   ulIFIndex;             /*℅車?車?迆?¯辰y*/
    ULONG   ulLocalIP;
    ULONG   ulPeerIP;             /*lns ip */
    USHORT  usReqApnIdx;
    USHORT  usApnIndex;
    USHORT  usTraceId;             /*豕?赤??迆℅迄*/
...
typedef struct
{
    ULONG  ulVpnId;
    ULONG  ulGiifIndex;
    USHORT usGroupNum;                          /* VPDN℅谷米?㊣那o?,㊣?米??????谷??車DD∫ */
...
typedef struct
{
    ULONG ulFsmId;          /* ℅∩足??迆那米角yo?*/
    ULONG ulCurState;       /* 米㊣?～℅∩足?*/
    ULONG ulEvent;          /* 那?豕?那??t */
    ULONG ulRelCode;
