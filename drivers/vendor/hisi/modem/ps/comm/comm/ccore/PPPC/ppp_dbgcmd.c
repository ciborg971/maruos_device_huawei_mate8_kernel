...
#if 1
ULONG ToLowerString ( CHAR * szSource, CHAR * szDest )
{
    ULONG ulTmp;
    ULONG ulLen = 0;
...
VOID PPP_DebugCbgCntProc( ULONG argc, CHAR *argv[] )
{
    LONG lReturn = 0;
...
VOID SRPppCfgCompress( ULONG argc, CHAR * argv []   )
{
    ULONG ulDmpuId = 0, ulDpeId = 0;
...
VOID PPP_DebugSetPrint(ULONG ulPrintLevel)
{
    if (ulPrintLevel > 1)
    {
...
VOID PPP_DebugSetPPPInfo(ULONG ulArgc, CHAR *szArgv[])
{
    ULONG ulRpIndex = 0;
    ULONG ulPeerId = 0;
    ULONG ulAAAType = 0;
    PPP_LUPPARA_S stUpPara = {0};
...
#if 0
    if (0 != _VerifyAndCvtUlong(szArgv[1], 0, PPP_MAX_USER_NUM, &ulRpIndex))
    {
        (VOID)vos_printf("\r\n wrong para!");
    }


    if (0 != _VerifyAndCvtUlong(szArgv[2], 0, 520, &ulPeerId))
    {
        (VOID)vos_printf("\r\n wrong para!");
    }

    if (0 != _VerifyAndCvtUlong(szArgv[3], 0, 1, &ulAAAType))
    {
        (VOID)vos_printf("\r\n wrong para!");
    }
#endif
    if (!VOS_StrCmp(szArgv[0], "add"))
    {
        ...
        pstPppInfo->usPeerId = (USHORT)ulPeerId;
...
VOID SRPppShowDebug( ULONG argc, CHAR * argv []   )
{

    CHAR ucSet = 1;
    ULONG ulType = PPP_DEBUG_EVENT;
    USHORT usProtocol = PPP_LCP;
    ...
    else if (argc == 3)  /* added by gxf 20030311 */
    {
        /*--------------------------------------------------------------------*/
        /* in favour of search,                                               */
        /*    1. change the upper letters in the word to lower                */
...
VOID SRPppShowVSNCPDetailInfo(PPPINFO_S *pstPppInfo)
{
    ...
    for(i = 0; i < A11_MAX_PDN_NUM; i++)
    {
        ...
        {
            pstPdnContext = NULL;
            ulRet = SDB_GetPdnContextByIndex((ULONG)pstRpContext->ausPdnIndexArray[i], &pstPdnContext);
 ...
 * Other        : None                                                     *
 ***************************************************************************/
VOID SRPppShowCBDetailInfo( ULONG argc, CHAR * argv []   )
{
    ULONG ulRPIndex = 0, ulSwitch = 0xffffffff;
    ...
    if (ulSwitch & 1)    /* PPPINFO_S */
    {
        vos_printf("\r\n------------------"
                   ...
                   (ULONG)pstPppInfo->pstConfigInfo->bAuthMethod, pstPppInfo->pstConfigInfo->bAuthRmtChap,
                   ...
                   (ULONG)pstPppInfo->ucDmpuId,
                   pstPppInfo->ucDpeId,
                   (ULONG)pstPppInfo->usPeerId,
    ...
    if ((ulSwitch & 2) && pstLcpInfo)    /* PPPLCPINFO_S */
    {
        vos_printf("\r\n------------------"
                   ...
                   (ULONG)pstLcpInfo->stHisOptions.neg_asyncmap, (VOS_UINT)pstLcpInfo->stHisOptions.asyncmap,
    ...
    if ((ulSwitch & 4) && pstIpcpInfo)    /* PPPIPCPINFO_S */
    {
        vos_printf("\r\n------------------"
                   ...
                   (ULONG)pstIpcpInfo->stFsm.sRetransmits, (ULONG)pstIpcpInfo->stFsm.usNakloops,
    ...
    if ((ulSwitch & 64) && pstCcpInfo)    /* PPPCCPINFO_S */
    {
        vos_printf("\r\n------------------"
                   ...
                   (ULONG)pstCcpInfo->stGotOptions.bStac_compress, (ULONG)pstCcpInfo->stGotOptions.bMppc_compress,
                   ...
                   (ULONG)pstCcpInfo->stHisOptions.bStac_compress, (ULONG)pstCcpInfo->stHisOptions.bMppc_compress,
...
#endif
VOID PPP_ShowFsmStat(ULONG *pulState)
{
    ULONG i, j , len;
    CHAR aucTemp[200] = { 0 };
    vos_printf("\r\n      | State");
    ...
    {
        len = (ULONG)VOS_sprintf(aucTemp,"%s |", g_cPPP_Debug_event[i]);
...
VOID PPP_ClearCnt(VOID)
{
    ...
    PPP_MemSet((void *)g_ulPppDBGStatisc, 0, sizeof(ULONG) * PPP_DBG_BUTT);
    PPP_MemSet((void *)g_ulPppFsmStat[0], 0, (sizeof(ULONG) * 5 * PPP_EVENT_FSM_BUTT * PPP_STATE_BUTT));
...
{
    ULONG i             = 0;
    ULONG ulPrintCnt    = 100; /* 一次性打印个数上限 */
    ULONG ulUsedCnt     = 0;
...
ULONG PPP_DbgShowPppInfo(ULONG ulRpIndex)
{
    PPPINFO_S *pstPppInfo = NULL;
    ...
    vos_printf("\r\nulNegoStartTime: \t%u", (ULONG)pstPppInfo->ulNegoStartTime);
    vos_printf("\r\nulNegoEndTime: \t%u", (ULONG)pstPppInfo->ulNegoEndTime);
...
VOID PPP_ShowRPIndexByDynAAAIndex(USHORT usDynAAAIndex)
{
    ULONG i = 0;
    ULONG ulCounter = 0;
 ...
 * Other        : None                                                     *
 ***************************************************************************/
VOID SRPppShowInfo( ULONG argc, CHAR * argv []   )
{
...
#endif
        PPP_ShowRPIndexByDynAAAIndex((USHORT)ulIndex);
        return;
    }
...
#if 0
        /*         USHORT usIndex; */ /* Deleted by zhudaoming 62333 at 2008-03-24 V9R7C01B02 for pc-lint */
        /*lint -e746*/
        if (E_CRM_ACTIVE != CRM_GetSlotPrimaryInfoOfSelfGroup((UCHAR)DVM_SELF_SLOTID))
        {
            /*             vos_printf("\nL2tpUserNum: %d", g_ulPppL2tpUserNum); */ /* Deleted by zhudaoming 62333 at 2008-04-14 V9R7C01B02 for L2TP BACKUP */
        }
        /*lint +e746*/
        else
        {
...
VOID SRPppShowCBInfo( ULONG argc, CHAR * argv []   )
{
    ULONG i = 0;
    ULONG ulSwitch = 0;
    ULONG ulCounter = 0;
    ULONG ulNegoCounter = 0;
    ULONG ulPDNNegoCounter = 0;
    ULONG ulRPNegoCounter = 0;
    ULONG ullcpCounter = 0;
    ULONG ulipcpCounter = 0;
    ULONG ulchapCounter = 0;
    ULONG ulIpv6cpCnt = 0;
    ULONG ulpapCounter = 0;
    ULONG ulccpCounter = 0;
    ULONG ulconfigcounter = 0;
    ULONG uleapCounter = 0;
...
VOID StartPPPRego( ULONG argc, CHAR * argv []   )
{
    ULONG ulA10Index = 0;
...
VOID PPP_PrintPerfVar( ULONG argc, CHAR * argv [] )
{
    /* PPP_LAP_Debug(); */
...
VOID PPP_ShowEcho( ULONG argc, CHAR * argv [] )
{
...
VOID PPP_SetDebugInfo( ULONG argc, CHAR * argv [] )
{
...
VOID PPP_PrintMemInfo(UCHAR *pucStructName, ULONG ulUnitSize, ULONG ulUnitNum, ULONG ulMemSize, UCHAR *pucMemType, UCHAR ucStatic)
{
...
VOID PPP_DbgShowMemInfo(ULONG argc, CHAR* argv[])
{
    if (0 == PPP_SELF_CSI)
    {
        return;
    ...
    //D板和E板
    PPP_PrintMemInfo((UCHAR *)"g_ulPppDBGStatisc", sizeof(ULONG), PPP_DBG_BUTT, sizeof(ULONG)*PPP_DBG_BUTT, (UCHAR *)"SC", 'N');

    PPP_PrintMemInfo((UCHAR *)"g_ulPppFsmStat", sizeof(ULONG), 5 * PPP_EVENT_FSM_BUTT * PPP_STATE_BUTT,
                 sizeof(ULONG) * 5 * PPP_EVENT_FSM_BUTT * PPP_STATE_BUTT, (UCHAR *)"SC", 'N');
...
#if (VRP_MODULE_LINK_PPP_EAP == VRP_YES)
    PPP_PrintMemInfo((UCHAR *)"g_astPppEapPool", sizeof(PPPEAPINFO_S), PPP_MAX_USER_NUM + 1,
                 sizeof(PPPEAPINFO_S) * (PPP_MAX_USER_NUM + 1), (UCHAR *)"SC", 'N');
#endif
