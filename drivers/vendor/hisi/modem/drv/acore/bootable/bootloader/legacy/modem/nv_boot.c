...
u32 nv_boot_dload_info_init(void)
{
	...
	/*lint -save -e838*/
    ret = g_file_func_table.file_read((char*)NV_DLOAD_SEC_NAME,0, &nv_dload, sizeof(nv_dload),NULL);
	/*lint -restore +e838*/
    if(ret)
    {
    ...
    if(multi_card)
    {
        ...
        g_boot_file_info.other_card_info = (STRU_XNV_MAP_FILE_INFO *)nv_malloc(nv_dload.ulSimNumEx * sizeof(STRU_XNV_MAP_FILE_INFO));
        ...
        ret = g_file_func_table.file_read((char*)NV_DLOAD_SEC_NAME, sizeof(nv_dload), other_card_info, \
    ...
    memcpy(&g_boot_file_info.nv_dload, &nv_dload, sizeof(nv_dload));
    if(multi_card)
    {
		/*lint -save -e668*/
        memcpy(g_boot_file_info.other_card_info, other_card_info, nv_dload.ulSimNumEx * sizeof(STRU_XNV_MAP_FILE_INFO));
		/*lint -restore +e668*/
        nv_free(other_card_info);
    }
	/*lint -save -e438*/
...
/*lint -save -e838*/
    ...
    ret = (u32)g_file_func_table.file_read((char*)name,0,&ctrl_info,sizeof(ctrl_info),NULL);
    if(NAND_OK != ret)
    {
...
/*lint -restore +e838*/
    ...
    ret = (u32)g_file_func_table.file_read((char*)name,sizeof(struct nv_ctrl_file_info_stru),file_info,ctrl_info.file_size,NULL);
    if(NAND_OK != ret)
    {
