...
static int logger_open(struct inode *inode, struct file *file)
{
	...
	if(chsell_get_work_flag())
	{
		permission = cshell_get_permit();
		ret = (int)bsp_icc_send((u32)ICC_CPU_MODEM, (ICC_CHN_IFC << 16)|IFC_RECV_FUNC_CSHELL, (unsigned char*)&permission, sizeof(permission));
		if(ret <= 0)
		{
...
//    now = current_kernel_time();
	...
	header.sec = 0;
	header.nsec = 0;
	header.len = min_t(size_t, log_left, LOGGER_ENTRY_MAX_PAYLOAD);
	...
	ret = do_write_log_from_user(log, in_buff, header.len);
	if(unlikely(ret < 0)) {
		log->w_off = orig;
		mutex_unlock(&log->mutex);
		return ret;
