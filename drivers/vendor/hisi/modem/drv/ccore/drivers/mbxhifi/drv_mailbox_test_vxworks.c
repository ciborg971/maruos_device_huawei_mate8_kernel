...
extern void* mailbox_memcpy(void * dst, const void * src, unsigned int size);

extern void *mailbox_memset(void * m, int c, unsigned int size);
...
#ifndef _DRV_LLT_
        time_us = mdrv_timer_get_normal_timestamp();
#else
        time_us--;
#endif
...
char* tm_mailbox_msg_pack(unsigned long rcode, unsigned long size)
{
    ...
    mailbox_memcpy((usr_str + 1), &g_tm_mailbox_data_str[0], size);
...
)
{
    ...
    snprintf(TaskName,255,"tm_mailbox_msg_multi_send(%u)",(unsigned int)task_num);
...
int ifc_acore_printk(const char *fmt, ... )
{
    ...
    mailbox_memcpy(out_buf, head_str, strlen(head_str));
...
{
    return (long)mailbox_memcpy(dest, src, dest_len);
}
...
IFC_GEN_EXEC6(ccpu_ifc_base_test2,
 ...
 long ccpu_ifc_test_acpu(unsigned int len, unsigned int loop_num)
 {
    ...
    while((flag == 1) || (i < loop_num)){
        mailbox_memset(src, 0x5a, len);
        mailbox_memset(dst, 0, len);
...
long ifc_test_acore_fs(char* name, long size)
{
    ...
    mailbox_memset(string_w, 0x5a, size);
    mailbox_memset(string_r, 0x00, size);
