...
/*双核编码测试，M核上发送，A核上接收*/
BSP_S32 SOCP_ST_ENCODE_DCORE_STRESS()
{
    char task_name[30] = {0};
...
BSP_S32 SOCP_ST_ENCODE_STRESS_TCM(u32 addr,u32 ulDatalen)
{
...
BSP_S32 SOCP_ST_ENCODE_STRESS_SRAM(u32 addr,u32 ulDatalen)
{
    SOCP_ST_CASE_S *pCase = &g_stCase[2];
    u32 ret;
    char task_name[30] = {0};

    pCase->SrcCh = SOCP_CODER_SRC_LBBP1;
    pCase->eMode = SOCP_ENCSRC_CHNMODE_CTSPACKET;
...
extern s32 bsp_socp_drx_bak_reg(void);
extern s32 BSP_BBPDMA_DrxBakReg(void);
extern s32 BSP_BBPDMA_DrxRestoreReg(void);
extern void bsp_socp_drx_restore_reg(void);
extern u32 bsp_socp_cansleep(void);
extern void bsp_socp_stop_dsp(void);
extern void bsp_socp_start_dsp(void);
extern void  bsp_socp_enable_lte_dsp_bbp(u32 ulChanId);
...
BSP_S32 SOCP_ST_DEBUG_TEST(void)
{
    ...
    (void)bsp_socp_cansleep();
    (void)bsp_socp_drx_bak_reg();
    (void)bsp_socp_drx_restore_reg();
