...
/******************************************************************************/
/*lint --e{123,124,522,525,539,537,438}*/
...
int pm_register_sleep_ops(unsigned char* pname,PWC_SLEEP_FUNCPTR suspend,PWC_SLEEP_FUNCPTR resume){
    ...
    strncpy((char *)ops.name,(const char *)pname,MAX_PWR_ID_NAME_SIZE-1);
...
void balong_pm_enter(suspend_state_t state)
{
    ...
    if(pm_ctrl_info.sleep_flag)
    {
        if (0 == SRE_LowPowerSuspend(LOW_POWER_TYPE_GIC))
        {
            ...
            (void)SRE_LowPowerSuspend(LOW_POWER_TYPE_L2CACHE);
           ...
           tcxo_suspend();
            ...
            (void)SRE_LowPowerResume(LOW_POWER_TYPE_L2CACHE);
            pm_timer_stamp(pm_stamp_start_addr + STAMP_AFTER_SLEEPOPS_RESUME);
            SRE_LowPowerResume(LOW_POWER_TYPE_GIC);
...
static inline int bsp_get_pm_dts_value(void){
	 u32 state_offset[6] = {0},pm_boot_offset=0,boot_addr_shift=0,pm_remap_offset=0,ret = 0,sysctrl_enum = 0,id = 0;;
	...
	if(of_property_read_u32(node, "boot_offset_sysctrl", &sysctrl_enum)){
	...
	if(of_property_read_u32(node, "boot_addr_shift", &boot_addr_shift)){
        boot_addr_shift = 0; /* no need to do shift */
	}
	if(of_property_read_u32(node, "boot_offset", &pm_boot_offset)){
		pm_printk(BSP_LOG_LEVEL_ERROR,"pm_boot_offset not find. pm init failed\n");
		return -1;
	}
	else{
		writel(pm_mem_ccore_base_addr>>boot_addr_shift,(u32)sys_baseaddr+pm_boot_offset);
	}
