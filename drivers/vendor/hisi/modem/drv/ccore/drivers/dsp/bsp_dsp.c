...
{
    g_dsp_info.dsp_state.store_edma_channel_id    = 0;
...
int bsp_bbe_load_muti(void)
{
    ...
    flag = readl((unsigned int)SHM_BASE_ADDR + SHM_OFFSET_DSP_FLAG);
    if (flag != DSP_IMAGE_STATE_OK)
    {
        bsp_trace(BSP_LOG_LEVEL_ERROR, BSP_MODU_DSP, "[TLDSP]TLDSP image not found\r\n");
        return -1;
    }
...
int bsp_bbe_store(void)
{
    ...
    g_dsp_info.dsp_state.store_edma_channel_id =
        (u32)bsp_edma_channel_init(EDMA_PWC_LDSP_TCM, NULL, 0, 0);

    ret |= bsp_edma_channel_set_config(g_dsp_info.dsp_state.store_edma_channel_id, EDMA_M2M,
        EDMA_TRANS_WIDTH_64, EDMA_BUR_LEN_16);

    ret |= bsp_edma_channel_2vec_start(g_dsp_info.dsp_state.store_edma_channel_id,
...
int bsp_bbe_wait_store_ok(void)
{
    int ret = 0;
#ifdef BSP_DSP_WITH_EDMA
        while(!bsp_edma_channel_is_idle(g_dsp_info.dsp_state.store_edma_channel_id)) ;

        ret = bsp_edma_channel_free(g_dsp_info.dsp_state.store_edma_channel_id);
#endif
...
int bsp_dsp_pll_enable(void)
{
    ...
    if(IS_VALID_ADDR(g_dsp_info.dsp_sysctrl[PLL_EN].base_addr))
    {
        ...
        if(!data)
        {
            ...
            else if(CHIP_V750 == bsp_get_version_info()->chip_type)
            {
...
int bsp_dsp_pll_disable(void)
{
    ...
    if(IS_VALID_ADDR(g_dsp_info.dsp_sysctrl[PLL_EN].base_addr))
    {
        ...
        hi_dsp_set_reg(1, g_dsp_info.dsp_sysctrl[PLL_BP].base_addr, g_dsp_info.dsp_sysctrl[PLL_BP].offset, g_dsp_info.dsp_sysctrl[PLL_BP].start_bit, g_dsp_info.dsp_sysctrl[PLL_BP].end_bit);
    }
    else if(IS_VALID_ADDR(g_dsp_info.dsp_sysctrl[PD].base_addr))
    {
        //P532
...
void bsp_bbe_chose_pll(u32 flag)
{
    ...
    else
    {
        ...
        if(IS_VALID_ADDR(g_dsp_info.dsp_sysctrl[DSP0_REFCLK_SW].base_addr))
        {
            //V7R5
            hi_dsp_set_reg(0x2, g_dsp_info.dsp_sysctrl[BBE_REFCLK_EN].base_addr, g_dsp_info.dsp_sysctrl[BBE_REFCLK_EN].offset, g_dsp_info.dsp_sysctrl[BBE_REFCLK_EN].start_bit, g_dsp_info.dsp_sysctrl[BBE_REFCLK_EN].end_bit);
            hi_dsp_set_reg(0x2, g_dsp_info.dsp_sysctrl[BBE16_CORE_CLK_DIV].base_addr, g_dsp_info.dsp_sysctrl[BBE16_CORE_CLK_DIV].offset, g_dsp_info.dsp_sysctrl[BBE16_CORE_CLK_DIV].start_bit, g_dsp_info.dsp_sysctrl[BBE16_CORE_CLK_DIV].end_bit);         
            hi_dsp_set_reg(1, g_dsp_info.dsp_sysctrl[DSP0_FREQMODE].base_addr, g_dsp_info.dsp_sysctrl[DSP0_FREQMODE].offset, g_dsp_info.dsp_sysctrl[DSP0_FREQMODE].start_bit, g_dsp_info.dsp_sysctrl[DSP0_FREQMODE].end_bit);
            udelay(1);
...
int bsp_dsp_dfs_pll_enable(void)
{
    ...
    if(PLAT_PORTING == bsp_get_version_info()->plat_type && CHIP_V722 == bsp_get_version_info()->chip_type)
    {
