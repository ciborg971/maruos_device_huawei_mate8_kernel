...
void pmic_log_level_set_debug(u32 level)
{
    ...
    pmu_print_dbg("bbp log id=%d, set to level=%d)\n",BSP_MODU_PMU,level);
}
/*****************************************************************************
...
* 说 明 :
*****************************************************************************/
void pmic_log_level_get_debug(void)
{
    u32 log_level = 0;

    log_level = bsp_log_module_cfg_get(BSP_MODU_PMU);
    pmu_print_dbg("bbp log id=%d, level=%d\n",BSP_MODU_PMU,log_level);
}
...
void pmic_xo_reg_debug(void)
{
    ...
    pmu_print_dbg("****************c1c2 addr************************\n");
    pmu_print_dbg("%-20s%-20s%-20s%-20s\n","c1_base","c2_base","c2_mask","c2_offset");
    xo_ctrl = &g_pmic_xoinfo.xo_ctrl;
    pmu_print_dbg("0x%-20x0x%-20x0x%-20x0x%-20x\n",xo_ctrl->c1_base,xo_ctrl->c2_base,xo_ctrl->c2_mask,xo_ctrl->c2_offset);

    pmu_print_dbg("****************xorf addr************************\n");
    pmu_print_dbg("%-20s%-20s%-20s%","rf_id","onoff_addr","onoff_bit");
    ...
    for(rf_id = 0; rf_id < g_pmic_xoinfo.xorf_num; rf_id++)
    {
        ...
        pmu_print_dbg("%-20d0x%-20x%-20d%",xorf_ctrl->rf_id,xorf_ctrl->reg_ctrl.reg_base,xorf_ctrl->reg_ctrl.reg_bit);
...
void pmu_regulator_debug(void)
{
    ...
    pmu_print_dbg("%-10s%-15s%-15s%-15s%-15s", "regu_id", "pmic_type", "volt_name","is_enabled","voltage(uV)");
    pmu_print_dbg("%-10s%-20s\n", "gpio_num","consumer");
    ...
    list_for_each_entry(regulator, &regulator_list, list)
    {
        if(strcmp(regulator->regulator_pmic_type, "pmic") == 0)
        {
            ...
            pmu_print_dbg("%-10d%-15s%-15s%-15d%-15d", regulator->id,regulator->regulator_pmic_type,\
                volt_ctrl->name,onoff,voltage);
            pmu_print_dbg("%-10s","NULL");
        }
        ...
        else if(strcmp(regulator->regulator_pmic_type, "dcdc") == 0)
        {
            ...
            pmu_print_dbg("%-10d%-15s%-15s%-15d%-15s", regulator->id,regulator->regulator_pmic_type,\
                dcdc_ctrl->name,onoff,"ctrlbypdm");
            pmu_print_dbg("%-10d",dcdc_ctrl->gpio_id);
        }
        else if(strcmp(regulator->regulator_pmic_type, "dummy_pmic") == 0)
        {
            pmu_print_dbg("%-10d%-15s%-15s%-15s%-15s%", regulator->id,regulator->regulator_pmic_type,\
                "dummy","NULL","NULL");
            pmu_print_dbg("%-10s","NULL");
        }
        /*print consumers*/
        ...
        {
            pmu_print_dbg("%s,", regulator->consumer_supplies[i].supply);
        }
        pmu_print_dbg("\n");
...
void pmu_help(void)
{
    ...
    pmu_print_dbg("pmic_volt_set_debug(volt_id,voltage)             :set volt voltage\n");
    ...
    pmu_print_dbg("dcdc_state_debug         :show exdcdc state and info\n");
    pmu_print_dbg("pmu_regulator_debug      :show power all relationship\n");
    pmu_print_dbg("pmic_xo_state_debug      :show xo c1c2 and rfclk state\n");
    pmu_print_dbg("pmic_xo_reg_debug        :show xo c1c2 and rfclk reg info\n");
    pmu_print_dbg("pmu_modem_init_debug     :init modem volt\n");
