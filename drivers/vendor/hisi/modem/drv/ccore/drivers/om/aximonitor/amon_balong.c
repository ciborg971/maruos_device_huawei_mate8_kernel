...
s32 axi_socp_src_chan_init(void)
{
    ...
    socp_chan.sCoderSetSrcBuf.pucInputStart  = AXI_VITR_TO_PHYS(bd_addr);                         /* 输入通道起始地址 */
    socp_chan.sCoderSetSrcBuf.pucInputEnd    = AXI_VITR_TO_PHYS(bd_addr) + SOCP_BD_BUF_SIZE -1;   /* 输入通道结束地址 */
    socp_chan.sCoderSetSrcBuf.pucRDStart     = AXI_VITR_TO_PHYS(rd_addr);                         /* RD buffer起始地址 */
    socp_chan.sCoderSetSrcBuf.pucRDEnd       = AXI_VITR_TO_PHYS(rd_addr) + SOCP_RD_BUF_SIZE -1;   /* RD buffer结束地址 */
