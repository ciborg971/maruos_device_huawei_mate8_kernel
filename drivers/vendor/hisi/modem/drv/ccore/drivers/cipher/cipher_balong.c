...
static inline void cipher_move_bdq_w_ptr(struct cipher_chn_ctl * chn_ctl)
{
	...
	//reg_rwptr = chn_ctl->bd_fifo.write_idx;
	//reg_rwptr &= CIPHER_RW_MASK;
	
	if(chn_ctl->straight) {
		reg_rwptr = chn_ctl->bd_fifo.write_idx & CIPHER_WMASK;
		bsp_psam_move_cbdq_ptr(reg_rwptr);
	} else {
												...
												CIPHER_SUBMIT_TYPE_E subm_attr)
{
	...
	do{
		cipher_get_bdq_rwptr(cur_chn, (unsigned int *)&r_idx, 
											(unsigned int *)&w_idx);
	}while(r_idx != w_idx);
...
int mdrv_cipher_enable(void)
{
	...
	/*In some platform, cipher clock isn't come from BBP clock. So we use clk_switch to tag it,
	*  the cipher clock will alwasy on, so we will stub it.
	*/
	if(cipher_module_ctl.clk_switch)
		return MDRV_OK;
...
void cipher_init_reg(void)
{
	...
	while(0x80000000 & readl(cipher_reg_base + HI_CIPHER_CTRL_OFFSET))
		;
	writel(4096, cipher_reg_base + HI_CIPHER_PRI_OFFSET);
...
int cipher_para_dts(void)
{
	...
	ret = of_property_read_u32_array(dts_node, "clk_switch", &cipher_module_ctl.clk_switch, 1);
	if(ret) {
...
int cipher_init(void)
{
	...
	/*In some platform, cipher clock isn't come from BBP clock. So we use clk_switch to tag it,
	*  the cipher clock won't be closed.
	*/
	if(cipher_module_ctl.clk_switch)
	{
		if(clk_enable(cipher_module_ctl.pclk))
		...
		CIPHER_INFO("Cipher clock won't be closed!\n");
	}
...
int cipher_suspend(struct dpm_device *dev)
{
	...
	if(cipher_reg_backup())
	{
		CIPHER_INFO("CIPHER:stop suspend\n");
		return CIPHER_UNKNOWN_ERROR;
	}
	(void)mdrv_cipher_disable();
	return MDRV_OK;
