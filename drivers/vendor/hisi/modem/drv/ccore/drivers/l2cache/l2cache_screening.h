...
/************************DDR使用说明*************************************************************************************
                                         |(DDR_MEM_ADDR --- DDR_MEM_ADDR+0x4000)            **页表16K
*(DDR_MEM_ADDR -DDR_MEM_ADDR+0x100000)   |(DDR_MEM_ADDR + 0x4000 --- DDR_MEM_ADDR+0x10000)  **SP
*开始的1M空间不做测试使用                |(DDR_MEM_ADDR + 0x100000 -0x4)                      **用于存放测试标记

*(DDR_MEM_ADDR + 0x100000 -DDR_MEM_ADDR+0x300000)  可做测试使用 3M  
*(DDR_MCORE_ADDR + 0x400000 -DDR_MCORE_ADDR+0x900000)  用于存放A9的镜像(MCORE_TEXT_START_ADDR)
*(DDR_MCORE_ADDR + 0xA00000 -DDR_MCORE_ADDR+DDR_MCORE_SIZE) MCORE DDR可做测试用
*************************************************************************************************************************/
...
#define MINI_TLB_BASE_ADDR       (MINI_DDR_BASE_ADDR) /*页表基地址*/
#define MINI_PAGE_SIZE           (0x100000)           /*页大小1M*/
#define L2CACHE_TEST_FLAG_ADDR   (MINI_DDR_BASE_ADDR + MINI_PAGE_SIZE - 0x4)/*测试结果存放*/
#define L2CACHE_TEST_START_ADDR  (MINI_DDR_BASE_ADDR + 0xA00000)           /*测试开始的地址*/
#define L2CACHE_TEST_END_ADDR    (DDR_MCORE_ADDR + DDR_MCORE_SIZE) /*测试结束的地址*/
