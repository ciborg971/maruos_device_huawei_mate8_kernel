...
void m3_log_print(char ch)
{
    ...
    LOG_BUF(g_log_info->log_write) = ch;
    g_log_info->log_write++;

    if(g_log_info->log_write >= g_log_info->log_buf_len)
    {
        g_log_info->log_write = 0;
    }
}

void m3_log_init(void)
{
    log_mem_stru * log_info = (log_mem_stru *)DUMP_M3_LOG_ADDR;

    log_info->magic = M3_LOG_MAGIC;
    log_info->log_read = 0;
    log_info->log_write = 0;
    log_info->logged_chars = 0;
    log_info->log_buf_len = DUMP_M3_LOG_SIZE - sizeof(log_mem_stru);
    log_info->log_buf = (u8 *)DUMP_M3_LOG_ADDR + sizeof(log_mem_stru);
    log_info->w_mark = (log_info->log_buf_len)/2;
    log_info->full_flag = 0;
