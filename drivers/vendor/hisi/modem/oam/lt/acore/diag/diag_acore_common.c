...
{
    VOS_UINT32 ulPtr = g_stDumpInfo.ulCur;
    
    if(g_stDumpInfo.pcDumpAddr)
    {
        g_stDumpInfo.pcDumpAddr[ulPtr]      = ulSenderPid;
        g_stDumpInfo.pcDumpAddr[ulPtr+4]    = ulMsgId;
        g_stDumpInfo.pcDumpAddr[ulPtr+8]    = ulSize;
        g_stDumpInfo.pcDumpAddr[ulPtr+12]   = mdrv_timer_get_normal_timestamp();

        g_stDumpInfo.ulCur = (g_stDumpInfo.ulCur + 16);
        if(g_stDumpInfo.ulCur >= DIAG_DUMP_LEN)
        {
            g_stDumpInfo.ulCur = 0;
        }
    }
}
...
VOS_UINT32 diag_TransReqProcEntry(DIAG_FRAME_INFO_STRU *pstReq, DIAG_TRANS_HEADER_STRU *pstHead)
{
    ...
    if (pstMsg != NULL)
    {
        ...
        VOS_MemCpy(&pstMsg->ulMsgId, &pstSendReq->ulMsgId, (ulCmdParasize - VOS_MSG_HEAD_LENGTH));
...
VOS_UINT32 MSP_AppDiagFidInit(enum VOS_INIT_PHASE_DEFINE ip)
{
    ...
    switch (ip)
    {
        case VOS_IP_LOAD_CONFIG:
            ...
            {
                g_stDumpInfo.pcDumpAddr = (VOS_VOID * )VOS_UnCacheMemAlloc(DIAG_DUMP_LEN, &realAddr);
                if(VOS_NULL == g_stDumpInfo.pcDumpAddr)
                {
                    diag_printf("g_stDumpInfo.pcDumpAddr malloc failed.\n");
                    return VOS_ERR;
                }
            }
            g_stDumpInfo.ulCur  = 0;
            g_stDumpInfo.ulLen  = DIAG_DUMP_LEN;
