...
VOS_UINT32 DIAG_LogReport(VOS_UINT32 ulModuleId, VOS_UINT32 ulPid, VOS_CHAR *cFileName, VOS_UINT32 ulLineNum, VOS_CHAR* pszFmt, ...)
{
    ...
    DIAG_Api_Statistic_Ind(DIAG_DEBUG_API_PRINT,EN_DIAG_CBT_API_PRINTFV_OK,EN_DIAG_CBT_API_PRINTFV_ERR);
    return ret;
}
...
/*****************************************************************************
 函 数 名  : DIAG_LogReport
 功能描述  : 打印上报接口(多模OM融合迭代二阶段此接口将删除)
 输入参数  : ulModuleId( 31-24:modemid,23-16:modeid,15-12:level,11-0:pid )
             pcFileName(上报时会把文件路径删除，只保留文件名)
             ulLineNum(行号)
             pszFmt(可变参数)
*****************************************************************************/
VOS_UINT32 DIAG_PrintfV(VOS_UINT32 ulModuleId, VOS_UINT32 ulPid, VOS_CHAR *cFileName, VOS_UINT32 ulLineNum, VOS_CHAR* pszFmt, ...)
{
    return ERR_MSP_SUCCESS;
}
...
*****************************************************************************/
unsigned int DIAG_TransReport_Ex(DIAG_TRANS_IND_STRU *pstData)
{
    VOS_UINT32 ret;
    ...
    DIAG_Api_Statistic_Ind(DIAG_DEBUG_API_TRANS,EN_DIAG_CBT_API_TRANS_OK,EN_DIAG_CBT_API_TRANS_ERR);
...
unsigned int DIAG_TransReport(DIAG_TRANS_IND_STRU *pstData)
{
    /*检查DIAG是否初始化且HSO是否连接上*/
    if(!DIAG_IS_CONN_ON)
    {
...
VOS_UINT32 DIAG_EventReport(DIAG_EVENT_IND_STRU *pstEvent)
{
    ...
    DIAG_Api_Statistic_Ind(DIAG_DEBUG_API_EVENT,EN_DIAG_CBT_API_EVENT_OK,EN_DIAG_CBT_API_EVENT_ERR);
...
VOS_UINT32 DIAG_AirMsgReport(DIAG_AIR_IND_STRU *pstAir)
{
    ...
    DIAG_Api_Statistic_Ind(DIAG_DEBUG_API_AIR,EN_DIAG_CBT_API_AIR_OK,EN_DIAG_CBT_API_AIR_ERR);
...
VOS_UINT32 DIAG_ReportVoLTELog(DIAG_VOLTE_LOG_STRU* pRptMessage)
{
    ...
    DIAG_Api_Statistic_Ind(DIAG_DEBUG_API_VOLTE,EN_DIAG_CBT_API_VOLTE_OK,EN_DIAG_CBT_API_VOLTE_ERR);
...
VOS_VOID DIAG_TraceReport(VOS_VOID *pMsg)
{
    ...
    DIAG_Api_Statistic_Ind(DIAG_DEBUG_API_TRACE,EN_DIAG_CBT_API_TRACE_OK,EN_DIAG_CBT_API_TRACE_ERR);
...
VOS_UINT32 DIAG_UserPlaneReport(DIAG_USER_IND_STRU *pstUser)
{
    ...
    DIAG_Api_Statistic_Ind(DIAG_DEBUG_API_USERPLANE,EN_DIAG_CBT_API_USER_OK,EN_DIAG_CBT_API_USER_ERR);
...
VOS_UINT32 Diag_GetLogLevel(VOS_UINT32 ulModuleId)
{
    VOS_UINT8 level;
