...
const FTM_BAND_INFO_STRU g_stBandInfo[] = {
    ...
    {      0,      0,      0,      0,      0,      0,  EN_FCHAN_MODE_UNKONWN,    255 }, /* Band26  */
...
/*lint -restore*/
/*****************************************************************************
 函 数 名  : at_ftm_getLteBandswId
 功能描述  : 通过频段ID获得Band Switch ID和频段模式
 输入参数  : ucBandID 频段ID
 输出参数  : pucBandSwitchID Band Switch ID
             pemBandMode     频段模式
 返 回 值  : 频率值
*****************************************************************************/
/*lint -save -e958*/
VOS_UINT32 at_ftm_getLteBandswId(VOS_UINT8 ucBandID, VOS_UINT8* pucBandSwitchID, FCHAN_MODE_ENUM* pemBandMode)
{
    *pucBandSwitchID = 255;
    *pemBandMode     = EN_FCHAN_MODE_UNKONWN;

    if((g_stBandInfo[ucBandID].ucBandSWID== 255)
        || (g_stBandInfo[ucBandID].ucBandMode == EN_FCHAN_MODE_UNKONWN))
    {
        return ERR_MSP_FAILURE;
    }

    *pucBandSwitchID = g_stBandInfo[ucBandID].ucBandSWID;
    *pemBandMode     = g_stBandInfo[ucBandID].ucBandMode;
