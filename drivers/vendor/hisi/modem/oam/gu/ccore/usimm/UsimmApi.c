...
VOS_UINT32 USIMM_IsServiceAvailable(UICC_SERVICES_TYPE_ENUM_UINT32 enService)
{
    ...
    {
        return USIMM_IsUIMServiceAvailable(enService);
    }

    if (enService < CSIM_SVR_BUTT)
    {
        return USIMM_IsCSIMServiceAvailable(enService);
    }
...
VOS_VOID USIMM_FdnQuery(VOS_UINT32 *pulState)
{
    if(VOS_NULL_PTR != pulState)
    {
        ...
        *pulState = USIMM_FDNSTATUS_OFF;
#else
        *pulState = USIMM_CCB_GetAppFDN(USIMM_GUTL_APP);
#endif
    }
...
VOS_UINT32 USIMM_GetCardCGType(VOS_UINT32 *pulCardMode, VOS_BOOL *pbHasCModule, VOS_BOOL *pbHasWGModule)
{
    if ((VOS_NULL_PTR == pulCardMode) || (VOS_NULL_PTR == pbHasCModule) || (VOS_NULL_PTR == pbHasWGModule))
    {
        return USIMM_API_WRONG_PARA;
    }

    *pulCardMode    = USIMM_CCB_GetCardType();

    /* 这里不区分ICC或UICC，ICC卡初始化时也会把AID填进去 */
