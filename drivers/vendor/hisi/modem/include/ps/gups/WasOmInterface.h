...
typedef struct
{
    ...
    VOS_UINT16                              usMeasQuantity;
     ...
     /* 如果usCellNum为N（N > 0），表示激活集中有多个小区，
        此时aucActOrServeCellInfo中存贮N个小区
        WAS_MNTN_REPORT_WCDMA_CELL_RSLT_STRU的信息,此处
        占据4个字节的位置*/
...
typedef struct
{
    ...
    /* 如果usCellNum为N（N > 0），表示邻区中有多个小区，
       此时aucWcdmaNeighberCellInfo中存贮N个小区
       WAS_MNTN_REPORT_WCDMA_CELL_RSLT_STRU的信息,此处
       占据4个字节的位置*/
...
typedef struct
{
    ...
    /***************************************************************************
     如果usCellNum为N（N > 0），此时aucLTENeighberFreqInfo中存贮N个小区
     WAS_MNTN_REPORT_LTE_CELL_RSLT_STRU的信息,此处占据4个字节的位置
    ***************************************************************************/
...
typedef struct
{
    ...
    VOS_UINT16                              usMeasQuantity;
     ...
     /* 如果usCellNum为N（N > 0），表示Second Cell中有多个小区，
        此时aucSecondCellInfo中存贮N个小区
        WAS_MNTN_REPORT_WCDMA_CELL_RSLT_STRU的信息,此处
        占据4个字节的位置*/
...
typedef struct
{
    ...
    /* 如果usCellNum为N（N > 0），
        此时aucGsmNeighberCellInfo中存贮N个小区
        WAS_MNTN_REPORT_GSM_CELL_RSLT_STRU的信息,此处
        占据4个字节的位置*/
