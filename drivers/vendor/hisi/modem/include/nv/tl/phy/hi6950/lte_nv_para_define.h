...
typedef struct
{
    ...
    UINT16 usUlModemLoss[MAX_TX_CHAN_NUM]; /*十六分之一单位*/
}LTE_OTHER_COMP_STRU;
#else
typedef struct
{
...
typedef struct
{
    ...
    UINT16  sRes;/*sDbbValue; 暂时不开发这个配置，避免引起麻烦*/
...
#define LPHY_FE_RF_INIT_NV_DDR_BASE         (BAND_MAILBOX_NV_IDX_BASE_ADDR)     /*rfic初始化nv ddr首地址*/
#define LPHY_FE_BAND_NV_DDR_BASE            (LPHY_FE_RF_INIT_NV_DDR_BASE + sizeof(UCOM_NV_RF_INIT_CONFIG_NEW_RAM_STRU *))/*单band nv地址列表存储*/
#define LPHY_FE_BANDCOMB_NV_DDR_BASE        (LPHY_FE_BAND_NV_DDR_BASE    + sizeof(LRRC_LPHY_FENV_SINGLE_BAND_DDR_INFO_STRU))/*多band nv地址列表存储*/
