...
typedef struct _TDS_ASU_OM_REPORT_STRU_{
    ...
    UINT16                         usRsv2;
    INT32                          slGiveConnAnt0;         //连接态主天线收到的RSCP值
    INT32                          slGiveConnAnt1;         //辅天线RSCP值，这里是逻辑概念，主天线对应信号好的天线

    INT32                          slGiveConnRsrp;         //连接态用于上报GOOD/POOR等事件所依据的测量值
    INT32                          slGiveConnScale;
    INT32                          slGiveIdleRsrp;         //空闲态用于上报GOOD/POOR等事件所依据的测量值
