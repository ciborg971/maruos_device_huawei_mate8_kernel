 ...
 */
#define pr_fmt(fmt) "hi36xx-smmu:  " fmt
...
#ifdef IOMMU_DEBUG
#define dbg(format, arg...)  printk(KERN_INFO "[iommu]"format, ##arg);
#else
...
typedef struct {unsigned int pgd_entry[4];} smmu_pgd_t ;
...
static unsigned int dbg_iova_start = 0;
static unsigned int dbg_iova_size = 0;

static inline unsigned int smmu_pgd_none(smmu_pgd_t pgd) {
...
static inline void *smmu_pte_page_vaddr(smmu_pgd_t* pgd)
{
...
static inline void smmu_set_pgd(smmu_pgd_t*pgdp, unsigned int pgd)
{
	unsigned int i=0;
...
static inline void smmu_pgd_populate(smmu_pgd_t *pgdp, pgtable_t ptep,pgdval_t prot)
{
	smmu_set_pgd(pgdp, (unsigned int)(page_to_phys(ptep) | prot));
}

static inline unsigned long  smmu_pgd_addr_end(unsigned long addr, unsigned long end)
{
...
static int get_domain_data(struct device_node* np ,struct iommu_domain_data *data)
{
...
static int hisi_smmu_domain_init(struct iommu_domain *domain)
{
	...
	if(!iommu_info){
		printk(KERN_ERR"alloc iommu_domain_data fail \n");
		return -EINVAL;
	}
...
static void hisi_smmu_free_ptes(smmu_pgd_t pgd)
{
	pgtable_t table = smmu_pgd_to_pte(pgd.pgd_entry[0]);
	if (!table) {
		printk(KERN_ERR"pte table is null \n");
		return ;
	}
...
static int hisi_smmu_alloc_init_pte(smmu_pgd_t *ppgd,
		...
		unsigned long pfn, int prot)
{
	...
	if (smmu_pgd_none(*ppgd)) {
		...
		if(prot & IOMMU_SEC)
			smmu_pgd_populate(ppgd, table,SMMU_PGD_TYPE_TABLE|SMMU_PGD_NS_BIT(0));
		else
			smmu_pgd_populate(ppgd, table,SMMU_PGD_TYPE_TABLE|SMMU_PGD_NS_BIT(1));
		hisi_smmu_flush_pgtable(ppgd, sizeof(*ppgd));
	}
	start = (smmu_pte_t*)smmu_pte_page_vaddr(ppgd) + smmu_pte_index(addr);
	pte = start;

	/* by default, setting pages by read only ,non-cacheable attribute and memory type.
	 * ap[2]=0b1, ap[1:0]=0b11, tex=0b100 , c_b=0b00.
	 */
	if (!prot) {
		pteval |= SMMU_PTE_RW | SMMU_PTE_TEX(4) | SMMU_PTE_C_B(0) | SMMU_PTE_XN_BIT(0);
	} else {
		/*
		 * set page to device type.
		 */
		if (prot & IOMMU_DEVICE) {
			pteval |= SMMU_PTE_RW | SMMU_PTE_TEX(0) | SMMU_PTE_C_B(1) | SMMU_PTE_XN_BIT(1);
		} else {
			/*
		...
		size_t size, int prot)
{
	...
	unsigned long next;
	unsigned long flags;
	smmu_pgd_t *pgd = (smmu_pgd_t *)hisi_smmu->va_pgtable_addr;
	if (!pgd) {
		dbg("pgd is null \n");
		return -EINVAL;
	}
	...
	do {
		...
		ret = hisi_smmu_alloc_init_pte(pgd, iova, next, __phys_to_pfn(paddr),prot);
		if (ret)
			goto out_unlock;
		paddr += next - iova;
		iova = next;
		...
		phys_addr_t paddr, size_t size, int prot)
{
	...
	if(iova < data->iova_start){
		printk(KERN_INFO "iova failed: iova = 0x%lx, start = 0x%8x \n", iova, data->iova_start);
		goto error;
	}
...
static unsigned int hisi_smmu_clear_pte(smmu_pgd_t* pgdp, unsigned int iova, unsigned int end)
{
...
static unsigned int hisi_smmu_handle_unmapping(struct iommu_domain* domain, unsigned long iova, size_t size )
{
	...
	pgdp = (smmu_pgd_t*)hisi_smmu->va_pgtable_addr;
	end = iova + size;
	dbg("%s:end=0x%x \n",__func__, end);
...
static phys_addr_t hisi_smmu_iova_to_phys(struct iommu_domain *domain,dma_addr_t iova)
{
	smmu_pgd_t *pgdp, pgd;
	smmu_pte_t pte;
	...
	pte = *((unsigned int*)smmu_pte_page_vaddr(&pgd) + smmu_pte_index(iova));
		...
		iova,struct scatterlist *sg, size_t size, int prot)
{
	...
	while (sg) {
		...
		if (ret) {
			printk(KERN_ERR "hisi_smmu_map failed!\n");
			break;
		}
...
static size_t hisi_smmu_unmap_range(struct iommu_domain *domain, unsigned long iova,
		size_t size)
{
	size_t ret = 0;
...
static size_t hisi_map_tile_row(struct iommu_domain *domain, unsigned long
	...
	while ( 1 ) {
		...
		ret = hisi_smmu_map(domain, iova + mapped_size,phys_addr,map_size,prot);
		if (ret) {
			printk(KERN_ERR "[%s] hisi_smmu_map failed!\n", __func__);
			break;
		}
		...
		if ( mapped_size < size ) {
			...
			sg = sg_next(sg);
			if (!sg) {
				printk(KERN_ERR "[%s] error physical memory not enough \n",__func__ );
				break;
			}
		}else{
			sg_offset += map_size;
...
static int hisi_smmu_map_tile(struct iommu_domain *domain, unsigned long iova,
	...
	if ( header_size ) {
		mapped_size =
		hisi_map_tile_row(domain,iova,header_size,sg_node,sg_offset,&map_position,prot);
	...
	for (row = 0; row < row_number; row++) {
		...
		if ( map_position.sg ) {
			...
			printk(KERN_ERR"[%s]:physical memory is not enough \n",__func__);
			break;
		}
		/* map one row*/
		mapped_size = hisi_map_tile_row(domain, iova + (size_virt * row),size_phys, sg_node, sg_offset, &map_position,prot);
...
static size_t hisi_smmu_unmap_tile(struct iommu_domain *domain, unsigned long iova,
		size_t size)
{
...
smmu_pte_t  hisi_dbg_show_ptb(unsigned int iova,smmu_pgd_t *pgdp)
{
	smmu_pgd_t pgd;
	smmu_pte_t pte;
	if (!pgdp)
...
static ssize_t dbg_smmu_read(struct file *file, char __user *buf, size_t count, loff_t *ppos)
{
	char* msg = NULL;
	...
	if (!msg) {
		...
		return simple_read_from_buffer(buf,count,ppos,"alloc buffer fail \n" , sizeof("alloc buffer fail \n"));
	}
	iova_end = dbg_iova_start+dbg_iova_size ;
	dbg("iova_end=0x%x \n",iova_end);
	if ((dbg_iova_start==INVALID_IO_ADDR)||(iova_end > MAX_IO_ADDR)) {
		dbg("parameter error :dbg_iova_start=0x%x, dbg_iova_size=0x%x, iova_end=0x%x \n",dbg_iova_start, dbg_iova_size,iova_end);
		kfree(msg);
		return simple_read_from_buffer(buf,count,ppos, "parameter error \n", sizeof("parameter error \n"));
	}else{
		...
		for(io_addr=dbg_iova_start; io_addr < iova_end; io_addr += SMMU_PAGE_SIZE){
			printk("pgd[%d],pte[%d]=0x%x \n",
					smmu_pgd_index(io_addr),smmu_pte_index(io_addr),
					hisi_dbg_show_ptb(io_addr,(smmu_pgd_t *)hisi_smmu->va_pgtable_addr));
		}
		...
		size_t count, loff_t *ppos)
{
	...
	dbg("%s \n",msg);
	num = sscanf(msg,"start=0x%8x size=0x%8x", &dbg_iova_start, &dbg_iova_size);
...
static int hisi_smmu_probe(struct platform_device *pdev)
{
	...
	if (!hisi_smmu) {
		printk(KERN_ERR"failed to allocate arm_smmu_device\n");
	...
	ret=of_property_read_u64(np,"phy_pgd_base",&hisi_smmu->smmu_phy_pgtable_addr);
	if (ret) {
	...
	file = debugfs_create_file("smmu_pgtbl", (S_IRUSR | S_IRGRP | S_IWUSR | S_IWGRP),
			debugfs_root, &pdev->dev, &smmu_pgtable_fops);
	if (!file){
...
static int __init hisi_smmu_init(void)
{
	...
	if ( ret ){
		printk(KERN_ERR"register smmu driver fail \n");
	}
	return ret;
}
