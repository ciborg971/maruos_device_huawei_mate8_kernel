...
struct ts_tui_finger *tp_ptr =NULL;
static void tc_notify_fn(struct work_struct *dummy)
{
    ...
    if(notify_data_entry_timer == NULL) {
       ...
       // ts_tui_report_input((void *)notify_data);
...
static irqreturn_t tc_secure_notify(int irq, void *dev_id)
{
    ...
    if(notify_data_entry_tp != NULL && notify_data_entry_tp->filled){
        ...
        if(tp_nofity_data->irq_type == IRQ_TYPE_TP) {
            tp_ptr =(struct ts_tui_finger *)(&tp_nofity_data->tui_notify_data);
            //TCERR("notify_data is NULL  sting=%s \n",(char *)notify_data);
            //TCERR("x=%d, y=%d--------=-=-=-=-=-=-=-=-=-=-=-=-==-=-=-=-=-=-=-\n",tp_ptr->x, tp_ptr->y);
...
static int TC_NS_tui_event(TC_NS_DEV_File *dev_file, void* argp)
{
	...
	copy_to_user(argp, &tui_param, sizeof(TEEC_TUI_Parameter));
    }
...
static __init int tc_init(void)
{
	...
	if(!notify_data){
		...
		//phys_addr_t notify_data_phys = virt_to_phys(notify_data);
