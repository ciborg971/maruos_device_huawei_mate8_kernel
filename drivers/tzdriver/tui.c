					...
					size_t cnt, loff_t *ppos)
{
	...
	r = sprintf(buf, "tui state:%s\n", state_name[atomic_read(&tui_state)]);

	r += sprintf(buf + r, "drv config state:");

	mutex_lock(&tui_drv_lock);
	list_for_each_entry(pos, &tui_drv_head, list)
		r += sprintf(buf + r, "%s-%s,", pos->name,
					...
					size_t cnt, loff_t *ppos)
{
	...
	r = sprintf(buf, "%s", state_name[atomic_read(&tui_state)]);
					...
					size_t cnt, loff_t *ppos)
{
	...
	r = sprintf(buf, "%s", "event format: event_type:val\n"
						   "event type:\n");

	/* event type list */
	for (i = 0; i < TUI_POLL_MAX - 1; i++)
		r += sprintf(buf + r, "%s, ", poll_event_type_name[i]);
	r += sprintf(buf + r, "%s\n", poll_event_type_name[i]);

	/* cfg drv type list */
	r += sprintf(buf + r, "val type for %s or %s:\n",
				poll_event_type_name[TUI_POLL_CFG_OK],
				poll_event_type_name[TUI_POLL_CFG_FAIL]);
	list_for_each_entry(pos, &tui_drv_head, list)
		r += sprintf(buf + r, "%s,", pos->name);
	buf[r-1] = '\n';
