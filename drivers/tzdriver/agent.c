...
static char *exception_mem_addr;
...
int TC_NS_sync_sys_time(TC_NS_Time *tc_ns_time)
{
    ...
    smc_cmd.err_origin = (unsigned int)tc_ns_time->seconds;
    smc_cmd.ret_val = (unsigned int)tc_ns_time->millis;
...
static int tee_agent_kernel_init()
{
...
static int tee_agent_kernel_exit()
{
    struct tee_agent_kernel_ops *agent_ops = NULL;
...
extern int rpmb_agent_register();
#endif
int agent_init(void)
{
