...
extern int TC_NS_get_rdr_mem_addr();
extern int TC_NS_get_rdr_mem_len();
...
start:
    ...
    if(ring_flag != reader->r_ver /*ring_flag not same*/){
        ...
        if(log->w_off < reader->r_off){
            TLOGERR("tlogcaterrorinfo----ring_flag=%d  read->r_ver=%d log->w_off=%llx reader->r_off=%llx \n",ring_flag, reader->r_ver, log->w_off, reader->r_off);
...
int tlogger_store_lastmsg(void)
{
    ...
    {
        TLOGERR("Succeed to Write to last_teemsg, write_len=%d\n", write_len);
    }
    pos = 0;
