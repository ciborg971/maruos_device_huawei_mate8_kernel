						...
						int offset, int avail)
{
	...
	dev_dbg(dev, "memory rsc: da %x, pa %x, len %x, flags %x\n",
			rsc->da, rsc->pa, rsc->len, rsc->flags);
                                  ...
                                  int offset, int avail)
{
    ...
    dev_dbg(dev, "memory rsc: da %x, pa %x, len %x, name %s, flags %x, rdr_addr 0x%x\n",
            rsc->da, rsc->pa, rsc->len, rsc->name, rsc->flags, g_isp_rdr_addr);
                                  ...
                                  int offset, int avail)
{
    ...
    va = ioremap_nocache(rsc->pa, rsc->len);
    if (!va) {
        dev_err(dev, "ioremap failed \n");    
        ret = -ENOMEM;
        goto free_memory;
...
out:
    ...
    if (ret) {
        ...
        pr_err("%s: handle resources failed.\n", __func__);
    } else {
