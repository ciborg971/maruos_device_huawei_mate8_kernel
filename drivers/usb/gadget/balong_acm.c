...
extern int is_modem_port(int port_num);
...
static void gs_acm_cdev_write_cb(struct gs_acm_cdev_port *port)
{
	...
	for (;;) {
		...
		if (NULL != write_cb) {
			...
			if (is_modem_port(port->port_num) && write_priv != NULL) {
				port->stat_tx_callback++;
				free_cb(write_priv->user_p_skb);
			}
		}
	}
}
...
// following macros don't need to define
// use cdc-acm.h ACM_CTRL_DSR...

#define MODEM_CTRL_RI        (1 << 3) // 8
#define MODEM_CTRL_DSR        (1 << 1) // 2
#define MODEM_CTRL_DCD        (1 << 0) // 1
		...
		MODEM_MSC_STRU *flow_msg)
{
	...
	if (port->port_usb) {
		...
		pr_err("%s: write cmd when balong_acm disconnected\n", __func__);
		return -ENODEV;
	}
	...
	if (SIGNALCH == flow_msg->OP_Cts) {
		...
		printk(KERN_INFO  "acm[%s] flow_control: %s\n",
			ACM_CDEV_GET_NAME(port->port_num),
	...
	if ((SIGNALCH == flow_msg->OP_Ri) || (SIGNALCH == flow_msg->OP_Dsr)
		...
		if ((SIGNALCH == flow_msg->OP_Ri)
			...
			printk(KERN_INFO  "acm[%s] OP_Ri CHANGE capability=0x%x\n",
		...
		if ((SIGNALCH == flow_msg->OP_Dsr)
			...
			printk(KERN_INFO  "acm[%s] OP_Dsr CHANGE capability=0x%x\n",
				ACM_CDEV_GET_NAME(port->port_num), capability);
		};
		...
		if ((SIGNALCH == flow_msg->OP_Dcd)
			...
			printk(KERN_INFO  "acm[%s] OP_Dcd CHANGE capability=0x%x\n",
				ACM_CDEV_GET_NAME(port->port_num), capability);
		};
	...
	switch (cmd) {
		...
		case ACM_IOCTL_SET_FREE_CB:
			if (is_modem_port(port->port_num)) {
				...
				pr_err("ACM_IOCTL_SET_FREE_CB only support for modem port\n");
				status = -EINVAL;
			}
			break;
...
static int gs_acm_cdev_port_alloc(unsigned port_num, struct usb_cdc_line_coding *coding)
{
	...
	scnprintf(port->read_domain, 4, "%d_rd", port_num);
	scnprintf(port->write_domain, 4, "%d_wt", port_num);
	scnprintf(port->debug_tx_domain, 4, "dtx%d", port_num);
	scnprintf(port->debug_rx_domain, 4, "drx%d", port_num);
	scnprintf(port->debug_port_domain, 4, "dpt%d", port_num);

	/* mark the asic string for debug */
	scnprintf(gs_acm_cdev_ports[port_num].name_domain, 4, "acm%d", port_num);
...
#define ACM_PRINT_IOCTL(cmd) pr_emerg("comand:%s\t\t\t\tcode:0x%x\n", #cmd, cmd);
...
void acm_cdev_dump(int port_num)
{
	...
	mdelay(10);
	pr_emerg("\n=== dump call back info ===\n");
	pr_emerg("read_done_cb              %p\n", port->read_done_cb);
	pr_emerg("write_done_cb             %p\n", port->write_done_cb);
	pr_emerg("write_done_free_cb        %p\n", port->write_done_free_cb);
	pr_emerg("event_notify_cb           %p\n", port->event_notify_cb);
	pr_emerg("read_sig_cb               %p\n", port->read_sig_cb);
	pr_emerg("rel_ind_cb                %p\n", port->rel_ind_cb);
}
