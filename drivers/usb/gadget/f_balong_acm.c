...
struct acm_name_type_tbl g_acm_cdev_type_table[ACM_CDEV_COUNT] = {
	...
	/* !!if you change the acm_modem position, please also revise
	   ACM_MODEM_PORT above to the array index */
	{"acm_modem",       USB_IF_PROTOCOL_MODEM},
};

static int g_acm_is_single_interface = ACM_IS_SINGLE_INTF;

int is_modem_port(int port_num)
{
	return port_num == ACM_MODEM_PORT;
}

static inline int is_support_notify(int port_num)
{
	return port_num == ACM_MODEM_PORT;
}
...
static int acm_set_alt(struct usb_function *f, unsigned intf, unsigned alt)
{
	 ...
	 */
	if (intf == acm->ctrl_id) { // ctrl_id是控制接口的id，还有一个data_id
		is_setting = 1;
		if (acm->notify) {
...
// 此函数被connect和disconnect调用，用来向host通知状态
...
{
	int status;
#ifdef CONFIG_JUNGO_HOST_DRIVER
	u16 value;
#endif
...
#else
		u32 flow_control[2];
		flow_control[0] = acm->tx_is_on ? 0x1 : 0x0;
		flow_control[1] = acm->rx_is_on ? 0x1 : 0x0;
		status = acm_cdc_notify(acm, USB_CDC_NOTIFY_SPEED_CHANGE,
			0, flow_control,8,0);
#endif
	} else {
		acm->pending = true;
