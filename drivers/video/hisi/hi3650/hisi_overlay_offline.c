	...
	bool enable_cmdlist)
{
	...
	hisi_dss_prev_module_set_regs(hisifd, pov_req, cmdlist_idxs, enable_cmdlist);
...
static int hisi_vactive0_start_wait(struct hisi_fb_data_type *hisifd, dss_overlay_t *pov_req)
{
	...
	hisifd_primary = hisifd_list[PRIMARY_PANEL_IDX];
	if (hisifd_primary &&
		hisifd_primary->panel_power_on &&
		is_mipi_cmd_panel(hisifd_primary) &&
		(hisifd_primary->vactive0_start_flag == 0) &&
		(pov_req->ovl_idx != DSS_OVL3)) {

		hisifb_get_timestamp(&tv0);

	REDO:
		ret = wait_event_interruptible_timeout(hisifd_primary->vactive0_start_wq, hisifd_primary->vactive0_start_flag,
...
int hisi_ov_offline_play(struct hisi_fb_data_type *hisifd, void __user *argp)
{
	...
	for (m = 0; m < pov_req->ov_block_nums; m++) {
		...
		for (k = 0; k < block_num; k++) {
			...
			for (i = 0; i < pov_h_v_block->layer_nums; i++) {
				...
				if ((layer->need_cap & CAP_AFBCD) && (layer_cap_afbc == false))
					layer_cap_afbc = true;
	...
	ret = hisi_add_clear_module_reg_node(hisifd, pov_req, cmdlist_idxs, enable_cmdlist);
	if (ret != 0) {
	...
	if (enable_cmdlist) {
		...
		if (g_debug_ovl_offline_composer_timediff & 0x2) {
			...
			if (timediff >= g_debug_ovl_offline_composer_time_threshold)
				HISI_FB_INFO("wb_type=%d, OFFLINE_VACTIVE_TIMEDIFF is %u us!\n", wb_type, timediff);
		}
	#endif

	#ifdef CONFIG_HISI_DSS_CMDLIST_LAST_USED
		hisi_cmdlist_exec(hisifd, cmdlist_idxs);
		hisi_cmdlist_config_start(hisifd, pov_req->ovl_idx, cmdlist_idxs);
	#else
		if (cmdlist_idxs_need_start) {
			hisi_cmdlist_config_start(hisifd, pov_req->ovl_idx, cmdlist_idxs_need_start);
		}
		hisi_cmdlist_exec(hisifd, cmdlist_idxs);
	#endif
	}
...
REDO:
	...
	if ((ret <= 0) ||(ret_rch_state < 0)) {
		HISI_FB_ERR("compose timeout, layer_cap_afbc=%d, wb_type=%d, GLB_CPU_OFF_INTS=0x%x, ret=%d, ret_rch_state=%d,"
			"diff =%d usecs!\n",
			layer_cap_afbc, wb_type, inp32(hisifd->dss_base + GLB_CPU_OFF_INTS), ret, ret_rch_state,
...
err_return_sem1:
	...
	if (g_debug_ovl_offline_composer_timediff & 0x1) {
		...
		if (timediff >= g_debug_ovl_offline_composer_time_threshold)
			HISI_FB_INFO("wb_type=%d, OFFLINE_TIMEDIFF is %u us!\n", wb_type, timediff);
	}
