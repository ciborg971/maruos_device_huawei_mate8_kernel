...
*/
int hisi_cmdlist_config_start(struct hisi_fb_data_type *hisifd, int ovl_idx, uint32_t cmdlist_idxs)
{
	char __iomem *cmdlist_base = NULL;
	...
	for (i = 0; i < HISI_DSS_CMDLIST_MAX; i++) {
		if ((cmdlist_idxs_temp & 0x1) == 0x1) {
			...
			if (ovl_idx >= DSS_OVL2) {
				cmdlist_node = list_first_entry(&(hisifd->cmdlist_data_tmp[ovl_idx - DSS_OVL2]->cmdlist_head_temp[i]), dss_cmdlist_node_t, list_node);
			} else {
	...
	if (ovl_idx < DSS_OVL2) {
		...
		for (i = 0; i < DSS_WCHN_W0; i++) {
			if ((cmdlist_idxs_temp & 0x1) == 0x1) {
				...
				while (1) {
					tmp = inp32(dss_base + DSS_MIF_OFFSET + MIF_STAT1 + 0x10 * i);
					if (((tmp & 0x00008400) == 0x00008400) || delay_count > 1000) {
				...
				if (is_timeout) {
					HISI_FB_ERR("2mif_ch%d MIF_STAT1=0x%x !\n", i, tmp);
				}
			}
	...
	uint32_t cmdlist_idxs)
{
	...
	for (i = 0; i < HISI_DSS_CMDLIST_MAX; i++) {
		if (0x1 == (cmdlist_idxs_temp & 0x1)) {
			if (ovl_idx >= DSS_OVL2) {
				hisi_dump_cmdlist_one_node(&(hisifd->cmdlist_data_tmp[ovl_idx - DSS_OVL2]->cmdlist_head_temp[i]), i);
			} else {
	...
	uint32_t cmdlist_idxs)
{
	...
	for (i = 0; i < HISI_DSS_CMDLIST_MAX; i++) {
		if ((cmdlist_idxs_temp & 0x1) == 0x1) {
			if (ovl_idx >= DSS_OVL2) {
				hisi_cmdlist_del_all_node(&(hisifd->cmdlist_data_tmp[ovl_idx - DSS_OVL2]->cmdlist_head_temp[i]));
			} else {
...
void hisi_cmdlist_data_get_offline(struct hisi_fb_data_type *hisifd, dss_overlay_t *pov_req)
{
	...
	hisifd->cmdlist_data = hisifd->cmdlist_data_tmp[pov_req->ovl_idx - DSS_OVL2];
