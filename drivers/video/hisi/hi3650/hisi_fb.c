...
int g_debug_ovl_offline_composer_time_threshold = 8000;  //us
...
int g_dss_effect_sharpness2D_en = 1;
...
static int hisifb_dirty_region_updt_set(struct fb_info *info, void __user *argp)
{
	...
	if (g_enable_dirty_region_updt
		...
		&& (DSS_SEC_IDLE == hisifd->secure_ctrl.secure_status)) {
		enable = 1;
	}
...
static int hisifb_esd_recover(struct hisi_fb_data_type *hisifd)
{
	...
	pinfo->esd_recover_step = LCD_ESD_RECOVER_POWER_OFF;
	...
	pinfo->esd_recover_step = LCD_ESD_RECOVER_POWER_ON;
	...
	pinfo->esd_recover_step = LCD_ESD_RECOVER_NONE;
