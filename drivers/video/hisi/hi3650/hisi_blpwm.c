...
static int cabc_pwm_thread(void *p)
{
	while(!kthread_should_stop()) {
		...
		if (g_bl_info.index_cabc_dimming > CABC_DIMMING_STEP_TOTAL_NUM) {
			...
			int32_t backlight = g_bl_info.current_cabc_pwm * g_bl_info.ap_brightness / g_bl_info.blpwm_input_precision;
...
void updateCabcPwm(struct hisi_fb_data_type *hisifd)
{
	char __iomem *blpwm_base = NULL;
	struct hisi_panel_info *pinfo = NULL;
	...
	if (!pwm_in_num) {
		HISI_FB_INFO("pwm_in_num is null!\n");
		return -EINVAL;
	}
...
int hisi_blpwm_set_backlight(struct hisi_fb_data_type *hisifd, uint32_t bl_level)
{
	...
	char __iomem *blpwm_base = NULL;
	uint32_t brightness = 0;
	uint32_t blpwm_out_precision = 0;
	uint32_t pwm_in_num	= 0;
	uint32_t pwm_in_high_num = 0;
	uint32_t pwm_in_low_num  = 0;
	...
	if ((pinfo->bl_min < 0) || (pinfo->bl_max < 1)) {
		HISI_FB_ERR("bl_min(%d) or bl_max(%d) is out of range!!",
			pinfo->bl_min, pinfo->bl_max);
		return -EINVAL;
	}
