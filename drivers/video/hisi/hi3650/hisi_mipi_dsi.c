	...
	const char *buf, size_t count)
{
	...
	if (count <= 1) {
		HISI_FB_ERR("invalid input parameter:count = %ld\n", count);
		return count;
	}
...
static void mipi_dsi_bit_clk_upt_set(struct hisi_fb_data_type *hisifd,
	char __iomem *mipi_dsi_base, struct mipi_dsi_phy_ctrl phy_ctrl)
{
	...
	if (!is_mipi_cmd_panel(hisifd)) {
		...
		*/
		...
		hline_time = (pinfo->ldi.h_pulse_width + pinfo->ldi.h_back_porch + rect.w +
			pinfo->ldi.h_front_porch) * phy_ctrl.lane_byte_clk / pixel_clk;

		outp32(mipi_dsi_base + MIPIDSI_VID_HSA_TIME_OFFSET, hsa_time);
		outp32(mipi_dsi_base + MIPIDSI_VID_HBP_TIME_OFFSET, hbp_time);
		outp32(mipi_dsi_base + MIPIDSI_VID_HLINE_TIME_OFFSET, hline_time);
	}
...
int mipi_dsi_bit_clk_upt_isr_handler(struct hisi_fb_data_type *hisifd)
{
	...
	mipi_dsi_bit_clk_upt_set(hisifd, hisifd->mipi_dsi0_base, phy_ctrl);
	if (is_dual_mipi_panel(hisifd))
		mipi_dsi_bit_clk_upt_set(hisifd, hisifd->mipi_dsi1_base, phy_ctrl);
	...
	if (!is_mipi_cmd_panel(hisifd)) {
		...
		*/
		...
		outp32(hisifd->mipi_dsi0_base + MIPIDSI_VID_HSA_TIME_OFFSET, hsa_time);
		outp32(hisifd->mipi_dsi0_base + MIPIDSI_VID_HBP_TIME_OFFSET, hbp_time);
		outp32(hisifd->mipi_dsi0_base + MIPIDSI_VID_HLINE_TIME_OFFSET, hline_time);
	}
#endif

	// Delay 1 line, read pll status, if pll lock, pll is stably
