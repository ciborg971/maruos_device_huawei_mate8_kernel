...
void init_post_scf(struct hisi_fb_data_type *hisifd)
{
	...
	hisi_dss_post_scl_load_filter_coef(hisifd, false, scf_lut_base, SCL_COEF_YUV_IDX);
}
...
void single_frame_update(struct hisi_fb_data_type *hisifd)
{
	...
	int secu_cfg_ing = 0, secu_pay = 0, count = 0;
	...
	if (hisifd->index == PRIMARY_PANEL_IDX) {
		...
		if (secure_ctrl->secure_status == DSS_SEC_RUNNING) {
			secu_pay = inp32(dss_base + DSS_MCTRL_SYS_OFFSET + MCTL_CTL_SECU_CFG);
			secu_pay = secu_pay & BIT(4);

			if ((secu_pay) && (!secure_ctrl->tui_frame_processed)) {
...
void init_igm_gmp_xcc_gm(struct hisi_fb_data_type *hisifd)
{
	...
	if (pinfo->xcc_support == 1) {
		...
		if (pinfo->xcc_table_len > 0 && pinfo->xcc_table) {
			...
			outp32(lcp_base + LCP_XCC_COEF_01, pinfo->xcc_table[1] * g_led_rg_csc_value[0] / 32768);
			...
			outp32(lcp_base + LCP_XCC_COEF_12, pinfo->xcc_table[6] * g_led_rg_csc_value[4] / 32768);
			...
			outp32(lcp_base + LCP_XCC_COEF_23, pinfo->xcc_table[11] * g_led_rg_csc_value[8] / 32768 * DISCOUNT_COEFFICIENT(g_comform_value));
		}
...
int dpe_set_led_rg_ct_cscValue(struct hisi_fb_data_type *hisifd)
{
	...
	//XCC
	if (1 == g_is_led_rg_csc_set && pinfo->xcc_support == 1) {
