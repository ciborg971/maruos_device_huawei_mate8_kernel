...
static void *g_smmu_rwerraddr_virt = NULL;
...
** DSS AIF
*/
static void hisi_dss_aif_init(char __iomem *aif_ch_base,
	dss_aif_t *s_aif)
{
...
static void hisi_dss_aif_ch_set_reg(struct hisi_fb_data_type *hisifd,
...
** DSS MIF
*/
static uint32_t hisi_dss_mif_get_invalid_sel(uint32_t format, uint32_t transform,
	...
	dss_mif_t *s_mif, int chn_idx)
{
	...
	*/
	...
	rw_type = (chn_idx < DSS_WCHN_W0) ? 0x0 : 0x1;
...
void hisi_dss_mif_on(struct hisi_fb_data_type *hisifd)
{
	...
	/*
	set_reg(hisifd->dss_base + MIF_CH0_OFFSET + MIF_CTRL1, 0xA, 4, 0);
	set_reg(hisifd->dss_base + MIF_CH1_OFFSET + MIF_CTRL1, 0x1, 4, 0);
	set_reg(hisifd->dss_base + MIF_CH2_OFFSET + MIF_CTRL1, 0x2, 4, 0);
	set_reg(hisifd->dss_base + MIF_CH3_OFFSET + MIF_CTRL1, 0x3, 4, 0);
	set_reg(hisifd->dss_base + MIF_CH4_OFFSET + MIF_CTRL1, 0x4, 4, 0);
	set_reg(hisifd->dss_base + MIF_CH5_OFFSET + MIF_CTRL1, 0x5, 4, 0);
	set_reg(hisifd->dss_base + MIF_CH6_OFFSET + MIF_CTRL1, 0x6, 4, 0);
	set_reg(hisifd->dss_base + MIF_CH7_OFFSET + MIF_CTRL1, 0x7, 4, 0);
	set_reg(hisifd->dss_base + MIF_CH8_OFFSET + MIF_CTRL1, 0x20008, 32, 0);
	set_reg(hisifd->dss_base + MIF_CH9_OFFSET + MIF_CTRL1, 0x20009, 32, 0);
	*/
	...
	dss_layer_t *layer, dss_wb_layer_t *wb_layer, bool rdma_stretch_enable)
{
	...
	if (img->mmu_enable == 0) {
		...
		if (need_cap & (CAP_AFBCD | CAP_AFBCE)) {
			...
			invalid_sel = hisi_dss_mif_get_invalid_sel(img->format,
				transform, ((need_cap & CAP_TILE) ? 1 : 0), rdma_stretch_enable);
		}
...
void hisi_dss_smmu_on(struct hisi_fb_data_type *hisifd)
{
	...
	set_reg(smmu_base + SMMU_SCR, 0xF, 4, 16);
...
static void hisi_dss_smmu_init(char __iomem *smmu_base,
	dss_smmu_t *s_smmu)
{
...
static void hisi_dss_smmu_ch_set_reg(struct hisi_fb_data_type *hisifd,
...
int hisi_dss_sharpness_config(struct hisi_fb_data_type *hisifd, dss_layer_t *layer)
{
	...
	if (hisifd->panel_info.prefix_sharpness1D_support == 1
		...
		sharpthd1 = 0x10 & 0xFF;
		sharpthd1mul = 0x100 & 0xFFF;
		sharpamt1 = 0x40 & 0xFFF;
...
static void hisi_dss_dpp_acm_ce_set_reg(struct hisi_fb_data_type *hisifd,
	char __iomem *ce_base)
{
	struct hisi_panel_info *pinfo = NULL;
	dss_display_effect_t *ce_ctrl = NULL;
	dss_ce_info_t *ce_info = NULL;
	...
	char __iomem *mctl_base, dss_mctl_t *s_mctl, int ovl_idx)
{
	...
	if ((ovl_idx == DSS_OVL0) || (ovl_idx == DSS_OVL1)) {
		...
		if (is_mipi_cmd_panel(hisifd) && (hisifd->ldi_data_gate_en == 1)) {
			...
			hisifd->set_reg(hisifd, mctl_base + MCTL_CTL_DBG, 0x782620, 32, 0);
		} else {
			//open underflow clear
			hisifd->set_reg(hisifd, mctl_base + MCTL_CTL_DBG, 0x78A620, 32, 0);
		}
	}
...
void hisi_dss_mctl_on(struct hisi_fb_data_type *hisifd, int ovl_idx, bool enable_cmdlist, bool fastboot_enable)
{
	...
	set_reg(mctl_base + MCTL_CTL_DBG, 0x202600, 32, 0);

	//TODO: delete it
	//set_reg(mctl_base + MCTL_CTL_DBG, 0xB02620, 32, 0);
	//set_reg(mctl_base + MCTL_CTL_DBG, 0x2600, 32, 0);
...
int hisi_dss_ch_module_set_regs(struct hisi_fb_data_type *hisifd, int ovl_idx, int chn_idx, uint32_t wb_type, bool enable_cmdlist)
{
	...
	if (dss_module->scl_used[i] == 1) {
		hisi_dss_chn_scl_load_filter_coef_set_reg(hisifd, enable_cmdlist, chn_idx, dss_module->scl[i].fmt);
	...
	bool enable_cmdlist, int task_end, int last, bool is_first_ov_block)
{
	...
	if (dss_module->mctl_used[i] == 1) {
		hisi_dss_mctl_ov_set_reg(hisifd, dss_module->mctl_base[i],	&(dss_module->mctl[i]), ovl_idx);
	}
	...
	hisi_dss_dpp_acm_ce_set_reg(hisifd, hisifd->dss_base + DSS_DPP_ACM_CE_OFFSET);
...
int hisi_dss_prev_module_set_regs(struct hisi_fb_data_type *hisifd,
	dss_overlay_t *pov_req, uint32_t cmdlist_pre_idxs, bool enable_cmdlist)
{
	dss_module_reg_t *dss_module = NULL;
	dss_layer_t *layer = NULL;
	...
	if (pov_req->wb_enable && (ovl_idx > DSS_OVL1)) {
		...
		hisifd->cmdlist_idx = DSS_CMDLIST_OV0 + ovl_idx;
		hisi_dss_mctl_ov_set_reg(hisifd, dss_module->mctl_base[ovl_idx], &(dss_module->mctl[ovl_idx]), ovl_idx);
	}
...
void* hisi_dss_mmbuf_init(struct hisi_fb_data_type *hisifd)
{
	...
	if (!prev_refcount) {
		...
		if (!g_smmu_rwerraddr_virt) {
			...
			BUG_ON(g_smmu_rwerraddr_virt == NULL);
...
void hisi_dss_mmbuf_on(struct hisi_fb_data_type *hisifd)
{
	...
	if (g_fpga_flag == 1) {
		outp32(hisifd->sctrl_base + SCPWREN, 0x8);
		outp32(hisifd->sctrl_base + SCPEREN1, 0x3c00000);
		outp32(hisifd->sctrl_base + SCPERRSTDIS1, 0x00001800);
		outp32(hisifd->sctrl_base + SCISODIS, 0x8);
	}
...
int hisi_overlay_off(struct hisi_fb_data_type *hisifd)
{
	...
	if ((hisifd->index == PRIMARY_PANEL_IDX) ||
		...
		hisi_dss_prev_module_set_regs(hisifd, pov_req_prev, cmdlist_pre_idxs, enable_cmdlist);
		...
		if (hisifd->ov_online_play) {
			...
			if (g_debug_ovl_online_composer_timediff & 0x1) {
				...
				if (timediff >= g_debug_ovl_online_composer_time_threshold)
					HISI_FB_INFO("ONLING_IOCTL_TIMEDIFF is %u us!\n", timediff);
			}
...
int hisi_overlay_init(struct hisi_fb_data_type *hisifd)
{
	...
	hisifd->crc_flag = 0;
	init_waitqueue_head(&hisifd->crc_wq);
	...
	if ((hisifd->index == PRIMARY_PANEL_IDX) ||
		...
		if (HISI_DSS_SUPPORT_DPP_MODULE_BIT(DPP_MODULE_ACM_CE)) {
			...
			INIT_WORK(&hisifd->dpp_ce_end_work, hisi_dss_dpp_acm_ce_end_handle_func);
		}
	...
	if (hisifd->index == PRIMARY_PANEL_IDX) {
		...
		hisifd->crc_isr_handler = hisi_crc_isr_handler;
...
void hisi_vactive0_start_isr_handler(struct hisi_fb_data_type *hisifd)
{
	...
	if (secure_ctrl->secure_status == DSS_SEC_RUNNING) {
		...
		wake_up_interruptible_all(&secure_ctrl->tui_wq);
	}
		...
		REDO_0:
			...
			if (ret <= 0) {
				...
				HISI_FB_ERR("fb%d, 1wait_for vactive0_start_flag timeout!ret=%d, "
					...
					cmdlist_idxs_prev, cmdlist_idxs_prev_prev, cmdlist_idxs, inp32(hisifd->dss_base + GLB_GLB_CPU_ITF0_INTS));
				#if defined (CONFIG_HUAWEI_DSM)
