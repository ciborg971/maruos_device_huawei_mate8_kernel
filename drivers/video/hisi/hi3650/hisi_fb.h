...
struct hisifb_secure {
	...
	uint32_t tui_vactive_start_flag;
	uint32_t tui_frame_processed;

	spinlock_t spin_lock;
	wait_queue_head_t tui_wq;

	struct work_struct secure_ctrl_work;
	struct task_struct *tui_thread;
	...
	void (*check_single_update) (struct hisi_fb_data_type *hisifd);
...
struct hisi_fb_data_type {
	...
	dss_ce_info_t prefix_ce_info[DSS_CHN_MAX];
