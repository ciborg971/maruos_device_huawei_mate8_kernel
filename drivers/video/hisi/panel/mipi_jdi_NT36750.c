...
static int mipi_jdi_panel_set_backlight(struct platform_device *pdev)
{
	...
	HISI_FB_DEBUG("mipi_jdi_panel_set_backlight wxh exit %d.\n",bl_cmd[0].payload[1]);
	...
	char *buf)
{
	...
	ret = snprintf(buf, PAGE_SIZE, "jdi_NT36750 5.2' CMD TFT 1080 x 1920\n");
...
static int mipi_jdi_probe(struct platform_device *pdev)
{
	...
	HISI_FB_ERR("mipi_jdi_probe 36750 wxh!\n");
	...
	pinfo->dsi_bit_clk_upt_support = 1; //zhangshijun asked modify
