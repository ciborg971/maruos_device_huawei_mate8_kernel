...
#define io_trace_print(fmt, arg...) printk("[@@@@@@@%s:%d]"fmt, __FUNCTION__, __LINE__, ##arg)
...
enum IO_TRACE_ACTION
{
    ...
    IO_TRACE_PARENT_END = EXT4_SYNC_ENTER_TAG,
    ...
    IO_TRACE_JANK_TAG = EXT4_SYNC_EXIT_TAG,
    EXT4_MAP_SUBMIT_TAG,
        ...
        struct kobj_attribute *attr, const char *buf, size_t count)
{
    ...
    if(out_itface->magic == 0x5A)
    {
        ...
        if(!io_copy_interface_data(my_itface, out_itface))
        {
