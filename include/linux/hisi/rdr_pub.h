...
typedef enum
{
    ...
    REBOOT_REASON_LABEL1 = 0x14,    /*小于0x14的复位原因是一个域，
    ...
    REBOOT_REASON_LABEL2 = 0x23,    /*大于等于0x14，且小于0x23的复位原因是一个域，
                                    表示是sctrl系统状态寄存器记录的异常，流程上不需要框架记录复位原因，但是
                                    在复位之后，需要通过框架保存log*/
    RESERVED2          = REBOOT_REASON_LABEL2,
    AP_S_PANIC         = 0x24,
    AP_S_NOC           = 0x25,
    AP_S_PMU           = 0x26,
    AP_S_DDRC_SEC      = 0x27,
    ...
    AP_S_MAILBOX   = 0x2b,
    REBOOT_REASON_LABEL3 = 0x2c,    /*大于等于0x23，且小于0x2c的复位原因是一个域，
    ...
    REBOOT_REASON_LABEL4 = 0x40,    /*大于等于0x2c，且小于等于0x40的复位原因是一个域，
    ...
    BR_POWERON_BY_SMPL                    = 0x53,   /*大于0x40的是一个域，表示框架无关*/
} EXCH_SOURCE;
